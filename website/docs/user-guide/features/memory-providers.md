...
```bash
kopi memory setup      # interactive picker + configuration
kopi memory status     # check what's active
kopi memory off        # disable external provider
```

You can also select the active memory provider via `kopi plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.kopi/config.yaml`:
...
**Setup Wizard:**
```bash
kopi memory setup        # select "honcho" — runs the Honcho-specific post-setup
```
...
:::tip Migrating from `kopi honcho`
If you previously used `kopi honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::
...
Off-gateway these keys do nothing. `kopi memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.kopi/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.kopi/.env
```
...
**Setup (Platform):**
```bash
kopi memory setup    # select "mem0" → "Platform"
# Or manually:
kopi config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.kopi/.env
```

**Setup (OSS):**
```bash
kopi memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
kopi memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
kopi memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
kopi memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
kopi memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.kopi/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.kopi/.env
```
...
**Config:** `$KOPI_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.kopi/.env`.
...
| `user_id` | `kopi-user` | User identifier |
| `agent_id` | `kopi` | Agent identifier |
...
**Switching modes:** Re-run `kopi memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
kopi memory setup    # select "hindsight"
# Or manually:
kopi config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.kopi/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p kopi ui start`

**Config:** `$KOPI_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `kopi` | Memory bank identifier |
...
**Setup:**
```bash
kopi memory setup    # select "holographic"
# Or manually:
kopi config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.kopi-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$KOPI_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
kopi memory setup    # select "retaindb"
# Or manually:
kopi config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.kopi/.env
```
...
- Knowledge tree stored at `$KOPI_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=kopi) |
...
**Setup:**
```bash
kopi memory setup    # select "supermemory"
# Or manually:
kopi config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.kopi/.env
```

**Config:** `$KOPI_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `kopi` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
| **Requires** | `pip install kopi-memori` + `kopi-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install kopi-memori
kopi-memori install
kopi config set memory.provider memori
kopi memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `kopi-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$KOPI_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$KOPI_HOME/` so each profile has its own credentials
