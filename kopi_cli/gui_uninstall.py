...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``kopi desktop`` / ``kopi gui``)
     Built inside the agent checkout under ``$KOPI_HOME/kopi-ai-agent/``:
       ...
       - ``$KOPI_HOME/desktop-build-stamp.json`` (the build freshness stamp)
...
the app name ("Hermes"), separate from ``$KOPI_HOME``:
...
``kopi uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(kopi_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return kopi_home / "kopi-ai-agent"
...
def source_built_gui_artifacts(kopi_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``kopi desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``kopi-ai-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(kopi_home)
    ...
    return [
        ...
        kopi_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    if sys.platform == "darwin":
        ...
        paths += [
            data_base / "applications" / "kopi.desktop",
            data_base / "applications" / "Hermes.desktop",
        ]
    return paths


def agent_is_installed(kopi_home: Path) -> bool:
    """Return True when a usable Python agent install exists under KOPI_HOME.
...
def gui_is_installed(kopi_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(kopi_home):
        if p.exists():
            return True
...
def gui_install_summary(kopi_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = kopi_home if kopi_home is not None else get_kopi_home()
...
def uninstall_gui(kopi_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``kopi-ai-agent/kopi_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$KOPI_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove kopi' or "
            "'sudo dnf remove kopi'). AppImage builds are a single file you "
