# KOPI AI AGENT Security Policy

This document describes KOPI AI AGENT's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/LINYIQ66/kopi-ai-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **KOPI AI AGENT does not operate a bug
bounty program.**
...
- Environment details (`kopi version`, commit SHA, OS, Python
  version).
...
KOPI AI AGENT is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running KOPI AI AGENT,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted KOPI AI AGENT access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in KOPI AI AGENT's documentation
...
KOPI AI AGENT supports two OS-level isolation postures. They address
...
KOPI AI AGENT supports this in two ways:

- **KOPI AI AGENT's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, KOPI AI AGENT's in-process heuristics
...
KOPI AI AGENT filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in KOPI AI AGENT
itself. Bugs in KOPI AI AGENT's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   KOPI AI AGENT does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, KOPI AI AGENT's own documentation, or reasonable
  operator expectations would predict — including cases where
  KOPI AI AGENT has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  kopi-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not KOPI AI AGENT's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in KOPI AI AGENT. Bugs in Hermes
...
- KOPI AI AGENT includes supply-chain guards for MCP server
