...
| | |
|---|---|
| Source | Optional — install with `kopi skills install official/productivity/telephony` |
...
- save provider credentials into `${KOPI_HOME:-~/.kopi}/.env`
...
```bash
SCRIPT="$(find ~/.kopi/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
kopi skills search telephony
kopi skills install official/productivity/telephony
```
...
3. Buy it and save it into `${KOPI_HOME:-~/.kopi}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `${KOPI_HOME:-~/.kopi}/.env`
