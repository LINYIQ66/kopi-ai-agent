...
kopi kanban watch --tenant <project-slug>

# Snapshot of the board
kopi kanban list --tenant <project-slug>
kopi kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
kopi kanban stats --tenant <project-slug>

# Visual dashboard (browser)
kopi dashboard

# Inspect a specific task (includes comments + events)
kopi kanban show <task-id>

# Follow a single task's event stream
kopi kanban tail <task-id>
```

Verify available subcommands with `kopi kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `kopi kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `kopi kanban block <id>` then `kopi kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `kopi kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `kopi kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `kopi profile list` (profile exists?), `kopi status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(kopi kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
kopi kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
kopi kanban block <task-id>
kopi kanban archive <task-id>

# Diagnose what happened
kopi kanban show <task-id>      # task body, comments, recent events
kopi kanban tail <task-id>      # follow the live event stream
kopi kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `kopi kanban create` for the director
...
while true; do
    clear
    kopi kanban list --tenant <slug>
    echo "---"
    kopi kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `kopi kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `kopi profile list` and start
