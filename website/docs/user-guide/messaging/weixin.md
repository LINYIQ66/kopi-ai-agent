...
sidebar_position: 15
title: "Weixin (WeChat)"
description: "Connect KOPI AI AGENT to personal WeChat accounts via the iLink Bot API"
---
...
5. Save the account credentials automatically to `~/.kopi/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.kopi/.env`:
...
- **QR code login** — scan-to-connect setup via `kopi gateway setup`
...
1. Pair Hermes once with `kopi gateway setup` and note the connected iLink bot
   account.
...
- Tokens are saved per account+peer to `~/.kopi/weixin/accounts/<account_id>.context-tokens.json`
...
| `Weixin startup failed: WEIXIN_TOKEN is required` | Run `kopi gateway setup` to complete QR login, or set `WEIXIN_TOKEN` manually |
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | Set `WEIXIN_ACCOUNT_ID` in your `.env` or run `kopi gateway setup` |
...
| Session expired (`errcode=-14`) | Your login session has expired. Re-run `kopi gateway setup` to scan a new QR code |
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `cd ~/.kopi/kopi-ai-agent && uv pip install -e ".[messaging]"`. Alternatively, open the URL printed above the QR |
