...
description: "Connect KOPI AI AGENT to external tool servers via MCP — and control exactly which MCP tools Hermes loads"
---

# MCP (Model Context Protocol)

MCP lets KOPI AI AGENT connect to external tool servers so the agent can use tools that live outside Hermes itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.
...
2. Add an MCP server to `~/.kopi/config.yaml`:
...
```bash
kopi mcp                # interactive picker (default)
kopi mcp catalog        # plain-text list, scriptable
kopi mcp install n8n    # install a catalog entry by name
```
...
`optional-mcps/` in the kopi-ai-agent repo — presence in that directory means
...
- **API key** — Hermes prompts at install time and writes the value to
  `~/.kopi/.env`. Non-secret values (base URLs) go to the same file.
...
- **OAuth** (third-party provider like Google/GitHub) — Hermes points you at
  `kopi auth <provider>` if you haven't authenticated already.
...
written (if not). Re-run `kopi mcp configure <name>` once the server is
reachable to refine.
...
the kopi-ai-agent repo, so Nous has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/LINYIQ66/kopi-ai-agent/tree/main/optional-mcps)
...
Hermes`) for that entry instead of silently hiding it. Run `kopi update`
...
from environment variables (which include everything in `~/.kopi/.env`).
...
MCPs are never auto-updated. Re-run `kopi mcp install <name>` to refresh
after a Hermes update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/LINYIQ66/kopi-ai-agent/tree/main/optional-mcps).
...
On first connect, Hermes prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.kopi/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.
...
See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `kopi mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `kopi mcp login` can list the tools and look like it worked, but every real tool call later times out. `kopi mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `kopi mcp login googledrive` — with the pre-registered client, Hermes skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.kopi/config.yaml` from inside a running Hermes session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `kopi mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.
...
Hermes reads MCP config from `~/.kopi/config.yaml` under `mcp_servers`.
...
For well-known MCP servers, `kopi mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
kopi mcp add codex --preset codex
```
...
You can pick any local name (`kopi mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.
...
```json
{
  "mcpServers": {
    "hermes": {
      "command": "/home/user/.kopi/kopi-ai-agent/venv/bin/kopi",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server reads conversation data directly from Hermes's session store (`~/.kopi/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `kopi send` CLI.
...
- The embedded `kopi mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Hermes, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
