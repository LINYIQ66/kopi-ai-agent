"""
Microsoft Teams platform adapter for KOPI AI AGENT.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        kopi_action = data.get("kopi_action", "")
        session_key = data.get("session_key", "")

        if not kopi_action or not session_key:
        ...
        choice = choice_map.get(kopi_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="kopi_approve",
                    data={**btn_data_base, "kopi_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="kopi_approve",
                    data={**btn_data_base, "kopi_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="kopi_approve",
                    data={**btn_data_base, "kopi_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="kopi_approve",
                    data={**btn_data_base, "kopi_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print()
    print_success("Teams configuration saved to ~/.kopi/.env")
