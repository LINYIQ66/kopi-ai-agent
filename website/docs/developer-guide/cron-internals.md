...
| `kopi_cli/cron.py` | CLI `kopi cron` subcommands |
...
Jobs are stored in `~/.kopi/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `kopi cron` commands are run or during active CLI sessions.
...
```python
# ~/.kopi/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.kopi/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `kopi cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `kopi cron` CLI provides direct job management:

```bash
kopi cron list                    # Show all jobs
kopi cron create                  # Interactive job creation (alias: add)
kopi cron edit <job_id>           # Edit job configuration
kopi cron pause <job_id>          # Pause a running job
kopi cron resume <job_id>         # Resume a paused job
kopi cron run <job_id>            # Trigger immediate execution
kopi cron remove <job_id>         # Delete a job
```
