...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with kopi-ai-agent)
2. User-installed providers: ``$KOPI_HOME/plugins/<name>/``
...
_USER_NAMESPACE = "_kopi_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_kopi_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_kopi_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$KOPI_HOME/plugins/`` or None if unavailable."""
    try:
        from kopi_constants import get_kopi_home
        d = get_kopi_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$KOPI_HOME/plugins/<name>/``) directories.  Bundled takes
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _kopi_user_memory.<name>.cli, usually
