...
_KOPI_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on KOPI_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (KOPI_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `kopi tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (KOPI_KANBAN_TASK env "
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in kopi_cli/tools_config.py).
        "posture": True,
    },
    ...
    # the `kopi send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "kopi-cron": {
        # Mirrors kopi-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `kopi tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as kopi-cli; gated by `kopi tools`",
        "tools": _KOPI_CORE_TOOLS,
        "includes": []
    },
    ...
    "kopi-webhook": {
        ...
        "tools": _KOPI_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "kopi-gateway": {
        ...
        "tools": [],
        "includes": ["kopi-telegram", "kopi-discord", "kopi-whatsapp", "kopi-slack", "kopi-signal", "kopi-bluebubbles", "kopi-homeassistant", "kopi-email", "kopi-sms", "kopi-mattermost", "kopi-matrix", "kopi-dingtalk", "kopi-feishu", "kopi-wecom", "kopi-wecom-callback", "kopi-weixin", "kopi-qqbot", "kopi-webhook", "kopi-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``kopi-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_KOPI_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``kopi-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (kopi-<name>).
        # Gives them _KOPI_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("kopi-"):
            platform_name = name[len("kopi-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_KOPI_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
