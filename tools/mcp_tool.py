...
transport, discovers their tools, and registers them into the kopi-ai-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.kopi/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.kopi/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            kopi_home = os.path.expanduser(
                os.getenv(
                    "KOPI_HOME", os.path.join(os.path.expanduser("~"), ".kopi")
                )
            )
            candidates = [
                os.path.join(kopi_home, "node", "bin", resolved_command),
...
class ElicitationHandler:
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (KOPI_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit KOPI_SESSION_PLATFORM, so the
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `kopi mcp login {server_name}` (or delete the tokens "
            f"file under ~/.kopi/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local KOPI_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_kopi_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_kopi_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local KOPI_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_kopi_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.kopi/.env`` loaded at startup).
    """
    try:
        from kopi_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("KOPI_SAFE_MODE"):
            return {}
        config = load_config()
