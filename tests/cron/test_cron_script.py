...
- Path resolution (absolute, relative to KOPI_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp KOPI_HOME."""
    kopi_home = tmp_path / ".kopi"
    kopi_home.mkdir()
    (kopi_home / "cron").mkdir()
    (kopi_home / "cron" / "output").mkdir()
    (kopi_home / "scripts").mkdir()
    monkeypatch.setenv("KOPI_HOME", str(kopi_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "KOPI_DIR", kopi_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", kopi_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", kopi_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", kopi_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        ...
        from tools.environments.local import _KOPI_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_KOPI_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.kopi/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("KOPI_SESSION_PLATFORM") is None
        assert os.environ.get("KOPI_SESSION_CHAT_ID") is None
        assert os.environ.get("KOPI_SESSION_CHAT_NAME") is None
