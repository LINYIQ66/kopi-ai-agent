---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into KOPI AI AGENT"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into KOPI AI AGENT"
---
...
Migrate a user's OpenClaw customization footprint into KOPI AI AGENT. Imports Hermes-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `kopi skills install official/migration/openclaw-migration` |
...
Use this skill when a user wants to move their OpenClaw setup into KOPI AI AGENT with minimal manual cleanup.
...
```bash
kopi claw migrate              # Full interactive migration
...
kopi claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `kopi setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_kopi.py` to:
...
- copy OpenClaw skills into `~/.kopi/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Hermes workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.kopi/tts/`
...
- `scripts/openclaw_to_kopi.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.kopi/skills/migration/openclaw-migration/scripts/openclaw_to_kopi.py`

Do not guess a shorter path like `~/.kopi/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.kopi/skills/migration/openclaw-migration/`.
...
```bash
python3 ~/.kopi/skills/migration/openclaw-migration/scripts/openclaw_to_kopi.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.kopi/skills/migration/openclaw-migration/scripts/openclaw_to_kopi.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.kopi/skills/migration/openclaw-migration/scripts/openclaw_to_kopi.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.kopi/skills/migration/openclaw-migration/scripts/openclaw_to_kopi.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.kopi/skills/migration/openclaw-migration/scripts/openclaw_to_kopi.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.kopi/skills/migration/openclaw-migration/scripts/openclaw_to_kopi.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
- OpenClaw skills available under `~/.kopi/skills/openclaw-imports/`
