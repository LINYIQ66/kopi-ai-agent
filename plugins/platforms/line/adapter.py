"""
LINE Messaging API platform adapter for KOPI AI AGENT.
...
to KOPI AI AGENT. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``KOPI_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            kopi_home = Path(get_kopi_home()).resolve()
        except Exception:
            kopi_home = Path.home().joinpath(".kopi").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``kopi status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``kopi status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``kopi setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.kopi/.env`` via ``kopi_cli.config``.
    """
    print()
    ...
    try:
        from kopi_cli.config import get_env_var, set_env_var
    except ImportError:
        print("kopi_cli.config not available; set LINE_* vars manually in ~/.kopi/.env")
        return
