...
在进程启动时从 [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) 拉取 API 密钥，而不是以明文形式存储在 `~/.kopi/.env` 中。一个引导密钥（机器账户访问令牌）替代了 N 个提供商密钥，轮换凭据只需在 Bitwarden Web 应用中修改一次即可。

## 工作原理

1. 在 Bitwarden Secrets Manager 中创建一个**机器账户**，授予其对某个项目的读取权限，并生成一个**访问令牌**。
2. Hermes 将该单一令牌以 `BWS_ACCESS_TOKEN` 的形式存储在 `~/.kopi/.env` 中。
3. 每次 `kopi`（或 gateway，或 cron 任务）启动时，在加载 `~/.kopi/.env` 之后，Hermes 会调用 `bws secret list <project_id>` 并将返回的密钥写入 `os.environ`。
...
`bws` 二进制文件在首次使用时会自动下载到 `~/.kopi/bin/`，无需 `apt`、`brew` 或 `sudo`。
...
1. 下载并验证 `bws v2.0.0`，存放至 `~/.kopi/bin/bws`。
2. 提示输入访问令牌（输入内容隐藏）。以 `BWS_ACCESS_TOKEN` 形式存储在 `~/.kopi/.env` 中。
...
此后，每次调用 `kopi` 都会在启动时拉取最新 secret。进程中首次应用 secret 时，stderr 会显示一行摘要信息。

## CLI

| 命令 | 功能 |
|---|---|
| `kopi secrets bitwarden setup` | 交互式向导（安装二进制文件、提示输入令牌、选择项目、测试拉取） |
| `kopi secrets bitwarden status` | 显示配置、二进制版本及令牌是否存在 |
| `kopi secrets bitwarden sync` | 演习模式：立即拉取 secret 并显示将应用的内容 |
| `kopi secrets bitwarden sync --apply` | 拉取并导出到当前 shell 的环境中 |
| `kopi secrets bitwarden install` | 仅下载固定版本的 `bws` 二进制文件（无需认证） |
| `kopi secrets bitwarden disable` | 将 `enabled` 设为 `false`；保留令牌和项目 ID |

## 配置

`~/.kopi/config.yaml` 中的默认值：
...
| `cache_ttl_seconds` | `300` | 进程内拉取结果的复用时长。设为 `0` 可禁用缓存。缓存按进程隔离；新的 `kopi` 调用从头开始。 |
| `override_existing` | `true` | 为 true 时，Bitwarden 的值会覆盖环境中已有的任何值（使 Web 应用中的轮换真正生效）。如果希望本地 `.env` / shell 导出优先，设为 `false`。 |
| `auto_install` | `true` | 为 true 时，首次使用时自动将 `bws` 下载到 `~/.kopi/bin/`。 |
...
| 现象 | 原因 | 修复方法 |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | 配置中已启用，但令牌已从 `.env` 中清除 | 重新运行 `kopi secrets bitwarden setup` |
...
- **单机个人使用**，`~/.kopi/.env` 已经够用。你只是用一个凭据换了另一个，并在启动时增加了网络依赖。
