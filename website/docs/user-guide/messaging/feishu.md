...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up KOPI AI AGENT as a Feishu or Lark bot"
---

# Feishu / Lark Setup

KOPI AI AGENT integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.
...
5. Run `kopi gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `kopi update --gateway` needs confirmation, the adapter records the selected answer in Hermes's `.update_response` file and replaces the card inline with a resolved state.
...
Rules live in `~/.kopi/feishu_comment_rules.json` (pairing grants in `~/.kopi/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
|---------|---------|---------|
| Quiet period | `KOPI_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `KOPI_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `KOPI_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `KOPI_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.kopi/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `KOPI_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `KOPI_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `KOPI_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `KOPI_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `KOPI_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `KOPI_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `kopi gateway setup` |
...
Feishu / Lark uses the `kopi-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
