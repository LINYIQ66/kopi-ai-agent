...
sidebar_position: 12
title: "Google Chat"
description: "Set up KOPI AI AGENT as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect KOPI AI AGENT to Google Chat as a bot. The integration uses Cloud Pub/Sub
...
> Run `kopi gateway setup` and pick **Google Chat** for a guided walk-through.
...
`~/.kopi/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a kopi restart)
...
- Principal: `kopi-chat-bot@<your-project>.iam.gserviceaccount.com`
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/kopi-chat-events`.
...
Add the Google Chat section to `~/.kopi/.env`:
...
kopi -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Hermes home (e.g.
`~/.kopi/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.kopi/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.kopi/google_chat_user_token.json` (if present from
...
3. Check `kopi gateway` logs for `[GoogleChat] Connected`. If you see
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.kopi/google_chat_user_tokens/<sanitized_email>.json` (filesystem
