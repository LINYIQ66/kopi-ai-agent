...
By default every profile is seeded with the bundled skill catalog, and each `kopi update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.kopi` profile):

```bash
curl -fsSL https://kopi-ai-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
kopi profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
kopi skills opt-out            # stop future seeding — nothing on disk is touched
kopi skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
kopi skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `kopi update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `kopi skills opt-in`) to re-enable.

:::note Safe by default
`kopi skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Hermes installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Hermes to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.kopi/plans/` relative to the active workspace/backend working directory.
...
/home/user/.kopi/cache/chart-q4-2025.png
...
When a missing value is encountered, Hermes asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `kopi setup` or `~/.kopi/.env` locally instead.
...
- **Create locally, update in place**: New agent-created skills are written to `~/.kopi/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
...
Bundles live in **`~/.kopi/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `kopi bundles list`.
...
kopi bundles create research

# Overwrite an existing bundle
kopi bundles create backend-dev --skill ... --force

# Delete a bundle
kopi bundles delete backend-dev

# Re-scan ~/.kopi/skill-bundles/ and report changes
kopi bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.kopi/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.kopi/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.kopi/pending/skills/` and are
...
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `kopi skills search <query> --source skills-sh`. Hermes resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
[Skills Hub](https://kopi-ai-agent.nousresearch.com/docs) page — instead of a
...
```bash
kopi skills search airbnb --source browse-sh
kopi skills inspect browse-sh/airbnb.com/search-listings-ddgioa
kopi skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Hermes users add with `kopi skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.kopi.tags`, `version`, `author`, `platforms`, `metadata.kopi.config`).
...
Hermes ships with a set of bundled skills in `skills/` inside the repo. On install and on every `kopi update`, a sync pass copies those into `~/.kopi/skills/` and records a manifest at `~/.kopi/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).
