...
Let the kopi agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where kopi runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `kopi meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `kopi meet` CLI |
...
| `cli.py` | `kopi meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$KOPI_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `kopi meet node {run,list,approve,remove,status,ping}` |
...
kopi meet install                                      # pip + Chromium
kopi meet setup                                        # preflight
kopi meet auth                                         # optional
kopi meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
kopi meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.kopi/.env
kopi meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
kopi meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
kopi meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.kopi/.env
kopi meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, kopi will **not** switch your system audio input automatically — the
...
kopi plugins enable google_meet
kopi meet node run --display-name my-mac --host 0.0.0.0 --port 18789
