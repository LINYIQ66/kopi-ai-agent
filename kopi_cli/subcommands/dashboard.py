"""``kopi dashboard`` / ``kopi serve`` subcommand parsers.
...
``kopi_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `kopi dashboard` / `kopi serve` cmdlines and SIGTERM them directly —
    # the same path `kopi update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the KOPI AI AGENT web dashboard for managing config, API keys, and sessions",
    )
    ...
    # backend as `kopi dashboard --no-open --tui --host ... --port ...`. The
    ...
    # (cmd_dashboard exports KOPI_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `kopi dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.kopi/.env.
    # Nested subparser so bare `kopi dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "KOPI_DASHBOARD_OAUTH_CLIENT_ID into ~/.kopi/.env, and prints "
            "how to engage the login gate. Requires being logged in (kopi setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://kopi.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via KOPI_DASHBOARD_PORTAL_URL. Mainly for "
