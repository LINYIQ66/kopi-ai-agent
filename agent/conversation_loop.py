...
from kopi_logging import set_session_context
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `kopi fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from kopi_cli.moa_config import decode_moa_turn
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from kopi_cli.middleware import apply_llm_request_middleware
                ...
                if env_var_enabled("KOPI_DUMP_REQUESTS"):
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: kopi auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `kopi doctor` for credential-chain diagnostics, or")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: kopi config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: kopi config set ANTHROPIC_API_KEY \"\"")
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `kopi auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `kopi model`.", force=True)
                            else:  # nous
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: kopi setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        kopi fallback add   (interactive picker — same as `kopi model`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.kopi/config.yaml` to extend the per-call "
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    from kopi_cli.plugins import get_pre_verify_continue_message, has_hook
