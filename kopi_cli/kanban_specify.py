...
Used by ``kopi kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``kopi_cli/goals.py`` — same aux
...
KOPI_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("KOPI_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the KOPI AI AGENT board.
...
def _profile_author() -> str:
    """Mirror of ``kopi_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("KOPI_PROFILE")
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=KOPI_KANBAN_SPECIFY_MAX_TOKENS,
