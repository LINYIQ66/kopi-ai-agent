...
sidebar_position: 7
title: "电子邮件"
description: "通过 IMAP/SMTP 将 KOPI AI AGENT 设置为电子邮件助手"
---
...
- **为 KOPI AI AGENT 准备一个专用邮箱账户**（不要使用个人邮箱）
- **在该邮箱账户上启用 IMAP**
...
sudo kopi gateway install --system   # 仅 Linux：开机自启的系统服务
```
...
| **重复回复** | 确保只有一个 gateway 实例在运行。检查 `kopi gateway status`。 |
...
- 密码存储在 `~/.kopi/.env` 中——请保护此文件（`chmod 600`）
