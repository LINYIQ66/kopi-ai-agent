...
session-scoped state file under ``$KOPI_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $KOPI_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_kopi_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this kopi install, leave it first —
    ...
    env = os.environ.copy()
    env["KOPI_MEET_URL"] = url
    env["KOPI_MEET_OUT_DIR"] = str(out)
    env["KOPI_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["KOPI_MEET_HEADED"] = "1"
    if auth_state:
        env["KOPI_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["KOPI_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if KOPI_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["KOPI_MEET_MODE"] = mode
    if realtime_model:
        env["KOPI_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["KOPI_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["KOPI_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["KOPI_MEET_REALTIME_KEY"] = realtime_api_key
