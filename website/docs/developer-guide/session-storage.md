...
Source file: `kopi_state.py`


## Architecture Overview

```
~/.kopi/state.db (SQLite, WAL mode)
...
Multiple kopi processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.kopi/state.db
...
Default path: `~/.kopi/state.db`

This is derived from `kopi_constants.get_kopi_home()` which resolves to
`~/.kopi/` by default, or the value of `KOPI_HOME` environment variable.
