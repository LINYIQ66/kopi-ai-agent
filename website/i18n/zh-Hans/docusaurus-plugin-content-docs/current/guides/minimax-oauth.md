...
sidebar_position: 15
title: "MiniMax OAuth"
description: "通过浏览器 OAuth 登录 MiniMax，在 KOPI AI AGENT 中使用 MiniMax-M2.7 模型——无需 API 密钥"
---

# MiniMax OAuth

KOPI AI AGENT 通过基于浏览器的 OAuth 登录流程支持 **MiniMax**，使用与 [MiniMax 门户](https://www.minimax.io) 相同的凭据。无需 API 密钥或信用卡——登录一次，Hermes 即可自动刷新您的会话。
...
首次登录后，凭据将存储在 `~/.kopi/auth.json` 下，并在每次会话前自动刷新。
...
```bash
kopi auth add minimax-cn --type oauth   # 如果您的中国账户支持 OAuth
# 或更简单的方式：
...
5. 令牌（`access_token`、`refresh_token`、过期时间）以 `minimax-oauth` 为键保存到 `~/.kopi/auth.json`。
...
```bash
kopi config set model MiniMax-M2.7
kopi config set provider minimax-oauth
```

## 配置参考

登录后，`~/.kopi/config.yaml` 将包含类似如下的条目：
...
```bash
kopi --provider minimax-oauth    # 规范名称
kopi --provider minimax-portal   # 别名
kopi --provider minimax-global   # 别名
kopi --provider minimax_oauth    # 别名（下划线形式）
```
...
要将 `minimax-oauth` 设为活跃 provider，请在 `config.yaml` 中设置 `model.provider: minimax-oauth`（使用 `kopi setup` 进行引导式配置），或在单次调用时传入 `--provider minimax-oauth`：
...
**解决方法：** 再次运行 `kopi auth add minimax-oauth` 以开始全新登录。下一次成功交换后隔离状态将自动清除。

### 授权超时

设备码流程有有限的过期窗口。如果您未在规定时间内批准登录，Hermes 将抛出超时错误。

**解决方法：** 重新运行 `kopi auth add minimax-oauth`（或 `kopi model`）。流程将重新开始。
...
如果 `kopi` 无法打开浏览器窗口，请使用 `--no-browser`：
...
**解决方法：** 运行 `kopi model` 并选择 MiniMax (OAuth)，或运行 `kopi auth add minimax-oauth`。
