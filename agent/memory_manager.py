...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_KOPI_CORE_TOOLS)
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``kopi_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_kopi_home()`` themselves.
        """
        if "kopi_home" not in kwargs:
            from kopi_constants import get_kopi_home
            kwargs["kopi_home"] = str(get_kopi_home())
        for provider in self._providers:
            try:
