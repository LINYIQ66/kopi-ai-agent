"""
Photon Spectrum (iMessage) platform adapter for KOPI AI AGENT.
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `kopi photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `kopi setup` / status.
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `kopi update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``kopi photon install-sidecar``: ``npm ci`` for an exact,
...
def register(ctx) -> None:
    ...
    # gateway-setup hook and the `kopi photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: kopi photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `kopi gateway setup` alongside every other
    ...
    # Register CLI subcommands — `kopi photon ...`
