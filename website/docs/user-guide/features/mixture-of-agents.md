...
- **`kopi model`** and the **Dashboard model picker** — a `Mixture of Agents` provider row appears with your preset names as its models.
...
```bash
kopi moa list
kopi moa configure              # update the default preset
kopi moa configure review       # create or update a named preset
kopi moa delete review
```
...
- MoA is no longer listed under `kopi tools`; there is no `moa` toolset to enable.
