...
engine plugins (e.g. kopi-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``kopi plugins`` picker."""
    from kopi_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "kopi_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "kopi_cli.plugins.get_plugin_context_engine",
