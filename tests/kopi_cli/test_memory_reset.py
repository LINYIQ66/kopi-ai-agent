"""Tests for the `kopi memory reset` CLI command.
...
- Profile-scoped reset (uses KOPI_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake KOPI_HOME with memory files."""
    kopi_home = tmp_path / ".kopi"
    memories = kopi_home / "memories"
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `kopi memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        kopi_home = tmp_path / ".kopi"
        (kopi_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("KOPI_HOME", str(kopi_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's KOPI_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_kopi_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
