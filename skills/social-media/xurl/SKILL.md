...
version: 1.1.1
author: xdevplatform + openclaw + KOPI AI AGENT
license: MIT
...
> **Docker HOME pitfall:** In the official Hermes Docker layout, `/opt/data` is `KOPI_HOME`, but Hermes tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Hermes-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
