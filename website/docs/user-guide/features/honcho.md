...
```bash
kopi memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `kopi` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
The `kopi honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `kopi memory setup honcho` (or run `kopi memory setup` and pick it from the list); the `kopi honcho` subcommand then appears on the next invocation.

```bash
kopi memory setup honcho    # Configure Honcho directly (works before activation)
kopi honcho status          # Connection status, config, and key settings
kopi honcho setup           # Redirects to `kopi memory setup` (post-activation alias)
kopi honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
kopi honcho peer            # Show or update peer names + dialectic reasoning level
kopi honcho mode            # Show or set recall mode (hybrid/context/tools)
kopi honcho tokens          # Show or set token budget for context and dialectic
kopi honcho identity        # Seed or show the AI peer's Honcho identity
kopi honcho sync            # Sync Honcho config to all existing profiles
kopi honcho peers           # Show peer identities across all profiles
kopi honcho sessions        # List known Honcho session mappings
kopi honcho map             # Map current directory to a Honcho session name
kopi honcho enable          # Enable Honcho for the active profile
kopi honcho disable         # Disable Honcho for the active profile
kopi honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `kopi honcho`

If you previously used the standalone `kopi honcho setup`:
...
No re-login or re-setup needed. Run `kopi memory setup` and select "honcho" — the wizard detects your existing config.
