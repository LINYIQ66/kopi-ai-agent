...
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.kopi/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.kopi/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
Each hook is a directory under `~/.kopi/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".kopi" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.kopi/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.kopi/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".kopi" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.kopi/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.kopi/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/kopi-events"
...
A popular pattern from the community: drop a Markdown checklist at `~/.kopi/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."
...
1. A file at `~/.kopi/BOOT.md` with natural-language startup instructions.
...
Create `~/.kopi/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `kopi cron list` and check if any scheduled jobs failed overnight.
...
**`~/.kopi/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.kopi/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.kopi/hooks/boot-md/handler.py`**

```python
"""Run ~/.kopi/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".kopi" / "BOOT.md"
...
```bash
kopi logs --follow --level INFO | grep boot-md
```
...
Delete `~/.kopi/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.kopi/hooks/`
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`kopi_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.kopi/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.kopi/agent-hooks/` (by convention) | `~/.kopi/plugins/<name>/` | `~/.kopi/hooks/<name>/` |
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.kopi/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.kopi/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.kopi/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.kopi/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.kopi/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.kopi/agent-hooks/inject-cwd-context.sh
...
hooks:
  subagent_stop:
    - command: "~/.kopi/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.kopi/agent-hooks/log-orchestration.sh
log=~/.kopi/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Hermes sees it, then persists the decision to `~/.kopi/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `kopi --accept-hooks chat`)
2. `KOPI_ACCEPT_HOOKS=1` environment variable
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `kopi hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

#### Manual allowlisting

Manual allowlisting is useful for non-TTY or service-account deployments where an operator cannot answer the first-use prompt interactively. The allowlist file is `~/.kopi/shell-hooks-allowlist.json`, and the expected format is an `approvals` array. Each approval records the hook `event` and the exact `command` string:
...
```json
{
  "approvals": [
    {
      "event": "post_llm_call",
      "command": "/home/kopi/.kopi/hooks/my-hook.py"
    }
  ]
}
```

The command string must match the configured hook command exactly. A path-keyed object with a `sha256` field is not the expected format and will not approve the hook. Verify manual entries with `kopi hooks list`.

### The `kopi hooks` CLI

| Command | What it does |
|---------|--------------|
| `kopi hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `kopi hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `kopi hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `kopi hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.kopi/agent-hooks/` so the path is easy to audit.
- Re-run `kopi hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
