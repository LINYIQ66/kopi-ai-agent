...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``kopi_constants.get_subprocess_home()`` so that ``~`` resolves
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where kopi previously hard-rejected an oversized read
...
_kopi_config_resolved: str | None = None
_kopi_config_resolved_loaded = False


def _get_kopi_config_resolved() -> str | None:
    """Return the resolved absolute path of the Hermes config file (cached)."""
    global _kopi_config_resolved, _kopi_config_resolved_loaded
    if _kopi_config_resolved_loaded:
        return _kopi_config_resolved
    _kopi_config_resolved_loaded = True
    try:
        from kopi_cli.config import get_config_path
        _kopi_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _kopi_config_resolved = str(Path(_expand_tilde("~/.kopi/config.yaml")).resolve())
        except Exception:
            _kopi_config_resolved = None
    return _kopi_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    # this file.
    kopi_config = _get_kopi_config_resolved()
    if kopi_config and (resolved == kopi_config or normalized == kopi_config):
        return (
            f"Refusing to write to Hermes config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.kopi/config.yaml directly or use 'kopi config' instead."
        )
    return None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.kopi/…`` mirror created by a
    ...
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.kopi/…`` path.
    ...
    # in a session that cd'd into ``~/.kopi/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # and block credential stores under KOPI_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == KOPI_HOME (e.g. "auth.json") still hits the
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with kopi_tools.write_file() for very "
            "large files."
        )
