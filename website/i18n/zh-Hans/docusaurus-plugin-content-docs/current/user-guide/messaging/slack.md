...
sidebar_position: 4
title: "Slack"
description: "使用 Socket Mode 将 KOPI AI AGENT 设置为 Slack 机器人"
---

# Slack 设置

使用 Socket Mode 将 KOPI AI AGENT 作为机器人连接到 Slack。Socket Mode 使用 WebSocket 而非公开 HTTP 端点，因此你的 Hermes 实例无需公开访问——它可以在防火墙后、笔记本电脑上或私有服务器上正常运行。
...
1. 生成 manifest：
   ...
   ```
   此命令会将 `~/.kopi/slack-manifest.json` 写入磁盘并打印粘贴说明。
...
2. 点击 **Create New App**
3. 选择 **From scratch**
4. 输入应用名称（例如 "KOPI AI AGENT"）并选择你的工作区
5. 点击 **Create App**
...
```bash
kopi gateway setup    # 提示时选择 Slack
```
...
sudo kopi gateway install --system   # 仅 Linux：开机启动系统服务
```
...
底层实现：Hermes 附带一个生成的 Slack 应用 manifest（见第一步，方式 A），它将 [`COMMAND_REGISTRY`](https://github.com/LINYIQ66/kopi-ai-agent/blob/main/kopi_cli/commands.py) 中的每个命令声明为斜杠命令。在 Socket Mode 下，无论 manifest 的 `url` 字段如何，Slack 都会通过 WebSocket 路由命令事件。

### 更新后刷新斜杠命令

当 Hermes 添加新命令时（例如执行 `kopi update` 后），重新生成 manifest 并更新你的 Slack 应用：
...
3. 粘贴 `~/.kopi/slack-manifest.json` 的新内容
4. **保存**。如果权限范围或斜杠命令有变化，Slack 会提示重新安装应用。

### 旧版 `/kopi <子命令>` 仍然有效

为了向后兼容旧版 manifest，你仍然可以输入 `/kopi btw run the tests`——Hermes 会以与 `/btw run the tests` 相同的方式路由它。自由形式的问题也有效：`/kopi what's the weather?` 会被当作普通消息处理。
...
|---------|----------|
| **私信** | 机器人响应每条消息——无需 @ 提及 |
| **频道** | 机器人**仅在被 @ 提及时响应**（例如 `@KOPI AI AGENT what time is it?`）。在频道中，Hermes 在该消息附带的话题中回复。 |
...
除了第八步中的必需环境变量外，你还可以通过 `~/.kopi/config.yaml` 自定义 Slack 机器人行为。
...
确保机器人已被**邀请到该频道**（`/invite @KOPI AI AGENT`）。
...
| 机器人在私信中正常但在频道中不响应 | **最常见问题。** 将 `message.channels` 和 `message.groups` 添加到事件订阅，重新安装应用，并用 `/invite @KOPI AI AGENT` 邀请机器人加入频道 |
...
| 机器人响应但无法在频道中发帖 | 用 `/invite @KOPI AI AGENT` 邀请机器人加入频道 |
...
7. ✅ 已**邀请**机器人加入频道（`/invite @KOPI AI AGENT`）
...
- Token 应存储在 `~/.kopi/.env` 中（文件权限 `600`）
