#!/usr/bin/env python3
"""
SQLite State Store for KOPI AI AGENT.
...
DEFAULT_DB_PATH = get_kopi_home() / "state.db"
...
# kopi_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``kopi_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see kopi_cli/kanban_db.py) would
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    try:
        ...
        probe_session_id = f"_kopi_fts_health_probe_{time.time_ns()}"
        try:
...
class SessionDB:
    ...
    # With multiple kopi processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``kopi_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `kopi update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._kopi_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._kopi_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def delete_session_if_empty(
        ...
        and ``kopi sessions list`` output. (Pattern ported from
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``KOPI_DISABLE_FTS_TRIGRAM`` or not yet created), so
