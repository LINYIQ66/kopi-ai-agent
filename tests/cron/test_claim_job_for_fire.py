...
These exercise the real store against a temp KOPI_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated KOPI_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("KOPI_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_kopi_home() reads the env live.
    yield tmp_path
