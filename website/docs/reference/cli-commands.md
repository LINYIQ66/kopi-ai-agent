...
```bash
kopi [global-options] <command> [subcommand/options]
```
...
| `--ignore-user-config` | Ignore `~/.kopi/config.yaml` and fall back to built-in defaults. Credentials in `.env` are still loaded. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, memory, and preloaded skills. |
| `--tui` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI. Equivalent to `KOPI_TUI=1`. Always wins over `display.interface`. |
...
| Command | Purpose |
|---------|---------|
| `kopi chat` | Interactive or one-shot chat with the agent. |
| `kopi model` | Interactively choose the default provider and model. |
| `kopi moa` | Configure named Mixture of Agents presets selectable from the model picker. |
| `kopi fallback` | Manage fallback providers tried when the primary model errors. |
| `kopi gateway` | Run or manage the messaging gateway service. |
| `kopi proxy` | Local OpenAI-compatible proxy that attaches OAuth provider credentials. See [Subscription Proxy](../user-guide/features/subscription-proxy.md). |
| `kopi lsp` | Manage Language Server Protocol integration (semantic diagnostics for write_file/patch). |
| `kopi setup` | Interactive setup wizard for all or part of the configuration. |
| `kopi whatsapp` | Configure and pair the WhatsApp bridge. |
| `kopi whatsapp-cloud` | Configure the official Meta WhatsApp Business Cloud API adapter (Business account + public webhook required). Distinct from `kopi whatsapp` (Baileys personal-account bridge). |
| `kopi slack` | Slack helpers (currently: generate the app manifest with every command as a native slash). |
| `kopi auth` | Manage credentials — add, list, remove, reset, status, logout. Handles OAuth flows for Codex/Nous/Anthropic. |
| `kopi login` / `logout` | **Deprecated** — use `kopi auth` instead. |
| `kopi send` | Send a one-shot message to a configured messaging platform (Telegram, Discord, Slack, Signal, SMS, …). Useful from shell scripts, cron jobs, CI hooks, and monitoring daemons — no agent loop, no LLM. |
| `kopi secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.kopi/.env`. |
| `kopi migrate` | Diagnose and (optionally) rewrite `config.yaml` to replace references to retired models or deprecated settings (e.g. `migrate xai`). |
| `kopi status` | Show agent, auth, and platform status. |
| `kopi cron` | Inspect and tick the cron scheduler. |
| `kopi kanban` | Multi-profile collaboration board (tasks, links, dispatcher). |
| `kopi project` | Manage named, multi-folder workspaces (projects). Anchors desktop session grouping and, when bound to a kanban board, gives tasks a deterministic worktree + branch convention. State is per-profile. |
| `kopi webhook` | Manage dynamic webhook subscriptions for event-driven activation. |
| `kopi hooks` | Inspect, approve, or remove shell-script hooks declared in `config.yaml`. |
| `kopi doctor` | Diagnose config and dependency issues. |
| `kopi security audit` | On-demand supply-chain audit (OSV.dev) for the venv, plugin requirements, and pinned MCP servers. |
...
| `kopi skills` | Browse, install, publish, audit, and configure skills. |
...
| `kopi sessions` | Browse, export, prune, rename, and delete sessions. |
| `kopi insights` | Show token/cost/activity analytics. |
...
For programmatic callers (shell scripts, CI, cron, parent processes piping in a prompt), `kopi -z` is the purest one-shot entry point: **single prompt in, final response text out, nothing else on stdout or stderr.** No banner, no spinner, no tool previews, no `Session:` line — just the agent's final reply as plain text.

```bash
kopi -z "What's the capital of France?"
# → Paris.

# Parent scripts can cleanly capture the response:
answer=$(kopi -z "summarize this" < /path/to/file.txt)
```

Per-run overrides (no mutation to `~/.kopi/config.yaml`):

| Flag | Equivalent env var | Purpose |
|---|---|---|
| `-m` / `--model <model>` | `KOPI_INFERENCE_MODEL` | Override the model for this run |
| `--provider <provider>` | _(none)_ | Override the provider for this run |

```bash
kopi -z "…" --provider openrouter --model openai/gpt-5.5
# or:
KOPI_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 kopi -z "…"
```

Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `kopi chat -q` instead; `-z` is explicitly for "I only want the final answer".
...
If you've only configured OpenRouter, `/model` will only show OpenRouter models. To add another provider (Anthropic, DeepSeek, Copilot, etc.), exit your session and run `kopi model` from the terminal.
:::
...
| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `KOPI_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `kopi update`. |
| `--no-supervise` | On `run`: inside the s6-overlay Docker image, opt out of auto-supervision and use pre-s6 foreground semantics — gateway runs as the container's main process with no auto-restart. No-op outside the s6 image. Equivalent to setting `KOPI_GATEWAY_NO_SUPERVISE=1`. |

`kopi gateway enroll` accepts `--token`, `--connector-url`, `--gateway-id`, and `--wake-url`. It exchanges the enrollment token with the connector and writes the resulting `GATEWAY_RELAY_ID`, `GATEWAY_RELAY_SECRET`, `GATEWAY_RELAY_DELIVERY_KEY`, optional `GATEWAY_RELAY_URL`, and (when `--wake-url` is given) `GATEWAY_RELAY_WAKE_URL` values to the active profile's `.env`.

:::tip WSL users
Use `kopi gateway run` instead of `kopi gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s kopi 'kopi gateway run'`. See [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-kopi-gateway-start-fails) for details.
:::
...
```bash
kopi slack manifest              # print manifest to stdout
kopi slack manifest --write      # write to ~/.kopi/slack-manifest.json
kopi slack manifest --slashes-only  # just the features.slash_commands array
```
...
Run `kopi slack manifest --write` again after `kopi update` to pick
up any new commands.
...
```bash
kopi send --to <target> "message text"
kopi send --to <target> --file <path>
echo "message" | kopi send --to <target>
kopi send --list [platform]
```

Send a one-shot message to a configured messaging platform without spinning up an agent or gateway loop. Reuses the gateway's already-configured credentials (`~/.kopi/.env` + `~/.kopi/config.yaml`) so ops scripts, cron jobs, CI hooks, and monitoring daemons can post status updates without reimplementing each platform's REST client.

For bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp-CloudAPI) no running gateway is required — `kopi send` talks directly to the platform's REST endpoint. Plugin platforms that need a persistent adapter still require a live gateway.
...
If neither a positional `message` argument nor `--file` is provided, `kopi send` reads from stdin when it is not a TTY. Exit codes: `0` on success, `1` on delivery/backend failure, `2` on usage errors.
...
```bash
kopi send --to telegram "MEDIA:/tmp/screenshot.png"
kopi send --to telegram "Build chart for today MEDIA:/tmp/chart.png"   # with caption
kopi send --to discord:#ops "MEDIA:/tmp/report.pdf"
```
...
```bash
kopi send --to telegram "[[as_document]] MEDIA:/tmp/screenshot.png"
```

Examples:

```bash
kopi send --to telegram "deploy finished"
echo "RAM 92%" | kopi send --to telegram:-1001234567890
kopi send --to discord:#ops --file /tmp/report.md
kopi send --to slack:#eng --subject "[CI]" --file build.log
kopi send --list                  # all platforms
kopi send --list telegram         # filter by platform
```
...
```bash
kopi secrets bitwarden <subcommand>
kopi secrets bw <subcommand>          # short alias
```

Pull API keys from an external secret manager at process startup instead of storing them in `~/.kopi/.env`. Currently supports **Bitwarden Secrets Manager**. See the full guide: [Bitwarden integration](../user-guide/secrets/bitwarden.md).
...
## `kopi login` / `kopi logout` *(Deprecated)*

:::caution
`kopi login` has been removed. Use `kopi auth` to manage OAuth credentials, `kopi model` to select a provider, or `kopi setup` for full interactive setup.
:::
...
```bash
kopi auth                                              # Interactive wizard
kopi auth list                                         # Show all pools
kopi auth list openrouter                              # Show specific provider
kopi auth add openrouter --api-key sk-or-v1-xxx        # Add API key
kopi auth add anthropic --type oauth                   # Add OAuth credential
kopi auth remove openrouter 2                          # Remove by index
...
```bash
kopi cron <list|create|edit|pause|resume|run|remove|status|tick>
```
...
`$KOPI_HOME/plugins/<name>/`. An unknown or unavailable provider falls back to
...
```bash
kopi kanban [--board <slug>] <action> [options]
```

Multi-profile, multi-project collaboration board. Each install can host many boards (one per project, repo, or domain); each board is a standalone queue with its own SQLite DB and dispatcher scope. New installs start with one board called `default`, whose DB is `~/.kopi/kanban.db` for back-compat; additional boards live at `~/.kopi/kanban/boards/<slug>/kanban.db`. The gateway-embedded dispatcher sweeps every board per tick.

**Global flags (apply to every action below):**

| Flag | Purpose |
|------|---------|
| `--board <slug>` | Operate on a specific board. Defaults to the current board (set via `kopi kanban boards switch`, the `KOPI_KANBAN_BOARD` env var, or `default`). |

**This is the human / scripting surface.** Agent workers spawned by the dispatcher drive the board through a dedicated `kanban_*` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board) (`kanban_show`, `kanban_complete`, `kanban_block`, `kanban_create`, `kanban_link`, `kanban_comment`, `kanban_heartbeat`; orchestrator profiles also get `kanban_list` and `kanban_unblock`) instead of shelling to `kopi kanban`. Workers have `KOPI_KANBAN_BOARD` pinned in their env so they physically cannot see other boards.
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.kopi/kanban/current`). |
...
kopi kanban boards create atm10-server --name "ATM10 Server" --icon 🎮
kopi kanban --board atm10-server create "Restart server" --assignee ops

# Switch the active board for subsequent calls.
kopi kanban boards switch atm10-server
kopi kanban list                  # shows atm10-server tasks

# Archive a board (recoverable) or hard-delete it.
kopi kanban boards rm atm10-server
kopi kanban boards rm atm10-server --delete
```

Board resolution order (highest precedence first): `--board <slug>` flag → `KOPI_KANBAN_BOARD` env var → `~/.kopi/kanban/current` file → `default`.

All actions are also available as a slash command in the gateway (`/kanban …`), with the same argument surface — including `boards` subcommands and the `--board` flag.

For the full design — comparison with Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise, eight collaboration patterns, four user stories, concurrency correctness proof — see `docs/kopi-kanban-v1-spec.pdf` in the repository or the [Kanban user guide](/user-guide/features/kanban).

## `kopi project`

```bash
kopi project <create|list|show|add-folder|remove-folder|rename|set-primary|use|archive|restore|bind-board>
```
...
```bash
kopi webhook <subscribe|list|remove|test>
```
...
### `kopi webhook subscribe`

```bash
kopi webhook subscribe <name> [options]
```
...
| `--script` | Filter/transform script under `~/.kopi/scripts/`. The webhook payload is passed as JSON on stdin; JSON stdout replaces the payload, and empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. See [Script Filters and Transforms](../user-guide/messaging/webhooks.md#script-filters-and-transforms). |

Subscriptions persist to `~/.kopi/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart.
...
:::tip
`kopi dump` is specifically designed for sharing. For interactive diagnostics, use `kopi doctor`. For a visual overview, use `kopi status`.
:::

## `kopi debug`

```bash
kopi debug share [options]
```
...
```bash
kopi debug share              # Upload debug report, print URL
kopi debug share --lines 500  # Include more log lines
kopi debug share --expire 30  # Keep paste for 30 days
kopi debug share --nous       # Upload a private diagnostics bundle for Nous support
kopi debug share --local      # Print report to terminal (no upload)
```
...
| Option | Description |
|--------|-------------|
| `-o`, `--output <path>` | Output path for the zip file (default: `~/kopi-backup-<timestamp>.zip`). |
...
- The `kopi-ai-agent` code itself (this is a user-data backup, not a repo snapshot).

### Examples

```bash
kopi backup                           # Full backup to ~/kopi-backup-*.zip
kopi backup -o /tmp/kopi.zip        # Full backup to specific path
kopi backup --quick                   # Quick state-only snapshot
kopi backup --quick --label "pre-upgrade"  # Quick snapshot with label
```

## `kopi checkpoints`

```bash
kopi checkpoints [COMMAND]
```

Inspect and manage the shadow git store at `~/.kopi/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.

| Subcommand | Description |
|------------|-------------|
| `status` (default) | Show total size, project count, and per-project breakdown. Bare `kopi checkpoints` is equivalent. |
...
```bash
kopi checkpoints                                  # status overview
kopi checkpoints prune --retention-days 3         # aggressive cleanup
kopi checkpoints prune --max-size-mb 200          # tighten size cap once
kopi checkpoints clear-legacy -f                  # drop v1 archive dirs
kopi checkpoints clear -f                         # wipe everything
```
...
### Examples
```bash
kopi import ~/kopi-backup-20260423.zip           # Prompts before overwriting existing config
kopi import ~/kopi-backup-20260423.zip --force   # Overwrite without prompting
```
...
kopi logs gateway -n 100

# Show only warnings and errors from the last hour
kopi logs --level WARNING --since 1h

# Filter by a specific session
kopi logs --session abc123

# Follow errors.log, starting from 30 minutes ago
kopi logs errors --since 30m -f
...
kopi logs --level WARNING --since 2h --session tg-12345
```
...
```bash
kopi prompt-size [--platform <name>] [--json]
```
...
kopi prompt-size --platform telegram

# Machine-readable output for scripts
kopi prompt-size --json
```

:::tip
The skills index and tool schemas scale with how many skills and tools you have
enabled. To shrink the prompt, disable unused toolsets (`kopi tools`) or
uninstall skills you don't need (`kopi skills`). Context files (AGENTS.md,
...
```bash
kopi pairing <list|approve|revoke|clear-pending>
```
...
kopi skills inspect official/security/1password
kopi skills inspect skills-sh/vercel-labs/json-render/json-render-react
kopi skills install official/migration/openclaw-migration
...
kopi skills opt-out                  # stop future bundled-skill seeding (nothing deleted)
kopi skills opt-out --remove --yes   # also delete UNMODIFIED bundled skills
kopi skills opt-in --sync            # undo: remove marker and re-seed now
```
...
```bash
kopi bundles <subcommand>
```

Skill bundles group several skills under one `/<bundle-name>` slash command. Invoking the bundle loads every referenced skill into a single combined user message. Storage: `~/.kopi/skill-bundles/<slug>.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles) for the YAML schema and behavior.
...
| `reload` | Re-scan `~/.kopi/skill-bundles/` and report added/removed bundles |

Examples:

```bash
kopi bundles create backend-dev \
...
kopi bundles list
kopi bundles show backend-dev
kopi bundles delete backend-dev
```
...
| `backup` | Take a manual tar.gz snapshot of `~/.kopi/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.kopi/skills/` from a snapshot (defaults to newest) |
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `kopi update`. Use `kopi curator run --dry-run` to preview before that happens.
...
| `add` | Pick a provider + model (same picker as `kopi model`) and append to the chain |
...
```bash
kopi hooks <subcommand>
```

Inspect shell-script hooks declared in `~/.kopi/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.kopi/shell-hooks-allowlist.json`.
...
When an external memory provider is active, it may register its own top-level `kopi <provider>` command for provider-specific management (e.g. `kopi honcho` when Honcho is active). Inactive providers do not expose their subcommands. Run `kopi --help` to see what's currently wired in.
:::
...
```bash
cd ~/.kopi/kopi-ai-agent && uv pip install -e '.[acp]'
```
...
```bash
kopi plugins [subcommand]
```

Unified plugin management — general plugins, memory providers, and context engines in one place. Running `kopi plugins` with no subcommand opens a composite interactive screen with two sections:
...
`kopi computer-use install` is the stable entry point for installing the
[cua-driver](https://github.com/trycua/cua) binary used by the
`computer_use` toolset. It runs the same upstream installer that
`kopi tools` invokes when you first enable Computer Use, so it's safe
...
```bash
kopi insights [--days N] [--source platform]
```
...
kopi claw migrate --dry-run

# Full migration (all compatible settings, no secrets)
kopi claw migrate --preset full

# Full migration including API keys
kopi claw migrate --preset full --migrate-secrets

# Migrate user data only (no secrets), overwrite conflicts
kopi claw migrate --preset user-data --overwrite

# Migrate from a custom OpenClaw path
kopi claw migrate --source /home/user/old-openclaw
```
...
```bash
kopi dashboard [options]
```

Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. (For a headless backend with no browser UI — e.g. what the desktop app spawns — use [`kopi serve`](#kopi-serve) above.) Requires `cd ~/.kopi/kopi-ai-agent && uv pip install -e ".[web]"` (FastAPI + Uvicorn). The embedded browser Chat tab is always available and additionally needs the `pty` extra (`cd ~/.kopi/kopi-ai-agent && uv pip install -e ".[web,pty]"`) plus a POSIX PTY environment such as Linux, macOS, or WSL2. See [Web Dashboard](/user-guide/features/web-dashboard) for full documentation.
...
| `--stop` | — | Stop running `kopi dashboard` processes and exit. |
| `--status` | — | List running `kopi dashboard` processes and exit. |
...
kopi dashboard --port 8080 --no-open
...
kopi profile alias work --name h-work
kopi profile export work -o work-backup.tar.gz
kopi profile import work-backup.tar.gz --name restored
kopi profile install github.com/user/my-distro --alias
kopi profile update work
kopi -p work chat -q "Hello from work profile"
```
...
```bash
kopi update [--gateway] [--check] [--no-backup] [--backup] [--yes]
```
...
| `--backup` | Create a labeled pre-update snapshot of `KOPI_HOME` (config, auth, sessions, skills, pairing data) before pulling. Default is **off** — the previous always-backup behavior was adding minutes to every update on large homes. Flip it on permanently via `updates.pre_update_backup: true` in `config.yaml`. |
| `--yes`, `-y` | Assume yes for interactive prompts such as config migration and stash restore. API-key entry is skipped; run `kopi config migrate` separately for those. |
