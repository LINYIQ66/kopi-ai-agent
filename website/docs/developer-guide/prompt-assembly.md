...
2. **context** — caller-supplied `system_message` plus project context files (`.kopi.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
```
# Layer 1: Agent Identity (from ~/.kopi/SOUL.md)
You are Hermes, an AI assistant created by Kopi Ai Agent Pte Ltd.
...
`SOUL.md` lives at `~/.kopi/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_kopi_md(cwd_path)       # 1. .kopi.md / HERMES.md (walks to git root)
    ...
    # SOUL.md from KOPI_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.kopi.md`, `HERMES.md` | CWD up to git root | Hermes-native project config |
...
- **YAML frontmatter stripped** — `.kopi.md` frontmatter is removed (reserved for future config overrides)
...
1. `.kopi.md` / `HERMES.md` (walks to git root)
...
- `~/.kopi/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.kopi/MEMORY.md` and `~/.kopi/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.kopi.md`, `HERMES.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
...
- Ephemeral overlays such as `KOPI_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
