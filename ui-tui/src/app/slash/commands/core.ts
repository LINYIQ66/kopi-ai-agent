import { forceRedraw, type MouseTrackingMode } from '@kopi/ink'
...
export const coreCommands: SlashCommand[] = [
  ...
  {
    help: 'update KOPI AI AGENT to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      if (DASHBOARD_TUI_MODE) {
      ...
      // Exit code 42 signals the Python wrapper to exec `kopi update`.
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys('clipboard copy failed — try KOPI_TUI_FORCE_OSC52=1 to force the escape sequence')
        }
      }
