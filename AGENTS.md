# KOPI AI AGENT - Development Guide

Instructions for AI coding assistants and developers working on the kopi-ai-agent codebase.
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`kopi tools`, `kopi setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `KOPI_HOME`. Mocks hide integration bugs.
...
- **New `KOPI_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `kopi tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.kopi/plugins/` (or via a
  pip entry point), and promote them in the Kopi Ai Agent Pte Ltd Discord
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `kopi <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `kopi webhook`, `kopi cron`, `kopi tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.kopi/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.kopi/kopi-ai-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _KOPI_CORE_TOOLS list
├── cli.py                # HermesCLI class — interactive CLI orchestrator (~11k LOC)
├── kopi_state.py       # SessionDB — SQLite session store (FTS5 search)
├── kopi_constants.py   # get_kopi_home(), display_kopi_home() — profile-aware paths
├── kopi_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── kopi_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── kopi-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `kopi --tui`
...
**User config:** `~/.kopi/config.yaml` (settings), `~/.kopi/.env` (API keys only).
**Logs:** `~/.kopi/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_kopi_home()`.
Browse with `kopi logs [--follow] [--level ...] [--session ...]`.
...
- **Skin engine** (`kopi_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `HermesCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.kopi/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`kopi_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/kopi` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `kopi_cli/commands.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `kopi --tui` or `KOPI_TUI=1`.
...
npm run dev       # watch mode (rebuilds kopi-ink + tsx --watch)
npm start         # production
npm run build     # full build (kopi-ink + tsc)
...
### TUI in the Dashboard (`kopi dashboard` → `/chat`)

The dashboard embeds the real `kopi --tui` — **not** a rewrite.  See `kopi_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `kopi_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `kopi --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `kopi --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@kopi/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `kopi serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `KOPI_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.cjs` + `backendSupportsServe()` in `main.cjs`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `kopi` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `kopi --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `kopi-desktop-app-work` skill, not `kopi-dashboard-work`.
...
Use the plugin route instead: create `~/.kopi/plugins/<name>/plugin.yaml`
and `~/.kopi/plugins/<name>/__init__.py`, then register tools with
...
**2. Add to `toolsets.py`** — either `_KOPI_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_KOPI_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_kopi_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `KOPI_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_kopi_home()` for the base directory — never `Path.home() / ".kopi"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `kopi_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `kopi_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `kopi tools`, `kopi setup`, most CLI subcommands | `kopi_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`kopi_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
kopi_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.kopi/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
Add to `_BUILTIN_SKINS` dict in `kopi_cli/skin_engine.py`:
...
Users create `~/.kopi/skins/<name>.yaml`:
...
`~/.kopi/plugins/` and pip-installed entry points.

### General plugins (`kopi_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.kopi/plugins/`, `./.kopi/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `kopi` at startup so
  `kopi <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(kopi_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `kopi <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `kopi --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `kopi_cli/main.py`, etc.).
...
into `~/.kopi/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `kopi memory setup` / `post_setup()` without
...
`~/.kopi/plugins/` (or via pip entry points). They register through
...
don't own. Promote standalone plugins in the Kopi Ai Agent Pte Ltd Discord
...
2. User: `$KOPI_HOME/plugins/model-providers/<name>/`
...
[`kopi-example-plugins`](https://github.com/LINYIQ66/kopi-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `kopi skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.kopi.tags`, `metadata.kopi.category`,
`metadata.kopi.related_skills`, `metadata.kopi.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.kopi.*` by the loader.
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "KOPI AI AGENT" is the secondary collaborator. If the
   contributor's commit shows "KOPI AI AGENT" as author (because they
...
lives in the `kopi-ai-agent-dev` skill at
...
`"messaging"`); `_KOPI_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `kopi tools` (the curses UI) or the
...
go to `~/.kopi/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `kopi_cli/curator.py` wires `kopi curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.kopi/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `kopi cron <verb>`
...
- File lock at `~/.kopi/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `kopi kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `kopi_cli/kanban.py` wires `kopi kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`kopi-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `KOPI_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
in config.yaml (or `KOPI_BACKGROUND_NOTIFICATIONS` env var):
...
`KOPI_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `kopi_cli/main.py` sets
`KOPI_HOME` before any module imports. All `get_kopi_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_kopi_home()` for all KOPI_HOME paths.** Import from `kopi_constants`.
   NEVER hardcode `~/.kopi` or `Path.home() / ".kopi"` in code that reads/writes state.
   ```python
   # GOOD
   from kopi_constants import get_kopi_home
   config_path = get_kopi_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".kopi" / "config.yaml"
   ```

2. **Use `display_kopi_home()` for user-facing messages.** Import from `kopi_constants`.
   This returns `~/.kopi` for default or `~/.kopi/profiles/<name>` for profiles.
   ```python
   # GOOD
   from kopi_constants import display_kopi_home
   print(f"Config saved to {display_kopi_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.kopi/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_kopi_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_kopi_home()`,
   not `Path.home() / ".kopi"`.

4. **Tests that mock `Path.home()` must also set `KOPI_HOME`** — since code now uses
   `get_kopi_home()` (reads env var), not `Path.home() / ".kopi"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"KOPI_HOME": str(tmp_path / ".kopi")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not KOPI_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".kopi" / "profiles"`, NOT `get_kopi_home() / "profiles"`.
   This is intentional — it lets `kopi -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.kopi` paths
Use `get_kopi_home()` from `kopi_constants` for code paths. Use `display_kopi_home()`
for user-facing print/log messages. Hardcoding `~/.kopi` breaks profiles — each profile
has its own `KOPI_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `kopi_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `kopi_cli/curses_ui.py` — see
`kopi_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `KOPI_HOME`.

### Tests must not write to `~/.kopi/`
The `_isolate_kopi_home` autouse fixture in `tests/conftest.py` redirects `KOPI_HOME` to a temp dir. Never hardcode `~/.kopi/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_kopi_home()` resolve within the temp dir.
Use the pattern from `tests/kopi_cli/test_profiles.py`:
```python
@pytest.fixture
...
| HOME / `~/.kopi/` | Your real config+auth.json                  | Temp dir per test                         |
