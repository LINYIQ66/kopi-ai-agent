"""
Status command for kopi CLI.

Shows the status of all KOPI AI AGENT components.
"""
...
from kopi_cli.auth import AuthError, resolve_provider
from kopi_cli.colors import Colors, color
from kopi_cli.config import get_env_path, get_env_value, get_kopi_home, load_config
...
from kopi_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``kopi config``'s
...
def show_status(args):
    """Show status of all KOPI AI AGENT components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ KOPI AI AGENT Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: kopi model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from kopi_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from kopi_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_kopi_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_kopi_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'kopi doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'kopi setup' to configure", Colors.DIM))
    print()
