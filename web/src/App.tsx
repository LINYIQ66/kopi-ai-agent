...
const SIDEBAR_COLLAPSED_KEY = "kopi-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="kopi-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="kopi-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateHermes = status?.can_update_kopi === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'kopi update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "kopi update";
    return (
      t.status.updateHermesConfirmMessage ??
      `This will run 'kopi update' (${cmd}) and restart the gateway when it finishes.`
    );
