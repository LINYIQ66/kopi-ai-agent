...
sidebar_position: 11
title: "ACP 编辑器集成"
description: "在 VS Code、Zed 和 JetBrains 等兼容 ACP 的编辑器中使用 KOPI AI AGENT"
---

# ACP 编辑器集成

KOPI AI AGENT 可作为 ACP 服务器运行，让兼容 ACP 的编辑器通过 stdio 与 Hermes 通信并渲染：
...
Hermes 使用专为编辑器工作流设计的精选 `kopi-acp` 工具集运行，包括：
...
```bash
kopi acp --setup-browser           # 交互式（下载约 400 MB 前会提示确认）
kopi acp --setup-browser --yes     # 非交互式接受下载
```

这是独立命令。Zed registry 的终端认证流程（`kopi acp --setup`）在模型选择后也会将浏览器引导作为后续问题提供，因此大多数用户无需直接运行 `--setup-browser`。

具体操作：

- 若缺少 Node.js 22 LTS，将其安装到 `~/.kopi/node/`
...
2. 从内置 agent 列表中选择 **KOPI AI AGENT**。
3. 连接并开始聊天。
...
- 先通过 `kopi model` 配置 Hermes provider 凭据，或在 `~/.kopi/.env` / `~/.kopi/config.yaml` 中设置。
- 安装 `uv`，以便 registry 启动器可以运行 `uvx --from 'kopi-ai-agent[acp]==<version>' kopi-acp`。
...
上游 registry PR 将这些文件复制到 `agentclientprotocol/registry` 中的顶层 `kopi-ai-agent/` 目录。

Registry 条目使用直接指向 `kopi-ai-agent` PyPI 发行版的 `uvx` 发行版：
...
- 在 Zed 中，使用 `zed: acp registry` 打开 ACP Registry 并搜索 **KOPI AI AGENT**。
- 对于手动/本地开发，验证自定义 `agent_servers` 命令是否指向 `kopi acp`。
- Hermes 已安装且在 PATH 中。
...
或编辑 `~/.kopi/.env`。Registry 客户端也可以触发 Hermes 的终端认证流程，该流程运行相同的交互式 provider/模型设置。

### Zed registry 启动器找不到 uv

从官方 uv 安装文档安装 `uv`，然后从 Zed 重试 KOPI AI AGENT 线程。
