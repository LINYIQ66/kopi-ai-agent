...
Entry points (``cli.py`` ``main``, ``kopi_cli/main.py`` CLI dispatch,
...
def configure_windows_stdio() -> bool:
    ...
    Set ``KOPI_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("KOPI_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
...
def _default_windows_editor() -> str:
    ...
    ``kopi config edit`` both honour ``$EDITOR``.  Users who prefer a
...
def _augment_path_with_known_tools() -> None:
    ...
    ``%LOCALAPPDATA%\\kopi\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``kopi`` from right after install) retain their old PATH.
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "kopi", "git", "cmd"),
        os.path.join(local_appdata, "kopi", "git", "bin"),
        os.path.join(local_appdata, "kopi", "git", "usr", "bin"),
        # Hermes venv Scripts directory — host of the kopi.exe shim itself,
        ...
        os.path.join(local_appdata, "kopi", "kopi-ai-agent", "venv", "Scripts"),
