...
When ``KOPI_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.kopi/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.kopi            (has its own auth.json fixture)
    * Profile     -> tmp_path/.kopi/profiles/coder   (active, KOPI_HOME points here)
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from kopi_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    kopi_home = tmp_path / "classic"
    kopi_home.mkdir()
    monkeypatch.setenv("KOPI_HOME", str(kopi_home))

    _write(kopi_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (KOPI_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.kopi as the "real home" — which is NOT equal
    # to the KOPI_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    kopi_home = tmp_path / "classic"
    ...
    _write(kopi_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from kopi_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: KOPI_HOME is set to a custom path that is NOT under
    # ~/.kopi/profiles/ — get_default_kopi_root() returns KOPI_HOME
