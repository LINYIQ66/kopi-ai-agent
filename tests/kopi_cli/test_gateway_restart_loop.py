...
Covers:
- Defense 1: gateway stop/restart refuse when _KOPI_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.kopi.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.kopi.gateway.plist",
        "launchctl stop ai.kopi.gateway",
        "systemctl restart kopi-gateway",
        "systemctl stop kopi-gateway.service",
    ...
    @pytest.mark.parametrize("text", [
        ...
        "kopi cron list",
        "kopi update",
        "kopi config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `kopi gateway start` is benign — starting a gateway from inside a
        ...
        "systemctl restart kopi-meta.service",
        "systemctl restart kopi-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_kopi_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade kopi then run kopi gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.kopi.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_kopi_gateway_once.sh). The script must live under
        # KOPI_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("KOPI_HOME", str(tmp_path / ".kopi"))
        scripts_dir = tmp_path / ".kopi" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify kopi gateway stop/restart refuse when _KOPI_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_KOPI_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_KOPI_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _KOPI_GATEWAY=1.

    Issue #37453: systemctl --user restart kopi-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart kopi-gateway",
        "systemctl --user restart kopi-gateway",
        "systemctl stop kopi-gateway.service",
        "kopi gateway restart",
        "launchctl kickstart gui/501/ai.kopi.gateway",
        "pkill -f kopi.*gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart kopi-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-kopi systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _KOPI_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart kopi-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart kopi-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run kopi gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("kopi gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfekopi gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under KOPI_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.kopi.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    kopi_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run kopi gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KOPI_HOME", str(tmp_path / ".kopi"))
        (tmp_path / ".kopi").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run kopi gateway restart nightly",
        ))
