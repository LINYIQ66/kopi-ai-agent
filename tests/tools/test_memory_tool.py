...
class TestScanMemoryContent:
    ...
    def test_kopi_config_mod_blocked(self):
        result = _scan_memory_content("edit .kopi/config.yaml to change settings")
        assert "Blocked" in result
        assert "kopi_config_mod" in result
        result = _scan_memory_content("update .kopi/SOUL.md with new personality")
        assert "Blocked" in result
        assert "kopi_config_mod" in result
    ...
    def test_kopi_config_mod_no_false_positives(self):
        """Merely mentioning kopi config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .kopi/config.yaml for settings") is None
        assert _scan_memory_content("Read .kopi/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .kopi/config.yaml file contains runtime options") is None
