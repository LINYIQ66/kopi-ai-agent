...
def kopi_home(tmp_path, monkeypatch):
    ...
    # /etc/kopi on the dev/CI box can't influence the test.
    monkeypatch.setenv("KOPI_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(kopi_home, monkeypatch):
    from kopi_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(kopi_home, monkeypatch):
    from kopi_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(kopi_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_kopi_dotenv(kopi_home=str(home))
