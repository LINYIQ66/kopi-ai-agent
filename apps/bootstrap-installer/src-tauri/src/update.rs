...
//!      rebuild apps/desktop by design — see cmd_update in kopi_cli/main.py),
//!   3. run `kopi desktop --build-only` (the rebuild step update skips),
...
/// `kopi update` exit code meaning "another kopi process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// kopi_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `kopi update`'s own guard decide.
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    // async on Windows. If it still holds the venv shim, `kopi update`
    ...
    // ---- stage 2: kopi update -----------------------------------------
    // Pass --branch so `kopi update` targets the branch this installer was
    ...
    // without --branch, `kopi update` switches the checkout to `main` (a
    ...
    // --force skips `kopi update`'s Windows running-exe guard (which would
    ...
    // Retry-once for the update-boundary crash. `kopi update` lazily imports
    ...
    match update.exit_code {
        ...
        other => {
            let msg = format!(
                "kopi update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::kopi_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: kopi desktop --build-only ----------------------------
    // `kopi update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `kopi update` already does above, and `kopi update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `kopi desktop` \
    ...
    if let Some(target_app) = launch_target {
        ...
        crate::bootstrap::launch_kopi_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
/// Spawn `kopi <args>` from `cwd`, stream stdout/stderr as Log events on the
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        "KOPI_HOME".to_string(),
        kopi_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        kopi_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let tmp = PathBuf::from(format!("{}.kopi-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.kopi-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::kopi_home())
        .status()
        .await
...
mod tests {
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".kopi-update-in-progress");
