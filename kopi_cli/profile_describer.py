...
Used by ``kopi profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``kopi_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the KOPI AI AGENT kanban board.
...
Rules:
  ...
  - Never write "KOPI AI AGENT profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_kopi_home())
        else:
