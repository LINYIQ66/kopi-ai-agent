...
const KOPI_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type HermesRefType = (typeof KOPI_REF_TYPES)[number]
...
const KOPI_DIRECTIVE_RE = new RegExp(
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(KOPI_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...kopiDirectiveFormatter.parse(text)]
  } catch {
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.kopiDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.kopiDesktop?.readFileDataUrl(id)
