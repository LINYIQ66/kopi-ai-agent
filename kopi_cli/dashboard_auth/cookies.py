...
Three cookies in play:
  - kopi_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - kopi_session_rt:   the OAuth refresh token
  ...
  - kopi_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
setter wrote ``__Secure-kopi_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "kopi_session_at"
SESSION_RT_COOKIE = "kopi_session_rt"
PKCE_COOKIE = "kopi_session_pkce"
...
SSO_ATTEMPT_COOKIE = "kopi_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /kopi`` we want ``Path=/kopi`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (kopi #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/kopi``)
