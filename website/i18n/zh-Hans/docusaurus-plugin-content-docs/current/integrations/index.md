...
KOPI AI AGENT 可连接外部系统，用于 AI 推理、工具服务器、IDE 工作流、程序化访问等。这些集成扩展了 Hermes 的能力边界与运行环境。

## AI 提供商与路由

Hermes 开箱即支持多个 AI 推理提供商。使用 `kopi model` 进行交互式配置，或在 `config.yaml` 中直接设置。
...
`web_search` 和 `web_extract` 工具支持四个后端提供商，通过 `config.yaml` 或 `kopi tools` 配置：
...
- **[IDE 集成（ACP）](/user-guide/features/acp)** — 在兼容 ACP 的编辑器（如 VS Code、Zed 和 JetBrains）中使用 KOPI AI AGENT。Hermes 作为 ACP 服务器运行，在编辑器内渲染聊天消息、工具活动、文件差异和终端命令。
...
- **[插件系统](/user-guide/features/plugins)** — 无需修改核心代码，通过自定义工具、生命周期 hook（钩子）和 CLI 命令扩展 Hermes。插件从 `~/.kopi/plugins/`、项目本地 `.kopi/plugins/` 以及通过 pip 安装的入口点自动发现。
