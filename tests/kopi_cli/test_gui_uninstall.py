...
def test_agent_is_installed_detects_source_and_venv(tmp_path):
    ...
    kopi_home.mkdir()
    assert gu.agent_is_installed(kopi_home) is False
    _make_agent(kopi_home)
    assert gu.agent_is_installed(kopi_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    kopi_home = tmp_path / ".kopi"
    (kopi_home / "kopi-ai-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(kopi_home) is True
...
def test_source_built_artifacts_lists_known_paths(tmp_path):
    ...
    artifacts = gu.source_built_gui_artifacts(kopi_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(kopi_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(kopi_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    kopi_home = tmp_path / ".kopi"
    agent_root = _make_agent(kopi_home)
    _make_gui_build(kopi_home)
    _make_user_data(kopi_home)
    ...
    removed = gu.uninstall_gui(kopi_home)
    ...
    assert not (kopi_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "kopi_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (kopi_home / "config.yaml").exists()
    assert (kopi_home / ".env").exists()
    assert (kopi_home / "sessions").exists()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(kopi_home, remove_userdata=False)
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    KOPI_HOME with all the destructive externals stubbed out.
    """
    ...
    monkeypatch.setattr(uninstall, "get_kopi_home", lambda: kopi_home)
    ...
    from kopi_cli import gui_uninstall as gu_mod
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole KOPI_HOME non-interactively."""
    import kopi_cli.uninstall as uninstall

    kopi_home = tmp_path / ".kopi"
    (kopi_home / "kopi-ai-agent" / "kopi_cli").mkdir(parents=True)
    (kopi_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not kopi_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m kopi_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_kopi_home", lambda: kopi_home)
    ...
    assert not (desktop / "release").exists()
    assert not (kopi_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "kopi_cli").exists()
    assert (kopi_home / "config.yaml").exists()
