...
export const zh: Translations = {
  ...
  settings: {
    ...
    gateway: {
      ...
      envOverrideDesc: '取消设置 KOPI_DESKTOP_REMOTE_URL 和 KOPI_DESKTOP_REMOTE_TOKEN 后才会使用下面保存的设置。',
      localTitle: '本地网关',
      ...
      remoteUrlDesc: '远程 dashboard 后端的基础 URL。支持路径前缀，例如 /kopi。',
  ...
  skills: {
    ...
    skillArchivedMessage: '可通过 kopi curator restore 恢复。',
    hub: {
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      weixin:
        '运行 `kopi gateway setup`，选择 Weixin，然后使用个人微信账号扫描并确认二维码。Hermes 会通过腾讯 iLink Bot API 连接并保存凭据。',
  ...
  install: {
    ...
    failedTitle: '安装失败',
    settingUpTitle: '正在设置 KOPI AI AGENT',
    finishingTitle: '正在收尾',
    failedDesc:
  ...
  onboarding: {
    headerTitle: '开始设置 KOPI AI AGENT',
  ...
  assistant: {
    ...
    approval: {
      ...
      alwaysDescription: pattern =>
        `这会将“${pattern}”模式加入永久允许列表 (~/.kopi/config.yaml)。Hermes 对类似命令将不再询问，包括当前会话和未来会话。`,
      alwaysAllow: '始终允许'
    },
    clarify: {
      notReady: '澄清请求尚未就绪',
  ...
  desktop: {
    ...
    handoff: {
      ...
      timedOut: '等待网关超时。`kopi gateway` 是否正在运行？'
    }
  },
