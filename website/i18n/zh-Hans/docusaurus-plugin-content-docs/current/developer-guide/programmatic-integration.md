...
sidebar_position: 8
title: "程序化集成"
description: "从外部程序驱动 kopi-ai-agent 的三种协议：ACP、TUI gateway JSON-RPC 以及兼容 OpenAI 的 HTTP API"
---
...
`kopi acp` 启动一个基于 stdio 的 JSON-RPC 服务器，使用 ACP 协议。已在 VS Code（Zed Industries 的 ACP 扩展）、Zed 以及所有安装了 ACP 插件的 JetBrains IDE 中投入生产使用。
...
```bash
kopi acp                  # 在 stdio 上提供 ACP 服务
kopi acp --bootstrap      # 打印适用于支持 ACP 的 IDE 的安装代码片段
```
...
`tui_gateway/server.py` 是 Ink TUI（`kopi --tui`）和嵌入式仪表板 PTY 桥接所使用的协议。任何外部宿主均可通过 stdio（或经由 `tui_gateway/ws.py` 的 WebSocket）使用相同协议。
...
Pi-mono RPC 规范（[issue #360](https://github.com/LINYIQ66/kopi-ai-agent/issues/360)）中的每条命令均有对应的 TUI gateway 等价项：
...
GET  /v1/models                  列出 kopi-ai-agent
...
内置 provider 感知解析（相同的模型名称会根据当前 provider 自动选择正确格式）。参见 `kopi_cli/model_switch.py`。
