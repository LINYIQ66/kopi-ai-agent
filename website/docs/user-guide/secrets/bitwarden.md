...
Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.kopi/.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
2. Hermes stores that single token in `~/.kopi/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `kopi` (or the gateway, or a cron job) starts, after `~/.kopi/.env` has loaded, Hermes calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
...
The `bws` binary is auto-downloaded into `~/.kopi/bin/` on first use — no `apt`, no `brew`, no `sudo`.
...
1. Download and verify `bws v2.0.0` into `~/.kopi/bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/.kopi/.env` as `BWS_ACCESS_TOKEN`.
...
```bash
kopi secrets bitwarden status
```

From now on, every `kopi` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `kopi secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `kopi secrets bitwarden status` | Show config + binary version + token presence |
| `kopi secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `kopi secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `kopi secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `kopi secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |
...
| `cache_ttl_seconds` | `300` | How long an in-process fetch result is reused. Set to `0` to disable caching. Cache is per-process; new `kopi` invocations start fresh. |
...
| `auto_install` | `true` | When true, `bws` is auto-downloaded into `~/.kopi/bin/` on first use. |
...
| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `kopi secrets bitwarden setup` |
...
- **Single-machine personal setups** where `~/.kopi/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
