...
Behavioral settings live in `$KOPI_HOME/mem0.json` (set them via `kopi memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.kopi/.env`.
...
| `user_id` | `kopi-user` | User identifier on Mem0 |
...
```bash
kopi memory setup mem0 --mode oss \
...
```bash
kopi memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$KOPI_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.kopi/mem0_qdrant"}}
  }
}
```
...
```bash
kopi memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$KOPI_HOME/mem0.json`).
