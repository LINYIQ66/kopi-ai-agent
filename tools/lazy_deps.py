"""
Lazy dependency installer for opt-in KOPI AI AGENT backends.
...
(``kopi-ai-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``kopi tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``KOPI_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/kopi`` read-only), setting
  ``KOPI_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (KOPI_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (kopi trace upload / /upload-trace).
...
_LAZY_TARGET_ENV = "KOPI_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``KOPI_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("KOPI_DISABLE_LAZY_INSTALLS") == "1":
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``kopi update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="kopi-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``kopi_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = kopi_subprocess_env(inherit_credentials=False)
...
def active_features() -> list[str]:
    ...
    Used by ``kopi update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``kopi update``. Never raises; lazy-install failures
