"""
IRC Platform Adapter for KOPI AI AGENT.
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "kopi-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :KOPI AI AGENT")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :KOPI AI AGENT shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: kopi_, kopi_1, kopi_2...
...
def check_requirements() -> bool:
    ...
    # kopi setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `kopi gateway setup` flow for the IRC platform.

    Lazy-imports ``kopi_cli.setup`` helpers so the plugin stays importable
    ...
    channel = prompt(
        "Channel to join (e.g. #kopi — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.kopi/.env")
    print_info("Restart the gateway for changes to take effect: kopi gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "kopi-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "kopi-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :KOPI AI AGENT (cron)")
