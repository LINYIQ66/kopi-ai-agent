...
Owner: kopi-achievements plugin
...
### `GET /api/plugins/kopi-achievements/achievements`
...
### `POST /api/plugins/kopi-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/kopi-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/kopi-achievements/overview`
...
- Persist plugin data under:
  - `~/.kopi/plugins/kopi-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.kopi/plugins/kopi-achievements/scan_checkpoint.json`
...
- `~/.kopi/plugins/kopi-achievements/`
