...
kopi auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
kopi auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
kopi auth add anthropic --type oauth
...
Run `kopi auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `kopi auth` | Interactive pool management wizard |
| `kopi auth list` | Show all pools and credentials |
| `kopi auth list <provider>` | Show a specific provider's pool |
| `kopi auth add <provider>` | Add a credential (prompts for type and key) |
| `kopi auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `kopi auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `kopi auth remove <provider> <index>` | Remove credential by 1-based index |
| `kopi auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `kopi auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `kopi model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via kopi model:
kopi auth list
# Shows:
...
kopi auth add Together.ai --api-key sk-together-second-key
```
...
| Hermes PKCE OAuth | `~/.kopi/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `kopi auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `kopi auth add`) are never auto-pruned.
...
2. **`kopi_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`kopi_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.kopi/auth.json` under the `credential_pool` key:
