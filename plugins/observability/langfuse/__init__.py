...
load when listed in ``plugins.enabled`` (via ``kopi plugins enable
observability/langfuse`` or ``kopi tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``kopi tools`` or ~/.kopi/.env):
  KOPI_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  KOPI_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  KOPI_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  KOPI_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  KOPI_LANGFUSE_RELEASE     - release/version tag
  KOPI_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  KOPI_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  KOPI_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("KOPI_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    public_key = _env("KOPI_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("KOPI_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("KOPI_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("KOPI_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset KOPI_LANGFUSE_PUBLIC_KEY / KOPI_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("KOPI_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("KOPI_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("KOPI_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("KOPI_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid KOPI_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("KOPI_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
