...
KOPI AI AGENT uses a dual compression system and Anthropic prompt caching to
...
Configure via `kopi plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.
...
| `codex_app_server_auto` | `native` | `native`, `kopi`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
notice is shown once per profile — a marker under `$KOPI_HOME`
...
```bash
kopi config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
kopi config set compression.codex_gpt55_autoraise_notice false
```
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  ...
  events). Set `kopi` to let Hermes' compression threshold initiate
