"""
Interactive setup wizard for KOPI AI AGENT.
...
from kopi_constants import get_optional_skills_dir
...
# display_kopi_home imported lazily at call sites (stale-module safety during kopi update)
...
from kopi_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from kopi_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print_info("  kopi config set model.provider custom")
    print_info("  kopi config set model.base_url http://localhost:8080/v1")
    print_info("  kopi config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'kopi setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``KOPI_NONINTERACTIVE=1`` (see ``kopi_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("KOPI_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``KOPI_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from kopi_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'kopi setup')")


def _print_setup_summary(config: dict, kopi_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'kopi setup' to configure"))
    ...
    # Video generation — opt-in via `kopi tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from kopi_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'kopi setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'kopi setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'kopi setup terminal'"))
    ...
    # Spotify (OAuth via kopi auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'kopi setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {kopi_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('kopi setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('kopi setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('kopi setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('kopi setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('kopi setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('kopi config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('kopi config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('kopi config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('kopi', Colors.GREEN)}              Start chatting")
    print(f"   {color('kopi gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('kopi doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `kopi tools` and `kopi setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``kopi model``)
    ...
    provider added to ``kopi model`` is automatically available here.
    ...
    # Delegate to the shared kopi model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from kopi_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: kopi model")
    ...
    # on demand via `kopi auth add`, `kopi setup` vision, and
    # `kopi setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from kopi_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``kopi model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from kopi_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.kopi/.env."
            )
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via kopi setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'kopi setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `kopi setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `kopi setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into KOPI_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("KOPI_MAX_ITERATIONS")
    ...
    print_info("  Run `kopi setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # KOPI_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.kopi/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from kopi_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_kopi_home(Path(get_kopi_home()))
    except Exception:
        pass
...
def _profile_name_from_kopi_home(kopi_home) -> str | None:
    """Return the active profile name when KOPI_HOME is a profile dir."""
    if kopi_home.parent.name == "profiles":
        return kopi_home.name
    return None
...
def _setup_qqbot():
    ...
    from kopi_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from kopi_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if not selected:
        print_info("No platforms selected. Run 'kopi setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     kopi config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_kopi_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: kopi gateway install --system")
                print_info("  Or run in foreground:  kopi gateway")
        else:
            from kopi_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   kopi gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   kopi gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `kopi setup tools` and `kopi tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``kopi_cli.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from kopi_cli.config
    ...
    if section_key == "model":
        ...
        from kopi_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_kopi migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_kopi", _OPENCLAW_SCRIPT
    )
...
def _offer_openclaw_migration(kopi_home: Path) -> bool:
    ...
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: kopi claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: kopi claw migrate"
        )
        print_info(
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hermes (use kopi claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``kopi setup --portal`` and ``kopi portal``. This is the
    ...
    ``kopi setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``kopi model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``kopi portal`` always offers a model picker,
    ...
    # setup calls, so `kopi portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `kopi portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `kopi portal`: %s", exc)
        print()
    ...
    print_info("  Run `kopi portal info` to inspect routing.")
    print_info("  Run `kopi` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      kopi setup           — full or quick (auto-detected)
      kopi setup model     — just model/provider
      kopi setup tts       — just text-to-speech
      kopi setup terminal  — just terminal backend
      kopi setup gateway   — just messaging platforms
      kopi setup tools     — just tool configuration
      kopi setup agent     — just agent settings
    """
    from kopi_cli.config import is_managed, managed_error
    if is_managed():
    ...
    print(
        color(
            "│             ⚕ KOPI AI AGENT Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your KOPI AI AGENT installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, kopi_home)
            return
        ...
        print_info("")
        print_info("Tip: jump straight to a section with 'kopi setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(kopi_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, kopi_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, kopi_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {kopi_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'kopi config edit'")
    ...
    # Tune later with `kopi setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, kopi_home)


def _run_first_time_quick_setup(config: dict, kopi_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``kopi setup <section>``
    or switch providers with ``kopi model``.
    """
    from kopi_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'kopi setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    kopi setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  kopi setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``kopi tools`` (which
    ...
    try:
        ...
        from kopi_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``kopi setup agent`` /
    ``kopi config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, kopi_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `kopi update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        kopi tools")
        print_info("  Seed skills:         kopi skills opt-in --sync")
        print_info("  Add MCP servers:     kopi mcp add")
        print_info("  Enable plugins:      kopi plugins")
        print_info("  Tune agent settings: kopi setup agent")
        print()
        _print_setup_summary(config, kopi_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, kopi_home)


def _blank_slate_walkthrough(config: dict, kopi_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`kopi update` runs from re-injecting them. Opt back in any")
            print_info("time with `kopi skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `kopi tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `kopi plugins list` / `kopi plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `kopi plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `kopi mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `kopi mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   kopi tools")
    print_info("  Seed skills:         kopi skills opt-in --sync")
    print_info("  Add MCP servers:     kopi mcp add")
    print_info("  Tune agent settings: kopi setup agent")
    print()
...
def _run_quick_setup(config: dict, kopi_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'kopi setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("You can configure these later with 'kopi setup gateway'.")
