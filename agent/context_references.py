...
_SENSITIVE_KOPI_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    kopi_home = get_kopi_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(kopi_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(kopi_home / rel for rel in _SENSITIVE_KOPI_DIRS)
    ...
    # message text into reference expansion, so `@file:~/.kopi/auth.json` from a
