...
export default function SystemPage() {
  ...
  // spawned `kopi import` runs non-interactively (stdin is /dev/null), so
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_kopi === false) return;
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_kopi === false) {
      showToast(
    ...
    try {
      ...
      setActiveAction(resp.name ?? "kopi-update");
  ...
  const canUpdateHermes = status?.can_update_kopi !== false;
      ...
      <ConfirmDialog
        ...
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'kopi update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'kopi update' (${updateInfo?.update_command ?? "kopi update"}) and restart the gateway when it finishes.`
        }
          ...
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <span>v{stats?.kopi_version}</span>
                  {canUpdateHermes &&
                    updateInfo &&
          ...
          <CardContent className="flex flex-col gap-3 py-4">
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Log in with <span className="font-mono">kopi portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$KOPI_HOME/backups/kopi-backup.zip"
                />
              </div>
              <Button
                size="sm"
