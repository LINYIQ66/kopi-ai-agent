...
2. **Finds or creates** the `KOPI AI AGENT` project on your account.
...
Runtime credentials are written to `~/.kopi/.env`
...
(device token, dashboard project id) lives in `~/.kopi/auth.json` under
...
```bash
kopi pairing approve photon <CODE>
```

Use `kopi pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.kopi/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.kopi/.env`):
...
- **`sidecar deps : ✗ run kopi photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `kopi photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `kopi photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `kopi photon install-sidecar` completed without errors.
