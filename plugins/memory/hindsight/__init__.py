...
Or via $KOPI_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $KOPI_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_kopi_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "kopi": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "kopi"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    ...
    profile = config.get("profile", "kopi")
    return str(profile or "kopi")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    collapsed — e.g. ``kopi-{user}`` with no user becomes ``kopi``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between KOPI AI AGENT and the User"
    ...
    def save_config(self, values, kopi_home):
        """Write config to $KOPI_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(kopi_home) / "hindsight"
    ...
    def post_setup(self, kopi_home: str, config: dict) -> None:
        ...
        from kopi_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "kopi")
        ...
        self.save_config(provider_config, kopi_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(kopi_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(kopi_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "kopi"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: kopi-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between KOPI AI AGENT and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "kopi"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "kopi"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "kopi", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "kopi")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between KOPI AI AGENT and the User")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "to cloud / local_external mode via 'kopi memory setup'."
                )
                logger.warning(msg)
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See kopi-ai-agent#6672.
