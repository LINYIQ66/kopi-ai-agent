"""
Top-level argparse construction for the kopi CLI.
...
# argparse runs (it sets ``KOPI_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``kopi_cli.relaunch`` should carry over when
...
Examples:
    ...
    kopi chat -q "Hello"        Single query mode
    kopi --tui                  Launch the modern TUI (or set display.interface: tui)
    kopi --cli                  Force the classic REPL (overrides display.interface: tui)
    kopi -c                     Resume the most recent session
    kopi -c "my project"        Resume a session by name (latest in lineage)
    kopi --resume <session_id>  Resume a specific session by ID
    kopi setup                  Run setup wizard
    kopi logout                 Clear stored authentication
    kopi auth add <provider>    Add a pooled credential
    kopi auth list              List pooled credentials
    kopi auth remove <p> <t>    Remove pooled credential by index, id, or label
    kopi auth reset <provider>  Clear exhaustion status for a provider
    kopi model                  Select default model
    kopi fallback [list]        Show fallback provider chain
    kopi fallback add           Add a fallback provider (same picker as `kopi model`)
    kopi fallback remove        Remove a fallback provider from the chain
    kopi config                 View configuration
    ...
    kopi sessions browse        Interactive session picker
    kopi sessions rename ID T   Rename/title a session
    kopi logs                   View agent.log (last 50 lines)
    kopi logs -f                Follow agent.log in real time
    ...
    kopi dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    # Mirrors `kopi chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via KOPI_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `kopi setup` or edit the file to change it."
        ),
    )
    parser.add_argument(
        "-t",
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to KOPI_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.kopi/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also KOPI_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.kopi/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
