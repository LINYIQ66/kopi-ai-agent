...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for KOPI AI AGENT. Use 'kopi debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    kopi debug share              Upload debug report (asks for confirmation)
    kopi debug share --yes        Skip confirmation (for scripts/CI)
    kopi debug share --lines 500  Include more log lines
    kopi debug share --expire 30  Keep paste for 30 days
    kopi debug share --local      Print report locally (no upload)
    kopi debug share --no-redact  Disable upload-time secret redaction
    kopi debug share --nous       Upload to Nous-internal storage (private)
    kopi debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'kopi debug share'",
    )
