...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.kopi/kopi-ai-agent/utils.py)
...
2. ``kopi update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``kopi_cli/main.py`` (the ``kopi update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``kopi_constants`` after the
