...
| | |
|---|---|
| 来源 | 可选 — 通过 `kopi skills install official/autonomous-ai-agents/honcho` 安装 |
...
| 相关 skills | [`kopi-ai-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-kopi-ai-agent) |
...
参见：https://docs.honcho.dev/v3/guides/integrations/kopi#running-honcho-locally-with-hermes
...
手动覆盖：`kopi honcho map my-project-name`
...
配置文件：`$KOPI_HOME/honcho.json`（配置文件本地）或 `~/.honcho/config.json`（全局）。
...
运行 `kopi honcho setup`。确保 `~/.kopi/config.yaml` 中包含 `memory.provider: honcho`。

### 记忆未跨会话持久化
检查 `kopi honcho status` -- 验证 `saveMessages: true` 且 `writeFrequency` 不是 `session`（该选项仅在退出时写入）。

### 配置文件未获得自己的 peer
创建时使用 `--clone`：`kopi profile create <name> --clone`。对于现有配置文件：`kopi honcho sync`。
...
| 命令 | 描述 |
|---------|-------------|
| `kopi honcho setup` | 交互式设置向导（云端/本地、身份、观察、召回、会话） |
| `kopi honcho status` | 显示当前配置文件的已解析配置、连接测试、peer 信息 |
| `kopi honcho enable` | 为当前配置文件启用 Honcho（如需则创建 host 块） |
| `kopi honcho disable` | 为当前配置文件禁用 Honcho |
| `kopi honcho peer` | 显示或更新 peer 名称（`--user <name>`、`--ai <name>`、`--reasoning <level>`） |
| `kopi honcho peers` | 显示所有配置文件的 peer 身份 |
| `kopi honcho mode` | 显示或设置召回模式（`hybrid`、`context`、`tools`） |
| `kopi honcho tokens` | 显示或设置 token 预算（`--context <N>`、`--dialectic <N>`） |
| `kopi honcho sessions` | 列出已知的目录到会话名称映射 |
| `kopi honcho map <name>` | 将当前工作目录映射到 Honcho 会话名称 |
| `kopi honcho identity` | 为 AI peer 身份播种，或显示两个 peer 的表示 |
| `kopi honcho sync` | 为所有尚未拥有 host 块的 Hermes 配置文件创建 host 块 |
| `kopi honcho migrate` | 从 OpenClaw 原生记忆迁移到 Hermes + Honcho 的分步指南 |
| `kopi memory setup` | 通用记忆提供商选择器（选择 "honcho" 运行相同向导） |
| `kopi memory status` | 显示当前活跃的记忆提供商及配置 |
| `kopi memory off` | 禁用外部记忆提供商 |