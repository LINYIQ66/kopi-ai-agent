...
description: "Step-by-step guide to running KOPI AI AGENT entirely on your own machine with Ollama and open-weight models like Gemma 4, no cloud API keys or paid subscriptions needed"
---
...
```bash
# ~/.kopi/.env
KOPI_API_TIMEOUT=1800   # 30 minutes — generous for slow local models
```
:::
...
Alternatively, edit `~/.kopi/config.yaml` directly:
...
2. Add to your `~/.kopi/config.yaml`:
