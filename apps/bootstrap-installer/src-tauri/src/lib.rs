...
pub fn run() {
    // Tracing → bootstrap-installer.log under KOPI_HOME/logs/ so install
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // startup-grace handling used by launch_kopi_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::kopi_home().join("kopi-ai-agent");
                if bootstrap::kopi_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_kopi_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "kopi already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
