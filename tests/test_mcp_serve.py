...
def _isolate_kopi_home(tmp_path, monkeypatch):
    """Redirect KOPI_HOME to a temp directory."""
    monkeypatch.setenv("KOPI_HOME", str(tmp_path))
    try:
        import kopi_constants
        monkeypatch.setattr(kopi_constants, "get_kopi_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking kopi_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <KOPI_HOME>/state.db for its mtime gate; the autouse
        # fixture points KOPI_HOME at tmp_path.
        db_path = tmp_path / "state.db"
