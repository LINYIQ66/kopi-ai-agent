...
// (e.g. https://mission-control.tilos.com/kopi/). The Python backend
// injects ``window.__KOPI_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__KOPI_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const KOPI_BASE_PATH = readBasePath();
const BASE = KOPI_BASE_PATH;
...
declare global {
  interface Window {
    __KOPI_SESSION_TOKEN__?: string;
    __KOPI_BASE_PATH__?: string;
...
export async function fetchJSON<T>(
  ...
  const token = window.__KOPI_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``kopi update``, ``kopi gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__KOPI_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__KOPI_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
 ...
 *
 * The dashboard's gated-mode WS auth (``kopi_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__KOPI_AUTH_REQUIRED__) {
 ...
 * ``window.__KOPI_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export interface SkillHubSource {
  ...
  /** kopi-index only: whether the centralized index loaded. */
  available?: boolean;
}
