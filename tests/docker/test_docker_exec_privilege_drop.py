...
The shim (docker/kopi-exec-shim.sh, installed at /opt/kopi/bin/kopi)
...
1. ``docker exec <c> kopi …`` (defaulting to root) gets dropped to the
   kopi user before the real binary runs.
2. ``docker exec --user kopi <c> kopi …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $KOPI_HOME from a ``docker exec`` session land
   as kopi:kopi — the actual user-visible invariant.
4. The KOPI_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-kopi-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown kopi:kopi`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to kopi:kopi,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$KOPI_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (kopi_cli.container_boot), which s6 runs
    *strictly after* ``01-kopi-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_kopi_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `kopi` with a Python-style `-c` shim equivalent — there's no
    pure-kopi "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `kopi` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `kopi`
    do anything that writes to disk, then check the file's owner.

    Use `kopi config set` which writes config.yaml under KOPI_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "kopi", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "kopi:kopi", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected kopi:kopi. "
        "The shim did not drop privileges before invoking kopi."
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/kopi/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid kopi kopi <args>` which resolves `kopi`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> kopi login`` (root) wrote
    ...
    `docker exec` path produces is kopi-readable.

    Specifically: pretend the operator ran `kopi login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the kopi UID. We use `kopi auth list` since that touches the
    ...
    # `kopi auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `kopi config set` first to
    # provoke a write into KOPI_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "kopi", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # KOPI_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "kopi", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the kopi user "
