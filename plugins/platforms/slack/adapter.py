...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `kopi slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_kopi_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /kopi prefix). A single regex
            ...
            # manifest (see `kopi slack manifest`). In Socket Mode, Slack
            ...
            from kopi_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/kopi$")

            @self._app.command(_slash_pattern)
            async def handle_kopi_command(ack, command):
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "kopi_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "kopi_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "kopi_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "kopi_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "kopi_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "kopi_confirm_once": "once",
            "kopi_confirm_always": "always",
            "kopi_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "kopi_approve_once": "once",
            "kopi_approve_session": "session",
            "kopi_approve_always": "always",
            "kopi_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/kopi <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/kopi
        ...
        if slash_name in {"kopi", ""}:
            # Legacy /kopi <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from kopi_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/kopi   ``).
        ...
        # "Running /cmd…" ack shown by handle_kopi_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/kopi <question>" must produce public replies so
...
# ``kopi_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``kopi_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under KOPI_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from kopi_cli.slack_cli import _build_full_manifest
            from kopi_constants import get_kopi_home
            import json as _json
            ...
            print_info(
                "   Re-run `kopi slack manifest --write` anytime to refresh after "
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `kopi update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://kopi-ai-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``kopi_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'kopi-ai-agent[slack]'",
        # Interactive setup wizard — replaces kopi_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in kopi_cli/gateway.py.
