...
existing Codex CLI tokens via `kopi auth openai-codex`. The old
...
@pytest.fixture()
def kopi_auth_only_env(tmp_path, monkeypatch):
...
def test_normal_path_still_works(kopi_auth_only_env):
...
def test_codex_picker_uses_live_codex_catalog(kopi_auth_only_env, tmp_path, monkeypatch):
