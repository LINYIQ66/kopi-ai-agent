...
4. Store the token in `${KOPI_HOME:-~/.kopi}/.env` (or via `kopi setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `${KOPI_HOME:-~/.kopi}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
