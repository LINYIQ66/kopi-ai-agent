...
test('desktop background child processes opt into hidden Windows consoles', () => {
  ...
  requireHiddenChildOptions(source, /kopiProcess = spawn\(\s*backend\.command,\s*backend\.args/)
  requireHiddenChildOptions(source, /spawn\(\s*py,\s*\['-m', 'kopi_cli\.main', 'uninstall', '--gui-summary'\]/)
  ...
  assert.match(source, /args: \['-m', 'kopi_cli\.main', \.\.\.backendArgs\]/)
})
...
test('desktop backend launches console python so child consoles are inherited, not pythonw', () => {
  ...
  // KOPI_DASHBOARD_READY stdout line — no ready-file side channel is set.
...
test('desktop backend teardown tree-kills Windows backend descendants', () => {
  ...
  assert.doesNotMatch(resetSnippet, /kopiProcess\.kill\('SIGTERM'\)/)
  ...
  assert.doesNotMatch(quitSnippet, /kopiProcess\.kill\('SIGTERM'\)/)
})
