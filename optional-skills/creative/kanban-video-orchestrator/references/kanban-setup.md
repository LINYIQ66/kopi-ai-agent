...
2. **Create profiles** — `kopi profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.kopi/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `kopi kanban create` assigned to the director
...
```bash
kopi profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.kopi/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.kopi/profiles/{profile}/config.yaml")
with open(p) as f:
...
Each profile gets a `SOUL.md` at `~/.kopi/profiles/<name>/SOUL.md` that
...
```bash
kopi kanban create "Direct production of <video title>" \
    --assignee director \
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _kopi_env="${KOPI_HOME:-$HOME/.kopi}/.env"
    if grep -q "^${var}=" "$_kopi_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_kopi_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_kopi_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY kopi ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY kopi OPENROUTER_API_KEY || exit 1
# ...
```
