...
Or set environment variables directly in `~/.kopi/.env`:
...
```bash
kopi pairing approve bluebubbles <CODE>
```
Use `kopi pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.kopi/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.kopi/.env`):
```bash
...
Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.kopi/config.yaml` (default: `true`). There is no corresponding environment variable.
...
- Check `kopi logs gateway` for webhook errors (or `kopi logs -f` to follow in real-time)
