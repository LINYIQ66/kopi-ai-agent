...
> Run `kopi gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
(Use `kopi gateway start` only after `kopi gateway install` has registered the systemd/launchd service.)
...
in `~/.kopi/.env` values around `=` will also break signature checks. After
fixing, restart `kopi gateway run`.
...
Check `kopi gateway run` logs for the bound host/port. If the adapter bound to
