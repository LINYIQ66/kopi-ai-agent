...
1. The agent writes a Python script using `from kopi_tools import ...`
2. Hermes generates a `kopi_tools.py` stub module with RPC functions
...
from kopi_tools import web_search, web_extract
...
```python
from kopi_tools import search_files, read_file
import json
...
```python
from kopi_tools import search_files, read_file, patch
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.kopi/config.yaml`:
...
### `KOPI_*` variables in the child

The child process receives only a small, fixed set of operational `KOPI_*`
...
(plus `KOPI_RPC_DIR` / `KOPI_RPC_SOCKET` / `TZ` / `HOME`, which Hermes
injects explicitly so the RPC channel works).

:::note Behavior change
Earlier versions passed **any** variable whose name began with `KOPI_`
through to the child. That broad prefix was removed for security hardening: it
could leak `KOPI_*`-named configuration that doesn't match a secret substring
(for example `KOPI_BASE_URL`, `KOPI_KANBAN_DB`, or a `KOPI_*_WEBHOOK`
endpoint) into arbitrary sandboxed code.

If an `execute_code` script — or a repo/plugin module it imports at import time
— relied on a `KOPI_*` variable outside the four operational names above, it
...
**Diagnosing it.** When the child drops one or more non-allowlisted `KOPI_*`
variables, Hermes emits a one-line `debug` log naming them and pointing at the
`env_passthrough` escape hatch. Run with debug logging (`kopi logs --level
DEBUG`, or check `~/.kopi/logs/agent.log`) and look for
`execute_code: dropped N non-allowlisted KOPI_* var(s)` if a script behaves
as though a `KOPI_*` variable is missing.

Hermes always writes the script and the auto-generated `kopi_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
