...
Handles: kopi honcho setup | status | sessions | map | peer
"""
...
from kopi_cli.config import cfg_get
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "kopi" else ""
...
def cmd_sync(args) -> None:
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'kopi honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'kopi honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `kopi update` -- no output, no exceptions.
    """
    try:
        from kopi_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _local_config_path() -> Path:
    ...
    Always returns $KOPI_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_kopi_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        kopi_host.get("pinUserPeer"),
        kopi_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in kopi_host:
        aliases_src = kopi_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in kopi_host:
        prefix_src = kopi_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(kopi_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom kopi_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    kopi_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        kopi_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(kopi_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = kopi_host.get("userPeerAliases")
    prefix = kopi_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(kopi_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(kopi_host)
    kopi_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        kopi_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        kopi_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    if is_local:
        ...
        current_host_key = kopi_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            kopi_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(kopi_host)
        ...
        if use_oauth:
            ...
            try:
                ...
                print("  Re-run 'kopi honcho setup' to retry, or choose an API key instead.\n")
                return
            kopi_host["apiKey"] = cred.access_token
            kopi_host["oauth"] = cred.oauth_block()
            # Default the peer prompt to the name entered at consent.
            if cred.consent_peer_name:
                kopi_host["peerName"] = cred.consent_peer_name
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'kopi honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = kopi_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        kopi_host["peerName"] = new_peer

    current_ai = kopi_host.get("aiPeer") or cfg.get("aiPeer", "kopi")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        kopi_host["aiPeer"] = new_ai

    current_workspace = kopi_host.get("workspace") or cfg.get("workspace", "kopi")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        kopi_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, kopi_host)
    ...
    if run_mapping:
        peer_target = kopi_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(kopi_host)
            kopi_host["pinUserPeer"] = False
            if prior_aliases:
                kopi_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                kopi_host, current_prefix, prefix_from_root,
            ...
            if merged:
                kopi_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                kopi_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                kopi_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = kopi_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        kopi_host["observationMode"] = new_obs
    else:
        kopi_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(kopi_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        kopi_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        kopi_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = kopi_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        kopi_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = kopi_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        kopi_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                kopi_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(kopi_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            kopi_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        kopi_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        kopi_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        kopi_host["dialecticReasoningLevel"] = new_reasoning
    else:
        kopi_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = kopi_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        kopi_host["sessionStrategy"] = new_strat

    kopi_host["enabled"] = True
    kopi_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        kopi_config = load_config()
        kopi_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(kopi_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: kopi config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    kopi honcho status     -- show full config")
    print("    kopi honcho mode       -- change recall/observation mode")
    print("    kopi honcho tokens     -- tune context and dialectic budgets")
    print("    kopi honcho peer       -- update peer names")
    print("    kopi honcho map <name> -- map this directory to a session name\n")
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: kopi honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'kopi honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'kopi honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: kopi honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        kopi = hosts.get(_host_key(), {})
        user = kopi.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = kopi.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = kopi.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = kopi.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Hermes' identity in Honcho. Seed with 'kopi honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: kopi honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: kopi honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = kopi.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = kopi.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = kopi.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("\n  Set with: kopi honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'kopi honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'kopi honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    kopi honcho identity --show        — show both peer representations")
        print("    kopi honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  kopi honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'kopi honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'kopi honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  kopi honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'kopi'")
        ...
        for f in user_files:
            print("    kopi honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'kopi honcho setup' first, then re-run this step.")
    else:
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'kopi honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    kopi honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  kopi honcho identity <path/to/SOUL.md>")
    ...
    print("              Map a custom name:  kopi honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. kopi honcho setup              — configure API key (required)")
        print("  2. kopi honcho migrate            — re-run this walkthrough")
    else:
        print("  1. kopi honcho status             — verify Honcho connection")
        print("  2. kopi                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. kopi honcho identity --show    — verify AI peer representation")
        print("  4. kopi honcho tokens             — tune context and dialectic budgets")
        print("  5. kopi honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'kopi memory setup'...\n")
        from kopi_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``kopi honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``kopi honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to kopi memory setup)",
    )
