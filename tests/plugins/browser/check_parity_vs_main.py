...
Run from the PR worktree:
    ...
    cd ~/.kopi/kopi-ai-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.kopi/kopi-ai-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.kopi/kopi-ai-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a kopi-ai-agent checkout"
)
...
# Isolated KOPI_HOME for the config write.
