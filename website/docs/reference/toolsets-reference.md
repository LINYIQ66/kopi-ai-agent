...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `kopi-cli` is the default for interactive CLI sessions)
...
```bash
kopi chat --toolsets web,file,terminal
kopi chat --toolsets debugging        # composite — expands to file + terminal + web
kopi chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - kopi-cli          # default for CLI
  # - kopi-telegram   # override for Telegram gateway
```

### Interactive management

```bash
kopi tools                            # curses UI to enable/disable per platform
```
...
| `discord` | `discord` | Core Discord text/embed/DM actions (gateway-only). Active on the `kopi-discord` toolset. |
| `discord_admin` | `discord_admin` | Discord moderation (bans, role changes, channel management). Active on the `kopi-discord` toolset; requires the bot to hold the relevant Discord permissions. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `kopi-cli` or other messaging toolsets. |
...
| `kanban` | `kanban_block`, `kanban_comment`, `kanban_complete`, `kanban_create`, `kanban_heartbeat`, `kanban_link`, `kanban_list`, `kanban_show`, `kanban_unblock` | Multi-agent coordination tools. Registered for dispatcher-spawned task workers (`KOPI_KANBAN_TASK`) and for profiles that explicitly list the `kanban` toolset by name (the `all`/`*` wildcard does **not** enable it). Workers mark tasks done, block, heartbeat, comment, and create/link follow-up tasks; orchestrator profiles additionally get board-routing tools like list/unblock. |
...
| `x_search` | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Off by default; opt in via `kopi tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | Yuanbao DM/group actions and sticker search. Registered only on `kopi-yuanbao`. |

## Platform Toolsets

Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `kopi-cli`:

| Toolset | Differences from `kopi-cli` |
|---------|-------------------------------|
| `kopi-cli` | Full toolset — the default for interactive CLI sessions. Includes file, terminal, web, browser, memory, skills, vision, image_gen, todo, tts, delegation, code_execution, cronjob, session_search, and clarify, plus the `safe` (read-only) bundle. |
| `kopi-acp` | Drops `clarify`, `cronjob`, `image_generate`, `text_to_speech`, and all four Home Assistant tools. Focused on coding tasks in IDE context. |
| `kopi-api-server` | Drops `clarify` and `text_to_speech`. Keeps everything else — suitable for programmatic access where user interaction isn't possible. |
| `kopi-cron` | Same as `kopi-cli`. |
| `kopi-telegram` | Same as `kopi-cli`. |
| `kopi-discord` | Adds `discord` and `discord_admin` on top of `kopi-cli`. |
| `kopi-slack` | Same as `kopi-cli`. |
| `kopi-whatsapp` | Same as `kopi-cli`. |
| `kopi-signal` | Same as `kopi-cli`. |
| `kopi-matrix` | Same as `kopi-cli`. |
| `kopi-mattermost` | Same as `kopi-cli`. |
| `kopi-email` | Same as `kopi-cli`. |
| `kopi-sms` | Same as `kopi-cli`. |
| `kopi-bluebubbles` | Same as `kopi-cli`. |
| `kopi-dingtalk` | Same as `kopi-cli`. |
| `kopi-feishu` | Adds the five `feishu_doc_*` / `feishu_drive_*` tools (only used by the document-comment handler, not the regular chat adapter). |
| `kopi-qqbot` | Same as `kopi-cli`. |
| `kopi-wecom` | Same as `kopi-cli`. |
| `kopi-wecom-callback` | Same as `kopi-cli`. |
| `kopi-weixin` | Same as `kopi-cli`. |
| `kopi-yuanbao` | Adds the five `yb_*` tools (DM/group/sticker) on top of `kopi-cli`. |
| `kopi-homeassistant` | Same as `kopi-cli` (the Home Assistant tools are already present by default and activate when `HASS_TOKEN` is set). |
| `kopi-webhook` | Same as `kopi-cli`. |
| `kopi-gateway` | Internal gateway orchestrator toolset — union of every `kopi-<platform>` toolset; used when the gateway needs to accept any message source. |
...
- **Workflow-gated** tools — the `kanban` toolset — are deliberately opt-in. `all`/`*` does **not** enable kanban; you must list `kanban` explicitly (or be a dispatcher-spawned worker with `KOPI_KANBAN_TASK` set). Kanban tools mutate shared board state, so they stay off by default even under `all`.

## Relationship to `kopi tools`

The `kopi tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
