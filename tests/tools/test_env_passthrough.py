...
class TestTerminalIntegration:
    ...
    def test_blocklisted_var_blocked_by_default(self):
        from tools.environments.local import _sanitize_subprocess_env, _KOPI_PROVIDER_ENV_BLOCKLIST

        # Pick a var we know is in the blocklist
        blocked_var = next(iter(_KOPI_PROVIDER_ENV_BLOCKLIST))
    ...
    def test_non_kopi_api_key_still_registerable(self):
    ...
    def test_provider_blocklist_import_failure_fails_closed(self, monkeypatch):
        ...
        Verifies the full path: _is_kopi_provider_credential returns True,
        ...
        assert _ep_mod._is_kopi_provider_credential("OPENAI_API_KEY")
        assert _ep_mod._is_kopi_provider_credential("ANTHROPIC_API_KEY")
        assert _ep_mod._is_kopi_provider_credential("GH_TOKEN")
