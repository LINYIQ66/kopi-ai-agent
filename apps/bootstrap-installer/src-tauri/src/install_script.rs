...
//!
//! Resolution order:
//!   1. Dev shortcut: a sibling repo checkout via $KOPI_SETUP_DEV_REPO_ROOT
...
pub async fn resolve(
    ...
    // 1. Dev shortcut.
    if let Ok(repo_root) = std::env::var("KOPI_SETUP_DEV_REPO_ROOT") {
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = format!(
        "https://raw.githubusercontent.com/Kopi Ai Agent Pte Ltd/kopi-ai-agent/{}/scripts/{}",
        commit_or_ref,
        kind.filename()
    );
