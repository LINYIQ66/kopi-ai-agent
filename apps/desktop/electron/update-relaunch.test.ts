 ...
 *      internals; KOPI_HOME / KOPI_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.kopi/kopi-ai-agent'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'kopi')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  ...
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'kopi'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'kopi')
...
test('collectRelaunchEnv preserves KOPI_HOME + KOPI_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    KOPI_HOME: '/home/u/.kopi',
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  ...
  const tmp = path.join(os.tmpdir(), `kopi-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
