...
export function setZoomPercent(percent: number): void {
  window.kopiDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.kopiDesktop?.zoom) {
  void window.kopiDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.kopiDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
