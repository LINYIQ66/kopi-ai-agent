...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate KOPI_HOME under tmp_path."""
    monkeypatch.setenv("KOPI_HOME", str(tmp_path / ".kopi"))
    (tmp_path / ".kopi").mkdir()

    # Force the constants/image cache helpers to re-read KOPI_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("kopi_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_kopi_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under KOPI_HOME — gateway cleanup
