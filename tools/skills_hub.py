...
Used by kopi_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _kopi_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            kopi_meta = metadata.get("kopi", {})
            if isinstance(kopi_meta, dict):
                tags = kopi_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Kopi Ai Agent Pte Ltd) but not activated
    ...
    OFFICIAL_REPO = "Kopi Ai Agent Pte Ltd/kopi-ai-agent"
...
def _kopi_index_cache_file() -> Path:
    return _index_cache_dir() / "kopi-index.json"


def _load_kopi_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for KOPI_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    kopi_index_cache_file = _kopi_index_cache_file()
    if kopi_index_cache_file.exists():
        try:
            age = time.time() - kopi_index_cache_file.stat().st_mtime
            if age < KOPI_INDEX_TTL:
                return json.loads(kopi_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    # Cache locally
    try:
        kopi_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        kopi_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class HermesIndexSource(SkillSource):
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "kopi-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "kopi-index"),
            identifier=entry.get("identifier", ""),
