...
_KOPI_PROVIDER_ENV_FORCE_PREFIX = "_KOPI_FORCE_"
...
_KOPI_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
def _is_kopi_internal_secret(key: str) -> bool:
    """Return True for Hermes-internal secrets injected under *dynamic* names.

    ``_KOPI_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    non-terminal :func:`kopi_subprocess_env` helper all call it, so the
...
def _inject_context_kopi_home(env: dict) -> None:
    """Bridge the context-local Hermes home override into subprocess env."""
    try:
        from kopi_constants import get_kopi_home_override

        value = get_kopi_home_override()
        if value:
            env["KOPI_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (KOPI_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_KOPI_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_kopi_internal_secret(key):
            continue
        if key not in _KOPI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_KOPI_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_KOPI_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_kopi_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_kopi_internal_secret(key):
            continue
        elif key not in _KOPI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_kopi_home(sanitized)

    from kopi_constants import apply_subprocess_home_env
...
# narrow subset of _KOPI_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in kopi_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_kopi_internal_secret, but _ID has no secret suffix, so it must be
...
def kopi_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_KOPI_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 2 (conditional):** the rest of ``_KOPI_PROVIDER_ENV_BLOCKLIST``
    ...
    # legitimate use for them. See :func:`_is_kopi_internal_secret`.
    for key in list(env):
        if key.startswith(_KOPI_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_kopi_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _KOPI_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_kopi_home(env)
    from kopi_constants import apply_subprocess_home_env
    ...
    # copies os.environ, whose KOPI_SESSION_* mirror is a last-writer-wins
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("KOPI_GIT_BASH_PATH")
    ...
    #   PortableGit: %LOCALAPPDATA%\kopi\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\kopi\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _kopi_portable_git = os.path.join(_local_appdata, "kopi", "git") if _local_appdata else ""
    if _kopi_portable_git:
        for candidate in (
            os.path.join(_kopi_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_kopi_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. KOPI AI AGENT requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set KOPI_GIT_BASH_PATH to your bash.exe location."
    )
...
# Cached directory containing the ``kopi`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_KOPI_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_kopi_bin_dir() -> str | None:
    """Return the directory holding the ``kopi`` console-script, or None.
    ...
    kopi install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``kopi``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``kopi`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``kopi`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("kopi")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``kopi`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _KOPI_BIN_DIR
    if _KOPI_BIN_DIR is not _SENTINEL:
        return _KOPI_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("kopi")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "kopi" or base.startswith("kopi."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "kopi.exe" if _IS_WINDOWS else "kopi"
    ...
    _KOPI_BIN_DIR = candidate
    return candidate


def _prepend_kopi_bin_dir(existing_path: str) -> str:
    """Prepend the kopi install dir to ``existing_path`` if it's missing.
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_KOPI_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_KOPI_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_kopi_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_kopi_internal_secret(k):
            continue
        elif k not in _KOPI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    path_key = _path_env_key(run_env)
    if path_key is not None:
        new_path = _append_missing_sane_path_entries(run_env.get(path_key, ""))
        # Ensure the kopi install dir is reachable so plugins can shell out
        # to bare ``kopi`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_kopi_bin_dir(new_path)

    _inject_context_kopi_home(run_env)
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``KOPI_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under KOPI_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_kopi_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "kopi_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._kopi_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_kopi_pgid", None)
                    if pgid is None:
                        raise
