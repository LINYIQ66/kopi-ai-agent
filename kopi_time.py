...
Resolution order:
  1. ``KOPI_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.kopi/config.yaml``
...
from kopi_constants import get_config_path
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("KOPI_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
...
def reset_cache() -> None:
    ...
    config edit or ``KOPI_TIMEZONE`` update) to force ``get_timezone()`` /
