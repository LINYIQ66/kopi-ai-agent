"""Tests for the per-profile Projects store (kopi_cli/projects_db)."""
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' KOPI_HOME.
...
def test_db_path_under_kopi_home():
    # Resolves under KOPI_HOME (set by the autouse isolation fixture).
