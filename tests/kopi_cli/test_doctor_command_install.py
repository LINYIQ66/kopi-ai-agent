"""Tests for the Command Installation check in kopi doctor."""
...
import kopi_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal KOPI_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".kopi"
    ...
    kopi_bin = venv_bin_dir / "kopi"
    kopi_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    kopi_bin.chmod(0o755)
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, kopi_bin = _setup_doctor_env(monkeypatch, tmp_path)
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "kopi"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == kopi_bin.resolve()
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        kopi_bin = project / ".venv" / "bin" / "kopi"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "kopi"
        cmd_link.symlink_to(kopi_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/kopi" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/kopi is a regular file (not symlink), accept it."""
        home, project, kopi_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "kopi"
        cmd_link.write_text("#!/bin/sh\nexec python -m kopi_cli.main \"$@\"\n")
