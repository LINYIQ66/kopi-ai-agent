...
Architecture (two transports):
  ...
  **Local backend (UDS):**
  1. Parent generates a `kopi_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `kopi_tools.py` with file-based RPC stubs
...
# match a safe prefix, the operational KOPI_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "KOPI_" prefix was deliberately removed (#27303) — it leaked
# KOPI_*-named config that lacks a secret substring (e.g. KOPI_BASE_URL,
# KOPI_KANBAN_DB, KOPI_*_WEBHOOK).  The child only needs the few
# location/profile vars in _KOPI_CHILD_ALLOWED below; KOPI_RPC_SOCKET /
# KOPI_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational KOPI_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set kopi_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_KOPI_CHILD_ALLOWED = frozenset({
    "KOPI_HOME",
    "KOPI_PROFILE",
    "KOPI_CONFIG",
    "KOPI_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational KOPI_* vars (_KOPI_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret KOPI_* vars dropped by the tightened allowlist (#27303). The
    # broad "KOPI_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like KOPI_KANBAN_DB / KOPI_BASE_URL), but a sandbox script
...
def generate_kopi_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the kopi_tools.py stub module.
...
def _connect():
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["KOPI_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("KOPI_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
_RPC_DIR = os.environ.get("KOPI_RPC_DIR") or os.path.join(tempfile.gettempdir(), "kopi_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated kopi_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/kopi_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_kopi_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/kopi_tools.py", tools_src)
        ...
        env_prefix = (
            f"KOPI_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"KOPI_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("KOPI_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with kopi_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="kopi_sandbox_")
    ...
    # selector from KOPI_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"kopi_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated kopi_tools module.
        ...
        tools_src = generate_kopi_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "kopi_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  KOPI_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["KOPI_RPC_SOCKET"] = rpc_endpoint
        child_env["KOPI_RPC_TOKEN"] = rpc_token
        ...
        # Ensure the kopi-ai-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from kopi_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _kopi_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _kopi_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # KOPI_TIMEZONE is an internal Hermes setting and must not leak
        # into child processes.
        _tz_name = os.getenv("KOPI_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("KOPI_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.kopi/.env')).
...
def _load_config() -> dict:
    ...
    ``kopi --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (kopi-ai-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``kopi tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and kopi-ai-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.kopi/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        ...
        f"Available via `from kopi_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into kopi_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from kopi_tools import {import_str}` "
