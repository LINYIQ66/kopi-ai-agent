...
def _load_kopi_env_vars() -> dict[str, str]:
    """Load ~/.kopi/.env values without failing Docker command execution."""
    try:
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale kopi-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=kopi-ai-agent=1`` (created by this codebase)
    ...
    * (optional) ``label=kopi-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a kopi process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    filters = ["--filter", "label=kopi-ai-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=kopi-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``KOPI_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("KOPI_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using KOPI_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'kopi'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``kopi-ai-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.kopi/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. kopi-ai-agent:latest) already use /init as PID 1
        ...
        container_name = f"kopi-{uuid.uuid4().hex[:8]}"
        # Labels make kopi-created containers identifiable to:
        #   * the orphan reaper (`kopi-ai-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`kopi-task-id`, `kopi-profile`)
        #   * operators running `docker ps --filter label=kopi-ai-agent=1`
        ...
        label_args = [
            "--label", "kopi-ai-agent=1",
            "--label", f"kopi-task-id={task_label}",
            "--label", f"kopi-profile={profile_name}",
        ]
        ...
        self._labels = {
            "kopi-ai-agent": "1",
            "kopi-task-id": task_label,
            "kopi-profile": profile_name,
        }
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # name-based blocklist doesn't cover — see _is_kopi_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_kopi_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _KOPI_PROVIDER_ENV_BLOCKLIST)
        kopi_env = _load_kopi_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = kopi_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("kopi-task-id", "")
        profile_label = self._labels.get("kopi-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"kopi-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``kopi-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=kopi-ai-agent=1",
                    "--filter", f"label=kopi-task-id={task_label}",
                    "--filter", f"label=kopi-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"kopi-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``kopi /quit``
