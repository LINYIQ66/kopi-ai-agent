...
Hermes stores sessions in its own SQLite store (``kopi_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``kopi trace upload`` subcommand calls
...
DEFAULT_DATASET_NAME = "kopi-traces"
_KOPI_VERSION = "kopi-ai-agent"
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.kopi/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `kopi trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/kopi-traces`` dataset. Returns a
