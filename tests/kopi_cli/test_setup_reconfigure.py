...
- Bare `kopi setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `kopi setup --quick` runs the narrower "fill in missing items" flow.
- `kopi setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("kopi_cli.setup.ensure_kopi_home", {}),
        ("kopi_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("kopi_cli.config.is_managed", {"return_value": False}),
        ("kopi_cli.setup.load_config", {"return_value": {}}),
        ("kopi_cli.setup.save_config", {}),
        ("kopi_cli.setup.get_env_value", {"return_value": None}),
        ("kopi_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("kopi_cli.setup._print_setup_summary", {}),
        ("kopi_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("kopi_cli.setup.load_config", {"return_value": {}}),
        ("kopi_cli.setup.save_config", {}),
        ("kopi_cli.auth.get_active_provider", {"return_value": None}),
        ("kopi_cli.setup.get_env_value", {"return_value": None}),
        ("kopi_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `kopi setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="kopi_cli.setup.prompt_choice",
                quick="kopi_cli.setup._run_quick_setup",
                model="kopi_cli.setup.setup_model_provider",
                terminal="kopi_cli.setup.setup_terminal_backend",
                agent="kopi_cli.setup.setup_agent_settings",
                gateway="kopi_cli.setup.setup_gateway",
                tools="kopi_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`kopi setup --reconfigure` behaves the same as bare `kopi setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("kopi_cli.setup.prompt_choice", {"return_value": 0}),
                first="kopi_cli.setup._run_first_time_quick_setup",
            )
            from kopi_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        import sys
        from kopi_cli.main import main

        captured = {}
        monkeypatch.setattr(
            "kopi_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["kopi", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["kopi", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["kopi", "setup"])
        try:
            main()
        except SystemExit:
            pass
