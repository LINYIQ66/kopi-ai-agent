...
from kopi_constants import get_default_kopi_root, get_kopi_dir, get_kopi_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.kopi/.env manually."
)
...
IMAGE_CACHE_DIR = get_kopi_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_kopi_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_kopi_dir(new_subpath, old_name)
...
AUDIO_CACHE_DIR = get_kopi_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_kopi_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_kopi_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_kopi_dir("cache/screenshots", "browser_screenshots")
...
_KOPI_ROOT = get_default_kopi_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "KOPI_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "KOPI_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "KOPI_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "KOPI_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _KOPI_HOME / "image_cache",
    _KOPI_HOME / "audio_cache",
    _KOPI_HOME / "video_cache",
    _KOPI_HOME / "document_cache",
    _KOPI_HOME / "browser_screenshots",
    ...
    _KOPI_HOME / "cache" / "images",
    _KOPI_HOME / "cache" / "audio",
    _KOPI_HOME / "cache" / "videos",
    _KOPI_HOME / "cache" / "documents",
    _KOPI_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    ...
    list only covers the *active* KOPI_HOME's cache, so a gateway running at
    the root (e.g. ``KOPI_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when KOPI_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _KOPI_ROOT / "profiles"
    try:
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # KOPI_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.kopi
    ...
    for kopi_root in (_KOPI_HOME, _KOPI_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(kopi_root / rel)
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    # ``MEDIA:~/.kopi/google_token.json``) remain rejected.
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``KOPI_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("KOPI_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "KOPI_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "KOPI_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.kopi/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from kopi_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          KOPI_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          KOPI_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          KOPI_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("KOPI_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("KOPI_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("KOPI_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _kopi_run_generation is set on
