...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    ...
    ``home_path`` is whatever ``load_kopi_dotenv()`` already resolved;
    falling back to ``$KOPI_HOME`` / ``~/.kopi`` keeps direct callers
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<kopi_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if KOPI_HOME is ever made traversable.
            try:
