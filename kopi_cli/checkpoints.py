"""`kopi checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.kopi/checkpoints/``.  Actions:

    kopi checkpoints               # same as `status`
    kopi checkpoints status        # total size, project count, breakdown
    kopi checkpoints list          # per-project checkpoint counts + workdir
    kopi checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    kopi checkpoints clear [-f]    # nuke the entire base (asks first)
    kopi checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    kopi checkpoints
    kopi checkpoints prune --retention-days 3 --max-size-mb 200
    kopi checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: kopi checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``kopi checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `kopi checkpoints` → status
