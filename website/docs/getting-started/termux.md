...
KOPI AI AGENT can run directly on an Android phone through [Termux](https://termux.dev/).
...
- links `kopi` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `kopi` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/kopi" "$PREFIX/bin/kopi"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `kopi` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.kopi/.env`.
...
### `kopi doctor` says ripgrep or Node is missing
