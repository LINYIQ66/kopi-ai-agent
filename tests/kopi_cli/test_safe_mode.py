"""Tests for `kopi chat --safe-mode` isolation."""
...
_VARS = ("KOPI_SAFE_MODE", "KOPI_IGNORE_USER_CONFIG", "KOPI_IGNORE_RULES")
...
def test_cmd_chat_safe_mode_sets_env_before_startup(monkeypatch):
    ...
    def fake_has_provider() -> bool:
        assert os.environ["KOPI_SAFE_MODE"] == "1"
        assert os.environ["KOPI_IGNORE_USER_CONFIG"] == "1"
        assert os.environ["KOPI_IGNORE_RULES"] == "1"
        return True
...
def test_prepare_agent_startup_applies_safe_mode_before_plugin_discovery(monkeypatch):
    ...
    plugins = types.ModuleType("kopi_cli.plugins")
    ...
    monkeypatch.setitem(sys.modules, "kopi_cli.plugins", plugins)
...
def test_plugin_discovery_skipped(monkeypatch):
    monkeypatch.setenv("KOPI_SAFE_MODE", "1")
