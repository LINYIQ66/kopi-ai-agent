    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "KOPI AI AGENT"
"""
...
from kopi_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__kopi_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses KOPI_HOME so each profile gets its own OAuth tokens.
    Layout: ``KOPI_HOME/mcp-tokens/``
    """
    try:
        from kopi_constants import get_kopi_home
        base = Path(get_kopi_home())
    except ImportError:
        base = Path(os.environ.get("KOPI_HOME", str(Path.home() / ".kopi")))
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``kopi mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `kopi mcp login <server>` interactively to (re)authorize, "
...
class HermesTokenStorage:
    ...
    File layout::
        ...
        KOPI_HOME/mcp-tokens/<server_name>.json         -- tokens
        KOPI_HOME/mcp-tokens/<server_name>.client.json   -- client info
        KOPI_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://kopi-ai-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `kopi mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "KOPI AI AGENT")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`kopi mcp login {server_name}` interactively first to complete "
