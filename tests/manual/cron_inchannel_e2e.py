...
Uses a throwaway KOPI_HOME so it never touches ~/.kopi. No real names.
"""
...
def _fresh_home():
    """Point KOPI_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_kopi_home() at import time)."""
...
# a different get_kopi_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"KOPI_HOME (throwaway): {HOME}")
