 ...
 * ~/.kopi/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__KOPI_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__KOPI_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "kopi-kanban-dot-triage",
    todo: "kopi-kanban-dot-todo",
    ready: "kopi-kanban-dot-ready",
    running: "kopi-kanban-dot-running",
    blocked: "kopi-kanban-dot-blocked",
    done: "kopi-kanban-dot-done",
    archived: "kopi-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-kopi-task";
  ...
  const DOCS_URL = "https://kopi-ai-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://kopi-ai-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="kopi-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])kopi-kanban-md-code\1/i.test(attrs)) {
      return ' class="kopi-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "kopi-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "kopi-kanban-md",
  ...
  // standard `drop` event and our `kopi-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("kopi-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("kopi-kanban-column--drop");
          if (target) target.classList.add("kopi-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("kopi-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("kopi-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("kopi-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__KOPI_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "kopi-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "kopi-kanban-attention",
        "kopi-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "kopi-kanban-attention-bar" },
        h("span", { className: "kopi-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "kopi-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "kopi-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "kopi-kanban-attention-row",
                  "kopi-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "kopi-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "kopi-kanban-attention-row-id" }, task.id),
                h("span", { className: "kopi-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "kopi-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "kopi-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "kopi-kanban-diag-action-btn",
      action.suggested ? "kopi-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " kopi-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".kopi-kanban-drawer-comment-row input, .kopi-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "kopi-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("kopi-kanban-diag", sevClass) },
      h("div", { className: "kopi-kanban-diag-header" },
        h("span", { className: "kopi-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "kopi-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "kopi-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "kopi-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "kopi-kanban-diag-data-row" },
                  h("span", { className: "kopi-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "kopi-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "kopi-kanban-diag-data-row" },
                h("span", { className: "kopi-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "kopi-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "kopi-kanban-diag-reassign-row" },
            h("span", { className: "kopi-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "kopi-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "kopi-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "kopi-kanban-diag-msg",
              msg.ok ? "kopi-kanban-diag-msg--ok" : "kopi-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "kopi-kanban-section" },
      h("div", { className: "kopi-kanban-section-head-row" },
        h("span", { className: "kopi-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "kopi-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "kopi-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "kopi-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "kopi-kanban-docs-link",
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `kopi kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "kopi-kanban-msg-ok" : "kopi-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "kopi-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "kopi-kanban-boardswitcher" },
      h("div", { className: "kopi-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "kopi-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "kopi-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "kopi-kanban-dialog-title" },
        ...
        h("div", { className: "kopi-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "kopi-kanban-bulk" },
      h("span", { className: "kopi-kanban-bulk-count" },
      ...
      h("div", { className: "kopi-kanban-bulk-priority",
      ...
      h("div", { className: "kopi-kanban-bulk-reassign",
      ...
      h("label", { className: "kopi-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("kopi-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("kopi-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "kopi-kanban-trash",
        dragOver ? "kopi-kanban-trash--drop" : "",
        props.draggingTaskId ? "kopi-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "kopi-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "kopi-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".kopi-kanban-card")) return true;
      if (target.closest && target.closest(".kopi-kanban-column-add")) return true;
      if (target.closest && target.closest(".kopi-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("kopi-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("kopi-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".kopi-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ...
      className: cn(
        "kopi-kanban-columns",
        isScrollable ? "kopi-kanban-columns--scrollable" : "",
        isPanning ? "kopi-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("kopi-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("kopi-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "kopi-kanban-column",
        dragOver ? "kopi-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "kopi-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "kopi-kanban-col-check",
        ...
        h("span", { className: cn("kopi-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "kopi-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "kopi-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "kopi-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "kopi-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "kopi-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "kopi-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "kopi-kanban-lane" },
                  h("div", { className: "kopi-kanban-lane-head" },
                    h("span", { className: "kopi-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "kopi-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "kopi-kanban-card--stale-red";
    if (age >= tier.amber) return "kopi-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".kopi-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "kopi-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "kopi-kanban-card",
        props.selected ? "kopi-kanban-card--selected" : "",
        props.failed ? "kopi-kanban-card--failed" : "",
        props.draggingSource ? "kopi-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "kopi-kanban-card-content" },
          h("div", { className: "kopi-kanban-card-row" },
            h("label", {
              className: "kopi-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "kopi-kanban-card-check",
            ...
            h("span", { className: "kopi-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or kopi kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "kopi-kanban-warning-badge",
                    "kopi-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "kopi-kanban-needs-assignee",
          ...
          h("div", { className: "kopi-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "kopi-kanban-card-row kopi-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "kopi-kanban-assignee",
                            title: `Assigned to Hermes profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "kopi-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "kopi-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "kopi-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "kopi-kanban-inline-create" },
      h("textarea", {
        value: title,
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "kopi-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "kopi-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "kopi-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "kopi-kanban-drawer-close",
        ...
        data ? h("div", { className: "kopi-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "kopi-kanban-section" },
      h("div", { className: "kopi-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "kopi-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "kopi-kanban-drawer-body" },
      h("div", { className: "kopi-kanban-drawer-title" },
        h("span", { className: cn("kopi-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "kopi-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "kopi-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "kopi-kanban-section" },
        h("div", { className: "kopi-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "kopi-kanban-section" },
        h("div", { className: "kopi-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "kopi-kanban-comment" },
                h("div", { className: "kopi-kanban-comment-head" },
                  h("span", { className: "kopi-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "kopi-kanban-comment-ago" },
      ...
      h("div", { className: "kopi-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "kopi-kanban-event",
              isDiag ? "kopi-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "kopi-kanban-event-header" },
                  h("span", { className: "kopi-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "kopi-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "kopi-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "kopi-kanban-event-header-plain" },
                  h("span", { className: "kopi-kanban-event-kind" }, e.kind),
                  h("span", { className: "kopi-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "kopi-kanban-event-phantom-row" },
                  h("span", { className: "kopi-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "kopi-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "kopi-kanban-event-payload" },
    ...
    return h("div", { className: "kopi-kanban-section" },
      h("div", { className: "kopi-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "kopi-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `kopi-kanban-run--${r.outcome || r.status || "ended"}`
          : "kopi-kanban-run--active";
        return h("div", { key: r.id, className: cn("kopi-kanban-run", outcomeClass) },
          h("div", { className: "kopi-kanban-run-head" },
            h("span", { className: "kopi-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "kopi-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "kopi-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "kopi-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "kopi-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "kopi-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "kopi-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "kopi-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "kopi-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "kopi-kanban-pre kopi-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "kopi-kanban-meta-row" },
      h("span", { className: "kopi-kanban-meta-label" }, props.label),
      h("span", { className: "kopi-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "kopi-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "kopi-kanban-meta-row" },
        h("span", { className: "kopi-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "kopi-kanban-meta-value kopi-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "kopi-kanban-meta-row" },
        h("span", { className: "kopi-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "kopi-kanban-meta-value kopi-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "kopi-kanban-section" },
      h("div", { className: "kopi-kanban-section-head-row" },
        h("span", { className: "kopi-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "kopi-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "kopi-kanban-section" },
      h("div", { className: "kopi-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "kopi-kanban-deps-row" },
        h("span", { className: "kopi-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "kopi-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "kopi-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "kopi-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "kopi-kanban-dep-chip-x",
      ...
      h("div", { className: "kopi-kanban-deps-row" },
        h("span", { className: "kopi-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "kopi-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "kopi-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "kopi-kanban-section" },
      ...
      h("div", { className: "kopi-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "kopi-kanban-home-sub kopi-kanban-home-sub--on"
              : "kopi-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__KOPI_PLUGINS__ && typeof window.__KOPI_PLUGINS__.register === "function") {
    window.__KOPI_PLUGINS__.register("kanban", KanbanPage);
  }
})();
