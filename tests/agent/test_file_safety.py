...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_kopi_env(self):
        """Hermes' own .env inside KOPI_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: kopi internal .env is in ~/.kopi/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.kopi/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.kopi/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    ...
    def test_hub_index_cache_blocked(self, tmp_path):
        ...
        cache = kopi_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._kopi_home_path", return_value=kopi_home):
    ...
    def test_hub_directory_blocked(self, tmp_path):
        ...
        kopi_home = tmp_path / ".kopi"
        hub = kopi_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_kopi_home(self, tmp_path):
        """The env basename guard does not depend on KOPI_HOME resolution."""
        kopi_home = tmp_path / ".kopi"
        kopi_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        kopi_home = tmp_path / ".kopi"
        cache = kopi_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
