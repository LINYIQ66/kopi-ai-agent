...
Guards against the 60-vs-500 bug where a stale `.env KOPI_MAX_ITERATIONS=60`
...
def _run_gateway_import(kopi_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
...
@pytest.fixture
def kopi_home(tmp_path: Path) -> Path:
    home = tmp_path / ".kopi"
    home.mkdir()
    return home


def test_config_max_turns_wins_over_stale_env(kopi_home: Path) -> None:
    """Regression: config.yaml:agent.max_turns=500 must beat .env=60."""
    _write_config(kopi_home, agent_cfg={"max_turns": 500})
    _write_env(kopi_home, {"KOPI_MAX_ITERATIONS": "60"})

    env = _run_gateway_import(kopi_home, initial_env={})

    assert env.get("KOPI_MAX_ITERATIONS") == "500", (
        f"expected config.yaml max_turns=500 to win; got {env.get('KOPI_MAX_ITERATIONS')!r}. "
        "Stale .env value is shadowing config — the bridge lost its override."
    )


def test_config_gateway_timeout_wins_over_stale_env(kopi_home: Path) -> None:
...
def test_config_display_busy_input_mode_wins_over_stale_env(kopi_home: Path) -> None:
    _write_config(kopi_home, display_cfg={"busy_input_mode": "interrupt"})
    _write_env(kopi_home, {"KOPI_GATEWAY_BUSY_INPUT_MODE": "queue"})

    env = _run_gateway_import(kopi_home, initial_env={})

    assert env.get("KOPI_GATEWAY_BUSY_INPUT_MODE") == "interrupt"


def test_config_display_busy_text_mode_wins_over_stale_env(kopi_home: Path) -> None:
    _write_config(kopi_home, display_cfg={"busy_text_mode": "queue"})
    _write_env(kopi_home, {"KOPI_GATEWAY_BUSY_TEXT_MODE": "interrupt"})

    env = _run_gateway_import(kopi_home, initial_env={})

    assert env.get("KOPI_GATEWAY_BUSY_TEXT_MODE") == "queue"


def test_config_timezone_wins_over_stale_env(kopi_home: Path) -> None:
    _write_config(kopi_home, timezone="America/Los_Angeles")
    _write_env(kopi_home, {"KOPI_TIMEZONE": "UTC"})

    env = _run_gateway_import(kopi_home, initial_env={})

    assert env.get("KOPI_TIMEZONE") == "America/Los_Angeles"


def test_env_value_survives_when_config_omits_key(kopi_home: Path) -> None:
    ...
    """
    _write_config(kopi_home, agent_cfg={})  # no max_turns
    _write_env(kopi_home, {"KOPI_MAX_ITERATIONS": "123"})

    env = _run_gateway_import(kopi_home, initial_env={})

    assert env.get("KOPI_MAX_ITERATIONS") == "123"
...
def test_config_platform_connect_timeout_supplies_env_when_unset(kopi_home: Path) -> None:
    ...
    _write_config(kopi_home, gateway_cfg={"platform_connect_timeout": 90})

    env = _run_gateway_import(kopi_home, initial_env={})

    assert env.get("KOPI_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"


def test_env_platform_connect_timeout_wins_over_config(kopi_home: Path) -> None:
    """Unlike the agent.*/display.*/timezone bridges (config-authoritative),
    KOPI_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        kopi_home,
        initial_env={"KOPI_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("KOPI_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
