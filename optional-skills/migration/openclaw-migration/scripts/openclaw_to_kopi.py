...
into KOPI AI AGENT, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.kopi/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.kopi/tts/.",
    },
    "discord-settings": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'kopi cron'.",
    },
    "hooks-config": {
...
# like ``openclaw`` → ``kopi`` (so filesystem paths like ``~/.openclaw``
# become ``~/.kopi`` — the real Hermes home — not the broken ``~/.Hermes``).
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Hermes`` but maps ``openclaw`` → ``kopi`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.kopi/config.yaml`` (the real Hermes home) instead of the broken
    ``~/.Hermes/config.yaml``.
    """
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-kopi/apply.ts "blocked by earlier
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-kopi/plan.ts warnings[].
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: kopi config set",
                        )
                    continue
        ...
        for oc_key, kopi_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and kopi_key not in secret_additions:
                secret_additions[kopi_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, kopi_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and kopi_key not in secret_additions:
                        secret_additions[kopi_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        kopi_config = load_yaml_file(destination)
        current_model = kopi_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = kopi_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                kopi_config["model"] = {"default": model_str}
            dump_yaml_file(destination, kopi_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        kopi_config = load_yaml_file(destination)
        existing_tts = kopi_config.get("tts", {})
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        kopi_cfg_path = self.target_root / "config.yaml"
        kopi_cfg = load_yaml_file(kopi_cfg_path)
        existing_mcp = kopi_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            kopi_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                kopi_srv["command"] = srv["command"]
                if srv.get("args"):
                    kopi_srv["args"] = srv["args"]
                if srv.get("env"):
                    kopi_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    kopi_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                kopi_srv["url"] = srv["url"]
                if srv.get("headers"):
                    kopi_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    kopi_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                kopi_srv["enabled"] = False
            if srv.get("timeout"):
                kopi_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                kopi_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                kopi_srv["tools"] = {}
                if tools_cfg.get("include"):
                    kopi_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    kopi_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(kopi_cfg_path)
            kopi_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(kopi_cfg_path, kopi_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'kopi cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = kopi_cfg.get("agent") or {}
        ...
        if compaction:
            compression = kopi_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'kopi gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("KOPI_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        kopi_cfg_path = self.target_root / "config.yaml"
        kopi_cfg = load_yaml_file(kopi_cfg_path)
        sr = kopi_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        kopi_cfg_path = self.target_root / "config.yaml"
        kopi_cfg = load_yaml_file(kopi_cfg_path)
        custom_providers = kopi_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(kopi_cfg_path)
            kopi_cfg["custom_providers"] = custom_providers
            dump_yaml_file(kopi_cfg_path, kopi_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if discord_cfg:
            kopi_cfg_path = self.target_root / "config.yaml"
            kopi_cfg = load_yaml_file(kopi_cfg_path)
            discord_kopi = kopi_cfg.get("discord") or {}
            changed = False
            ...
            if changed and self.execute:
                kopi_cfg["discord"] = discord_hermes
                dump_yaml_file(kopi_cfg_path, kopi_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        kopi_cfg_path = self.target_root / "config.yaml"
        kopi_cfg = load_yaml_file(kopi_cfg_path)
        browser_kopi = kopi_cfg.get("browser") or {}
        changed = False
        ...
        if changed:
            kopi_cfg["browser"] = browser_hermes
            if self.execute:
                self.maybe_backup(kopi_cfg_path)
                dump_yaml_file(kopi_cfg_path, kopi_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            kopi_mode = mode_map.get(mode, "manual")
            kopi_cfg.setdefault("approvals", {})["mode"] = kopi_mode
            if self.execute:
                self.maybe_backup(kopi_cfg_path)
                dump_yaml_file(kopi_cfg_path, kopi_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{kopi_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        ...
        notes.extend([
            ...
            "",
            "**Strongly recommended:** Run `kopi claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `kopi claw cleanup` to fix it.",
            "",
            "## Hermes-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `kopi claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `kopi setup` to configure any remaining settings",
            "- Run `kopi mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `kopi cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `kopi cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `kopi gateway install` if you need the gateway service",
            "- Review `~/.kopi/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into KOPI AI AGENT.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("KOPI_HOME") or str(Path.home() / ".kopi"), help="Hermes home directory")
    parser.add_argument(
        "--workspace-target",
...
def main() -> int:
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.kopi/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.kopi/config.yaml")
        print("    2. Run: kopi mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: kopi cron")
