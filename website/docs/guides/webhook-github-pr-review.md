...
This guide walks you through connecting KOPI AI AGENT to GitHub so it automatically fetches a pull request's diff, analyzes the code changes, and posts a comment — triggered by a webhook event with no manual prompting.
...
- KOPI AI AGENT installed and running (`kopi gateway`)
...
Add the following to your `~/.kopi/config.yaml`:
...
The GitHub webhook payload includes PR metadata (title, description, branch names, URLs) but **not the diff**. The prompt above instructs the agent to run `gh pr diff` to fetch the actual changes. The `terminal` tool is included in the default `kopi-webhook` toolset, so no extra configuration is needed.
:::
...
```bash
tail -f "${KOPI_HOME:-$HOME/.kopi}/logs/gateway.log"
```
...
:::note
`kopi webhook test <name>` only works for **dynamic subscriptions** created with `kopi webhook subscribe`. It does not read routes from `config.yaml`.
:::
