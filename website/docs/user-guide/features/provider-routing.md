...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, KOPI AI AGENT supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.kopi/config.yaml`:
...
- **CLI mode** — configured in `~/.kopi/config.yaml`, loaded at startup
