...
sidebar_label: "内置插件"
title: "内置插件"
description: "随 KOPI AI AGENT 附带并通过生命周期 hook 自动运行的插件——disk-cleanup 等"
---

# 内置插件

Hermes 随仓库附带了一小组插件。它们位于 `<repo>/plugins/<name>/`，与用户安装在 `~/.kopi/plugins/` 中的插件一同自动加载。它们使用与第三方插件相同的插件接口——hook、工具、斜杠命令——只是在仓库内维护。
...
2. **用户（User）** — `~/.kopi/plugins/<name>/`
3. **项目（Project）** — `./.kopi/plugins/<name>/`（需要 `KOPI_ENABLE_PROJECT_PLUGINS=1`）
4. **Pip 入口点（Entry points）** — `kopi_agent.plugins`

名称冲突时，后面的来源优先——名为 `disk-cleanup` 的用户插件会替换内置版本。

`plugins/memory/` 和 `plugins/context_engine/` 被刻意排除在内置扫描之外。这两个目录使用各自的发现路径，因为内存提供者和上下文引擎是通过 `kopi memory setup` / 配置中的 `context.engine` 进行单选配置的提供者。

## 内置插件默认不启用

内置插件随附时处于禁用状态。发现机制会找到它们（它们会出现在 `kopi plugins list` 和交互式 `kopi plugins` UI 中），但在你明确启用之前不会加载：

```bash
kopi plugins enable disk-cleanup
```

或通过 `~/.kopi/config.yaml`：
...
仓库在 `plugins/` 下附带了以下内置插件。所有插件均需手动启用——通过 `kopi plugins enable <name>` 启用。
...
| `kopi-achievements` | 仪表盘标签页 | Steam 风格的可收集徽章，根据你真实的 Hermes 会话历史生成 |
| `kanban/dashboard` | 仪表盘标签页 | 多智能体调度器的看板（Kanban）UI——任务、评论、扇出、切换看板。参见 [Kanban 多智能体](./kanban.md)。 |

内存提供者（`plugins/memory/*`）和上下文引擎（`plugins/context_engine/*`）在 [内存提供者](./memory-providers.md) 中单独列出——它们分别通过 `kopi memory` 和 `kopi plugins` 管理。以下是两个长期运行的基于 hook 的插件的详细说明。
...
| Hook | 行为 |
|---|---|
| `post_tool_call` | 当 `write_file` / `terminal` / `patch` 在 `KOPI_HOME` 或 `/tmp/kopi-*` 内创建匹配 `test_*`、`tmp_*` 或 `*.test.*` 的文件时，静默追踪为 `test` / `temp` / `cron-output`。 |
...
**状态** — 所有内容存储在 `$KOPI_HOME/disk-cleanup/`：
...
**安全性** — 清理操作仅涉及 `KOPI_HOME` 或 `/tmp/kopi-*` 下的路径。Windows 挂载点（`/mnt/c/...`）会被拒绝。已知的顶级状态目录（`logs/`、`memories/`、`sessions/`、`cron/`、`cache/`、`skills/`、`plugins/`、`disk-cleanup/` 本身）即使为空也不会被删除——全新安装不会在第一次会话结束时被清空。

**启用：** `kopi plugins enable disk-cleanup`（或在 `kopi plugins` 中勾选复选框）。

**再次禁用：** `kopi plugins disable disk-cleanup`。

### observability/langfuse

将 Hermes 的轮次、LLM 调用和工具调用追踪到 [Langfuse](https://langfuse.com)——一个开源 LLM 可观测性平台。每轮一个 span，每次 API 调用一个 generation，每次工具调用一个 tool observation。用量总计、各类型 token 数量和成本估算来自 Hermes 的标准 `agent.usage_pricing` 数据，因此 Langfuse 仪表盘看到的分类（input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`）与 `kopi logs` 中显示的一致。
...
或在交互式 `kopi plugins` UI 中勾选复选框。然后将凭据写入 `~/.kopi/.env`：

```bash
KOPI_LANGFUSE_PUBLIC_KEY=pk-lf-...
KOPI_LANGFUSE_SECRET_KEY=sk-lf-...
KOPI_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # 或你的自托管 URL
```
...
| `post_tool_call` | 关闭 tool observation，附加经过清理的 `result`。`read_file` 的内容会被摘要化（头部 + 尾部 + 省略行数），以使大文件读取保持在 `KOPI_LANGFUSE_MAX_CHARS` 以内。 |

会话分组基于 Hermes 会话 ID（或子 agent 的任务 ID），通过 `langfuse.propagate_attributes` 实现，因此单次 `kopi chat` 会话中的所有内容都归属于同一个 Langfuse session。

**验证：**

```bash
kopi plugins list                 # observability/langfuse 应显示 "enabled"
kopi chat -q "hello"              # 在 Langfuse UI 中检查是否有 "Hermes turn" trace
```
...
| `KOPI_LANGFUSE_RELEASE` | — | 发布/版本标签 |
| `KOPI_LANGFUSE_SAMPLE_RATE` | `1.0` | 传递给 SDK 的采样率（0.0–1.0） |
| `KOPI_LANGFUSE_MAX_CHARS` | `12000` | 消息内容 / 工具参数 / 工具结果的单字段截断长度 |
| `KOPI_LANGFUSE_DEBUG` | `false` | 向 `agent.log` 输出详细插件日志 |
...
**禁用：** `kopi plugins disable observability/langfuse`。插件模块仍会被发现，但在你重新启用之前不会运行任何模块代码。
...
- 会后产物（转录、带发言人归属的笔记、行动项）保存在 `~/.kopi/cache/google_meet/<meeting_id>/`
...
**禁用：** `kopi plugins disable google_meet`。已缓存的转录和录音保留在 `~/.kopi/cache/google_meet/`，直到你手动删除。
...
**API** — 路由挂载在 `/api/plugins/kopi-achievements/` 下：
...
**状态文件** — 位于 `$KOPI_HOME/plugins/kopi-achievements/`：
...
**启用：** 无需启用——`kopi-achievements` 是一个仅限仪表盘的插件（无生命周期 hook，无模型可见工具）。它在 `kopi dashboard` 首次启动时自动注册为标签页。`plugins.enabled` 配置仅控制生命周期/工具插件；仪表盘插件完全通过其 `dashboard/manifest.json` 发现。

**退出：** 删除或重命名 `plugins/kopi-achievements/dashboard/manifest.json`，或在 `~/.kopi/plugins/kopi-achievements/` 中用同名用户插件覆盖它（该插件不包含仪表盘）。`$KOPI_HOME/plugins/kopi-achievements/` 下的插件状态文件会保留——重新安装后你的解锁历史依然存在。

## 添加内置插件

内置插件的编写方式与其他 Hermes 插件完全相同——参见 [构建 Hermes 插件](/developer-guide/plugins)。唯一的区别是：

- 目录位于 `<repo>/plugins/<name>/`，而非 `~/.kopi/plugins/<name>/`
- 在 `kopi plugins list` 中，manifest 来源显示为 `bundled`
- 同名用户插件会覆盖内置版本
