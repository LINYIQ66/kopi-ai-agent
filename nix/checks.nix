    ...
    in {
      ...
      checks = {
        ...
        package-contents = pkgs.runCommand "hermes-package-contents" { } ''
          ...
          test -x ${kopi-ai-agent}/bin/kopi-ai-agent || (echo "FAIL: kopi-ai-agent binary missing"; exit 1)
        ...
        cli-commands = pkgs.runCommand "hermes-cli-commands" { } ''
          ...
          echo "=== Checking kopi --help ==="
          ${kopi-ai-agent}/bin/kopi --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${kopi-ai-agent}/bin/kopi --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "hermes-bundled-skills" { } ''
          ...
          SKILL_COUNT=$(find ${kopi-ai-agent}/share/kopi-ai-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "KOPI_BUNDLED_SKILLS" ${kopi-ai-agent}/bin/kopi || \
            (echo "FAIL: KOPI_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: KOPI_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "hermes-bundled-plugins" { } ''
          ...
          test -f ${kopi-ai-agent}/share/kopi-ai-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "KOPI_BUNDLED_PLUGINS" ${kopi-ai-agent}/bin/kopi || \
            (echo "FAIL: KOPI_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: KOPI_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "hermes-bundled-locales" { } ''
          ...
          LOC_COUNT=$(find ${kopi-ai-agent}/share/kopi-ai-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${kopi-ai-agent}/share/kopi-ai-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "KOPI_BUNDLED_LOCALES" ${kopi-ai-agent}/bin/kopi || \
        ...
        bundled-tui = pkgs.runCommand "hermes-bundled-tui" { } ''
          ...
          test -f ${kopi-ai-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "KOPI_TUI_DIR" ${kopi-ai-agent}/bin/kopi || \
            (echo "FAIL: KOPI_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: KOPI_TUI_DIR set in wrapper"
        ...
        managed-guard = pkgs.runCommand "hermes-managed-guard" { } ''
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(KOPI_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking KOPI_MANAGED guards ==="
          check_blocked "config set" ${kopi-ai-agent}/bin/kopi config set model foo
          check_blocked "config edit" ${kopi-ai-agent}/bin/kopi config edit
