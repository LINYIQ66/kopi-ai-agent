...
The PID file lives at ``{KOPI_HOME}/gateway.pid``.  KOPI_HOME defaults to
``~/.kopi`` but can be overridden via the environment variable.  This means
separate KOPI_HOME directories naturally get separate PID files — a property
...
from kopi_constants import get_kopi_home, _get_platform_default_kopi_home
...
def _get_process_kopi_home() -> Path:
    """Return the process-level KOPI_HOME, skipping context-local overrides.
    ...
    ``get_kopi_home()`` honors ``_KOPI_HOME_OVERRIDE`` contextvar used for
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting KOPI_HOME."""
    home = _get_process_kopi_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("KOPI_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "kopi" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    ...
    test also matched ``kopi_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\kopi-gateway.exe"``) survive, and strips
    ...
    for token in tokens:
        ...
        if basename in ("kopi-gateway", "kopi-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "kopi_cli.main" in joined
        or "kopi_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("kopi", "kopi.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `kopi gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a KOPI_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.kopi`` or ``$KOPI_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``kopi_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``KOPI_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"kopi_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit KOPI_HOME= on the argv. KOPI_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "kopi_home=" in command_lc and f"kopi_home={home_lc}" not in command_lc:
        return False
    return True
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `kopi status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``KOPI_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    KOPI_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different KOPI_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``kopi.service`` + ``kopi-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different KOPI_HOME. When two profile gateway
    # services share the same default ~/.kopi (KOPI_HOME not set
    ...
    replacer_home = record.get("replacer_kopi_home")
    if replacer_home is not None and replacer_home != str(_get_process_kopi_home()):
        return False
    ...
    # ``kopi gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(target_pid: int) -> bool:
    ...
    try:
        ...
        record = {
            ...
            "replacer_kopi_home": str(_get_process_kopi_home()),
