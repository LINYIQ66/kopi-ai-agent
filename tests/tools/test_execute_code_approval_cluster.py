...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad KOPI_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: KOPI_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_kopi_allowlist_and_secret_blocks():
...
def test_env_scrub_logs_dropped_kopi_vars(caplog):
    """Dropping a non-allowlisted, non-secret KOPI_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset KOPI_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "KOPI_HOME": "/h",          # allowlisted → kept, not logged
        "KOPI_BASE_URL": "https://x",   # dropped → logged
        "KOPI_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "KOPI_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "KOPI_HOME" in out and "PATH" in out
    assert "KOPI_BASE_URL" not in out and "KOPI_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "KOPI_BASE_URL" in msgs and "KOPI_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "KOPI_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped KOPI_* vars."""
    import logging
