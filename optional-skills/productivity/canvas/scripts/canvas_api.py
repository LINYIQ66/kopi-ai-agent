#!/usr/bin/env python3
"""Canvas LMS API CLI for KOPI AI AGENT.
...
def _check_config():
    ...
    if missing:
        kopi_env = os.path.join(
            os.environ.get("KOPI_HOME", os.path.expanduser("~/.kopi")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {kopi_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for KOPI AI AGENT"
    )
