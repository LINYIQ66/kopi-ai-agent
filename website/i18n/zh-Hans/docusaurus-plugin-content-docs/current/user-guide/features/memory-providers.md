...
KOPI AI AGENT 内置 8 个外部记忆提供者插件，为 Agent 提供跨会话的持久化知识，超越内置的 MEMORY.md 和 USER.md。同一时间只能激活**一个**外部提供者——内置记忆始终与其并行工作。

## 快速开始

```bash
kopi memory setup      # 交互式选择器 + 配置
kopi memory status     # 查看当前激活状态
kopi memory off        # 禁用外部提供者
```

也可以通过 `kopi plugins` → Provider Plugins → Memory Provider 选择激活的记忆提供者。

或在 `~/.kopi/config.yaml` 中手动设置：
...
**安装向导：**
```bash
kopi memory setup        # 选择 "honcho" — 运行 Honcho 专属的安装后配置
```

旧版 `kopi honcho setup` 命令仍然有效（现在会重定向到 `kopi memory setup`），但只有在 Honcho 被选为激活记忆提供者后才会注册。

**配置：** `$KOPI_HOME/honcho.json`（profile 本地）或 `~/.honcho/config.json`（全局）。解析顺序：`$KOPI_HOME/honcho.json` > `~/.kopi/honcho.json` > `~/.honcho/config.json`。参见[配置参考](https://github.com/kopi-ai/kopi-ai-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/kopi)。
...
:::tip 从 `kopi honcho` 迁移
如果你之前使用过 `kopi honcho setup`，你的配置和所有服务端数据均完好无损。只需通过安装向导重新启用，或手动设置 `memory.provider: honcho`，即可通过新系统重新激活。
:::
...
| **AI peer**（`aiPeer`） | 每个 Hermes profile 一个。host key `kopi` → 默认；其他 profile 使用 `hermes.<profile>`。 |
...
扫描所有 Hermes profile，为没有 host 块的 profile 创建 host 块，从默认 `kopi` 块继承设置，并提前创建新的 AI peer。幂等操作——跳过已有 host 块的 profile。
...
参见[配置参考](https://github.com/kopi-ai/kopi-ai-agent/blob/main/plugins/memory/honcho/README.md)和 [Honcho 集成指南](https://docs.honcho.dev/v3/guides/integrations/kopi)。
...
# 然后配置 Hermes
kopi memory setup    # 选择 "openviking"
# 或手动配置：
...
**安装：**
```bash
kopi memory setup    # 选择 "mem0"
# 或手动配置：
kopi config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.kopi/.env
```

**配置：** `$KOPI_HOME/mem0.json`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `user_id` | `kopi-user` | 用户标识符 |
| `agent_id` | `kopi` | Agent 标识符 |
...
**安装：**
```bash
kopi memory setup    # 选择 "hindsight"
# 或手动配置：
...
**本地模式 UI：** `hindsight-embed -p kopi ui start`

**配置：** `$KOPI_HOME/hindsight/config.json`
...
| `retain_context` | `conversation between KOPI AI AGENT and the User` | 保留记忆的上下文标签 |
...
完整配置参考参见[插件 README](https://github.com/LINYIQ66/kopi-ai-agent/blob/main/plugins/memory/hindsight/README.md)。
...
**安装：**
```bash
kopi memory setup    # 选择 "holographic"
# 或手动配置：
kopi config set memory.provider holographic
```

**配置：** `plugins.kopi-memory-store` 下的 `config.yaml`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `db_path` | `$KOPI_HOME/memory_store.db` | SQLite 数据库路径 |
...
# 然后配置 Hermes
kopi memory setup    # 选择 "byterover"
# 或手动配置：
...
- 知识树存储于 `$KOPI_HOME/byterover/`（profile 范围隔离）
...
| **依赖** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=kopi) |
...
**安装：**
```bash
kopi memory setup    # 选择 "supermemory"
# 或手动配置：
kopi config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.kopi/.env
```

**配置：** `$KOPI_HOME/supermemory.json`

| 键 | 默认值 | 描述 |
|-----|---------|-------------|
| `container_tag` | `kopi` | 用于搜索和写入的容器标签。支持 `{identity}` 模板用于 profile 范围隔离。 |
...
- **Profile 范围容器**——在 `container_tag` 中使用 `{identity}`（例如 `kopi-{identity}` → `kopi-coder`），按 Hermes profile 隔离记忆
...
- **本地存储提供者**（Holographic、ByteRover）使用 `$KOPI_HOME/` 路径，各 profile 路径不同
- **配置文件提供者**（Honcho、Mem0、Hindsight、Supermemory）将配置存储在 `$KOPI_HOME/` 中，每个 profile 拥有独立凭证
