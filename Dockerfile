...
# Do not write .pyc files at runtime: /opt/kopi is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/kopi/.playwright
...
# kopi process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main kopi process, the dashboard,
...
# lock on the @kopi/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/kopi/.venv at first
...
# gives the non-root kopi user read + traverse but no write; root retains
...
# Wire the exec shim and install-method stamp.  Files under /opt/kopi are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the kopi user (go-w from the --chmod above).
...
# host install's marker and wrongly block its ``kopi update``. A code-scoped
...
# container always returns nothing — meaning `kopi dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the KOPI_GIT_SHA build-arg to
# /opt/kopi/.kopi_build_sha at build time, and have
# kopi_cli/build_info.py read it at runtime.  Both `kopi dump` and
...
ARG KOPI_GIT_SHA=
RUN if [ -n "${KOPI_GIT_SHA}" ]; then \
        printf '%s\n' "${KOPI_GIT_SHA}" > /opt/kopi/.kopi_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-kopi + dashboard.
...
# slots from $KOPI_HOME/profiles/<name>/ after a container restart
...
# fast path (`node --expose-gc /opt/kopi/ui-tui/dist/entry.js`) and skip the
...
# is seeded + chowned to the kopi user by docker/stage2-hook.sh and lives
...
ENV KOPI_LAZY_INSTALL_TARGET=/opt/data/lazy-packages
...
ENV PATH="/opt/kopi/bin:/opt/kopi/.venv/bin:/opt/data/.local/bin:${PATH}"
...
ENTRYPOINT [ "/init", "/opt/kopi/docker/main-wrapper.sh" ]
CMD [ ]
