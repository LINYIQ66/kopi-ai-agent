...
Usage examples::
    ...
    kopi logs                    # last 50 lines of agent.log
    kopi logs -f                 # follow agent.log in real time
    kopi logs errors             # last 50 lines of errors.log
    kopi logs gateway -n 100    # last 100 lines of gateway.log
    kopi logs gui -f            # follow gui.log (dashboard/pty/ws)
    kopi logs desktop -f        # follow desktop.log (Electron app boot/backend)
    kopi logs --level WARNING    # only WARNING+ lines
    kopi logs --session abc123   # filter by session ID substring
    kopi logs --component tools  # only tool-related lines
    kopi logs --since 1h         # lines from the last hour
    kopi logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if follow:
        print(f"--- {display_kopi_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_kopi_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_kopi_home()}/logs/")
        return

    print(f"Log files in {display_kopi_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'kopi chat' to generate logs)")
