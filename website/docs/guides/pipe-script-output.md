...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `kopi send`."
---
...
`kopi send` resolves the message body in this order:

1. **Positional argument** — `kopi send --to telegram "hi"`
...
`kopi send`. If the Python interpreter can't load because the box is
...
## Does `kopi send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `kopi send` calls
the platform's REST endpoint directly using credentials from
`~/.kopi/.env` and `~/.kopi/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `kopi gateway start` and retry.
...
The listing is built from `~/.kopi/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`kopi gateway
...
`kopi send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `kopi send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `kopi send` shares with cron delivery.
