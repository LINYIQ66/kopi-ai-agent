...
// kopi_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - kopi_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - kopi_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / kopi #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-kopi_session_at', '__Secure-kopi_session_at', 'kopi_session_at']
const RT_COOKIE_VARIANTS = ['__Host-kopi_session_rt', '__Secure-kopi_session_rt', 'kopi_session_rt']
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://kopi.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a kopi session ACCESS-token cookie
