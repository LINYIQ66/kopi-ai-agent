...
_UNSET = object()
_KOPI_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_KOPI_HOME_OVERRIDE", default=_UNSET
)
...
def set_kopi_home_override(path: str | Path | None) -> Token:
    ...
    return _KOPI_HOME_OVERRIDE.set(value)
...
def get_kopi_home() -> Path:
    ...
    When ``KOPI_HOME`` is unset but an ``active_profile`` file indicates
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_kopi_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[KOPI_HOME fallback] KOPI_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass KOPI_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return _get_platform_default_kopi_home()
...
def get_default_kopi_root() -> Path:
    ...
    ``~/.kopi`` (e.g. ``/opt/data``), returns ``KOPI_HOME`` directly
    — that IS the root.

    In profile mode where ``KOPI_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.kopi/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_kopi_home()
    env_home = os.environ.get("KOPI_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # KOPI_HOME is under ~/.kopi (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — KOPI_HOME itself is the root
    return env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``KOPI_OPTIONAL_SKILLS``.
    """
    override = os.getenv("KOPI_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_kopi_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``KOPI_OPTIONAL_MCPS``.
    """
    override = os.getenv("KOPI_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_kopi_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``KOPI_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<KOPI_HOME>/skills`` last-resort
    """
    override = os.getenv("KOPI_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_kopi_dir(new_subpath: str, old_name: str) -> Path:
    ...
    Args:
        new_subpath: Preferred path relative to KOPI_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to KOPI_HOME (e.g. ``"image_cache"``).
...
def iter_kopi_node_dirs(home: Path | None = None) -> list[Path]:
    ...
    ``$KOPI_HOME/node/bin``. Include both shapes on every platform so mixed
...
_KOPI_NODE_TARGET_MAJOR = int(os.environ.get("KOPI_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    used to trust file presence alone, so ``kopi update`` would pick that
...
def kopi_managed_node_tree_present(home: Path | None = None) -> bool:
    ...
    for directory in iter_kopi_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%KOPI_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_kopi_home()
    index_url = f"https://nodejs.org/dist/latest-v{_KOPI_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_KOPI_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_kopi_managed_node() -> bool:
    ...
    if _managed_node_heal_attempted:
        return False
    if not kopi_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "KOPI_HOME": str(get_kopi_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
        ...
        for name in names:
            ...
            if candidate.is_file() and (
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_kopi_managed_node():
        for directory in iter_kopi_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable(command: str) -> str | None:
    ...
    """
    managed = find_kopi_node_executable(command)
    if managed:
        return managed
    if kopi_managed_node_tree_present():
        return None
...
def with_kopi_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    managed = [str(path) for path in iter_kopi_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``kopi update`` — leaving a **dangling symlink** that ``which`` still
...
def display_kopi_home() -> str:
    """Return a user-friendly display string for the current KOPI_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.kopi``
        profile:  ``~/.kopi/profiles/coder``
        custom:   ``/opt/kopi-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.kopi``.  For code that needs a real ``Path``, use
    :func:`get_kopi_home` instead.
    """
    home = get_kopi_home()
    try:
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{KOPI_HOME}/home`` when the profile-home directory exists."""
    kopi_home = get_kopi_home_override() or (env or {}).get("KOPI_HOME") or os.getenv("KOPI_HOME")
    if not kopi_home:
        return None
    profile_home = os.path.join(kopi_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("KOPI_REAL_HOME") or os.getenv("KOPI_REAL_HOME", "")).strip()
    if explicit:
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{KOPI_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{KOPI_HOME}/home`` when it exists, preserving the
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under KOPI_HOME.

    Replaces the ``get_kopi_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, kopi_logging.py, kopi_time.py, etc.).
    """
    return get_kopi_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under KOPI_HOME."""
    return get_kopi_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under KOPI_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_kopi_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._kopi_ipv4_patched = True  # type: ignore[attr-defined]
