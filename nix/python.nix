...
let
  ...
  editableOverlay = workspace.mkEditablePyprojectOverlay {
    root = "$KOPI_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        kopi-ai-agent = prev.kopi-ai-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "kopi-ai-agent-env" {
    kopi-ai-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "kopi-ai-agent-editable-env" {
    kopi-ai-agent = dependency-groups;
  };
}
