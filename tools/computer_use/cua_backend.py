...
etc.) surface as specific blocked checks via `kopi computer-use doctor`
...
# fetches the latest release, so a `KOPI_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `KOPI_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD = os.environ.get("KOPI_CUA_DRIVER_CMD", "cua-driver")
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of Kopi Ai Agent Pte Ltd/kopi-ai-agent#47072: instead of hardcoding
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or KOPI_CUA_DRIVER_CMD resolves."""
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `kopi computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        f"{installer}\n"
        "Or run `kopi tools` and enable the Computer Use toolset to install it automatically."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of Kopi Ai Agent Pte Ltd/kopi-ai-agent#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(self, bridge: _AsyncBridge) -> None:
        ...
        self._started = False
        # Surface 4 of Kopi Ai Agent Pte Ltd/kopi-ai-agent#47072: per-tool
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `kopi computer-use doctor` and check "
                f"{display_kopi_home()}/logs/agent.log for the phase timings."
            )
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    Kopi Ai Agent Pte Ltd/kopi-ai-agent#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        # Surface 6 of Kopi Ai Agent Pte Ltd/kopi-ai-agent#47072: per-snapshot
        ...
        self._session_id: str = f"kopi-{uuid.uuid4().hex[:12]}"
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps kopi `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of Kopi Ai Agent Pte Ltd/kopi-ai-agent#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of Kopi Ai Agent Pte Ltd/kopi-ai-agent#47072: prefer the
    ...
    def click(
        ...
        *,
        ...
        # Kopi Ai Agent Pte Ltd/kopi-ai-agent#47072). cua-driver-rs gained an explicit
