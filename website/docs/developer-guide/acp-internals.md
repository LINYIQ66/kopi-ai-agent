...
```text
kopi acp / kopi-acp / python -m acp_adapter
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'kopi-ai-agent[acp]==<version>' kopi-acp`, pointed at the `kopi-ai-agent` PyPI release.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["kopi-acp"])
...
- `acp_adapter/auth.py`
- `kopi_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Hermes provider/credentials. It also always advertises a terminal setup auth method (`kopi-setup`, args `--setup`) so first-run registry clients can open Hermes' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.kopi/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `kopi-acp` toolset definition
- `kopi_cli/main.py` — `kopi acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `kopi-acp` script
