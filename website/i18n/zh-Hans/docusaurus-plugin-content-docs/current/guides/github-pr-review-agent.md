...
- **已安装 KOPI AI AGENT** — 参见[安装指南](/getting-started/installation)
...
:::tip 没有消息通知？没关系
使用 `deliver: "local"` 将审查结果保存到 `~/.kopi/cron/output/`。在接入通知之前用于测试非常方便。
:::
...
创建 `~/.kopi/skills/code-review/SKILL.md`：
...
验证是否已加载——启动 `kopi`，你应该能在启动时的 skill 列表中看到 `code-review`。
