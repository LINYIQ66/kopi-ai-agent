...
provider** — turn on a backend by enabling a plugin (``kopi plugins
enable video_gen/<name>``) and selecting it in ``kopi tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/kopi-ai-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `kopi tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `kopi plugins list` to see "
            f"installed video gen backends, or `kopi tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `kopi tools` → "
...
# `kopi tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`kopi tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `kopi tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
