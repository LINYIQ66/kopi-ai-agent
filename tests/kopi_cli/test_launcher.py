"""Tests for the top-level `./kopi` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./kopi` should use `kopi_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "kopi"
    called = []

    fake_main_module = types.ModuleType("kopi_cli.main")

    def fake_main():
        called.append("kopi_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "kopi_cli.main", fake_main_module)
