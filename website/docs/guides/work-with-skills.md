...
kopi skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.kopi/skills/`
...
```bash
mkdir -p ~/.kopi/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.kopi/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
kopi chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.kopi/skills/` and it's live.
...
**Use categories.** Organize skills into subdirectories (`~/.kopi/skills/devops/`, `~/.kopi/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.
