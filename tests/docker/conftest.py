...
Override the image with ``KOPI_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("KOPI_TEST_IMAGE", "kopi-ai-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``KOPI_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("KOPI_TEST_IMAGE"):
        return IMAGE_TAG
