"""Tests for kopi_cli.gateway_windows."""
...
import kopi_cli.setup as setup
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda kopi_home: "")
    monkeypatch.setattr("kopi_cli.config.get_kopi_home", lambda: str(kopi_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "kopi_cli.main"]
    assert cwd == str(kopi_home.resolve())
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_kopi_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".kopi"
        home.mkdir()
        monkeypatch.setattr("kopi_cli.config.get_kopi_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".kopi"
        project = tmp_path / "checkout"
        monkeypatch.setattr("kopi_cli.config.get_kopi_home", lambda: missing)
        assert gateway_windows._stable_gateway_working_dir(project) == str(project)


def test_write_task_script_anchors_cmd_cd_at_kopi_home(monkeypatch, tmp_path):
    project = tmp_path / "project"
    kopi_home = tmp_path / "kopi-home"
    kopi_home.mkdir()
    ...
    assert f"cd /d {gateway_windows._quote_cmd_script_arg(str(kopi_home.resolve()))}" in content
...
def test_gateway_cmd_script_uses_pythonw_without_replace_or_start_churn(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda exe: (exe.replace("python.exe", "pythonw.exe"), r"C:\\Hermes\\kopi-ai-agent\\venv", []),
    )

    content = gateway_windows._build_gateway_cmd_script(
        r"C:\\Hermes\\kopi-ai-agent\\venv\\Scripts\\python.exe",
        r"C:\\Hermes\\kopi-ai-agent",
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "pythonw.exe" in content
    assert "kopi_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("KOPI_HOME", "PYTHONIOENCODING", "KOPI_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
def test_install_startup_fallback_does_not_auto_spawn_when_gateway_stopped(monkeypatch, tmp_path, capsys):
    ...
    assert "kopi --profile alice gateway start" in out
...
# resume_pending=True) never fires when `kopi gateway stop` kills the
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("kopi_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("kopi_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    `kopi gateway start` fails with "another instance is running".
    """
    pid = 77777
    events = []
