...
Mounted at /api/plugins/kopi-achievements/ by Hermes dashboard.
"""
...
try:
    ...
    def get_kopi_home() -> Path:  # type: ignore[misc]
        val = (_os.environ.get("KOPI_HOME") or "").strip()
...
def state_path() -> Path:
    return get_kopi_home() / "plugins" / "kopi-achievements" / "state.json"


def snapshot_path() -> Path:
    return get_kopi_home() / "plugins" / "kopi-achievements" / "scan_snapshot.json"


def checkpoint_path() -> Path:
    return get_kopi_home() / "plugins" / "kopi-achievements" / "scan_checkpoint.json"
...
def analyze_messages(session_id: str, title: str, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "plugin_events": len(re.findall(r"plugin|dashboard-plugins|__KOPI_PLUGIN|manifest\.json", full_text, re.I)),
