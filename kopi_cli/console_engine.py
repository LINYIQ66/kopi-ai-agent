...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'kopi doctor'")
        and last.startswith("Run 'kopi setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="kopi", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `kopi "):
        return ""
    return summary
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("kopi_cli.main")
    builder = getattr(module, builder_name)
...
def _register_command_family(
    ...
    *,
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `kopi {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `kopi {usage}`?",
...
class HermesConsoleEngine:
    ...
    def _register_broad_cli_surface(self) -> None:
        ...
        extracted = {
            ...
            "prompt-size": (
                "kopi_cli.subcommands.prompt_size",
            ...
            "import": (
                "kopi_cli.subcommands.import_cmd",
        ...
        self.register(
            ...
            _adder_handler("send", (), "kopi_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("kopi_cli.portal_cli", "add_parser"),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("kopi_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("kopi_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from kopi_cli.logs import list_logs, tail_log
...
def _config_set(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    key = args[0]
    value = " ".join(args[1:])
    from kopi_cli.config import set_config_value
...
def _sessions_repair(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from kopi_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    return _dispatch_extracted_subcommand(
        ...
        fixed=(),
        args=(),
        module_name="kopi_cli.subcommands.profile",
...
def _cron_list(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    ns = parser.parse_args(args)
    from kopi_cli.cron import cron_list
...
def _cron_pause(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    try:
        job = pause_job(args[0], reason="paused from kopi console")
...
def run_console_repl(
    *,
    ...
    """Run the local ``kopi console`` REPL."""
