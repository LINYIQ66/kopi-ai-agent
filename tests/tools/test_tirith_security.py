...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._kopi_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(kopi_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(kopi_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_kopi_bin(self):
        """After _INSTALL_FAILED, manual install in KOPI_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        kopi_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(kopi_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(kopi_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._kopi_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == kopi_bin
            assert _tirith_mod._resolved_path == kopi_bin
            mock_clear.assert_called_once()
...
class TestHermesHomeIsolation:
    def test_kopi_bin_dir_respects_kopi_home(self):
        ...
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"KOPI_HOME": tmpdir}):
            result = _kopi_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_kopi_home(self):
        """_failure_marker_path must use KOPI_HOME, not hardcoded ~/.kopi."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"KOPI_HOME": "/custom/kopi"}):
            result = _failure_marker_path()
        assert result == "/custom/kopi/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets KOPI_HOME; verify it's active."""
        kopi_home = os.getenv("KOPI_HOME")
        assert kopi_home is not None, "KOPI_HOME should be set by conftest"
        assert "kopi_test" in kopi_home, "Should point to test temp dir"

    def test_get_kopi_home_fallback(self):
        """Without KOPI_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_kopi_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove KOPI_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".kopi")
            result = _get_kopi_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._kopi_bin_dir",
