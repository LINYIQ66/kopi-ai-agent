...
def _save_mem0_json(kopi_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(kopi_home) / "mem0.json"
    existing = {}
...
def _setup_platform(kopi_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "kopi-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "kopi"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.kopi/.env. Warn
    ...
    provider.save_config(provider_config, kopi_home)

    if env_writes:
        _write_env(Path(kopi_home) / ".env", env_writes)
...
def _setup_selfhosted(kopi_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "kopi-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "kopi")
...
def _setup_oss(kopi_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(kopi_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "kopi-user")
    ...
    if env_writes:
        _write_env(Path(kopi_home) / ".env", env_writes)
    _save_mem0_json(kopi_home, {"mode": "oss", "user_id": user_id, "agent_id": "kopi", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, kopi_home: str) -> str:
...
_PGVECTOR_CONTAINER = "kopi-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "kopi"
...
def _setup_oss_interactive(kopi_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], kopi_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], kopi_home)
        if key:
    ...
    if env_writes:
        _write_env(Path(kopi_home) / ".env", env_writes)
    _save_mem0_json(kopi_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(kopi_home: str, config: dict) -> None:
    """Entry point called by kopi memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(kopi_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(kopi_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(kopi_home, config, flags)
        return
