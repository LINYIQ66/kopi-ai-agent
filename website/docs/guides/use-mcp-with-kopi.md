...
description: "A practical guide to connecting MCP servers to KOPI AI AGENT, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Hermes

This guide shows how to actually use MCP with KOPI AI AGENT in day-to-day workflows.
...
```bash
kopi mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
kopi mcp test chrome-devtools-win
```
...
```bash
kopi mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
kopi mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `kopi mcp add` prompt, or edit `config.yaml` afterward:
