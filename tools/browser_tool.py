...
from kopi_constants import agent_browser_runnable, get_kopi_home
from utils import env_int, is_truthy_value
from kopi_cli.config import DEFAULT_CONFIG, cfg_get
...
def _build_browser_env() -> dict:
    ...
    The ``kopi_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = kopi_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    kopi_node_bin = str(kopi_home / "node" / "bin")
    kopi_node_root = str(kopi_home / "node")
    kopi_nm_bin = str(kopi_home / "node_modules" / ".bin")
    return [kopi_node_bin, kopi_node_root, kopi_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.kopi/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.kopi/plugins/browser/<vendor>/``) participate
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-kopi_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed kopi processes — this way every clean kopi exit sweeps
    ...
    # Sweep orphans from other crashed kopi processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live kopi processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current kopi PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live kopi process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning kopi process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced kopi PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent kopi instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-kopi_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live kopi process.
            continue
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    # node_modules binary, which disappears on the next ``kopi update`` and
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_kopi_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_kopi_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_kopi_home() / "node")),
            ]
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this kopi PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = kopi_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    screenshots_dir = get_kopi_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/kopi-ai-agent:latest")
                else:
