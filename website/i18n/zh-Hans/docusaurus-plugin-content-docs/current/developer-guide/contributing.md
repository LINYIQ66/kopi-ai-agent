...
sidebar_position: 4
title: "贡献指南"
description: "如何为 KOPI AI AGENT 做贡献 — 开发环境配置、代码风格、PR 流程"
---

# 贡献指南

感谢您为 KOPI AI AGENT 做贡献！本指南涵盖开发环境配置、代码库结构说明以及 PR 合并流程。
...
对大多数贡献者来说，最好的开发启动方式和用户安装方式相同：运行标准安装器，然后在它克隆出的仓库里开发。安装器会创建 Hermes venv、配置 `kopi` 命令、为 `kopi update` 写入安装方式标记，并把完整 git 项目克隆到 `$KOPI_HOME/kopi-ai-agent`（通常是 `~/.kopi/kopi-ai-agent`）。这样你的开发环境会和 CLI、updater、lazy dependency installer、gateway、docs 默认假设的布局一致。
...
只有在你明确不想使用 Hermes managed install layout 时才使用这种方式（例如容器或 CI job 里的临时 clone）。如果这样安装，请确保运行的是这个 venv 里的 `kopi` entrypoint；运行系统 `python3 -m kopi_cli.main` 可能会加载无关的系统 Python 包。
...
```bash
# 标准安装器已经把 `kopi` 放到了 PATH 上。
kopi doctor
kopi chat -q "Hello"
```

如果你使用了手动克隆备用路径，可以在 checkout 中运行 `./kopi`，或显式把这个 clone 的 venv 链接到 PATH：
...
- **Profile 安全路径**：不得硬编码 `~/.kopi` — 代码路径使用 `kopi_constants` 中的 `get_kopi_home()`，面向用户的消息使用 `display_kopi_home()`。完整规则参见 [AGENTS.md](https://github.com/LINYIQ66/kopi-ai-agent/blob/main/AGENTS.md#profiles-multi-instance-support)。
...
- 使用 [GitHub Issues](https://github.com/LINYIQ66/kopi-ai-agent/issues)
- 请包含：操作系统、Python 版本、Hermes 版本（`kopi version`）、完整错误堆栈
- 包含复现步骤
- 创建前请检查是否已有重复 issue
- 安全漏洞请私下报告
...
提交贡献即表示您同意您的贡献将以 [MIT 许可证](https://github.com/LINYIQ66/kopi-ai-agent/blob/main/LICENSE) 授权。