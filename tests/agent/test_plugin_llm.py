...
""",
        ...
        from kopi_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        kopi_home = tmp_path / ".kopi"
        kopi_home.mkdir()
        (kopi_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("KOPI_HOME", str(kopi_home))
        from kopi_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from kopi_cli.plugins import PluginContext, PluginManifest, PluginManager
