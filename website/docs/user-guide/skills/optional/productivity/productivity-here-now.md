...
| | |
|---|---|
| Source | Optional — install with `kopi skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${KOPI_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${KOPI_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
```bash
DRIVE="${KOPI_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `kopi`)    |
