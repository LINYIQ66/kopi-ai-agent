...
import { getSessionMessages, type SessionMessage, triggerCronJob } from '../kopi'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.kopiDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.kopiDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.kopiDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.kopiDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  useEffect(() => {
    const unsubscribe = window.kopiDesktop?.onDeepLink?.(payload => {
    ...
    void window.kopiDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.kopiDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
