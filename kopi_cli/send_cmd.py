"""CLI subcommand: ``kopi send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"kopi send: {file_path} is not a text file. --file reads the "
                ...
                f'  kopi send --to telegram "MEDIA:{file_path}"\n'
                f'  kopi send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  kopi send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"kopi send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"kopi send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"kopi send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"kopi send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"kopi send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `kopi gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.kopi/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_kopi_env() -> None:
    """Populate ``os.environ`` from ``~/.kopi/.env`` AND bridge top-level
    ...
    startup that ``kopi send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.kopi/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.kopi/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``kopi send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.kopi/.env and ~/.kopi/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "kopi send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  kopi send --to telegram \"hello\"\n"
            "  kopi send --to discord:#ops --file report.md\n"
            "  kopi send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "kopi send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `kopi send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        epilog=(
            ...
            "  kopi send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `kopi send --list telegram`.",
    )
