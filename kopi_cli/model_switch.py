...
- ``kopi_cli.providers``        -- canonical provider identity + overlays
- ``kopi_cli.model_normalize``  -- per-provider name formatting
...
def is_nous_kopi_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_KOPI_NON_AGENTIC_RE.search(model_name))
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``kopi config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from kopi_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'kopi model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'kopi doctor' — config issues detected:"
    ...
    # =================================================================
        ...
        try:
            from kopi_cli.config import load_config
            from kopi_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from kopi_cli.providers import resolve_user_provider as _ruser
    ...
    # kopi_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from kopi_cli.models import normalize_opencode_base_url
    ...
    kopi_warn = _check_kopi_model_warning(new_model)
    if kopi_warn:
        warnings.append(kopi_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from kopi_cli.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from kopi_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from kopi_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from kopi_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by kopi provider ID
    ...
    for kopi_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(kopi_id)
        if (
            _alias_target
            and _alias_target != kopi_id
        ...
        pconfig = PROVIDER_REGISTRY.get(kopi_id)
        ...
        # /model picker sees the SAME list `kopi model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(kopi_id)
        if not model_ids:
    ...
    for pid, overlay in KOPI_OVERLAYS.items():
        ...
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(kopi_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, kopi_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or kopi_slug in providers_store):
                    has_creds = True
        ...
        # configured.
        if not has_creds and kopi_slug == "anthropic":
            try:
                ...
                if (kopi_creds and kopi_creds.get("accessToken")) or \
        ...
        if kopi_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(kopi_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(kopi_slug)
                model_ids = _ids if _ids else (curated.get(kopi_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(kopi_slug, []) or curated.get(pid, [])
        elif kopi_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `kopi model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from kopi_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(kopi_slug, []) or curated.get(pid, [])
                if kopi_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(kopi_slug, model_ids)
        total = len(model_ids)
        if kopi_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": kopi_slug,
            "name": get_label(kopi_slug),
            "is_current": kopi_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "kopi",
        ...
        seen_slugs.add(kopi_slug.lower())
        _record_builtin_endpoint(kopi_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or KOPI_OVERLAYS (keeps /model in sync
    # with `kopi model`).
    try:
        from kopi_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from kopi_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`kopi_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from kopi_cli.models import fetch_openrouter_models
