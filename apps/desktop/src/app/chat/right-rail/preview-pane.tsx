        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.kopiDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  useEffect(() => {
    if (
      ...
      !window.kopiDesktop?.watchPreviewFile ||
      !window.kopiDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.kopiDesktop.onPreviewFileChanged(payload => {
    ...
    void window.kopiDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.kopiDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.kopiDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:kopi-preview')
