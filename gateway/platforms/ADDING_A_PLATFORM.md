...
Create a plugin directory in `~/.kopi/plugins/` (or under `plugins/platforms/`
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `kopi gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `kopi_cli/config.py` so the setup
...
And add it to the `kopi-gateway` composite:

```python
"kopi-gateway": {
    "includes": [..., "kopi-your-platform"]
}
```
...
## 12. Status Display (`kopi_cli/status.py`)
...
## 13. Gateway Setup Wizard (`kopi_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ kopi_cli/ toolsets.py \
  --include="*.py" -l | sort -u
