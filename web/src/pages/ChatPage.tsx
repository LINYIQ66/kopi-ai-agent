/**
 * ChatPage — embeds `kopi --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (kopi_cli/web_server.py)                          .
 *          ▼                                                              .
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__KOPI_SESSION_TOKEN__ &&
    !window.__KOPI_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `kopi dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__KOPI_AUTH_REQUIRED__;
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[kopi-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__KOPI_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets KOPI_HOME pointed at the
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="kopi-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
