...
sidebar_position: 5
title: "内置技能目录"
description: "随 KOPI AI AGENT 附带的内置技能目录"
---

# 内置技能目录

Hermes 附带一个大型内置技能库，安装时会复制到 `~/.kopi/skills/`。下方每个技能均链接至专属页面，包含完整定义、配置和用法说明。

Hermes 在执行 `kopi update` 时也会同步内置技能，但同步清单会尊重本地删除和用户编辑。如果此处列出的某个技能在你的 `~/.kopi/skills/` 目录树中缺失，它仍随 Hermes 一同发布；可通过 `kopi skills reset <name> --restore` 恢复。
...
| [`kopi-ai-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-kopi-ai-agent) | 配置、扩展或贡献 KOPI AI AGENT。 | `autonomous-ai-agents/kopi-ai-agent` |
...
| 技能 | 描述 | 路径 |
|-------|-------------|------|
| [`kopi-ai-agent-skill-authoring`](/user-guide/skills/bundled/software-development/software-development-kopi-ai-agent-skill-authoring) | 编写仓库内 SKILL.md：frontmatter、验证器、结构规范。 | `software-development/kopi-ai-agent-skill-authoring` |
...
| [`plan`](/user-guide/skills/bundled/software-development/software-development-plan) | 计划模式：将 Markdown 计划写入 `.kopi/plans/`，不执行。 | `software-development/plan` |
