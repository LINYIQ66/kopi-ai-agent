...
def isolated_home(monkeypatch):
    """Temp KOPI_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="kopi_test_31179_")
    kopi_home = os.path.join(test_home, ".kopi")
    os.makedirs(kopi_home)
...
def _fresh_modules():
    """Drop cached kopi modules so each test reloads against current env."""
