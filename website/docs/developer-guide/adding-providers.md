...
description: "How to add a new inference provider to KOPI AI AGENT — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---
...
1. `kopi_cli/auth.py` decides how credentials are found.
2. `kopi_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `kopi_cli/models.py` and `kopi_cli/main.py` make the provider show up in the CLI. (`kopi_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `kopi_cli/auth.py`
2. `kopi_cli/models.py`
3. `kopi_cli/runtime_provider.py`
4. `kopi_cli/main.py`
...
:::tip
`kopi_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `kopi setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$KOPI_HOME/plugins/model-providers/` both get picked up.
...
7. `kopi model` menu includes the provider
8. `kopi setup` wizard delegates to `main.py` automatically
...
User plugins at `$KOPI_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `kopi model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `kopi_cli/auth.py`
- `_PROVIDER_LABELS` in `kopi_cli/models.py`
- `_PROVIDER_ALIASES` in both `kopi_cli/auth.py` and `kopi_cli/models.py`
- CLI `--provider` choices in `kopi_cli/main.py`
...
## Step 2: Add auth metadata in `kopi_cli/auth.py`
...
## Step 3: Add model catalog and aliases in `kopi_cli/models.py`
...
## Step 4: Resolve runtime data in `kopi_cli/runtime_provider.py`
...
## Step 5: Wire the CLI in `kopi_cli/main.py`

A provider is not discoverable until it shows up in the interactive `kopi model` flow.

Update these in `kopi_cli/main.py`:
...
:::tip
`kopi_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `kopi model` and `kopi setup` automatically.
:::
...
- `tests/kopi_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/kopi_cli/test_model_switch_custom_providers.py` (and adjacent `tests/kopi_cli/test_model_switch_*.py`)
- `tests/kopi_cli/test_setup_model_provider.py`
...
python -m pytest tests/kopi_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/kopi_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
...
```bash
source venv/bin/activate
python -m kopi_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m kopi_cli.main model
python -m kopi_cli.main setup
```
...
- [ ] `ProviderConfig` added in `kopi_cli/auth.py`
- [ ] aliases added in `kopi_cli/auth.py` and `kopi_cli/models.py`
- [ ] model catalog added in `kopi_cli/models.py`
- [ ] runtime branch added in `kopi_cli/runtime_provider.py`
- [ ] CLI wiring added in `kopi_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `kopi model` but not `kopi setup`
