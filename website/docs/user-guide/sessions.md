...
KOPI AI AGENT automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.kopi/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
`kopi sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`kopi` or `kopi chat`) |
...
Hermes automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `kopi sessions list` or `kopi sessions browse`.
...
Hermes provides a full set of session management commands via `kopi sessions`:
...
`kopi sessions export` is one surface for every export format, selected with `--format`:
...
kopi sessions export backup.jsonl --redact
```
...
kopi sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
kopi sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
kopi sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
kopi sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `kopi-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
kopi sessions export --format trace

# One session to a local trace file
kopi sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
kopi sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.kopi/session-exports`).

```bash
# Export one session to Markdown
kopi sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
kopi sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
kopi sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
kopi sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
kopi sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
kopi sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
kopi sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
kopi sessions prune --newer-than 5h

# Explicit window with absolute timestamps
kopi sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
kopi sessions prune --source telegram
kopi sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
kopi sessions prune --newer-than 5h --title "smoke test"   # title substring
kopi sessions prune --older-than 30 --max-messages 3        # tiny sessions
kopi sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
kopi sessions prune --model gpt-5 --older-than 1w           # by model (substring)
kopi sessions prune --provider openrouter --older-than 60   # by billing provider
kopi sessions prune --branch feature/old-experiment         # by git branch
kopi sessions prune --user 12345678 --chat-type group       # by messaging origin
kopi sessions prune --max-tokens 500 --older-than 7         # by token usage
kopi sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
kopi sessions prune --newer-than 5h --dry-run
...
the implicit 90-day default, so `kopi sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `kopi sessions prune` keeps the 90-day cutoff. Every
...
`kopi sessions archive` takes the same filters as `prune` but soft-hides
...
kopi sessions archive --newer-than 5h

# Archive by title substring, preview first
kopi sessions archive --title "dry run" --dry-run
kopi sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `kopi sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`kopi sessions list` and `/resume` but remain in the database and can be
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`kopi insights`](/reference/cli-commands#kopi-insights).
...
|------|------|-------------|
| SQLite database | `~/.kopi/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.kopi/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.kopi/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the gateway's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.kopi/sessions/sessions.json` is the **gateway routing index** — it maps
...
`kopi sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.kopi/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `kopi sessions list`, the cause is
`state.db` not receiving them — run `kopi sessions repair` and watch for a
...
`*.jsonl` files in `~/.kopi/sessions/`. They are no longer written or
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Hermes process in the same `KOPI_HOME`
