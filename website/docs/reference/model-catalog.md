...
Hermes fetches curated model lists for **OpenRouter** and **Nous Portal** from a JSON manifest hosted alongside the docs site. This lets maintainers update picker lists without shipping a new `kopi-ai-agent` release.
...
```
https://kopi-ai-agent.nousresearch.com/docs/api/model-catalog.json
```
...
| When | What happens |
|---|---|
| `/model` or `kopi model` | Fetches if disk cache is stale, else uses cache |
...
Cache location: `~/.kopi/cache/model_catalog.json`.
...
# editing OPENROUTER_MODELS or _PROVIDER_MODELS["nous"] in kopi_cli/models.py).
