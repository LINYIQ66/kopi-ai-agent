...
| | |
|---|---|
| Source | Optional — install with `kopi skills install official/autonomous-ai-agents/honcho` |
...
See: https://docs.honcho.dev/v3/guides/integrations/kopi#running-honcho-locally-with-hermes

### Verify

```bash
kopi honcho status    # shows resolved config, connection test, peer info
```
...
Manual override: `kopi honcho map my-project-name`
...
```bash
kopi honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Config file: `$KOPI_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `kopi honcho setup`. Ensure `memory.provider: honcho` is in `~/.kopi/config.yaml`.

### Memory not persisting across sessions
Check `kopi honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `kopi profile create <name> --clone`. For existing profiles: `kopi honcho sync`.
...
| Command | Description |
|---------|-------------|
| `kopi honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `kopi honcho status` | Show resolved config, connection test, peer info for active profile |
| `kopi honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `kopi honcho disable` | Disable Honcho for the active profile |
| `kopi honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `kopi honcho peers` | Show peer identities across all profiles |
| `kopi honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `kopi honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `kopi honcho sessions` | List known directory-to-session-name mappings |
| `kopi honcho map <name>` | Map current working directory to a Honcho session name |
| `kopi honcho identity` | Seed AI peer identity or show both peer representations |
| `kopi honcho sync` | Create host blocks for all Hermes profiles that don't have one yet |
| `kopi honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Hermes + Honcho |
| `kopi memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `kopi memory status` | Show active memory provider and config |
| `kopi memory off` | Disable external memory provider |
