...
`kopi skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.kopi.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.kopi.blueprint`` is a mapping containing
    ...
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.kopi.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.kopi.blueprint`` block) they can hand
    to ``kopi skills publish`` to share. ``body`` is the plain-language
