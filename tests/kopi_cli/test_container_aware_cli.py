...
def test_get_container_exec_info_none_inside_container(container_env):
    ...
    with patch("kopi_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_kopi_dev(container_env, monkeypatch):
    """Returns None when KOPI_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("KOPI_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_kopi_dev_zero(container_env, monkeypatch):
    """KOPI_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("KOPI_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        kopi_home = Path(tmpdir) / ".kopi"
        kopi_home.mkdir()
        (kopi_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("kopi_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_kopi_home": lambda: kopi_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KOPI_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=kopi-custom\n"
        "exec_user=myuser\n"
        "kopi_bin=/opt/kopi/bin/kopi\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "kopi-custom"
    assert info["exec_user"] == "myuser"
    assert info["kopi_bin"] == "/opt/kopi/bin/kopi"
