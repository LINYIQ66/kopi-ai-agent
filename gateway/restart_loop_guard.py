...
Defenses 1 and 2 (the ``_KOPI_GATEWAY`` guard on ``kopi gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.kopi.gateway")``,
...
State lives in ``<KOPI_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_kopi_home() / "gateway" / "restart_loop.json"
