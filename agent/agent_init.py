...
from kopi_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt5_autoraise_notice(autoraise: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: kopi config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$KOPI_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_kopi_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$KOPI_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .kopi.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / KOPI_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.kopi/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from kopi_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind KOPI_DEV_CREDITS.
    ...
    # both live under ~/.kopi/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from kopi_logging import setup_logging, setup_verbose_logging
    setup_logging(kopi_home=_ra()._kopi_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that kopi_logging.setup_logging() does not
        ...
        # handler level inside kopi_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from kopi_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from kopi_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `kopi model` to "
                        "select a provider, or run `kopi setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff KOPI_KANBAN_TASK is set).
    ...
    agent.logs_dir = kopi_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.kopi/sessions/session_{sid}.json)
    ...
    try:
        from kopi_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from kopi_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "kopi"
                    except Exception:
                        pass
    ...
    if codex_app_server_auto_compaction not in {"native", "kopi", "off"}:
        _ra().logger.warning(
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from kopi_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from kopi_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by KOPI AI AGENT.  Choose a model with at least "
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from kopi_cli.model_switch import _check_kopi_model_warning
