...
When KOPI AI AGENT runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.kopi/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.kopi/config.yaml` (switchable per session
via `/personality`).
...
Install MCP servers via `~/.kopi/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. KOPI AI AGENT's deliverable
