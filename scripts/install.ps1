...
param(
    ...
    #     `kopi desktop` already builds on demand.
...
function Resolve-UvCmd {
    ...
    # sibling process and KOPI_HOME wasn't propagated).
...
function Set-GitBashEnvVar {
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("KOPI_GIT_BASH_PATH", $candidate, "User")
            $env:KOPI_GIT_BASH_PATH = $candidate
            Write-Info "Set KOPI_GIT_BASH_PATH=$candidate"
            return
        }
    }
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed — mirror ``kopi update`` and
            ...
            try {
                ...
                Write-Info "terminals, running kopi processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\kopi-ai-agent-extract"
    ...
    # next `kopi update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running kopi process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running kopi processes before recreating venv..."
            ...
            # user (see kopi_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `kopi update` / the
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install kopi-ai-agent package even with no extras. Inspect the uv pip install output above."
    }
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m kopi_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `kopi dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\kopi_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `kopi dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `kopi dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: kopi_cli/web_server.py"
        }
    }
...
function Write-BootstrapMarker {
    ...
    $markerPath = Join-Path $InstallDir ".kopi-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'kopi setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
...
function Install-PlatformSdks {
    ...
    # ~/.kopi/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'kopi setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m kopi_cli.main setup
    } else {
...
function Start-GatewayIfConfigured {
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'kopi whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: kopi gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: kopi gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: kopi gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host ""
    Write-Host "   kopi              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   kopi setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   kopi config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   kopi config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   kopi gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   kopi update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://kopi-ai-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
