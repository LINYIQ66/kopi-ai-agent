...
- GET  /v1/models                  — lists kopi-ai-agent and any configured model_routes aliases
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to kopi-ai-agent
...
def _kopi_version() -> str:
    """Return the kopi-ai-agent version string, or "dev" if it can't be resolved.

    Tries the installed package metadata first (authoritative for a pip/uv
    install), then the in-tree ``kopi_cli.__version__`` (covers editable /
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_kopi_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted KOPI_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # kopi_state._WAL_INCOMPAT_MARKERS).
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through kopi-ai-agent's AIAgent.
    """
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``kopi sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the kopi-api-server default.
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "kopi-ai-agent", "version": _kopi_version()}
        )
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "status": "ok",
            "platform": "kopi-ai-agent",
            "version": _kopi_version(),
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list kopi-ai-agent and any configured model_routes aliases."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "kopi.api_server.capabilities",
            "platform": "kopi-ai-agent",
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "kopi.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "kopi.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "kopi.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "kopi.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {
                "object": "kopi.session.chat.completion",
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``kopi.tool.progress`` with ``status: running``.
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: kopi.tool.progress`` SSE event so
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "kopi.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # that is the exact surface the kopi-0day campaign abused to write
            # ~/.kopi/config.yaml and plant persistence. Sandboxing (Docker /
