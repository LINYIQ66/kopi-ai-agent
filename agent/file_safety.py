...
def _kopi_home_path() -> Path:
    """Resolve the active KOPI_HOME (profile-aware) without circular imports."""
    try:
        from kopi_constants import get_kopi_home  # local import to avoid cycles
        return get_kopi_home()
    except Exception:
        return Path(os.path.expanduser("~/.kopi"))


def _kopi_root_path() -> Path:
    """Resolve the Hermes root dir (always the parent of any profile, never per-profile)."""
    try:
        from kopi_constants import get_default_kopi_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    kopi_home = _kopi_home_path()
    kopi_root = _kopi_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(kopi_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(kopi_root / ".anthropic_oauth.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved KOPI_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("KOPI_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def is_write_denied(path: str) -> bool:
    ...
    kopi_dirs = []
    for base in (_kopi_home_path(), _kopi_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in kopi_dirs:
                kopi_dirs.append(real)
        except Exception:
            continue

    for base_real in kopi_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Three categories are blocked:
      ...
      * Internal Hermes cache files under ``KOPI_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under KOPI_HOME and the global Hermes
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.kopi/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active KOPI_HOME (profile-aware) AND the global
    # Hermes root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (KOPI_HOME points at
...
#
# Hermes profiles are separate KOPI_HOME dirs under
...
#
# Reference: May 2026 incident where a kopi-security profile session
# edited skills under both ``~/.kopi/profiles/kopi-security/skills/``
# AND ``~/.kopi/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under KOPI_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from KOPI_HOME.
    ...
    """
    try:
        home_real = _kopi_home_path().resolve()
        root_real = _kopi_root_path().resolve()
...
#
#   <KOPI_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.kopi/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.kopi/…`` segment and warns
...
# (the agent's view inside the container is plain ``/root/.kopi/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.kopi`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.kopi/…`` and returns the
    ...
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .kopi / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".kopi":
            return i + 4
    return None
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    ...
    Otherwise returns a dict with:
      ...
      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.kopi``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.kopi`` (what the
    ...
    KOPI_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"under the real KOPI_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.kopi/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.kopi/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host KOPI_HOME)
    """
...
def get_container_mirror_warning(
    ...
    return (
        ...
        f"{info['inner_path']!r} under the real KOPI_HOME. Use the "
