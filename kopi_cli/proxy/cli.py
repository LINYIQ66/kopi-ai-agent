"""CLI handlers for the ``kopi proxy`` subcommand."""
...
from kopi_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"kopi auth add {adapter.name}")
        print(
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print(
        "\nStart the proxy with: kopi proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``kopi proxy <subcommand>``."""
    ...
    print(
        "kopi proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  kopi proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
