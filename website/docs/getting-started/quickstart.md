...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with KOPI AI AGENT — from install to chatting in under 5 minutes"
---
...
To easily install the command-line and desktop applications, [download the Hermes Desktop installer](https://kopi-ai-agent.nousresearch.com/) from our website and run it.
...
The single most important setup step. Use `kopi model` to walk through the choice interactively:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `kopi update`. Re-enable anything later with `kopi tools`, seed skills with `kopi skills opt-in --sync`, or tune settings with `kopi setup agent`.
:::
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `kopi_cli/models.py` may change between releases) | `kopi model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `kopi model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `kopi model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `kopi model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `kopi model` (requires `copilot` CLI + `copilot login`) |
...
KOPI AI AGENT requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `kopi model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::
...
```bash
kopi config set model anthropic/claude-opus-4.6
kopi config set terminal.backend docker
kopi config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
kopi            # classic CLI
kopi --tui      # modern TUI (recommended)
```
...
```bash
kopi --continue    # Resume the most recent session
kopi -c            # Short form
```
...
```bash
kopi gateway setup    # Interactive platform configuration
```
...
- `kopi tools` — tune tool access per platform
- `kopi skills` — browse and install reusable workflows
...
```bash
kopi config set terminal.backend docker    # Docker isolation
kopi config set terminal.backend ssh       # Remote server
```
...
```bash
kopi skills browse                      # list everything available
kopi skills search kubernetes           # find skills by keyword
kopi skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `kopi skills browse` shows the exact slugs to use.
...
(If you installed without `[all]`, run `cd ~/.kopi/kopi-ai-agent && uv pip install -e ".[acp]"` first.)
...
| `kopi doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `kopi model`
3. `kopi setup`
4. `kopi sessions list`
5. `kopi --continue`
6. `kopi gateway status`
...
| `kopi tools` | Configure which tools are enabled per platform |
| `kopi setup` | Full setup wizard (configures everything at once) |
| `kopi doctor` | Diagnose issues |
| `kopi update` | Update to latest version |
| `kopi gateway` | Start the messaging gateway |
| `kopi --continue` | Resume last session |
