...
kopi meet install                 # pip deps + Chromium (transcribe only)
kopi meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
kopi meet auth                    # optional; skips guest-lobby wait
kopi meet setup                   # preflight checks
```

`kopi meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or KOPI_MEET_REALTIME_KEY in ~/.kopi/.env
```
...
pip install playwright websockets && python -m playwright install chromium
kopi plugins enable google_meet
kopi meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
kopi meet node approve my-mac ws://<mac-ip>:18789 <token>
kopi meet node ping my-mac                   # confirm reachable
```

Run `kopi meet setup` to preflight local prereqs.
...
- Guest mode sits in the lobby until a host admits. Warn the user; `kopi meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `KOPI_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `kopi_meet_src`). |
...
Local:
```
$KOPI_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$KOPI_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `kopi meet node approve`.
