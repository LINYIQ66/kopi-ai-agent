---
title: "KOPI AI AGENT — 配置、扩展或贡献 KOPI AI AGENT"
sidebar_label: "KOPI AI AGENT"
description: "配置、扩展或贡献 KOPI AI AGENT"
---
...
|---|---|
| 来源 | 内置（默认安装） |
| 路径 | `skills/autonomous-ai-agents/kopi-ai-agent` |
| 版本 | `2.1.0` |
| 作者 | KOPI AI AGENT + Teknium |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `kopi`, `setup`, `configuration`, `multi-agent`, `spawning`, `cli`, `gateway`, `development` |
...
KOPI AI AGENT 是 Kopi Ai Agent Pte Ltd 开发的开源 AI agent 框架，可在终端、消息平台和 IDE 中运行。它与 Claude Code（Anthropic）、Codex（OpenAI）和 OpenClaw 同属一类——使用工具调用（tool calling）与系统交互的自主编码和任务执行 agent。Hermes 支持任意 LLM 提供商（OpenRouter、Anthropic、OpenAI、DeepSeek、本地模型及 15+ 其他提供商），可在 Linux、macOS 和 WSL 上运行。
...
**此 skill 帮助你高效使用 KOPI AI AGENT** — 包括设置、配置功能、生成额外的 agent 实例、排查问题、找到正确的命令和设置，以及在需要扩展或贡献时理解系统的工作原理。

**文档：** https://kopi-ai-agent.nousresearch.com/docs/
...
kopi auth                 交互式凭据管理器
kopi auth add PROVIDER    添加 OAuth 或 API key 凭据（例如 nous、openai-codex、qwen-oauth）
kopi auth list            列出已存储的凭据
kopi auth remove PROVIDER 移除已存储的凭据
...
```
kopi mcp serve            Run Hermes as an MCP server
...
平台文档：https://kopi-ai-agent.nousresearch.com/docs/user-guide/messaging/
...
kopi uninstall            Uninstall Hermes
```
...
在交互式聊天会话中输入这些命令。新命令会不定期上线；如果以下内容看起来过时，请在会话内运行 `/help` 获取权威列表，或查看[实时斜杠命令参考](https://kopi-ai-agent.nousresearch.com/docs/reference/slash-commands)。命令注册表的权威来源是 `kopi_cli/commands.py` — 每个消费方（自动补全、Telegram 菜单、Slack 映射、`/help`）均从中派生。
...
Profiles 使用 `~/.kopi/profiles/<name>/`，布局相同。

### 配置节

使用 `kopi config edit` 或 `kopi config set section.key value` 编辑。
...
完整配置参考：https://kopi-ai-agent.nousresearch.com/docs/user-guide/configuration

### 提供商

支持 20+ 个提供商。通过 `kopi model` 或 `kopi setup` 设置。
...
完整提供商文档：https://kopi-ai-agent.nousresearch.com/docs/integrations/providers

### Toolset

通过 `kopi tools`（交互式）或 `kopi tools enable/disable NAME` 启用/禁用。
...
完整枚举位于 `toolsets.py` 的 `TOOLSETS` 字典中；`_KOPI_CORE_TOOLS` 是大多数平台继承的默认工具包。
...
常见的"为什么 Hermes 对我的输出/工具调用/命令做了 X？"开关——以及更改它们的确切命令。其中大多数需要新会话（聊天中的 `/reset`，或启动新的 `kopi` 调用），因为它们在启动时只读取一次。
...
```bash
kopi config set security.redact_secrets true       # 全局启用
```

**需要重启。** `security.redact_secrets` 在导入时快照 — 在会话中途切换（例如通过工具调用执行 `export KOPI_REDACT_SECRETS=true`）对正在运行的进程**不会**生效。告知用户在终端运行 `kopi config set security.redact_secrets true`，然后启动新会话。这是有意为之——防止 LLM 在任务中途自行切换该开关。
...
```bash
kopi config set privacy.redact_pii true    # 启用
kopi config set privacy.redact_pii false   # 禁用（默认）
```
...
```bash
kopi config set approvals.mode smart       # 推荐的折中方案
kopi config set approvals.mode off         # 绕过一切（不推荐）
```
...
某些 shell hook 集成在触发前需要明确加入允许列表。通过 `~/.kopi/shell-hooks-allowlist.json` 管理——在 hook 首次尝试运行时以交互方式提示。

### 禁用 web/browser/image-gen 工具

要完全阻止模型访问网络或媒体工具，打开 `kopi tools` 并按平台切换。在下次会话（`/reset`）后生效。参见上方的工具与 Skill 部分。
...
- **fire-and-forget 使用 `kopi chat -q`** — 无需 PTY
...
持久化调度器——`cron/jobs.py` + `cron/scheduler.py`。通过 `cronjob` 工具、`kopi cron` CLI（`list`、`add`、`edit`、`pause`、`resume`、`run`、`remove`）或 `/cron` 斜杠命令驱动。
...
用户文档：https://kopi-ai-agent.nousresearch.com/docs/user-guide/features/cron
...
- **遥测：** `~/.kopi/skills/.usage.json` 中的 sidecar 保存每个 skill 的 `use_count`、`view_count`、`patch_count`、`last_activity_at`、`state`、`pinned`。

配置：`curator.*`（`enabled`、`interval_hours`、`min_idle_hours`、`stale_after_days`、`archive_after_days`、`backup.*`）。
用户文档：https://kopi-ai-agent.nousresearch.com/docs/user-guide/features/curator

### Kanban（多 agent 工作队列）

用于多 profile/多 worker 协作的持久化 SQLite 看板（kanban）。用户通过 `kopi kanban <verb>` 驱动；调度器生成的 worker 看到由 `KOPI_KANBAN_TASK` 控制的专注 `kanban_*` toolset，orchestrator profile 可以选择加入更广泛的 `kanban` toolset。普通会话除非配置，否则没有任何 `kanban_*` schema 占用。
...
- **隔离：** 看板是硬边界（worker 在环境中固定 `KOPI_KANBAN_BOARD`）；租户是看板内用于工作区路径和记忆键隔离的软命名空间。

用户文档：https://kopi-ai-agent.nousresearch.com/docs/user-guide/features/kanban
...
**首次运行时 HTTP 400 "No models provided"。** `config.yaml` 保存时带有 UTF-8 BOM（Windows 应用写入时常见）。重新保存为不带 BOM 的 UTF-8。`kopi config edit` 写入时不带 BOM；手动在记事本中编辑是常见原因。
...
### 工具不可用
1. `kopi tools` — 检查 toolset 是否为你的平台启用
2. 某些工具需要环境变量（检查 `.env`）
3. 启用工具后执行 `/reset`

### 模型/提供商问题
1. `kopi doctor` — 检查配置和依赖
2. `kopi auth` — 重新认证 OAuth 提供商（或 `kopi auth add <provider>`）
3. 检查 `.env` 中是否有正确的 API key
4. **Copilot 403**：`gh auth login` 的 token **不适用于** Copilot API。必须通过 `kopi model` → GitHub Copilot 使用 Copilot 专用 OAuth 设备码流程。
...
### Skill 未显示
1. `kopi skills list` — 验证已安装
2. `kopi skills config` — 检查平台启用状态
3. 显式加载：`/skill name` 或 `kopi -s name`
...
- **Gateway 崩溃循环**：重置失败状态：`systemctl --user reset-failed kopi-gateway`
...
| 查找内容... | 位置 |
|----------------|----------|
| 配置选项 | `kopi config edit` 或[配置文档](https://kopi-ai-agent.nousresearch.com/docs/user-guide/configuration) |
| 可用工具 | `kopi tools list` 或[工具参考](https://kopi-ai-agent.nousresearch.com/docs/reference/tools-reference) |
| 斜杠命令 | 会话内 `/help` 或[斜杠命令参考](https://kopi-ai-agent.nousresearch.com/docs/reference/slash-commands) |
| Skill 目录 | `kopi skills browse` 或[Skill 目录](https://kopi-ai-agent.nousresearch.com/docs/reference/skills-catalog) |
| 提供商设置 | `kopi model` 或[提供商指南](https://kopi-ai-agent.nousresearch.com/docs/integrations/providers) |
| 平台设置 | `kopi gateway setup` 或[消息文档](https://kopi-ai-agent.nousresearch.com/docs/user-guide/messaging/) |
| MCP 服务器 | `kopi mcp list` 或[MCP 指南](https://kopi-ai-agent.nousresearch.com/docs/user-guide/features/mcp) |
| Profiles | `kopi profile list` 或[Profiles 文档](https://kopi-ai-agent.nousresearch.com/docs/user-guide/profiles) |
| Cron 任务 | `kopi cron list` 或[Cron 文档](https://kopi-ai-agent.nousresearch.com/docs/user-guide/features/cron) |
| 记忆 | `kopi memory status` 或[记忆文档](https://kopi-ai-agent.nousresearch.com/docs/user-guide/features/memory) |
| 环境变量 | `kopi config env-path` 或[环境变量参考](https://kopi-ai-agent.nousresearch.com/docs/reference/environment-variables) |
| CLI 命令 | `kopi --help` 或[CLI 参考](https://kopi-ai-agent.nousresearch.com/docs/reference/cli-commands) |
| Gateway 日志 | `~/.kopi/logs/gateway.log` |
| 会话文件 | `~/.kopi/sessions/` 或 `kopi sessions browse` |
| 源代码 | `~/.kopi/kopi-ai-agent/` |

---

## 贡献者快速参考

面向偶尔贡献者和 PR 作者。完整开发者文档：https://kopi-ai-agent.nousresearch.com/docs/developer-guide/
...
配置：`~/.kopi/config.yaml`（设置）、`~/.kopi/.env`（API key）。
...
**2. 添加到 `toolsets.py`** → `_KOPI_CORE_TOOLS` 列表。

自动发现：任何包含顶层 `registry.register()` 调用的 `tools/*.py` 文件都会自动导入——无需手动列出。

所有处理器必须返回 JSON 字符串。路径使用 `get_kopi_home()`，永远不要硬编码 `~/.kopi`。

### 添加斜杠命令

1. 在 `kopi_cli/commands.py` 的 `COMMAND_REGISTRY` 中添加 `CommandDef`
...
- 测试自动将 `KOPI_HOME` 重定向到临时目录——永远不会触及真实的 `~/.kopi/`
- 推送任何变更前运行完整套件
...
- POSIX 文件模式（0o600 等）→ `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")`（参见 `tests/kopi_cli/test_auth_toctou_file_modes.py`）
- `signal.SIGALRM` → 仅 Unix（参见 `tests/conftest.py::_enforce_test_timeout`）
...
- 所有路径使用 `kopi_constants` 中的 `get_kopi_home()`（profile 安全）
