"""
Gateway subcommand for kopi CLI.

Handles: kopi gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_kopi_home is imported lazily at call sites to avoid ImportError
# when kopi_constants is cached from a pre-update version during `kopi update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "kopi CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``kopi gateway status`` from falsely counting the ``kopi`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``kopi gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_kopi_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"kopi_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. KOPI_HOME
        ...
        # KOPI_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "kopi_home=" in command_lc
            and f"kopi_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``kopi update``
...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from kopi_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "kopi_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m kopi_cli.main gateway
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose KOPI_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``kopi update`` users
    # with no running gateway until they re-invoke ``kopi gateway``
    ...
    from kopi_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from kopi_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / KOPI_HOME) the windowless
        # base interpreter needs to import kopi_cli.  Empty on POSIX, where
...
def _sync_kopi_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``KOPI_HOME``.

    Under ``sudo``, ``KOPI_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_kopi_home` falls back to ``/root/.kopi`` — the wrong
    profile. The unit file pins ``KOPI_HOME`` for the actual gateway
    ...
    unit_home = env.get("KOPI_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("KOPI_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["KOPI_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}kopi gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}kopi gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from kopi_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: kopi gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `kopi gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``kopi gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from kopi_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (kopi_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (KOPI_HOME-scoped).
...
from kopi_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``kopi gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``KOPI_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("KOPI_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "kopi-gateway"
SERVICE_DESCRIPTION = "KOPI AI AGENT Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current KOPI_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (KOPI_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from kopi_constants import get_default_kopi_root

    home = get_kopi_home().resolve()
    default = get_default_kopi_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary KOPI_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(kopi_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when KOPI_HOME is a named profile.

    For ``~/.kopi/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        kopi_home: Optional explicit KOPI_HOME path. Defaults to the current
            ``get_kopi_home()`` value. Should be passed when generating a
        ...
        default_root: Optional Hermes root to compare against. Used when
            ...
            process, where ``Path.home()`` and ``get_default_kopi_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from kopi_constants import get_default_kopi_root

    home = Path(kopi_home or str(get_kopi_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_kopi_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(kopi_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".kopi"
    try:
        Path(kopi_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(kopi_home, default_root=target_root)
    except ValueError:
        return _profile_arg(kopi_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this KOPI_HOME.

    Default ``~/.kopi`` returns ``kopi-gateway`` (backward compatible).
    Profile ``~/.kopi/profiles/coder`` returns ``kopi-gateway-coder``.
    Any other KOPI_HOME appends a short hash for uniqueness.
    """
...
# kopi-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (kopi-gateway-*.service) and unrelated third-party
# "kopi" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("kopi.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "kopi_cli.main gateway",
    "kopi_cli/main.py gateway",
    "gateway/run.py",
    " kopi gateway ",
    "/kopi gateway ",
)
...
def _find_legacy_kopi_units() -> list[tuple[str, Path, bool]]:
    ...
    different service name (e.g. ``kopi.service`` before the rename to
    ``kopi-gateway.service``). When both a legacy unit and the current
    ``kopi-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``kopi-gateway-coder.service`` and unrelated third-party
      ``kopi-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``kopi.service`` running an unrelated
...
def has_legacy_kopi_units() -> bool:
    """Return True when any legacy Hermes gateway unit files exist."""
    return bool(_find_legacy_kopi_units())
...
def print_legacy_unit_warning() -> None:
    ...
    """
    legacy = _find_legacy_kopi_units()
    if not legacy:
        return
    ...
    print_info("  These run alongside the current kopi-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    kopi gateway migrate-legacy")


def remove_legacy_kopi_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Hermes gateway unit files.

    Iterates over whatever ``_find_legacy_kopi_units()`` returns — which is
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: kopi gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo kopi gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    kopi gateway uninstall")
    print_info("    sudo kopi gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: kopi gateway install"
            )
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.kopi`` → ``ai.kopi.gateway.plist`` (backward compatible).
    Profile ``~/.kopi/profiles/coder`` → ``ai.kopi.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.kopi.gateway-{suffix}" if suffix else "ai.kopi.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.kopi/kopi-ai-agent  -> /home/alice/.kopi/kopi-ai-agent
      /opt/kopi                 -> /opt/kopi  (kept as-is)
...
def _kopi_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current KOPI_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_kopi_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.kopi                    → /home/alice/.kopi
      /root/.kopi/profiles/coder     → /home/alice/.kopi/profiles/coder
      /opt/custom-kopi               → /opt/custom-kopi  (kept as-is)
    """
    current_kopi = get_kopi_home().resolve()
    current_default = (Path.home() / ".kopi").resolve()
    target_default = Path(target_home_dir) / ".kopi"

    # Default ~/.kopi → remap to target user's default
    if current_kopi == current_default:
        return str(target_default)

    # Profile or subdir of ~/.kopi → preserve the relative structure
    try:
        relative = current_kopi.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.kopi) — keep as-is
        return str(current_kopi)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    kopi_node = kopi_home / "node" / "bin"
    if _is_dir(kopi_node):
        candidates.append(str(kopi_node))
    kopi_nm = kopi_home / "node_modules" / ".bin"
    if _is_dir(kopi_nm):
        candidates.append(str(kopi_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m kopi_cli.main``, so module
    ...
    dir, or a clone that ``kopi update`` later relocates/removes — the path
    ...
    ``KOPI_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if KOPI_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        kopi_home = _kopi_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(kopi_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's KOPI_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(kopi_home) if kopi_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m kopi_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
"""
    ...
    kopi_home = str(get_kopi_home().resolve())
    profile_arg = _profile_arg(kopi_home)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir KOPI_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved KOPI_HOME
    ...
    exported a throwaway ``KOPI_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``kopi gateway restart``
    with ``KOPI_HOME=/tmp/kopi-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="KOPI_HOME=..."`` lines and launchd
    ``<key>KOPI_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'KOPI_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>KOPI_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp KOPI_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: KOPI_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported KOPI_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only KOPI_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp KOPI_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../kopi_test`` path into
    # ``Environment="KOPI_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/kopi_test"' in new_unit
        or "/kopi_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # KOPI_HOME into the unit (manual E2E homes like /tmp/kopi-e2e-NNN
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo kopi gateway uninstall --system")
    print_info("         kopi gateway install")
    print_info("         kopi gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("KOPI_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(
    ...
    # Offer to remove legacy units (kopi.service from pre-rename installs)
    # before installing the new kopi-gateway.service. If both remain, they
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}kopi gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}kopi gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}kopi gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_kopi_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `kopi gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `kopi gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}kopi gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}kopi gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}kopi gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}kopi gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.kopi.gateway-{suffix}" if suffix else "ai.kopi.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``kopi gateway start/stop/status`` call).
...
#      background process (the `nohup kopi gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_kopi_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_kopi_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m kopi_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active KOPI_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "kopi_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup kopi gateway run --replace`
    ...
    """
    from kopi_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_kopi_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup kopi gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.kopi/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See kopi-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_kopi_home() / "logs" / "launchd-reload.log"
        try:
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  kopi gateway status             # Check status")
    from kopi_constants import display_kopi_home as _dhh
...
def launchd_stop():
    ...
    # `kopi gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: kopi gateway stop")
            else:
    ...
    if deep:
        log_file = get_kopi_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``KOPI_S6_SUPERVISED_CHILD``.
    """
    if os.environ.get("INVOCATION_ID"):
        return True
    if os.environ.get("KOPI_S6_SUPERVISED_CHILD"):
        return True
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``kopi gateway run`` is always a misconfiguration, so we
    ...
    try:
        from kopi_constants import get_default_kopi_root
        default_root = get_default_kopi_root()
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``kopi gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``kopi gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other KOPI_HOME roots.
    """
    ...
    print("  Use 'kopi gateway restart' to replace it,")
    print("  or 'kopi gateway stop' first.")
    print("  Or use 'kopi gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("KOPI_ALLOW_ROOT_GATEWAY")):
        return
    ...
    print(
        "  The image entrypoint normally drops privileges to the 'kopi' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/kopi/docker/entrypoint.sh before the Hermes command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$KOPI_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set KOPI_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `kopi gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set KOPI_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `kopi gateway restart` which already
    ...
    # `kopi gateway start/restart` — leaving the gateway vulnerable to
    ...
    # KOPI_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("KOPI_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from kopi_constants import get_kopi_home as _ghh
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "6. To authorize users, use DM pairing: kopi pairing generate bluebubbles",
...
def _all_platforms() -> list[dict]:
    ...
    ``kopi setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run kopi under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.kopi/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_kopi_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from kopi_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'kopi pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: kopi pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'kopi gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print_info(
        "  3. Hermes will store the returned account_id/token in ~/.kopi/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `kopi gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_kopi_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `kopi pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `kopi pairing approve`."
        )
    elif access_idx == 1:
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``kopi_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from kopi_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.kopi/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.kopi/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: kopi gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: kopi gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: kopi gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo kopi gateway install --system"
                        )
                    print_info("  Or run in foreground:  kopi gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: kopi gateway run")
                print_info(
                    "  For persistence:   tmux new -s kopi 'kopi gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: kopi gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup kopi gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'kopi gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`kopi_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from kopi_cli.service_manager import (
    ...
    if profile is None:
        ...
        # KOPI_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``kopi gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``kopi gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``kopi gateway run`` are unaffected.
      2. ``KOPI_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``kopi gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``KOPI_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("KOPI_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("KOPI_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${KOPI_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if KOPI_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or KOPI_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-kopi service's
    # pattern in docker/s6-rc.d/main-kopi/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: kopi gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: kopi gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s kopi 'kopi gateway run'"
                )
                print()
            ...
            print()
            print(
                "  kopi gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s kopi 'kopi gateway run'         # persistent via tmux"
            )
            print(
                "  nohup kopi gateway run > ~/.kopi/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from kopi_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  kopi profile create <name>     # creates the s6 service slot")
                print("  kopi -p <name> gateway start   # bring it up via s6")
                print("  kopi status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: kopi gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: kopi gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: kopi gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  kopi profile delete <name>     # tears down the s6 service slot")
                print("  kopi -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `kopi -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: kopi gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_KOPI_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `kopi gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_KOPI_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `kopi gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: kopi gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  kopi gateway install")
                else:
                    print("To install as a service:")
                    print("  kopi gateway install")
                    print("  sudo kopi gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  kopi gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup kopi gateway run > ~/.kopi/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s kopi 'kopi gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  kopi gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  kopi gateway install  # Install as user service")
                    print(
                        "  sudo kopi gateway install --system  # Install as boot-time system service"
                    )
        ...
        # pre-rename installs (e.g. kopi.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_kopi_units(interactive=not yes, dry_run=dry_run)
