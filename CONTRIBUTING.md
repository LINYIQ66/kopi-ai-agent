# Contributing to KOPI AI AGENT

Thank you for contributing to KOPI AI AGENT! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo Kopi Ai Agent Pte Ltd/kopi-ai-agent "<your terms>"
  gh search prs --repo Kopi Ai Agent Pte Ltd/kopi-ai-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/LINYIQ66/kopi-ai-agent/issues?q=) · [PRs (all states)](https://github.com/LINYIQ66/kopi-ai-agent/pulls?q=is%3Apr).
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `kopi skills browse` (labeled "official") and install it with `kopi skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Kopi Ai Agent Pte Ltd Discord](https://discord.gg/Kopi Ai Agent Pte Ltd). Users can install it with `kopi skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.kopi/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(kopi_home, config)` for setup-wizard integration
...
- Integrate with `kopi memory setup` via `post_setup()` — no need to touch core code
...
echo "OPENROUTER_API_KEY=***" >> ~/.kopi/.env
```

### Run

```bash
# The standard installer already put `kopi` on PATH.
kopi doctor
kopi chat -q "Hello"
```

If you used the manual clone fallback, run `./kopi` from the checkout or
...
├── toolsets.py               # Tool groupings and presets (kopi-cli, kopi-telegram, etc.)
├── kopi_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── kopi_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.kopi/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (kopi-ai-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.kopi/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.kopi/`)

| Path | Purpose |
|------|---------|
| `~/.kopi/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.kopi/.env` | API keys and secrets |
| `~/.kopi/auth.json` | OAuth credentials (Nous Portal) |
| `~/.kopi/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.kopi/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.kopi/state.db` | SQLite session database |
| `~/.kopi/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.kopi/cron/` | Scheduled job data |
| `~/.kopi/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`kopi_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.kopi/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_KOPI_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.kopi`:
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `kopi setup` or update `~/.kopi/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "KOPI AI AGENT" is the secondary collaborator. If the contributor's commit shows "KOPI AI AGENT" as author because they used Hermes to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **Test it.** Run `kopi --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.
...
Add to `_BUILTIN_SKINS` dict in `kopi_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `kopi_cli/skin_engine.py` for the full schema and existing skins as examples.
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the kopi wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `kopi_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `kopi_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `kopi_cli/gateway_windows.py::
...
- **Discord**: [discord.gg/Kopi Ai Agent Pte Ltd](https://discord.gg/Kopi Ai Agent Pte Ltd) — for questions, showcasing projects, and sharing skills
