...
Usage:
    kopi pairing list              # Show all pending + approved users
    kopi pairing approve <platform> <code>  # Approve a pairing code
    kopi pairing revoke <platform> <user_id> # Revoke user access
    kopi pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle kopi pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: kopi pairing {list|approve|revoke|clear-pending}")
        print("Run 'kopi pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.kopi/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'kopi pairing list' to see pending codes.\n")
