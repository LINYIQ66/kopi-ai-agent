"""Shared fixtures for the kopi-ai-agent test suite.
...
2. **Isolated KOPI_HOME.** KOPI_HOME points to a per-test tempdir so
   code reading ``~/.kopi/*`` via ``get_kopi_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".kopi"`` instead of the canonical
   ``get_kopi_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No KOPI_SESSION_* inheritance** — the agent's current gateway
...
# KOPI_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_KOPI_BEHAVIORAL_VARS = frozenset({
    "KOPI_YOLO_MODE",
    "KOPI_INTERACTIVE",
    "KOPI_QUIET",
    "KOPI_TOOL_PROGRESS",
    "KOPI_TOOL_PROGRESS_MODE",
    "KOPI_MAX_ITERATIONS",
    "KOPI_SESSION_PLATFORM",
    "KOPI_SESSION_CHAT_ID",
    "KOPI_SESSION_CHAT_NAME",
    "KOPI_SESSION_THREAD_ID",
    "KOPI_SESSION_SOURCE",
    "KOPI_SESSION_KEY",
    "KOPI_GATEWAY_SESSION",
    "KOPI_CRON_SESSION",
    "_KOPI_GATEWAY",
    "KOPI_PLATFORM",
    "KOPI_MODEL",
    "KOPI_INFERENCE_MODEL",
    "KOPI_INFERENCE_PROVIDER",
    "KOPI_TUI_PROVIDER",
    "KOPI_MANAGED",
    "KOPI_MANAGED_DIR",
    "KOPI_DEV",
    ...
    "KOPI_TIMEZONE",
    "KOPI_REDACT_SECRETS",
    "KOPI_BACKGROUND_NOTIFICATIONS",
    "KOPI_EXEC_ASK",
    "KOPI_HOME_MODE",
    "KOPI_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.kopi/kanban.db instead of the per-test KOPI_HOME.
    "KOPI_KANBAN_DB",
    "KOPI_KANBAN_BOARD",
    "KOPI_KANBAN_HOME",
    "KOPI_KANBAN_WORKSPACES_ROOT",
    "KOPI_KANBAN_LOGS_ROOT",
    "KOPI_KANBAN_TASK",
    "KOPI_KANBAN_WORKSPACE",
    "KOPI_KANBAN_RUN_ID",
    ...
    "KOPI_DASHBOARD_OAUTH_CLIENT_ID",
    "KOPI_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and KOPI_HOME to per-test tempdirs so code that
    reads ``~/.kopi/*`` can't touch the real one, and pins TZ/LANG so
    ...
    #    fixture. Any code in the codebase reading ``~/.kopi/*`` via
    #    ``Path.home() / ".kopi"`` instead of ``get_kopi_home()``
    #    is a bug to fix at the callsite.
    fake_kopi_home = tmp_path / "kopi_test"
    fake_kopi_home.mkdir()
    (fake_kopi_home / "sessions").mkdir()
    (fake_kopi_home / "cron").mkdir()
    (fake_kopi_home / "memories").mkdir()
    (fake_kopi_home / "skills").mkdir()
    monkeypatch.setenv("KOPI_HOME", str(fake_kopi_home))
    ...
    #    ~/.kopi/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import kopi_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_kopi_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal kopi config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``kopi-gateway`` process
...
# correlated with a ``tests/kopi_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> kopi-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart kopi-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting kopi/python patterns are also blocked.
    """
    ...
    def _matches_kopi_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _KOPI_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_kopi_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch kopi-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m kopi_cli.main".
                if (
                    "kopi" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live kopi-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting kopi/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `kopi update` (or the
        # equivalent `python -m kopi_cli.main update`).  These commands
        ...
        if "update" in low and (
            # kopi update / kopi update --gateway / setsid bash -c ... kopi update
            ("kopi" in low and "update" in low.split())
            or
            # python -m kopi_cli.main update --gateway
            ("kopi_cli" in low and "update" in low.split())
            or
            # venv/bin/kopi update  (absolute path variant used in tests)
            (".venv/bin/kopi" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`kopi update` against the real checkout, fetching "
