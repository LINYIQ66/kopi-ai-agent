...
```bash
kopi profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `kopi profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
- **You just want to back up a profile on your own machine.** Use [`kopi profile export` / `import`](../reference/profile-commands.md#kopi-profile-export) — that's what those are for.
...
:::caution
**Hermes does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `kopi profile install` or `kopi profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
kopi profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.kopi/profiles/research-bot/SOUL.md
...
Create `~/.kopi/profiles/research-bot/distribution.yaml`:
...
Create `~/.kopi/profiles/research-bot/.gitignore` with at minimum:
...
Recipients who run `kopi profile update research-bot` will pull the latest.
...
5. Copies distribution-owned files into `~/.kopi/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
kopi profile install github.com/you/research-bot

# Full HTTPS
kopi profile install https://github.com/you/research-bot.git

# SSH
kopi profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
kopi profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
kopi profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
kopi profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
kopi profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
kopi profile install github.com/acme/support-bot --name support-eu --alias
```
...
```bash
cp ~/.kopi/profiles/research-bot/.env.EXAMPLE ~/.kopi/profiles/research-bot/.env
...
```bash
kopi profile info research-bot
```
...
`kopi profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
kopi profile delete research-bot
```
...
Path:    ~/.kopi/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `kopi profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
kopi profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `kopi profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.kopi/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git status   # confirm no secrets staged
git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/kopi-polymarket-trader.git
git push -u origin main --tags

# Anyone
kopi profile install github.com/you/kopi-polymarket-trader --alias
```
...
# Each on-call
kopi profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
kopi profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `kopi profile info <name>` and hold off on updates until you're ready.
:::
...
kopi profile info research-bot | grep Version
...
```yaml
# ~/.kopi/profiles/research-bot/local/my-overrides.yaml
...
kopi profile delete research-bot --yes
kopi profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
kopi profile update research-bot --force-config --yes
```
...
kopi profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.kopi/profiles/forked-research-bot/
...
kopi profile install ~/.kopi/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
kopi profile delete research-bot-test --yes
kopi profile install ~/.kopi/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `kopi -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `kopi profile` — not a parallel command tree.
...
- Reserved profile names (`kopi`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`kopi profile export` / `import`](../reference/profile-commands.md#kopi-profile-export) — local backup / restore (not distribution)
