#!/usr/bin/env python3
"""Live test harness for KOPI AI AGENT's Tool Search feature.
...
# Force-isolate the test environment BEFORE any kopi imports.
ORIGINAL_HOME = os.environ.get("KOPI_HOME")
ORIGINAL_AUTH = Path.home() / ".kopi" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.kopi/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.kopi/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="kopi_ts_live_"))
    kopi_home = home_dir / ".kopi"
    kopi_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, kopi_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".kopi" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, kopi_home / ".env")
        ...
        load_kopi_dotenv(kopi_home=str(Path.home() / ".kopi"))
    ...
    (kopi_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return kopi_home
...
def reset_module_state():
    """Drop cached modules so the new KOPI_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "kopi_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original KOPI_HOME
    if ORIGINAL_HOME is not None:
        os.environ["KOPI_HOME"] = ORIGINAL_HOME
    else:
