...
```bash
kopi plugins enable <plugin-name>
```

For isolated local testing, use one `KOPI_HOME` for plugin enablement and the
agent run:

```bash
export KOPI_HOME=/tmp/kopi-middleware-test
mkdir -p "$KOPI_HOME"
kopi plugins enable <plugin-name>
kopi chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run kopi plugins enable <plugin-name>
uv run kopi chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["kopi_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/kopi-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
