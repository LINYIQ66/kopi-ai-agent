 ...
 * the three user-facing uninstall modes to the `kopi uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m kopi_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `kopi_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'kopi_cli.uninstall', '--mode', mode]
}
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'win32') {
    ...
    if (/[\\/]Hermes$/i.test(dir) || /[\\/]kopi-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/kopi
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import kopi_cli`
 ...
 * modes + `pythonPath`=agentRoot so `import kopi_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  const lines = [
    ...
    `set "KOPI_HOME=${String(hermesHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
