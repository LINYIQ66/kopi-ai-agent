...
Drop a directory into `~/.kopi/plugins/` with a `plugin.yaml` and Python code:
...
**`~/.kopi/plugins/hello-world/plugin.yaml`**
...
**`~/.kopi/plugins/hello-world/__init__.py`**
...
Drop both files into `~/.kopi/plugins/hello-world/`, restart Hermes, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.kopi/plugins/` are disabled by default. Enable them only for trusted repositories by setting `KOPI_ENABLE_PROJECT_PLUGINS=true` before starting Hermes.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `kopi <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `kopi plugins install` |
| Distribute via pip | `[project.entry-points."kopi_agent.plugins"]` |
...
| User | `~/.kopi/plugins/` | Personal plugins |
| Project | `.kopi/plugins/` | Project-specific plugins (requires `KOPI_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `kopi_agent.plugins` entry_points | Distributed packages |
| Nix | `services.kopi-ai-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |
...
User plugins at `~/.kopi/plugins/model-providers/<name>/` and `~/.kopi/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `kopi plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.kopi/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
kopi plugins                    # interactive toggle (space to check/uncheck)
kopi plugins enable <name>      # add to allow-list
kopi plugins disable <name>     # remove from allow-list + add to disabled
```

After `kopi plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.
...
| **User-installed platforms** (under `~/.kopi/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.kopi/plugins/`.

### Migration for existing users

When you upgrade to a version of Hermes that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.kopi/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)
...
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.kopi/plugins/` |
...
| A **subcommand** for `kopi <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `kopi skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.kopi/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `kopi plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
kopi plugins                               # unified interactive UI
kopi plugins list                          # table: enabled / disabled / not enabled
kopi plugins install user/repo             # install from Git, then prompt Enable? [y/N]
kopi plugins install user/repo --enable    # install AND enable (no prompt)
kopi plugins install user/repo --no-enable # install but leave disabled (no prompt)
kopi plugins update my-plugin              # pull latest
kopi plugins remove my-plugin              # uninstall
kopi plugins enable my-plugin              # add to allow-list
kopi plugins disable my-plugin             # remove from allow-list + add to disabled
```

### Interactive UI

Running `kopi plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `kopi plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
