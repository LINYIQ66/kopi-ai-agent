...
KOPI AI AGENT is often used on large, long‑lived repositories. When you want to:
...
- CLI: the directory where you run `kopi` or `kopi chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.kopi/config.yaml`
...
git worktree add ../repo-feature feature/kopi-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/kopi-experiment` checked out in that directory
...
git worktree add ../repo-experiment-a feature/kopi-a
git worktree add ../repo-experiment-b feature/kopi-b
```
...
- Works on its own branch (`feature/kopi-a` vs `feature/kopi-b`).
...
- Hermes checkpoint data under `~/.kopi/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.
...
- **Name branches after the experiment**
  - e.g. `feature/kopi-checkpoints-docs`, `feature/kopi-refactor-tests`.
- **Commit frequently**
...
## Using `kopi -w` (Automatic Worktree Mode)
...
- Check out an isolated branch (e.g. `kopi/kopi-<hash>`).
...
```bash
kopi -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `kopi -w` in each — every invocation gets its own worktree and branch automatically.
