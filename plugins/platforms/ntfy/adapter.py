    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "kopi-in"              # subscribe topic (incoming)
          publish_topic: "kopi-out"     # optional — defaults to topic
...
_ECHO_TAG = "kopi-ai-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``kopi cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `kopi gateway status` without
