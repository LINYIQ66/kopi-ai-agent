...
| | |
|---|---|
| 来源 | 可选 — 通过 `kopi skills install official/mcp/fastmcp` 安装 |
...
- 使用 `native-mcp` skill，在 `~/.kopi/config.yaml` 中配置服务器，或
...
服务器构建部分可能正确，但 Hermes 配置有误。加载 `native-mcp` skill 并在 `~/.kopi/config.yaml` 中配置服务器，然后重启 Hermes。
