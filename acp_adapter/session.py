...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["kopi-acp"]):
...
class SessionManager:
    ...
    def _get_db(self):
        ...
        Note: we resolve ``KOPI_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_kopi_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=kopi_home / "state.db")
            return self._db_instance
        except Exception:
