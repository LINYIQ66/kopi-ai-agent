...
`mcp_tool.py`, and `kopi_cli/mcp_config.py`. This module is the ONLY place
...
def _make_kopi_provider_class() -> Optional[type]:
    ...
    class HermesMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._kopi_server_name = server_name
            ...
            self._kopi_preregistered = preregistered
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._kopi_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._kopi_server_name, asm.token_endpoint,
                        )
                        break
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``kopi mcp reauth``.
            ...
            back to ``kopi mcp reauth``.
            """
            try:
                if self._kopi_preregistered:
                    return
...
_KOPI_PROVIDER_CLS: Optional[type] = _make_kopi_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `kopi mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``kopi mcp remove <name>`` and (indirectly) by
        ``kopi mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
