...
description: "Plugins shipped with KOPI AI AGENT that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Hermes ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.kopi/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.
...
2. **User** — `~/.kopi/plugins/<name>/`
3. **Project** — `./.kopi/plugins/<name>/` (requires `KOPI_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `kopi_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `kopi memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `kopi plugins list` and the interactive `kopi plugins` UI), but none load until you explicitly enable them:

```bash
kopi plugins enable disk-cleanup
```

Or via `~/.kopi/config.yaml`:
...
```bash
kopi plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `kopi plugins enable <name>`.
...
| `kopi-achievements` | dashboard tab | Steam-style collectible badges generated from your real Hermes session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `kopi memory` and `kopi plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `KOPI_HOME` or `/tmp/kopi-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under KOPI_HOME | always | Never |
...
**State** — everything lives at `$KOPI_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `KOPI_HOME` or `/tmp/kopi-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `kopi plugins enable disk-cleanup` (or check the box in `kopi plugins`).

**Disabling again:** `kopi plugins disable disk-cleanup`.
...
**Enabling:** `kopi plugins enable security-guidance` (or check the box in `kopi plugins`).

**Disabling again:** `kopi plugins disable security-guidance`.
...
```bash
kopi tools          # → Langfuse Observability → Cloud or Self-Hosted
```
...
Then put the credentials in `~/.kopi/.env`:
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `KOPI_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Hermes session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `kopi chat` session lives under one Langfuse session.

**Verify:**

```bash
kopi plugins list                 # observability/langfuse should show "enabled"
kopi chat -q "hello"              # check the Langfuse UI for a "Hermes turn" trace
```
...
| `KOPI_LANGFUSE_RELEASE` | — | Release/version tag |
| `KOPI_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `KOPI_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `KOPI_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |
...
**Disabling:** `kopi plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.kopi/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
kopi plugins enable google_meet
...
**Disabling:** `kopi plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.kopi/cache/google_meet/` until you remove them.
...
- Scans your entire `~/.kopi/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$KOPI_HOME/plugins/kopi-achievements/state.json`
...
**API** — routes mount under `/api/plugins/kopi-achievements/`:
...
**State files** — live under `$KOPI_HOME/plugins/kopi-achievements/`:
...
**Enabling:** Nothing to enable — `kopi-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `kopi dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/kopi-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.kopi/plugins/kopi-achievements/` that ships no dashboard. The plugin's state files under `$KOPI_HOME/plugins/kopi-achievements/` survive — reinstalling preserves your unlock history.
...
- Directory lives at `<repo>/plugins/<name>/` instead of `~/.kopi/plugins/<name>/`
- Manifest source is reported as `bundled` in `kopi plugins list`
