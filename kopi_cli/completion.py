"""Shell completion script generation for kopi CLI.
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_kopi_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
...
_kopi_profiles() {{
    local profiles_dir="$HOME/.kopi/profiles"
...
_kopi_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_kopi_profiles)" -- "$cur"))
        return
    fi
...
_kopi_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.kopi/profiles" ]]; then
        profiles+=($HOME/.kopi/profiles/*(N/:t))
    fi
...
_hermes() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_kopi_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'kopi command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "function __kopi_profiles",
        "    echo default",
        "    if test -d $HOME/.kopi/profiles",
        "        for d in $HOME/.kopi/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c kopi -f -s p -l profile"
        " -d 'Profile name' -xa '(__kopi_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__kopi_profiles)' -d 'Profile name'"
                )
