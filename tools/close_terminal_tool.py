...
``read_terminal`` it is gated on ``KOPI_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    """Desktop GUI only — KOPI_DESKTOP is set on the gateway the app spawns."""
    return env_var_enabled("KOPI_DESKTOP")
