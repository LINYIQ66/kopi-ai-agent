...
Jobs are stored in ~/.kopi/cron/jobs.json
Output is saved to ~/.kopi/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own KOPI_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same KOPI_HOME — so a job authored in profile
# `coder` lives in `~/.kopi/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_kopi_home()` (the active profile home), NOT `get_default_kopi_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever KOPI_HOME the
...
# `_get_kopi_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
KOPI_DIR = get_kopi_home().resolve()
CRON_DIR = KOPI_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``kopi cron status`` process share it
...
# threshold in `kopi cron status` (kopi_cli/cron.py), so the two never
# drift apart.
...
# the cron inactivity timeout is disabled (KOPI_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. KOPI_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``KOPI_CRON_TIMEOUT`` (the cron inactivity timeout the
    ...
    """
    raw = os.getenv("KOPI_CRON_TIMEOUT", "").strip()
...
def _jobs_lock():
    ...
    and standalone ``kopi`` CLI invocations, which previously shared no lock
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            # against `kopi_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                kopi_tz = _kopi_now().tzinfo
                dt = dt.replace(tzinfo=kopi_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    try:
        ...
        run_at = _kopi_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall kopi-ai-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `kopi cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    concurrent reader in another process (``kopi cron status``) never sees a
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `kopi cron status` can tell a thread that is merely *alive and looping*
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _kopi_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_kopi_home() / "config.yaml"
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.kopi/scripts/; ``.sh`` / ``.bash`` files run via bash,
    ...
    # covered, not just `kopi cron create`.
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _kopi_now().isoformat(),
        },
    )
...
def _machine_id() -> str:
    ...
    Uses ``KOPI_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("KOPI_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from KOPI_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        if next_run_dt <= now:
            ...
            # desktop both run in-process 60s tickers on one KOPI_HOME)
            ...
            # (_oneshot_run_claim_ttl_seconds, derived from KOPI_CRON_TIMEOUT),
...
# execution. Unlike the quick-snapshot store (`kopi_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``kopi_cli.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _kopi_now().strftime("%Y-%m-%d_%H-%M-%S")
