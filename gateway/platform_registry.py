...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``kopi setup`` display).
    ...
    # built-ins).  Used by ``kopi gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `kopi`
        # invocation -- including plain `kopi chat`, which never touches any
        ...
        # `kopi setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``kopi setup``/``gateway status``, channel
