...
Capabilities:
- Persist telephony provider credentials to the Hermes .env file ($KOPI_HOME/.env)
...
def _env_path() -> Path:
    return _kopi_home() / ".env"


def _config_path() -> Path:
    return _kopi_home() / "config.yaml"


def _state_path() -> Path:
    return _kopi_home() / "telephony_state.json"
