...
| | |
|---|---|
| Source | Optional — install with `kopi skills install official/payments/stripe-link-cli` |
...
```
kopi mcp add stripe-link --command "npx" --args "@stripe/link-cli --mcp"
```

Then `kopi mcp list` should show `stripe-link`. The same approval rules apply — MCP doesn't bypass the Link app approval step.
