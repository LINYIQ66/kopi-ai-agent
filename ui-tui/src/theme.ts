...
// Mirrors @kopi/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @kopi/ink is bundled separately from packages/.
...
// Terminal. Explicit KOPI_TUI_THEME / COLORFGBG signals above still win,
...
// `KOPI_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `KOPI_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `KOPI_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `KOPI_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
export function detectLightMode(
  ...
  const lightFlag = (env.KOPI_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.KOPI_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.KOPI_TUI_BACKGROUND ?? '')
