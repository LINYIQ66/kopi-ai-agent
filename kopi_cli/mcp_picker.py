"""MCP picker — interactive `kopi mcp picker` (also the default `kopi mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``kopi mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `kopi plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from kopi_cli.cli_output import prompt_yes_no
from kopi_cli.curses_ui import curses_single_select
from kopi_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from kopi_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `kopi mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: kopi mcp install <name>    Picker: kopi mcp",
        Colors.DIM,
    ))
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Hermes — run `kopi update` "
...
def show_catalog() -> None:
    """`kopi mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`kopi mcp picker` (and default `kopi mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`kopi mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `kopi mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
