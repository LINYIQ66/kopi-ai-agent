...
Regression: the old implementation wrote ``kopi_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``kopi sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.kopi/sessions/saved/`` and prints
...
def kopi_home(tmp_path, monkeypatch):
    ...
    # Clear any cached kopi_home computation
    import kopi_constants
    if hasattr(kopi_constants, "_kopi_home_cache"):
        kopi_constants._kopi_home_cache = None
    return home
...
def test_save_conversation_writes_under_kopi_home(kopi_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.kopi/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the KOPI_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "kopi_constants"]:
    ...
    cwd_leak = list(work.glob("kopi_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.kopi/sessions/saved/
    saved_dir = kopi_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("kopi_conversation_*.json"))
    ...
    assert "kopi --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(kopi_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "kopi_constants"]:
        sys.modules.pop(mod, None)
    import cli
