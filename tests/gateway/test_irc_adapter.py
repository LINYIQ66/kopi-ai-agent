...
class TestIRCProtocolHelpers:
    ...
    def test_parse_numeric_reply(self):
        msg = _parse_irc_message(":server 001 kopi-bot :Welcome to IRC")
        assert msg["prefix"] == "server"
        assert msg["command"] == "001"
        assert msg["params"] == ["kopi-bot", "Welcome to IRC"]

    def test_parse_nick_collision(self):
        msg = _parse_irc_message(":server 433 * kopi-bot :Nickname is already in use")
...
class TestIRCAdapterInit:
    ...
    def test_init_from_config_extra(self, monkeypatch):
        ...
        assert adapter.nickname == "kopi"
        assert adapter.channel == "#kopi-dev"
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_welcome(self, adapter):
        ...
        await adapter._handle_line(":server 001 kopi :Welcome to IRC")
    ...
    async def test_handle_nick_collision(self, adapter):
        ...
        await adapter._handle_line(":server 433 * kopi :Nickname in use")
        assert adapter._current_nick == "kopi_"
        sent = writer.write.call_args[0][0]
        assert b"NICK kopi_" in sent
    ...
    async def test_handle_dm(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG kopi :private message")
        assert len(dispatched) == 1
    ...
    async def test_ctcp_action_converted(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG kopi :\x01ACTION waves\x01")
        assert len(dispatched) == 1
    ...
    async def test_nick_collision_retry(self, adapter):
        ...
        await adapter._handle_line(":server 433 * kopi :Nickname in use")
        assert adapter._current_nick == "kopi_"
        await adapter._handle_line(":server 433 * kopi_ :Nickname in use")
        assert adapter._current_nick == "kopi_1"
        await adapter._handle_line(":server 433 * kopi_1 :Nickname in use")
        assert adapter._current_nick == "kopi_2"
...
class TestIRCStandaloneSend:
    ...
    async def test_standalone_send_completes_handshake_and_sends_privmsg(self, monkeypatch):
        ...
        assert any(line.startswith("USER hermesbot-cron 0 * :KOPI AI AGENT (cron)")
