...
from kopi_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``KOPI_KANBAN_TASK`` / ``KOPI_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("KOPI_KANBAN_TASK") or os.getenv("KOPI_KANBAN_BOARD"):
            result = True
        else:
            try:
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``kopi_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``KOPI_PLATFORM`` or
            ``KOPI_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("KOPI_PLATFORM")
        or get_session_env("KOPI_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``kopi`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.kopi/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``kopi`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against KOPI_HOME, not cwd
        if not p.is_absolute():
            p = (kopi_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.kopi/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.kopi/skills/`` (including via symlinks) or configured
    ...
    # ~/.kopi/skills/<name> where <name> is a symlink to a checked-out
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    return {
        ...
        "requires_toolsets": kopi.get("requires_toolsets", []),
        "fallback_for_tools": kopi.get("fallback_for_tools", []),
        "requires_tools": kopi.get("requires_tools", []),
    }
