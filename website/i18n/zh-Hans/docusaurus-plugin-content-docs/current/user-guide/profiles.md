...
```bash
kopi profile create coder       # 创建 profile + "coder" 命令别名
...
```bash
kopi profile create researcher --description "Reads source code and external docs, writes findings."
```

你也可以稍后通过 `kopi profile describe` 设置或自动生成描述——完整路由模型请参阅 [Kanban 指南](./features/kanban#auto-vs-manual-orchestration)。
...
将当前 profile 的 `config.yaml`、`.env`、`SOUL.md` 和 skills 复制到新 profile。API 密钥、模型和能力相同，但会话和记忆是全新的。编辑 `~/.kopi/profiles/work/.env` 可使用不同的 API 密钥，编辑 `~/.kopi/profiles/work/SOUL.md` 可设置不同的人格。
...
复制**所有内容**——配置、API 密钥、人格、记忆、技能、cron 任务、插件。会排除每个 profile 自己的历史数据（会话历史、`state.db`、`backups/`、`state-snapshots/`、`checkpoints/`），这些数据属于源 profile 且可能达到数十 GB。若要包含历史的完整备份，请使用 `kopi profile export` 或 `kopi backup`。

### 从指定 profile 克隆

```bash
kopi profile create work --clone-from coder
```

`--clone-from <source>` 会直接选择源 profile，并隐含执行 config/skills/SOUL 克隆。若要完整复制该源 profile，请与 `--clone-all` 组合使用：

```bash
kopi profile create work-backup --clone-from coder --clone-all
```
...
别名支持所有 kopi 子命令——底层实际上是 `kopi -p <name>`。
...
kopi chat -p coder -q "hello"    # 可在任意位置使用
```

### 粘性默认值（`kopi profile use`）

```bash
kopi profile use coder
kopi chat                   # 现在指向 coder
kopi tools                  # 配置 coder 的工具
kopi profile use default    # 切换回默认
```

设置默认值后，普通 `kopi` 命令将指向该 profile。类似于 `kubectl config use-context`。
...
- **`kopi profile`**：显示当前 profile 名称、路径、模型、gateway 状态
...
```bash
coder gateway install         # 创建 kopi-gateway-coder systemd/launchd 服务
assistant gateway install     # 创建 kopi-gateway-assistant 服务
```

每个 profile 拥有独立的服务名称，各自独立运行。

:::note 在官方 Docker 镜像中
各 profile 的 gateway 由 [s6-overlay](https://github.com/just-containers/s6-overlay)（容器中的 PID 1）监管，因此 `kopi profile create <name>` 会自动在 `/run/service/gateway-<name>/` 注册 s6 服务槽。`kopi -p <name> gateway start/stop/restart` 会调度到 `s6-svc` 而非直接启动裸进程——崩溃后自动重启，`docker restart` 会保留之前运行的 gateway 集合。详见 [各 profile gateway 监管](/user-guide/docker#per-profile-gateway-supervision)。
:::
...
`kopi update` 拉取一次代码（共享），并自动将新的内置技能同步到**所有** profile：
...
```bash
kopi profile list           # 显示所有 profile 及其状态
kopi profile show coder     # 显示某个 profile 的详细信息
kopi profile rename coder dev-bot   # 重命名（同步更新别名和服务）
kopi profile export coder   # 导出为 coder.tar.gz
kopi profile import coder.tar.gz   # 从归档文件导入
```
...
使用 `--yes` 跳过确认：`kopi profile delete coder --yes`

:::note
你无法删除默认 profile（`~/.kopi`）。如需删除所有内容，请使用 `kopi uninstall`。
:::
...
profile 使用 `KOPI_HOME` 环境变量。运行 `coder chat` 时，包装脚本在启动 kopi 前将 `KOPI_HOME` 设置为 `~/.kopi/profiles/coder`。由于代码库中 119+ 个文件通过 `get_kopi_home()` 解析路径，Hermes 状态会自动限定在 profile 目录范围内——包括配置、会话、记忆、技能、状态数据库、gateway PID、日志和 cron 任务。

这与终端工作目录是分开的。工具执行从 `terminal.cwd` 开始（或在 local 后端使用 `cwd: "."` 时从启动目录开始），而非自动从 `KOPI_HOME` 开始。

默认 profile 就是 `~/.kopi` 本身。无需迁移——现有安装的工作方式完全不变。
