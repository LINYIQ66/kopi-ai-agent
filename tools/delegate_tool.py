...
# Must match kopi_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``kopi-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. kopi-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.kopi/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_kopi_dir("cache/delegation", "delegation_cache")
...
def delegate_task(
    ...
    if background:
        ...
        try:
            ...
            _source = get_session_env("KOPI_SESSION_SOURCE", "")
            _origin_ui_session_id = get_session_env("KOPI_UI_SESSION_ID", "")
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'kopi auth'."
        )
...
def _load_config() -> dict:
    ...
    ``KOPI_IGNORE_USER_CONFIG=1`` (``kopi chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("KOPI_IGNORE_USER_CONFIG") == "1"
