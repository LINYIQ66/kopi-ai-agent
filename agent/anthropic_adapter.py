"""Anthropic Messages API adapter for KOPI AI AGENT.
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``kopi models``) never mutates ``~/.kopi/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      ...
      4. Anthropic credential_pool OAuth entry (~/.kopi/auth.json)
...
# Stores credentials in ~/.kopi/.anthropic_oauth.json (our own file).
...
def _get_kopi_oauth_file() -> Path:
    return get_kopi_home() / ".anthropic_oauth.json"
...
def run_kopi_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    try:
        from kopi_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_kopi_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Hermes-managed OAuth credentials from ~/.kopi/.anthropic_oauth.json."""
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See kopi-ai-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See kopi-ai-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See kopi-ai-agent#13848 (Kimi) and
    kopi-ai-agent#16748 (DeepSeek).
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                ...
                text = text.replace("kopi-ai-agent", "claude-code")
                text = text.replace("Kopi Ai Agent Pte Ltd", "Anthropic")
                block["text"] = text
