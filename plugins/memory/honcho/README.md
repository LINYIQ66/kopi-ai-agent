...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/kopi>
...
```bash
kopi memory setup honcho   # configure Honcho directly (works on a fresh install)
...
Or manually:
```bash
kopi config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.kopi/.env
```

> `kopi honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `kopi memory setup honcho`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `kopi honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| `KOPI_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `kopi memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `kopi honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `kopi memory setup`) |
| `kopi honcho status` | Show resolved config for active profile |
| `kopi honcho enable` / `disable` | Toggle Honcho for active profile |
| `kopi honcho mode <mode>` | Change recall or observation mode |
| `kopi honcho peer --user <name>` | Update user peer name |
| `kopi honcho peer --ai <name>` | Update AI peer name |
