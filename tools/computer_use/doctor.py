"""
`kopi computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
def run_doctor(
    ...
    *,
    ...
    Honors `KOPI_CUA_DRIVER_CMD` via the same `_cua_driver_cmd()` resolver
    ...
    if driver_cmd is None:
        try:
            ...
            driver_cmd = os.environ.get("KOPI_CUA_DRIVER_CMD") or "cua-driver"
