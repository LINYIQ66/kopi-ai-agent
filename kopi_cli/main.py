...
Usage:
    kopi                     # Interactive chat (default)
    kopi chat                # Interactive chat
    kopi gateway             # Run gateway in foreground
    kopi gateway start       # Start gateway as service
    kopi gateway stop        # Stop gateway service
    kopi gateway status      # Show gateway status
    kopi gateway install     # Install gateway service
    kopi gateway uninstall   # Uninstall gateway service
    kopi setup               # Interactive setup wizard
    kopi logout              # Clear stored authentication
    kopi status              # Show status of all components
    kopi cron                # Manage cron jobs
    kopi cron list           # List cron jobs
    kopi cron status         # Check if cron scheduler is running
    kopi doctor              # Check configuration and dependencies
    kopi honcho setup                    # Configure Honcho AI memory integration
    kopi honcho status                   # Show Honcho config and connection status
    kopi honcho sessions                 # List directory → session name mappings
    kopi honcho map <name>               # Map current directory to a session name
    kopi honcho peer                     # Show peer names and dialectic settings
    kopi honcho peer --user NAME         # Set user peer name
    kopi honcho peer --ai NAME           # Set AI peer name
    kopi honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    kopi honcho mode                     # Show current memory mode
    kopi honcho mode [hybrid|honcho|local]  # Set memory mode
    kopi honcho tokens                   # Show token budget settings
    kopi honcho tokens --context N       # Set session.context() token cap
    kopi honcho tokens --dialectic N     # Set dialectic result char cap
    kopi honcho identity                 # Show AI peer identity representation
    kopi honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    kopi honcho migrate                  # Step-by-step migration guide: OpenClaw native → Hermes + Honcho
    kopi version             Show version
    kopi update              Update to latest version
    kopi uninstall           Uninstall KOPI AI AGENT
    kopi acp                 Run as an ACP server for editor integration
    kopi sessions browse     Interactive session picker with search

    kopi claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: kopi_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``kopi_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``kopi update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``kopi_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, kopi crashes on import and the user can't run
# ``kopi update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import kopi_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'kopi' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``kopi tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``kopi.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("kopi")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"kopi", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"kopi")
        # Windows: the .exe name is already ``kopi.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* kopi_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        if home:
            ...
            cfg_path = os.path.join(os.path.expanduser("~"), ".kopi", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``KOPI_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("KOPI_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``KOPI_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("KOPI_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`kopi --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from kopi_cli import __release_date__, __version__

    print(f"KOPI AI AGENT v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``kopi --version`` before config/logging imports on Termux."""
    if os.environ.get("KOPI_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from kopi_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from kopi_cli.subcommands.cron import build_cron_parser
from kopi_cli.subcommands.gateway import build_gateway_parser
from kopi_cli.subcommands.profile import build_profile_parser
...
from kopi_cli.subcommands.whatsapp import build_whatsapp_parser
from kopi_cli.subcommands.slack import build_slack_parser
from kopi_cli.subcommands.login import build_login_parser
from kopi_cli.subcommands.logout import build_logout_parser
from kopi_cli.subcommands.auth import build_auth_parser
from kopi_cli.subcommands.status import build_status_parser
from kopi_cli.subcommands.webhook import build_webhook_parser
from kopi_cli.subcommands.hooks import build_hooks_parser
from kopi_cli.subcommands.doctor import build_doctor_parser
from kopi_cli.subcommands.security import build_security_parser
from kopi_cli.subcommands.dump import build_dump_parser
from kopi_cli.subcommands.debug import build_debug_parser
...
from kopi_cli.subcommands.uninstall import build_uninstall_parser
from kopi_cli.subcommands.dashboard import build_dashboard_parser
from kopi_cli.subcommands.gui import build_gui_parser
...
from kopi_cli.subcommands.memory import build_memory_parser
from kopi_cli.subcommands.acp import build_acp_parser
from kopi_cli.subcommands.tools import build_tools_parser
from kopi_cli.subcommands.insights import build_insights_parser
from kopi_cli.subcommands.skills import build_skills_parser
from kopi_cli.subcommands.pairing import build_pairing_parser
from kopi_cli.subcommands.plugins import build_plugins_parser
from kopi_cli.subcommands.mcp import build_mcp_parser
from kopi_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (kopi tools, kopi setup, kopi model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'kopi {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any kopi module import.
#
# Many modules cache KOPI_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("KOPI_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.kopi/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set KOPI_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `kopi mcp add ... --args <command argv>`.
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo kopi -p <name>` against the invoking user's home.
        ...
        candidate = home / ".kopi" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`kopi chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors kopi_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If KOPI_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.kopi/profiles/coder or
    # /opt/data/profiles/coder).  If KOPI_HOME points to the kopi root
    # instead (e.g. systemd hardcodes KOPI_HOME=/root/.kopi), we must
    # still read active_profile — the user may have switched profiles via
    # `kopi profile use` and the gateway should honour that choice.
    # See issue #22502.
    kopi_home_env = os.environ.get("KOPI_HOME", "")
    if profile_name is None and kopi_home_env:
        if Path(kopi_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the kopi root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as KOPI_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``kopi gateway run`` to mean
    # "the root KOPI_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("KOPI_S6_SUPERVISED_CHILD"):
        try:
            from kopi_constants import get_default_kopi_root

            active_path = get_default_kopi_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set KOPI_HOME
    if profile_name is not None:
        try:
            from kopi_cli.profiles import resolve_profile_env

            kopi_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            kopi_home = _resolve_sudo_user_profile_env(profile_name)
            if not kopi_home:
            ...
            # A bug in profiles.py must NEVER prevent kopi from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["KOPI_HOME"] = kopi_home
...
# Load .env from ~/.kopi/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from kopi_cli.config import get_kopi_home
from kopi_cli.env_loader import load_kopi_dotenv

load_kopi_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → KOPI_REDACT_SECRETS env
# var BEFORE kopi_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_kopi_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from kopi_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "KOPI_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["KOPI_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `kopi` subcommands
...
try:
    from kopi_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from kopi_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if kopi_constants not importable yet
...
# Provider model-selection wizard flows extracted to kopi_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (kopi_cli.main._model_flow_*) keep resolving unchanged.
from kopi_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_kopi_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("KOPI_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("KOPI_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from kopi_cli.config import get_env_path, get_kopi_home, load_config
    from kopi_cli.auth import get_auth_status
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_kopi_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from kopi_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_kopi_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, kopi_bin
        cli_args: the original CLI arguments (everything after 'kopi')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    kopi_bin = container_info["kopi_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo kopi {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo kopi {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  kopi --tui --resume {target}")
    if title:
        print(f'  kopi --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @kopi/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@kopi" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/kopi-ink/package.json",
    "packages/kopi-ink/index.js",
    "packages/kopi-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``KOPI_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("KOPI_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``KOPI_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("KOPI_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    kopi_home = os.environ.get("KOPI_HOME") or str(Path.home() / ".kopi")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "KOPI_HOME": kopi_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(kopi_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(kopi_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if kopi_cli_dir is None:
        kopi_cli_dir = Path(__file__).parent
    bundled = kopi_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``kopi update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("KOPI_QUIET"):
    ...
    print(
        ...
        "This usually means `kopi update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Hermes checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `kopi --tui`\n"
        "If the checkout is still inconsistent, run `kopi update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (KOPI_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("KOPI_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from kopi_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("KOPI_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with KOPI_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset KOPI_TUI_DIR (e.g. `unset KOPI_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @kopi/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @kopi/ink resolves through
        # packages/kopi-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "kopi-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from kopi_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from kopi_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="kopi-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["KOPI_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["KOPI_PYTHON_SRC_ROOT"] = os.environ.get(
        "KOPI_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("KOPI_PYTHON", sys.executable)
    env.setdefault("KOPI_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["KOPI_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["KOPI_MODEL"] = model
        env["KOPI_INFERENCE_MODEL"] = model
    if provider:
        env["KOPI_TUI_PROVIDER"] = provider
        env["KOPI_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["KOPI_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["KOPI_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["KOPI_TUI_SKILLS"] = value
    if query:
        env["KOPI_TUI_QUERY"] = query
    if image:
        env["KOPI_TUI_IMAGE"] = image
    if checkpoints:
        env["KOPI_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["KOPI_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["KOPI_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["KOPI_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["KOPI_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["KOPI_ACCEPT_HOOKS"] = "1"
    ...
    # KOPI_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `kopi --tui` try to
    ...
    # still set KOPI_TUI_RESUME themselves.
    env.pop("KOPI_TUI_RESUME", None)
    if resume_session_id:
        env["KOPI_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `kopi update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``KOPI_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``kopi kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``kopi kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("KOPI_KANBAN_BOARD"):
        return
    try:
        from kopi_cli.kanban_db import get_current_board

        os.environ["KOPI_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.kopi/skills/`` with the bundled skill library on first launch.
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``KOPI_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("KOPI_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'kopi sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from kopi_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from kopi_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'kopi doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        ...
        print()
        print("  Run:  kopi setup")
        print()

        from kopi_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'kopi setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from kopi_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["KOPI_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.kopi/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["KOPI_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["KOPI_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["KOPI_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from kopi_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from kopi_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from kopi_cli.config import get_env_value, save_env_value
    ...
    # has no creds.json.  Every subsequent `kopi gateway` then paid a 30s
    ...
    session_dir = get_kopi_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: kopi gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `kopi gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ KOPI AI AGENT'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  kopi gateway")
        ...
        print("⚠ Pairing may not have completed. Run 'kopi whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``kopi whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``kopi_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from kopi_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from kopi_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from kopi_cli.config import stamp_install_method
    from kopi_cli.dep_ensure import ensure_dependency
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from kopi_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``kopi model``) and the setup wizard
    ...
    effective_provider = (
        config_provider or os.getenv("KOPI_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'kopi model' for "
                "available providers, or run 'kopi doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in kopi_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.kopi/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.kopi/.env if the active provider is not 'custom'.
    ...
    """
    from kopi_cli.config import get_env_value, save_env_value, load_config
...
# `kopi model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `kopi model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`kopi_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from kopi_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``kopi model`` flow, then route aux tasks to them here.
    """
    from kopi_cli.config import load_config
    from kopi_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from kopi_cli.auth import _prompt_model_selection
    from kopi_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from kopi_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from kopi_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from kopi_cli.runtime_provider import _detect_api_mode_for_url
...
def _remove_custom_provider(config):
    ...
    try:
        from kopi_cli.curses_ui import curses_radiolist
...
# downstream call sites read `kopi_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `kopi` invocation — including
# fast paths like `kopi --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from kopi_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``kopi setup`` / ``kopi model``.
    ...
    recover from a malformed paste without editing ``~/.kopi/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from kopi_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from kopi_cli.config import save_env_value
    from kopi_cli.secret_prompt import masked_secret_prompt
    ...
    from kopi_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `kopi setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from kopi_constants import display_kopi_home as _dhh_fn
...
def cmd_login(args):
    ...
    from kopi_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from kopi_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from kopi_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from kopi_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from kopi_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from kopi_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``kopi slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: kopi slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `kopi slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from kopi_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from kopi_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from kopi_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from kopi_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from kopi_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `kopi security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from kopi_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from kopi_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from kopi_cli.debug import run_debug
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from kopi_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from kopi_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from kopi_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from kopi_cli.config import detect_install_method
    from kopi_cli.banner import format_banner_version_label
    ...
    try:
        from kopi_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall KOPI AI AGENT (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from kopi_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from kopi_cli.uninstall import run_gui_uninstall
    ...
    from kopi_cli.uninstall import run_uninstall
...
_UPDATE_CRITICAL_FILES = (
    "kopi_cli/main.py",
    "kopi_cli/config.py",
    "kopi_cli/__init__.py",
    "kopi_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``kopi`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="kopi-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``kopi update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``kopi_cli/web_dist/`` (per vite.config.ts
    outDir: "../kopi_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "kopi_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``kopi`` launcher present but ``kopi_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the kopi venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — kopi venv python3, no nix-shell overhead
    ...
    # a kopi venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``kopi update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``kopi web``).
    ...
    # build path usable on a stock `py -m kopi_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (kopi web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``kopi update`` can unconditionally call ``kopi desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``kopi desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $KOPI_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $KOPI_HOME."""
    from kopi_constants import get_kopi_home
    return get_kopi_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``kopi update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        ...
        candidates = [
            release_dir / "linux-unpacked" / "kopi",
            release_dir / "linux-unpacked" / "Hermes",
            release_dir / "linux-arm64-unpacked" / "kopi",
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_kopi_node_path(env)
    if mirror:
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``kopi desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the KOPI_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from kopi_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "cwd", None):
        ...
        env["KOPI_DESKTOP_CWD"] = os.getcwd()
    ...
    # `KOPI_DESKTOP_DISABLE_GPU=... kopi desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "KOPI_DESKTOP_DISABLE_GPU" not in os.environ:
        env["KOPI_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  kopi gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if build_result.returncode != 0:
                ...
                print("    ELECTRON_MIRROR=<mirror-base-url> kopi desktop --force-build")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``kopi dashboard`` processes other than ourselves.

    ``kopi dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``kopi update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``kopi serve`` as
    ...
    environment variable ``KOPI_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "kopi dashboard",
        "kopi_cli.main dashboard",
        "kopi_cli/main.py dashboard",
        # The headless backend (`kopi serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "kopi serve",
        "kopi_cli.main serve",
        "kopi_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "kopi.*dashboard"`) keeps us consistent
            # with `kopi_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `kopi update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``kopi update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``kopi update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: kopi curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``kopi dashboard`` processes.

    Called at the end of ``kopi update`` (default ``reason``) and also
    from ``kopi dashboard --stop`` (which overrides ``reason``).  The
    ...
    # backend child, it sets KOPI_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("KOPI_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    kopi dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.kopi-update-staging"
    backup = f"{dst}.kopi-update-old"
...
def _update_via_zip(args):
    """Update KOPI AI AGENT by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `kopi update "
            f"--branch {branch}`, or update against main with `kopi update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/LINYIQ66/kopi-ai-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="kopi-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"kopi-ai-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for kopi-ai-agent itself
        ...
        # GitHub ZIPs extract to kopi-ai-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"kopi-ai-agent-{branch}")
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: kopi skills list-modified  "
    ...
    try:
        from kopi_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "kopi-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes kopi completely unrunnable (SyntaxError on import).
...
# =========================================================================
# Fork detection and upstream management for `kopi update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/LINYIQ66/kopi-ai-agent.git",
    "git@github.com:Kopi Ai Agent Pte Ltd/kopi-ai-agent.git",
    "https://github.com/LINYIQ66/kopi-ai-agent",
    "git@github.com:Kopi Ai Agent Pte Ltd/kopi-ai-agent",
}
OFFICIAL_REPO_URL = "https://github.com/LINYIQ66/kopi-ai-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_kopi_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_kopi_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from Kopi Ai Agent Pte Ltd/kopi-ai-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/LINYIQ66/kopi-ai-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/LINYIQ66/kopi-ai-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``kopi update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from kopi_constants import get_default_kopi_root

    default_home = get_default_kopi_root()
...
# Install-scoped breadcrumb dropped right before ``kopi update`` mutates the
...
# survives and the next ``kopi`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $KOPI_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``kopi update`` left half-done.
    ...
    protocol stream (``kopi acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if kopi.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _kopi_exe_shims(scripts_dir)
            if shims:
    ...
    try:
        ...
        print(
            "⚠ A previous `kopi update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from kopi_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``kopi update`` is still progressing even if pip/uv itself is silent.
    """
...
def _kopi_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_kopi_instances(
    ...
    The Hermes Desktop Electron app spawns ``kopi.EXE`` as a backend child,
    so during ``kopi update`` the user-invoked process and the desktop's
    ...
    shims (``kopi.exe`` / ``kopi-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``kopi update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``kopi.exe``) is a separate process from the Python
    ...
    for shim in _kopi_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated kopi.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``kopi update``
    ...
    #      kopi.exe sitting *under* a non-Hermes parent (e.g. a Hermes
...
def _format_concurrent_instances_message(
    ...
    lines = ["✗ Another kopi.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Hermes Desktop, exit any open `kopi` REPLs, and")
    lines.append("  stop the gateway (`kopi gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `kopi update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_kopi_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``kopi.exe``.
    ...
    when ``kopi update`` runs, ``kopi.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``kopi.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next kopi invocation by ``_cleanup_quarantined_exes``.
    ...
    for shim in _kopi_exe_shims(scripts_dir):
        ...
        print(
            "    Close Hermes Desktop, exit other `kopi` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `kopi update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_kopi_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``kopi.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``kopi.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``kopi`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_kopi_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``kopi.exe.old.*`` left by prior updates.

    Called early on every kopi invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``kopi update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `kopi update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``kopi.exe`` / ``kopi-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_kopi_exe`` for the rationale.
    """
    ...
    # hours later inside ``kopi update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``kopi.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``kopi.exe`` shim is locked during ``kopi update``, or when uv/distlib
    skips a launcher write. The symptom is ``kopi-ai-agent.exe`` and
    ``kopi-acp.exe`` present but ``kopi.exe`` missing, so ``kopi`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `kopi update --force` after "
            "closing other kopi processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m kopi_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``kopi update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``kopi.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``kopi`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `kopi update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `kopi update --force` after closing other kopi processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from kopi_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$KOPI_HOME/bin/uv``, but on Termux the official
    ...
    """
    from kopi_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `kopi update`, so we install root-only first
    ...
    nixos_env = with_kopi_node_path(_nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `kopi update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``kopi update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.kopi/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``kopi update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``kopi update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.kopi/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``kopi update --gateway``) the update is already
    ...
    try:
        ...
        # kopi_cli.config.get_kopi_home to simulate setup failure.
        from kopi_cli.config import get_kopi_home as _get_kopi_home

        logs_dir = _get_kopi_home() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.kopi/logs/update.log`` only, never the terminal.

    During ``kopi update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``kopi update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``kopi update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from kopi_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from kopi_cli.config import recommended_update_command
        from kopi_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``kopi update`` without requiring a reinstall.  The
    ...
    ``kopi`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/kopi.
    ...
    # /usr/local/bin/kopi, code at /usr/local/lib/kopi-ai-agent).
    fhs_link = Path("/usr/local/bin/kopi")
    ...
    path_comment = (
        "# KOPI AI AGENT — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of KOPI_HOME before running the update.
    ...
    KOPI_HOME directories.  The ``--backup`` flag on ``kopi update``
    ...
    # ``kopi_cli/config.py``). Fall back to false if the key is missing
    ...
    try:
        from kopi_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_kopi_home so the user sees ~/.kopi/...
    try:
        from kopi_constants import get_kopi_home, display_kopi_home

        home = get_kopi_home()
        try:
            display_path = f"{display_kopi_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  kopi import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``kopi update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``kopi update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run kopi from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.kopi-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".kopi-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The kopi.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m kopi_cli.main
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines.append("    kopi update")
    lines.append("  (or use `kopi update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    kopi.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from kopi_cli import gateway_windows
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m kopi_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: kopi gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``kopi gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from kopi_cli import gateway_windows
        from kopi_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``kopi update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update KOPI AI AGENT to the latest version.
    ...
    if is_managed():
        managed_error("update KOPI AI AGENT")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `kopi update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    ...
    from kopi_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "kopi-ai-agent"]
        ...
        cmd = [pipx, "upgrade", "kopi-ai-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "kopi-ai-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "kopi-ai-agent"]
    ...
    print("✓ Update complete! Restart kopi to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_kopi_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `kopi serve` backend) will keep .pyd
    ...
    # the kopi.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Hermes windows/gateways and re-run: kopi update")
            else:
                print("✓ Already up to date!")
            _resume_windows_gateways_after_update(_windows_gateway_resume)
            return
        ...
        # though `git pull` can't touch $KOPI_HOME, this is cheap
        ...
        # orphan merge-conflict markers in kopi_cli/config.py bricked
        # every user who ran ``kopi update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``kopi update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_kopi_home added to kopi_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``kopi`` launch finishes the
        ...
        # build.  ``kopi desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``kopi desktop`` shouldn't be forced into a full
        # Electron build by ``kopi update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        from kopi_constants import find_node_executable

        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "kopi_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `kopi desktop` to retry)")
                tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
                if tail:
                    print(tail)
                from kopi_constants import display_kopi_home as _dhh
        ...
        # current — so copy it straight over ~/.kopi/cache/model_catalog.json
        ...
        # modules in this process.  Reload kopi_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_kopi_home() added since the last release.
        try:
            import importlib
            import kopi_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit KOPI_HOME so
        # it is not affected by sync_skills()'s module-level KOPI_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's KOPI_HOME env var points at the default or a named profile.
        try:
            from kopi_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from kopi_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'kopi config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: kopi config migrate")
            else:
                print()
                print("Skipped. Run 'kopi config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from kopi_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `kopi curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``kopi update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from kopi_cli.tools_config import install_cua_driver
        ...
        # When running as ``kopi update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_kopi_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... kopi-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... kopi-gateway*)
            ...
            # Discover all kopi-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. kopi-gateway.service or kopi-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `kopi update` restart it automatically, allow\n"
                        ...
                        print(
                            ...
                            f"Check the gateway with: kopi gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: kopi gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: kopi -p <profile> gateway run  for each profile)"
                        )
        ...
        # When both kopi.service (from a pre-rename install) and the
        # current kopi-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `kopi update` surfaces the issue until the user migrates.
        try:
            from kopi_cli.gateway import (
                has_legacy_kopi_units,
                _find_legacy_kopi_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_kopi_units():
                print()
                print("⚠ Legacy Hermes gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_kopi_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (kopi.service) fight the current")
                print("  kopi-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    kopi gateway migrate-legacy")
                print()
        ...
        print("  kopi model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``kopi -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `kopi profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_kopi_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → kopi -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.kopi)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  kopi profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     kopi -p {name} chat")
                else:
        ...
        from kopi_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from kopi_constants import get_kopi_home as _hh
        ...
        from kopi_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `kopi profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → kopi -p {name}  ({wrapper})")
        print()
        ...
        from kopi_cli.profiles import profile_exists, validate_alias_name
        ...
        from kopi_cli.profiles import rename_profile
        ...
        from kopi_cli.profiles import export_profile
        ...
        from kopi_cli.profiles import import_profile
        ...
        import tempfile
        from kopi_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="kopi_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  kopi -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      kopi -p {plan.manifest.name} chat")
        ...
        from kopi_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `kopi profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"kopi -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from kopi_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("kopi_requires"):
            print(f"Requires:     Hermes {data['kopi_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from kopi_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.kopi_requires:
        print(f"  Requires: Hermes {mf.kopi_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``kopi dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``kopi dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No kopi dashboard processes running.")
        return 0

    print(f"{len(pids)} kopi dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``kopi dashboard register`` for OAuth.
    ...
    try:
        from kopi_cli.web_server import should_require_auth
    ...
    try:
        from kopi_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `kopi dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    try:
        from kopi_cli.plugins import discover_plugins
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No kopi dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `kopi update`.
    ...
    # launches the dashboard (`worker dashboard` → KOPI_HOME points into
    ...
    if (
        ...
        and os.environ.get("KOPI_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "kopi_cli.main",
            "-p", "default",
        ...
        # KOPI_HOME.  We must resolve the root explicitly instead of just
        # dropping KOPI_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV KOPI_HOME=/opt/data`), so an unset
        # KOPI_HOME falls back to $HOME/.kopi = /opt/data/.kopi — an
        ...
        # update-button guard also misfires).  get_default_kopi_root()
        # returns the root for both layouts: ~/.kopi for a standard install
        ...
        try:
            from kopi_constants import get_default_kopi_root
            env["KOPI_HOME"] = str(get_default_kopi_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop KOPI_HOME) rather than block the reroute.
            env.pop("KOPI_HOME", None)
    ...
    if _headless_backend:
        ...
        os.environ["KOPI_SERVE_HEADLESS"] = "1"
    elif "KOPI_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["KOPI_WEB_DIST"])
            if "KOPI_WEB_DIST" in os.environ
            else PROJECT_ROOT / "kopi_cli" / "web_dist"
        )
    ...
    try:
        from kopi_cli.mcp_startup import start_background_mcp_discovery
    ...
    from kopi_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from kopi_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from kopi_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from kopi_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from kopi_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from kopi_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    ...
    from kopi_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from kopi_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``kopi -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``kopi_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``kopi -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``kopi`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("KOPI_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from kopi_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from kopi_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["KOPI_DEFER_AGENT_STARTUP"] = "1"
            os.environ["KOPI_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["KOPI_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `kopi --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_kopi_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_kopi_home()}/memories/\n")
    else:
        from kopi_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch KOPI AI AGENT as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from kopi_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from kopi_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from kopi_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from kopi_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from kopi_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from kopi_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from kopi_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from kopi_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from kopi_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for kopi CLI."""
    # Cosmetic: make the process show up as 'kopi' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from kopi_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``kopi.exe.old.*`` quarantine files left by previous
    # ``kopi update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_kopi_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``kopi update``
    ...
    # ``kopi skills install update``) merely defers recovery one launch;
    # under-matching (missing ``kopi -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in kopi_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from kopi_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from kopi_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `kopi model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    from kopi_cli import secrets_cli as _secrets_cli
    from kopi_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from kopi_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in kopi_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in kopi_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in kopi_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in kopi_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `kopi whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in kopi_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from kopi_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in kopi_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in kopi_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in kopi_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in kopi_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in kopi_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in kopi_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from kopi_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from kopi_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from kopi_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in kopi_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in kopi_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in kopi_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in kopi_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in kopi_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in kopi_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.kopi/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo kopi uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from kopi_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in kopi_cli/subcommands/import_cmd.py)
    # =========================================================================
    ...
    # =========================================================================
    # pairing command  (parser built in kopi_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in kopi_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from kopi_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in kopi_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``kopi --help``, ``kopi version``, ``kopi logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from kopi_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from kopi_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from kopi_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from kopi_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in kopi_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in kopi_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `kopi computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `kopi tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `kopi computer-use doctor` to run cua-driver's\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from kopi_cli.tools_config import _cua_driver_cmd
            # Honor KOPI_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                version = ""
                try:
                    from kopi_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: kopi computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: kopi computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: kopi computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: kopi computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: kopi computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: kopi computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in kopi_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <kopi home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            from kopi_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from kopi_cli.session_export_md import redact_session_data
            ...
            if args.format == "html":
                if not args.output or args.output == "-":
                    ...
                    return
                from kopi_cli.session_export_html import (
                ...
                try:
                    if len(ids) == 1:
                        ...
                        out_dir = (
                            ...
                            else get_kopi_home() / "session-exports"
                        )
            ...
            # Markdown / QMD export
            from kopi_cli.session_export_md import (
            ...
            if args.output == "-":
                ...
                db.close()
                return
            output_dir = Path(args.output).expanduser() if args.output else get_kopi_home() / "session-exports"
            ...
            if args.session_id:
                ...
                if args.delete_after_verified:
                    ...
                    if not ok:
                        ...
                        db.close()
                        return
                    sessions_dir = get_kopi_home() / "sessions"
            ...
            # `kopi sessions prune`: no time window and no filters at all
            ...
            # Launch kopi --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in kopi_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in kopi_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in kopi_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in kopi_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in kopi_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in kopi_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in kopi_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in kopi_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # run `kopi desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in kopi_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in kopi_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in kopi_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``kopi -c Pokemon Agent Dev`` → ``kopi -c 'Pokemon Agent Dev'``
    ...
    from kopi_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'kopi -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (kopi hooks list, cron
