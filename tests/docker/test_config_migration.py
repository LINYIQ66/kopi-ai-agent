...
Build the real image and verify: a config.yaml present in $KOPI_HOME
is migrated by docker_config_migrate.py on boot, running as the kopi
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $KOPI_HOME must be migrated on boot by
    docker_config_migrate.py, running as the kopi user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $KOPI_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/kopi/scripts/docker_config_migrate.py && "
    ...
    assert r.stdout.strip() == "kopi", (
        f"config.yaml not owned by kopi (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
...
def test_config_migration_opt_out_env_var_respected(
    ...
    """KOPI_SKIP_CONFIG_MIGRATION=1 must skip the migration."""
    start_container(
        built_image, container_name, "KOPI_SKIP_CONFIG_MIGRATION=1",
    )
