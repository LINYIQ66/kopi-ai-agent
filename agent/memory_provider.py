...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `kopi backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - kopi_home (str): The active KOPI_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.kopi``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "kopi").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'kopi memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], kopi_home: str) -> None:
        ...
        Called by 'kopi memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``kopi_home`` is the active KOPI_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE KOPI_HOME.

        ``kopi backup`` only walks KOPI_HOME, so any provider state kept
        ...
        of the archive, and ``kopi import`` restores them to their original
