"""kopi webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.kopi/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _kopi_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: kopi gateway run
"""
...
def webhook_command(args):
    """Entry point for 'kopi webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: kopi webhook {subscribe|list|remove|test}")
        print("Run 'kopi webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (kopi gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: kopi webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from kopi webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (kopi gateway run)")
