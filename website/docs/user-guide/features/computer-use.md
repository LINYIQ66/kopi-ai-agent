...
KOPI AI AGENT can drive your desktop — clicking, typing, scrolling,
...
```
kopi computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `kopi computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `kopi tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Hermes app). `kopi computer-use doctor` will tell you which permission is missing. |
...
```
kopi -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.kopi/config.yaml`.

## `kopi computer-use doctor` — your first triage stop

`kopi computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ kopi computer-use doctor
...
**session id** (something like `kopi-3a7b9c14d2e8`); the cursor's
...
Pair with `approvals.mode: manual` in `~/.kopi/config.yaml` if you
...
```
KOPI_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
KOPI_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```
...
When it's on, `kopi computer-use doctor` reports `telemetry: enabled`;
...
`KOPI_CUA_DRIVER_VERSION`**, so a local build (reported as
...
`KOPI_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
KOPI_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
KOPI_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Hermes is using your build

- `kopi computer-use status` prints the resolved binary path and
  version.
- `kopi computer-use doctor` confirms the binary is reachable and
...
**First action when anything's off: run `kopi computer-use doctor`.**
...
Run `kopi computer-use install` to fetch the cua-driver binary, or
run `kopi tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `kopi computer-use doctor` will
