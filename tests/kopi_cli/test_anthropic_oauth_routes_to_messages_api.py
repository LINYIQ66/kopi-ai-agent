...
* ``kopi_cli.providers.determine_api_mode`` correctly mapped
  ``api.anthropic.com`` to ``anthropic_messages``.
* ``kopi_cli.runtime_provider._detect_api_mode_for_url`` did NOT, so
...
class TestPoolEntryForAnthropic:
    ...
    has added an OAuth credential via ``kopi auth add anthropic
