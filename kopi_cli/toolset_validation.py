...
``kopi-cli`` to the non-existent ``kopi``. ``resolve_toolset('kopi')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``kopi-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``kopi`` instead of ``kopi-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `kopi tools` to reconfigure."
        )
    return warnings
