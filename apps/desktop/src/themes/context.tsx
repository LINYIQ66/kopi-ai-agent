...
const SKIN_KEY = 'kopi-desktop-theme-v2'
const MODE_KEY = 'kopi-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'kopi-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'kopi-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'kopi-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.kopiTheme = skinName
  root.dataset.kopiMode = rendered
  ...
  window.kopiDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('kopi-boot-background', chromeBg)
    window.localStorage.setItem('kopi-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.kopiThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.kopiDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
