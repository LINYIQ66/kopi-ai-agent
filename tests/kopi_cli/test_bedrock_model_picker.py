...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (KOPI_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via KOPI_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in KOPI_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from kopi_cli.providers import KOPI_OVERLAYS
        assert "bedrock" in KOPI_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from kopi_cli.providers import KOPI_OVERLAYS
        assert KOPI_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from kopi_cli.providers import KOPI_OVERLAYS
        assert KOPI_OVERLAYS["bedrock"].auth_type == "aws_sdk"
