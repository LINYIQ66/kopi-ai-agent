"""
Doctor command for kopi CLI.

Diagnoses issues with KOPI AI AGENT setup.
"""
...
_DHH = display_kopi_home()  # user-facing display path (e.g. ~/.kopi or ~/.kopi/profiles/coder)

# Load environment variables from ~/.kopi/.env so API key checks work
_env_path = get_env_path()
load_kopi_dotenv(kopi_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from kopi_cli.colors import Colors, color
from kopi_cli.models import _KOPI_USER_AGENT
from kopi_constants import OPENROUTER_MODELS_URL
...
from kopi_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.kopi/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("KOPI_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from kopi_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches kopi_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``kopi --version`` reporting a stale version while
    ...
    """
    try:
        from kopi_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != kopi_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'kopi update', or set "
            "kopi_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-kopi and dashboard static services are up
    ...
    for static in ("main-kopi", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `kopi profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from kopi_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the KOPI_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("KOPI_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via KOPI_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `kopi`.
    os.environ.setdefault("KOPI_INTERACTIVE", "1")

    # Handle `kopi doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.kopi/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`kopi doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and kopi_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.kopi/.env (primary location for user config)
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'kopi setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'kopi setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'kopi setup' to create one")
                issues.append("Run 'kopi setup' to create .env")
    
    # Check ~/.kopi/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from kopi_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from kopi_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'kopi config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from kopi_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.kopi/.env or run 'kopi setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'kopi setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'kopi config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from kopi_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from kopi_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'kopi setup' to migrate config")
                else:
                    issues.append("Run 'kopi doctor --fix' or 'kopi setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'kopi doctor --fix'")
        except Exception:
            pass

        # Detect stale KOPI_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives KOPI_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from kopi_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("KOPI_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"KOPI_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `kopi setup` run)",
                )
                if should_fix:
                    if remove_env_value("KOPI_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale KOPI_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove KOPI_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the KOPI_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale KOPI_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'kopi doctor --fix'"
                    )
        except Exception:
            pass
    ...
    for subdir_name in expected_subdirs:
        subdir_path = kopi_home / subdir_name
    ...
    memories_dir = kopi_home / "memories"
    ...
    state_db_path = kopi_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from kopi_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'kopi doctor --fix' "
                        "(or 'kopi sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from kopi_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'kopi doctor --fix' "
                        "(or 'kopi sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = kopi_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'kopi doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "kopi"
        ...
        _cmd_link = _cmd_link_dir / "kopi"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(kopi not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/kopi → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/kopi points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/kopi → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/kopi — run 'kopi doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/kopi exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/kopi not found",
                    "(kopi command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/kopi → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/kopi symlink — run 'kopi doctor --fix'")
    ...
    try:
        from kopi_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_path.exists():
            ...
            # `kopi update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `kopi doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable KOPI_HOME mirror
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'kopi config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``kopi doctor``.
    ...
    # never the legitimate source for `kopi doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'kopi setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = KOPI_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: kopi skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = KOPI_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: kopi memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: kopi memory setup",
                    "No Honcho API key — run 'kopi memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run kopi memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: kopi memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: kopi memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from kopi_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "kopi -p" in content:
                            _m = _re.search(r"kopi -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'kopi doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
