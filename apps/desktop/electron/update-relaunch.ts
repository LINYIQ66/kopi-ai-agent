 ...
 * Background
 * ----------
 * After `kopi update` + `kopi desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything KOPI_DESKTOP_* is preserved
// plus KOPI_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['KOPI_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['KOPI_DESKTOP_']
