...
Before the fix, ``kopi whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``kopi gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from kopi_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("kopi_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``kopi whatsapp`` with an existing paired session and
