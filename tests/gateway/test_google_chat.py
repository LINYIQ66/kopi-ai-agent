...
def adapter(tmp_path):
    ...
    ~/.kopi/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.kopi/...)
    ...
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        answers = {
            ...
            "Pub/Sub subscription (projects/<proj>/subscriptions/<sub>)": (
                "projects/demo-project/subscriptions/kopi-chat"
            ),
        ...
        monkeypatch.setattr("kopi_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("kopi_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "kopi_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr(
            "kopi_cli.cli_output.print_info", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "kopi_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "kopi_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
