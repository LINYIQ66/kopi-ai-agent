...
Storage
-------
Bundles live in ``~/.kopi/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``KOPI_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``kopi bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under KOPI_HOME.

    Honors ``KOPI_BUNDLES_DIR`` for tests; falls back to
    ``<KOPI_HOME>/skill-bundles``.
    """
    override = os.environ.get("KOPI_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_kopi_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `kopi bundles` CLI subcommand.
# ---------------------------------------------------------------------------
