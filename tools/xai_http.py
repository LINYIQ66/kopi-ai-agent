...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``kopi tools`` repaint, tool-registration scans,
    ...
    2. ``~/.kopi/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_kopi_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.kopi/.env`` first, then ``os.environ``.

    Wraps :func:`kopi_cli.config.get_env_value` so tests can patch
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    ...
    try:
        ...
        marker_dir = get_kopi_home() / "state"
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    to ``XAI_API_KEY`` resolved via ``kopi_cli.config.get_env_value`` so keys
    stored in ``~/.kopi/.env`` (the standard Hermes location) are honored —
