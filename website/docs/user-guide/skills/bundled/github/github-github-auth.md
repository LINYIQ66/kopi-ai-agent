...
- Give it a name like "kopi-ai-agent"
- Select scopes:
...
- Give it a title like "kopi-ai-agent-&lt;machine-name>"
...
elif _kopi_env="${KOPI_HOME:-$HOME/.kopi}/.env"; [ -f "$_kopi_env" ] && grep -q "^GITHUB_TOKEN=" "$_kopi_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_kopi_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
