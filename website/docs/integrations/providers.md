...
This page covers setting up inference providers for KOPI AI AGENT — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Hermes.

## Inference Providers

You need at least one way to connect to an LLM. Use `kopi model` to switch providers and models interactively, or configure directly:
...
| **Anthropic** | `kopi model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.kopi/.env` |
| **NovitaAI** | `NOVITA_API_KEY` in `~/.kopi/.env` (provider: `novita`, 200+ models, Model API, Agent Sandbox, GPU Cloud) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.kopi/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.kopi/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.kopi/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.kopi/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.kopi/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.kopi/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.kopi/.env` (provider: `minimax-cn`) |
| **xAI (Grok) — Responses API** | `XAI_API_KEY` in `~/.kopi/.env` (provider: `xai`) |
| **xAI Grok OAuth (SuperGrok)** | `kopi model` → "xAI Grok OAuth (SuperGrok / Premium+)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.kopi/.env` (provider: `alibaba`) |
...
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.kopi/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.kopi/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.kopi/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.kopi/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.kopi/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.kopi/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.kopi/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.kopi/.env` (provider: `gemini`) |
| **Google Vertex AI** | `kopi model` → "Google Vertex AI" (provider: `vertex`; OAuth2 via service-account JSON or ADC, GCP billing) |
| **OpenAI API (direct)** | `OPENAI_API_KEY` in `~/.kopi/.env` (provider: `openai-api`, optional `OPENAI_BASE_URL`) |
| **Azure AI Foundry** | `kopi model` → "Azure AI Foundry" (provider: `azure-foundry`; uses Azure OpenAI / Foundry endpoint and key) |
| **AWS Bedrock** | `kopi model` → "AWS Bedrock" (provider: `bedrock`; standard AWS credentials chain via boto3) |
| **NVIDIA Build** | `NVIDIA_API_KEY` in `~/.kopi/.env` (provider: `nvidia`; NIM-hosted models on build.nvidia.com) |
| **Ollama Cloud** | `kopi model` → "Ollama Cloud" (provider: `ollama-cloud`; cloud-hosted Ollama API) |
| **Qwen OAuth** | `kopi model` → "Qwen OAuth" (provider: `qwen-oauth`; browser PKCE login) |
| **MiniMax OAuth** | `kopi model` → "MiniMax (OAuth)" (provider: `minimax-oauth`; browser PKCE login) |
| **StepFun** | `STEPFUN_API_KEY` in `~/.kopi/.env` (provider: `stepfun`) |
| **LM Studio** | `kopi model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `kopi model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Hermes stores the resulting credentials in its own auth store under `~/.kopi/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Hermes marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `kopi auth add codex-oauth` (or `kopi model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::
...
|---------|-------------|--------------|
| **`kopi model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a Hermes chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `kopi model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `kopi model`, complete the provider setup, then start a new session.
...
When you authenticate via `kopi model` → Anthropic OAuth (or via `kopi auth add anthropic --type oauth`), Hermes routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by Hermes — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
kopi chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `kopi model`
# Hermes will use Claude Code's credential store directly when available
kopi model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
kopi chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
kopi chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `kopi model`, Hermes prefers Claude Code's own credential store over copying the token into `~/.kopi/.env`. That keeps refreshable Claude credentials refreshable.
...
```bash
kopi chat --provider copilot --model gpt-5.4
```
...
If no token is found, `kopi model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `kopi model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `kopi model` to authenticate via OAuth instead.
:::
...
```bash
kopi chat --provider copilot-acp --model copilot-acp
...
| `KOPI_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `KOPI_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# NovitaAI Model API
kopi chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.kopi/.env

# z.ai / ZhipuAI GLM
kopi chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.kopi/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
kopi chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.kopi/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
kopi chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.kopi/.env

# MiniMax (global endpoint)
kopi chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.kopi/.env

# MiniMax (China endpoint)
kopi chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.kopi/.env

# Qwen Cloud / DashScope (Qwen models)
kopi chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.kopi/.env

# Xiaomi MiMo
kopi chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.kopi/.env

# Tencent TokenHub (Hy3 Preview)
kopi chat --provider tencent-tokenhub --model hy3-preview
# Requires: TOKENHUB_API_KEY in ~/.kopi/.env

# Arcee AI (Trinity models)
kopi chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.kopi/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
kopi chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.kopi/.env
```
...
xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.kopi/.env` and pick xAI in `kopi model`, or drop `grok` as a shortcut into `/model grok-4-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok / Premium+)** in `kopi model`, or run `kopi auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if Hermes runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `kopi tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

**Retired xAI model migration (May 15, 2026):** xAI is retiring `grok-4*`, `grok-3`, `grok-code-fast-1`, and `grok-imagine-image-pro` on 2026-05-15. `kopi doctor` and `kopi chat` startup both detect any config still pointing at a retired ref and print the recommended replacement. Use `kopi migrate xai` for a one-shot config rewrite — dry-run by default, add `--apply` to write changes (a timestamped `config.yaml.bak-pre-migrate-xai-*` backup is created automatically).

```bash
kopi migrate xai          # preview replacements
kopi migrate xai --apply  # rewrite ~/.kopi/config.yaml in place
```
...
# Short alias
kopi chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `kopi model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Hermes auto-discovers the available models.
...
kopi chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 kopi chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
echo "VERTEX_CREDENTIALS_PATH=/path/to/service-account.json" >> ~/.kopi/.env
# or Application Default Credentials
gcloud auth application-default login

kopi model   # → "Google Vertex AI" → project → region → model
```
...
`VERTEX_PROJECT_ID` / `VERTEX_REGION` env vars override the `config.yaml` values. Install with `pip install 'kopi-ai-agent[vertex]'` (or let Hermes lazy-install `google-auth` on first use). See the [Google Vertex AI guide](/guides/google-vertex) for the full walkthrough, and the [Google Gemini guide](/guides/google-gemini) for the static-API-key AI Studio path instead.

### Qwen Portal (OAuth)

Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `kopi model`, sign in through the browser, and Hermes persists the refresh token.

```bash
kopi model
# → pick "Qwen OAuth (Portal)"
# → browser opens; sign in with your Alibaba account
# → confirm — credentials are saved to ~/.kopi/auth.json

kopi chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `KOPI_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
```bash
kopi chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `kopi model`, sign in through the browser, and Hermes persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
kopi chat   # uses api.minimax.io/anthropic endpoint
```
...
kopi chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.kopi/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 kopi chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
```bash
# GMI Cloud
kopi chat --provider gmi --model deepseek-ai/DeepSeek-V3.2
# Requires: GMI_API_KEY in ~/.kopi/.env
```
...
```bash
# StepFun
kopi chat --provider stepfun --model step-3.5-flash
# Requires: STEPFUN_API_KEY in ~/.kopi/.env
```
...
```bash
# Use any available model
kopi chat --provider huggingface --model Qwen/Qwen3.5-397B-A17B
# Requires: HF_TOKEN in ~/.kopi/.env

# Short alias
kopi chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
KOPI AI AGENT works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Hermes at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`LLM_MODEL` in `.env` is **removed** — `config.yaml` is the single source of truth for model and endpoint configuration. `OPENAI_BASE_URL` is still honored, but **only** for the `openai-api` provider (it overrides the OpenAI endpoint for direct API-key access). For other providers and custom endpoints, use `kopi model` or set `model.base_url` in `config.yaml` directly. If you have stale entries in your `.env`, they are automatically cleared on the next `kopi setup` or config migration.
:::
...
:::warning kopi model vs /model
**`kopi model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.
...
**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `kopi model`, set up the new provider, then start a new session.
:::
...
KOPI AI AGENT requires at least **64,000 tokens** of context for agent use with tools. Smaller windows are rejected at startup because the system prompt, tool schemas, and working conversation state need enough room for reliable multi-step workflows.
...
Supported parsers: `kopi` (Qwen 2.5, Hermes 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Since KOPI AI AGENT requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Then configure Hermes with `kopi model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Hermes with `kopi model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `kopi model` → Custom endpoint, or in `config.yaml`:
...
`kopi model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  ...
  - name: work
    ...
    api_mode: chat_completions   # set explicitly by `kopi model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
The `kopi model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.
...
You can also select named custom providers from the interactive `kopi model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.kopi/config.yaml` and the matching API key goes in `~/.kopi/.env`.
...
Together's `/v1/models` endpoint works, so `kopi model` can auto-discover available models.
...
:::tip Troubleshooting
- `kopi doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `kopi model` will persist the model with a warning rather than hard-reject — see #15136.
...
:::tip
You can switch between providers at any time with `kopi model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Hermes at your instance (no API key needed):
   ```bash
   kopi config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.kopi/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.kopi/config.yaml`:
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `kopi fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/user-guide/features/fallback-providers).
:::
