...
Supersedes: PR #31781 (prompt_toolkit `kopi profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `kopi profile wizard` in the terminal.
...
needs, and a profile is just a KOPI_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.kopi/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — KOPI_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with KOPI_HOME override |
...
### Seam #1 — hub-skill install cannot use the KOPI_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = KOPI_HOME / "skills"` at **module
import time**. The context-local `set_kopi_home_override()` swap (which makes
...
The correct mechanism is the existing subprocess path: `_spawn_kopi_action`
runs `python -m kopi_cli.main <subcommand>`, and `_apply_profile_override()`
re-reads `sys.argv` at import in the fresh child. Prepend `-p <profile>`:

```python
_spawn_kopi_action(["-p", profile, "skills", "install", identifier], "skills-install")
```

A fresh subprocess re-imports `skills_hub` with the profile's KOPI_HOME bound
...
spawned detached (`_spawn_kopi_action` returns a PID immediately). So the
create flow is:

1. `create_profile()` — make the dir (synchronous)
2. write model (synchronous, KOPI_HOME override)
3. write selected MCP servers (synchronous, KOPI_HOME override)
4. seed/enable selected built-in + optional skills (synchronous)
5. spawn `kopi -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, KOPI_HOME override
...
page afterward. Mirror `_write_profile_model`'s KOPI_HOME-override helper for
...
- Backend E2E with isolated KOPI_HOME: POST a full create body
