...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *KOPI AI AGENT*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to KOPI_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/kopi), this function
    mirrors the bridge source to a writable KOPI_HOME location and returns that
    ...
    # Try KOPI_HOME location first
    kopi_home = get_kopi_home()
    kopi_home_bridge = kopi_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to KOPI_HOME if needed
    if kopi_home_bridge.exists():
        return kopi_home_bridge

    # Mirror the bridge source to KOPI_HOME
    try:
        kopi_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            kopi_home_bridge,
            dirs_exist_ok=False,
