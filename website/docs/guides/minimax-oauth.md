...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in KOPI AI AGENT — no API key required"
---

# MiniMax OAuth

KOPI AI AGENT supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Hermes automatically refreshes your session.
...
After the first login, credentials are stored under `~/.kopi/auth.json` and are refreshed automatically before each session.
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.kopi/.env
```
...
```bash
kopi auth add minimax-oauth --no-browser
```
...
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.kopi/auth.json` under the `minimax-oauth` key.
...
```bash
kopi config set model.default MiniMax-M2.7
kopi config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.kopi/config.yaml` will contain entries similar to:
...
```bash
kopi --provider minimax-oauth    # canonical
kopi --provider minimax-portal   # alias
kopi --provider minimax-global   # alias
kopi --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `kopi setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
kopi --provider minimax-oauth
```
...
**Fix:** run `kopi auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Hermes raises a timeout error.

**Fix:** re-run `kopi auth add minimax-oauth` (or `kopi model`). The flow starts fresh.
...
If `kopi` cannot open a browser window, use `--no-browser`:
...
**Fix:** run `kopi model` and select MiniMax (OAuth), or run `kopi auth add minimax-oauth`.
...
```bash
kopi auth logout minimax-oauth
```
...
- [kopi doctor](../reference/cli-commands.md)
