...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/LINYIQ66/kopi-ai-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    kopi_meta = (fm.get("metadata") or {}).get("kopi") or {}
    tags = kopi_meta.get("tags") or []
    related = kopi_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `kopi skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with KOPI AI AGENT"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Hermes ships with a large built-in skill library copied into `~/.kopi/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Hermes also syncs bundled skills on `kopi update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.kopi/skills/` tree, it is still shipped with Hermes; restore it with `kopi skills reset <name> --restore`.",
        "",
