...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("KOPI_HOME", "/users/alice/.kopi/profiles/backend-worker")
        monkeypatch.setenv("KOPI_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "KOPI_KANBAN_DB",
            "/users/alice/.kopi/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.kopi/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through kopi_subprocess_env(
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        for var, val in {
            ...
            "KOPI_DASHBOARD_SESSION_TOKEN": "dash-secret",
        ...
        for var in (
            ...
            "KOPI_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
    ...
    def test_home_still_preserved_through_helper(self, monkeypatch):
        """Regression guard: routing through kopi_subprocess_env must not
