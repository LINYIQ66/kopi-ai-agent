...
3. Point Hermes at the extracted bundle — the directory that contains
   ...
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<KOPI_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`kopi lsp status` reports `installed` once `pwsh` is found; if the
...
run `kopi lsp install typescript` or auto-install fires on first
use.

## CLI

```
kopi lsp status          # service state + per-server install status
kopi lsp list            # registry, optionally --installed-only
kopi lsp install <id>    # eagerly install one server
kopi lsp install-all     # try every server with a known recipe
kopi lsp restart         # tear down running clients
kopi lsp which <id>      # print resolved binary path
```

`kopi lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <KOPI_HOME>/lsp/bin
...
`<KOPI_HOME>/lsp/bin/`. NPM packages land in
`<KOPI_HOME>/lsp/node_modules/` with bin symlinks one level up.
...
**`kopi lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<KOPI_HOME>/lsp/bin/`. Run
`kopi lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `kopi lsp status`**
...
When `kopi lsp status` shows a `Backend warnings` section, install
...
Check `~/.kopi/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `kopi lsp restart` to clear the set;
