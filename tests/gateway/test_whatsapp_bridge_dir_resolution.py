...
(/opt/kopi/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable KOPI_HOME location instead.
"""
import importlib
...
def test_writable_install_returns_install_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kopi_constants.get_kopi_home", lambda: kopi_home
    )

    resolved = whatsapp_common.resolve_whatsapp_bridge_dir()
    assert resolved == install_bridge
    # Nothing mirrored into KOPI_HOME.
    assert not (kopi_home / "scripts" / "whatsapp-bridge").exists()


def test_readonly_install_mirrors_to_kopi_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable KOPI_HOME."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)
    ...
    expected = kopi_home / "scripts" / "whatsapp-bridge"
...
def test_readonly_install_reuses_existing_mirror(tmp_path, monkeypatch):
    """If the KOPI_HOME mirror already exists, return it without re-copying."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)

    kopi_home = tmp_path / "kopi_home"
    mirror = kopi_home / "scripts" / "whatsapp-bridge"
