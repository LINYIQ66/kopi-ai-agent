...
closure the PR changed, against a real temp ``KOPI_HOME``.
"""
...
def _fake_switch_result():
    ...
    from kopi_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "kopi_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_kopi_home() / config.yaml`` — point it here.
    monkeypatch.setattr("kopi_constants.get_kopi_home", lambda: kopi_home)
    monkeypatch.setattr("kopi_cli.config.get_kopi_home", lambda: kopi_home)
    return cfg_path
