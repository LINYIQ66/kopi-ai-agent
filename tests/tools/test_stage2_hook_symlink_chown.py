...
def _chown_kopi_tree_function(text: str) -> str:
...
def _run_helper(
    ...
    log_path: Path,
    *,
    kopi_home: Path | None = None,
) -> subprocess.CompletedProcess[str]:
    shell = shutil.which("sh")
    if shell is None:
        pytest.skip("sh not available")
    kopi_home = target if kopi_home is None else kopi_home
    script = (
        "set -eu\n"
        f'KOPI_HOME="{kopi_home}"\n'
        f"{_chown_kopi_tree_function(text)}\n"
        f'chown() {{ printf "%s\\n" "$*" >> "{log_path}"; }}\n'
        f'chown_kopi_tree "{target}"\n'
    )
...
def test_chown_helper_refuses_symlinked_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    real_home.mkdir()
    symlinked_home = tmp_path / "kopi-home"
    try:
...
def test_chown_helper_refuses_target_under_symlinked_home(
    ...
    proc = _run_helper(
        ...
        linked_home / "cron",
        log_path,
        kopi_home=linked_home,
    )

    assert proc.returncode == 0, proc.stderr
    assert not log_path.exists(), "must not chown through a symlinked KOPI_HOME"
    assert "refusing recursive chown through symlinked path" in proc.stdout
...
def test_stage2_uses_symlink_safe_helper_for_kopi_home_trees(stage2_text: str) -> None:
    assert 'chown_kopi_tree "$KOPI_HOME/$sub"' in stage2_text
    assert 'chown_kopi_tree "$KOPI_HOME/profiles"' in stage2_text
    assert 'chown_kopi_tree "$KOPI_HOME/cron"' in stage2_text
    assert 'chown -R kopi:kopi "$KOPI_HOME/$sub"' not in stage2_text
    assert 'chown -R kopi:kopi "$KOPI_HOME/profiles"' not in stage2_text
    assert 'chown -R kopi:kopi "$KOPI_HOME/cron"' not in stage2_text


def test_stage2_skips_top_level_chown_for_symlinked_kopi_home(
    stage2_text: str,
) -> None:
