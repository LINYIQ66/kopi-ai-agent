...
(``kopi``, ``kopi-ai-agent``, ``kopi-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    ...
    """
    root = src_root or os.environ.get("KOPI_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``KOPI_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("KOPI_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import kopi_bootstrap``
# (or ``from kopi_bootstrap import apply_windows_utf8_bootstrap``) at
