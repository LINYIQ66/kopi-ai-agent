...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to KOPI AI AGENT via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With KOPI AI AGENT's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
flowchart LR
    ...
    B["kopi-ai-agent<br/>gateway API server<br/>port 8642"]
    A -->|POST /v1/chat/completions| B
    B -->|SSE streaming response| A
```

Open WebUI connects to KOPI AI AGENT's API server just like it would connect to OpenAI. Hermes handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.
...
```bash
kopi config set API_SERVER_ENABLED true
kopi config set API_SERVER_KEY your-secret-key
```

`kopi config set` auto-routes the flag to `config.yaml` and the secret to `~/.kopi/.env`. If the gateway is already running, restart it so the change takes effect:

```bash
kopi gateway stop && kopi gateway
```

### 2. Start KOPI AI AGENT gateway
...
# {"object":"list","data":[{"id":"kopi-ai-agent", ...}]}
```
...
Go to **http://localhost:3000**. Create your admin account (the first user becomes admin). You should see your agent in the model dropdown (named after your profile, or **kopi-ai-agent** for the default profile). Start chatting!
...
Your agent model should now appear in the model dropdown (named after your profile, or **kopi-ai-agent** for the default profile).
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and KOPI AI AGENT responds accordingly. Each request includes the full conversation history.
...
2. Edit your kopi-ai-agent connection
3. Change **API Type** from "Chat Completions" to **"Responses (Experimental)"**
4. Save

With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and KOPI AI AGENT can preserve full tool call history across turns via `previous_response_id`. When `stream: true`, Hermes also streams spec-native `function_call` and `function_call_output` items, which enables custom structured tool-call UI in clients that render Responses events.
...
2. KOPI AI AGENT creates a server-side `AIAgent` instance using the API server's profile, model/provider config, memory, skills, and configured API-server toolsets
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | KOPI AI AGENT's API URL (include `/v1`) |
...
- **Check model listing**: `curl -H "Authorization: Bearer your-secret-key" http://localhost:8642/v1/models` should return a list with `kopi-ai-agent`
...
KOPI AI AGENT may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in KOPI AI AGENT.
...
```bash
kopi profile create alice
cat >> ~/.kopi/profiles/alice/.env <<EOF
API_SERVER_ENABLED=true
API_SERVER_PORT=8650
API_SERVER_KEY=alice-secret
EOF

kopi profile create bob
cat >> ~/.kopi/profiles/bob/.env <<EOF
API_SERVER_ENABLED=true
...
```bash
kopi -p alice config set API_SERVER_MODEL_NAME "Alice's Agent"
```
:::
