...
#
# Wired into the image as /etc/cont-init.d/01-kopi-setup by the
...
INSTALL_DIR="/opt/kopi"

# Drop to kopi via s6-setuidgid, but skip it when already non-root.
as_kopi() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid kopi "$@"; }
...
# starts non-root. The baked install tree under /opt/kopi is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $KOPI_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass KOPI_UID/KOPI_GID — or the PUID/PGID aliases — which the
...
# `id -u kopi` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# kopi UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u kopi)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-kopi UID).
...
and the baked /opt/kopi install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e KOPI_UID=\$(id -u) -e KOPI_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the kopi user to that UID/GID at boot and chowns the data
...
# --- Bootstrap KOPI_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid kopi mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom KOPI_HOME paths whose parents only
# root can create (e.g. `KOPI_HOME=/home/kopi/.kopi` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for KOPI_UID/KOPI_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  KOPI_UID/
# KOPI_GID still win when both are set.  See #15290, salvages #25872.
KOPI_UID="${KOPI_UID:-${PUID:-}}"
KOPI_GID="${KOPI_GID:-${PGID:-}}"

if [ -n "${KOPI_UID:-}" ] && validate_uid_gid "$KOPI_UID" && [ "$KOPI_UID" != "$(id -u kopi)" ]; then
    echo "[stage2] Changing kopi UID to $KOPI_UID"
    usermod -u "$KOPI_UID" kopi
fi
if [ -n "${KOPI_GID:-}" ] && validate_uid_gid "$KOPI_GID" && [ "$KOPI_GID" != "$(id -g kopi)" ]; then
    echo "[stage2] Changing kopi GID to $KOPI_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$KOPI_GID" kopi 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised kopi user (UID 10000)
...
# kopi process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes kopi. Idempotent across container
...
#   - kopi is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G kopi 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] kopi already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" kopi 2>/dev/null; then
        echo "[stage2] Added kopi to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group kopi failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When KOPI_UID is remapped or the top-level $KOPI_HOME isn't owned by
# the runtime kopi UID, restore ownership to kopi — but ONLY for the
# directories kopi actually writes to. The full $KOPI_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of kopi-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_kopi_uid=$(id -u kopi)
...
chown_kopi_tree() {
    ...
    chown -R kopi:kopi "$target" 2>/dev/null || \
...
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $KOPI_HOME (targeted) to kopi ($actual_kopi_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $KOPI_HOME: chown the directory itself (not its contents)
    # so kopi can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$KOPI_HOME"; then
        :
    else
...
# kopi user. Hosted/container instances keep mutable state under
# $KOPI_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# KOPI_DISABLE_LAZY_INSTALLS=1. Keeping /opt/kopi root-owned and
...
# $KOPI_HOME/lazy-packages dir on the data volume (Dockerfile sets
# KOPI_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to kopi
...
# Always reset ownership of $KOPI_HOME/profiles to kopi on every
...
# approval files that the unprivileged kopi gateway cannot read,
...
# Reset ownership of kopi-owned top-level state files on every boot.
# The targeted data-volume chown above only covers kopi-owned
# *subdirectories*; loose state files living directly under $KOPI_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> kopi …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged kopi runtime then hits
...
# sweep so host-owned files in a bind-mounted $KOPI_HOME are never
...
# entries of kopi_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$KOPI_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$KOPI_HOME/$f"; then
            :
        else
            chown kopi:kopi "$KOPI_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the kopi runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$KOPI_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$KOPI_HOME/config.yaml"; then
        :
    else
        chown kopi:kopi "$KOPI_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$KOPI_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as kopi user ---
# Run as kopi via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $KOPI_HOME values
...
as_kopi mkdir -p \
    ...
    "$KOPI_HOME/home" \
    "$KOPI_HOME/pairing" \
    "$KOPI_HOME/platforms/pairing" \
    "$KOPI_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/kopi/.install_method (see Dockerfile), NOT written here into
# $KOPI_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $KOPI_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.kopi:/opt/data) and sometimes shared with a
...
# 'kopi update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $KOPI_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/kopi stamp, so deleting the data-dir copy is safe).
if [ -f "$KOPI_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$KOPI_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$KOPI_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$KOPI_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$KOPI_HOME/$dest"; then
            :
        else
            as_kopi cp "$INSTALL_DIR/$src" "$KOPI_HOME/$dest"
        fi
    fi
}
...
if [ -f "$KOPI_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$KOPI_HOME/.env"; then
        :
    else
        chown kopi:kopi "$KOPI_HOME/.env" 2>/dev/null || true
        chmod 600 "$KOPI_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $KOPI_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `kopi update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set KOPI_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$KOPI_HOME/config.yaml" ]; then
    s6-setuidgid kopi "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$KOPI_HOME/auth.json" ] && [ -n "${KOPI_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$KOPI_HOME/auth.json"; then
        :
    else
        printf '%s' "$KOPI_AUTH_JSON_BOOTSTRAP" > "$KOPI_HOME/auth.json"
        chown kopi:kopi "$KOPI_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$KOPI_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via KOPI_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$KOPI_HOME/auth.json" ] && [ -n "${KOPI_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$KOPI_HOME/auth.json"; then
        :
    else
        s6-setuidgid kopi "$INSTALL_DIR/.venv/bin/python" \
...
# can set KOPI_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# KOPI_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$KOPI_HOME/gateway_state.json" ] && \
        [ "${KOPI_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$KOPI_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$KOPI_HOME/gateway_state.json"
        chown kopi:kopi "$KOPI_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$KOPI_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_kopi "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
...
# main-wrapper.sh propagates it into the supervised ``kopi`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-kopi, dashboard, etc.).
