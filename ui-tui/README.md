...
Interpreter resolution order is: `KOPI_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `kopi setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.kopi/.kopi_history` or under `KOPI_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `kopi setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/kopi-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external kopi setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.kopi/tui-parent.log
    ...
    types/
      kopi-ink.d.ts            type declarations for @kopi/ink
