"""``kopi slack ...`` CLI subcommands.

Today only ``kopi slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ kopi slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    ``/new``, etc. work inline.
    """
    from kopi_cli.commands import slack_app_manifest
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``kopi_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$KOPI_HOME/slack-manifest.json``)
