...
description: "How KOPI AI AGENT remembers across sessions — MEMORY.md, USER.md, and session search"
---

# Persistent Memory

KOPI AI AGENT has bounded, curated memory that persists across sessions. This lets it remember your preferences, your projects, your environment, and things it has learned.
...
Both are stored in `~/.kopi/memories/` and are injected into the system prompt as a frozen snapshot at session start. The agent manages its own memory via the `memory` tool — it can add, replace, or remove entries.
...
- All CLI and messaging sessions are stored in SQLite (`~/.kopi/state.db`) with FTS5 full-text search
...
```bash
kopi sessions list    # Browse past sessions
```
...
`~/.kopi/pending/skills/<id>.json` when you want to read the whole change.
...
```bash
kopi memory setup      # pick a provider and configure it
