...
def server():
    with patch.dict(
        "sys.modules",
        {
            "kopi_constants": MagicMock(
                get_kopi_home=MagicMock(return_value="/tmp/kopi_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'kopi-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'kopi-release' patched"
    }
