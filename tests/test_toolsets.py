...
class TestToolsetConsistency:
    ...
    def test_kopi_platforms_share_core_tools(self):
        """All kopi-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on kopi-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["kopi-cli", "kopi-telegram", "kopi-discord", "kopi-whatsapp", "kopi-slack", "kopi-signal", "kopi-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_kopi_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("kopi-whatsapp")

    def test_kopi_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("kopi-api-server")
