...
* ``kopi profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``kopi skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``kopi profile``, not a parallel tree):

    kopi profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "kopi-ai-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            kopi_requires=str(data.get("kopi_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.kopi_requires:
            out["kopi_requires"] = self.kopi_requires
        if self.author:
...
def check_kopi_requires(spec: str, current_version: str) -> None:
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``kopi profile update`` can
...
def plan_install(
    ...
    check_kopi_requires(manifest.kopi_requires, kopi_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.kopi).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="kopi_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `kopi profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`kopi profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="kopi_dist_update_") as tmp:
        plan = plan_install(
