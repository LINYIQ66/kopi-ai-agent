...
const DEV_SERVER = process.env.KOPI_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.KOPI_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with KOPI_DESKTOP_DISABLE_GPU
...
// KOPI_HOME — the user-facing root for everything Hermes-related. Mirrors
// scripts/install.ps1's $HermesHome and scripts/install.sh's $KOPI_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\kopi (matches install.ps1)
//   macOS / Linux: ~/.kopi (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.kopi directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\kopi yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// KOPI_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// KOPI_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.kopi / %LOCALAPPDATA%\kopi.
function resolveHermesHome() {
  if (process.env.KOPI_HOME) {
    return normalizeHermesHomeRoot(process.env.KOPI_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'kopi-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a KOPI_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\kopi and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('KOPI_HOME')
  ...
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'kopi')
    const legacy = path.join(app.getPath('home'), '.kopi')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.kopi setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.kopi')
}

const KOPI_HOME = resolveHermesHome()
...
// ACTIVE_KOPI_ROOT — the canonical mutable Hermes install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_KOPI_ROOT = path.join(KOPI_HOME, 'kopi-ai-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_KOPI_ROOT, 'venv')
...
//
// We deliberately put the marker INSIDE ACTIVE_KOPI_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_KOPI_ROOT, '.kopi-bootstrap-complete')
...
// local backend as. When set, startHermes() passes `kopi --profile <name>
// dashboard …`, which deterministically pins KOPI_HOME (see
// _apply_profile_override in kopi_cli/main.py) and bypasses the sticky
// ~/.kopi/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors kopi_cli.profiles._PROFILE_ID_RE so we never hand the backend a
...
// desktop.log lives under KOPI_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by kopi_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(KOPI_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (kopi_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.KOPI_DESKTOP_BOOT_FAKE === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.KOPI_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// electron-builder derives from the build `appId` (com.nousresearch.kopi) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.kopi')
}

// Seed the native About panel with the live Hermes version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `kopi update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveHermesVersion(),
  copyright: 'Copyright © 2026 Kopi Ai Agent Pte Ltd'
})
...
const MEDIA_PROTOCOL = 'kopi-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(kopi-media://…)`.
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.KOPI_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.KOPI_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// respawning kopi serve backend children in a tight loop while boot is broken.
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-kopi-wsl-windows-fonts.conf')
    let existing = ''
...
// The Tauri updater writes KOPI_HOME/.kopi-update-in-progress for the whole
...
function findOnPath(command) {
  ...
  // shell-script shim named `kopi` — must not shadow `hermes.cmd`/`hermes.exe`.
...
function unwrapWindowsVenvHermesCommand(command, backendArgs) {
  ...
  return {
    ...
    command: python,
    args: ['-m', 'kopi_cli.main', ...backendArgs],
    bootstrap: false,
...
// spawns `kopi serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `kopi` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'kopi_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, KOPI_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isHermesSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'kopi_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.KOPI_DESKTOP_PYTHON
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\kopi\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'kopi', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'kopi', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal KOPI_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\kopi\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'kopi', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'kopi', 'git', 'bin', 'git.exe'))
  }
...
// ─── Self-update (git-pull against the running backend's kopi root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_KOPI_ROOT.
// KOPI_DESKTOP_KOPI_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.KOPI_DESKTOP_KOPI_ROOT && path.resolve(process.env.KOPI_DESKTOP_KOPI_ROOT),
    !IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isHermesSourceRoot(ACTIVE_KOPI_ROOT) ? ACTIVE_KOPI_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_KOPI_ROOT
}
...
// KOPI_HOME/kopi-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_kopi_home). That binary owns
// ALL repo mutation — running `kopi update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'kopi-setup.exe' : 'kopi-setup'
  const candidate = path.join(KOPI_HOME, name)
...
// Path to the venv shim whose lock decides whether `kopi update` can write
...
function venvHermesShimPath(updateRoot) {
  return IS_WINDOWS
    ...
    : path.join(updateRoot, 'venv', 'bin', 'kopi')
}
...
// spawned its own grandchildren (a `kopi` REPL, a pty terminal session, the
...
// `kopi update`). Instead we spawn the staged Hermes-Setup binary with
// --update and quit, so it can run `kopi update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri kopi-setup. Unlike Windows
      ...
      // whole update itself: `kopi update` (backend) + `kopi desktop
    ...
    if (!updater) {
      ...
      // `kopi desktop`, never the Tauri installer that self-copies
      // kopi-setup.exe into KOPI_HOME). They DO have a working `kopi`
      ...
      // checkout they're on — bare `kopi update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `kopi update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `kopi update` if branch detection fails.
      }
    ...
    // `kopi update` will run (the venv shim is locked while we live).
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvHermes = path.join(venvBin, IS_WINDOWS ? 'hermes.exe' : 'kopi')
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvHermes) || fileExists(path.join(updateRoot, '.kopi-bootstrap-complete'))
...
// Resolve the kopi CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `kopi` on PATH.
function resolveHermesCliBinary(updateRoot) {
  const venvHermes = path.join(updateRoot, 'venv', 'bin', 'kopi')

  if (fileExists(venvHermes)) {
    return venvHermes
  }

  return findOnPath('kopi') || null
}
...
// macOS/Linux in-app update: backend (`kopi update`) + OS-aware GUI rebuild
// (`kopi desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const kopi = resolveHermesCliBinary(updateRoot)

  if (!hermes) {
    emitUpdateProgress({ stage: 'manual', message: 'kopi update', percent: null })

    return { ok: true, manual: true, command: 'kopi update', hermesRoot: updateRoot }
  }

  // Put the Hermes-managed Node and the venv on PATH so `kopi desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\kopi\node, not node\bin.
  const env: Record<string, string> = {
    KOPI_HOME,
    PATH: pathWithHermesManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `kopi update` reaps stale `kopi serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads KOPI_DESKTOP_CHILD_PID and excludes
  ...
  if (desktopChildPids.length) {
    env.KOPI_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'kopi update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'kopi update failed' }
  }
  ...
  // Linux in-app update terminal state (#45205). `kopi desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      ...
      const scriptPath = path.join(app.getPath('temp'), `kopi-desktop-update-${Date.now()}.sh`)
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.kopi-update-new"; then
    rm -rf "$DST.kopi-update-old" 2>/dev/null || true
    mv "$DST" "$DST.kopi-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.kopi-update-new" "$DST"
    rm -rf "$DST.kopi-update-old" 2>/dev/null || true
  fi
fi
...
// Marker-independent: is the canonical install at ACTIVE_KOPI_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isHermesSourceRoot(ACTIVE_KOPI_ROOT) &&
    fileExists(venvPython) &&
    canImportHermesCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_KOPI_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function resolveWebDist() {
  const override = process.env.KOPI_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set KOPI_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: kopi desktop --force-build`
  )
...
// createActiveBackend — build a backend pointing at ACTIVE_KOPI_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Hermes at ${ACTIVE_KOPI_ROOT}`,
    command,
    args: ['-m', 'kopi_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      hermesHome: KOPI_HOME,
      pythonPathEntries: [ACTIVE_KOPI_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_KOPI_ROOT,
    bootstrap: true,
    shell: false
  }
}
...
function resolveHermesBackend(backendArgs) {
  // 1. Explicit override -- KOPI_DESKTOP_KOPI_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.KOPI_DESKTOP_KOPI_ROOT && path.resolve(process.env.KOPI_DESKTOP_KOPI_ROOT)
  ...
  //    installed `kopi` on PATH so local Python edits are actually exercised.
  ...
  // 3. Bootstrap-complete ACTIVE_KOPI_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\kopi\kopi-ai-agent (Windows) or ~/.kopi/kopi-ai-agent.
  ...
  //    (applyUpdates -> git pull) or `kopi update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `kopi` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    KOPI_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.KOPI_DESKTOP_IGNORE_EXISTING !== '1') {
    let hermesCommand = null
    const hermesOverride = process.env.KOPI_DESKTOP_HERMES
    ...
    if (hermesCommand) {
      ...
      // Smoke-test the candidate before trusting it. A `kopi` shim
  ...
  // 5. Last-ditch: pip-installed kopi_cli module via system Python.
  ...
  if (python) {
    ...
    // having kopi_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\kopi\kopi-ai-agent\venv.
    if (canImportHermesCli(python)) {
      return {
        kind: 'python',
        label: `installed kopi_cli module via ${python}`,
        command: python,
        args: ['-m', 'kopi_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: kopi_cli is not importable; falling through to bootstrap.`)
  }
  ...
  return {
    ...
    label: 'KOPI AI AGENT not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    const bootstrapResult = await runBootstrap({
      ...
      hermesHome: KOPI_HOME,
      logRoot: path.join(KOPI_HOME, 'logs'),
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Hermes bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          ...
          `Check ${path.join(KOPI_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any
  ...
  if (!isHermesSourceRoot(ACTIVE_KOPI_ROOT)) {
    throw new Error(
      `Hermes install at ${ACTIVE_KOPI_ROOT} is missing or incomplete. ` +
  ...
  // %LOCALAPPDATA%\kopi\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `kopi` on PATH, this check still helps.
  ...
  backend.label = `Hermes at ${ACTIVE_KOPI_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
...
// Kopi Ai Agent Pte Ltd) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``kopi_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / kopi #37247).
//     Both are set as HttpOnly cookies (``kopi_session_at`` ~15 min,
//     ``kopi_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:kopi-remote-oauth'
...
// a valid stored value (pins the root KOPI_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.KOPI_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.KOPI_DESKTOP_REMOTE_URL || '') : String(block.url || '')
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (kopi #37247), and the
...
//   2. env override (KOPI_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.KOPI_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.KOPI_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'KOPI_DESKTOP_REMOTE_URL is set but KOPI_DESKTOP_REMOTE_TOKEN is not. ' +
...
function globalRemoteActive() {
  if (process.env.KOPI_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Kopi Ai Agent Pte Ltd" instead of a generic
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        KOPI_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        KOPI_DESKTOP: '1',
        KOPI_WEB_DIST: webDist,
        ...(readyFile ? { KOPI_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
...
// backend whose ensure_kopi_home() recreates the deleted profile directory.
...
async function startHermes() {
  ...
  connectionPromise = (async () => {
    ...
    // deterministic (it wins over the sticky ~/.kopi/active_profile file) and
    // resolves KOPI_HOME the same way `kopi -p <name>` does on the CLI. An
    ...
    hermesProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin KOPI_HOME for the child so Python's get_kopi_home()
          // resolves to the SAME location our resolveHermesHome() picked. Without
          // this pin, Python falls back to ~/.kopi on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\kopi, which differs from C:\Users\<u>\.kopi.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets KOPI_HOME via setx; the desktop
...
ipcMain.handle('hermes:profile:set', async (_event, name) => {
  ...
  // new KOPI_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.handle('hermes:api', async (_event, request) => {
  ...
  // backend calls ensure_kopi_home() which recreates the profile directory,
...
function terminalShellCommand() {
  // KOPI_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.KOPI_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // Let a kopi/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from KOPI_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.KOPI_DESKTOP_TERMINAL = '1'
...
// kopi_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
...
function resolveHermesVersion() {
  try {
    ...
    const initPath = path.join(root, 'kopi_cli', '__init__.py')
...
// just before showing it, so an in-place `kopi update` is reflected without
...
// the actual removal via `kopi uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (kopi_cli/uninstall.py + kopi_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_KOPI_ROOT
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'kopi_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, KOPI_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  // Python with PYTHONPATH=<agentRoot> so `import kopi_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_KOPI_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `kopi-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `kopi-uninstall-${Date.now()}.sh`)
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${KOPI_PROTOCOL}://`)) || null
}
...
ipcMain.handle('hermes:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${KOPI_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(KOPI_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(KOPI_PROTOCOL)
    }
  } catch (err) {
