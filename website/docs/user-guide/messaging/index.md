...
```bash
kopi gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
kopi gateway              # Run in foreground
kopi gateway setup        # Configure messaging platforms interactively
kopi gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo kopi gateway install --system   # Linux only: install a boot-time system service
kopi gateway start        # Start the default service
kopi gateway stop         # Stop the default service
kopi gateway status       # Check default service status
kopi gateway status --system         # Linux only: inspect the system service explicitly
```
...
with the `session_reset` section in `~/.kopi/config.yaml`:
...
Configure per-platform overrides in `~/.kopi/gateway.json`:
...
kopi pairing approve telegram XKGH5N7P

# Other pairing commands:
kopi pairing list          # View pending + approved users
kopi pairing revoke telegram 123456789  # Remove access
```
...
Control how much tool activity is displayed in `~/.kopi/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.kopi/config.yaml`:
...
```bash
KOPI_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
kopi gateway install               # Install as user service
kopi gateway start                 # Start the service
kopi gateway stop                  # Stop the service
kopi gateway status                # Check status
journalctl --user -u kopi-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo kopi gateway install --system
sudo kopi gateway start --system
sudo kopi gateway status --system
...
A system service needs root for every restart — including the automatic gateway restart at the end of `kopi update`. When `kopi update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart kopi-gateway` command (it never blocks on an interactive password prompt).
...
```bash
kopi gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `kopi update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo kopi update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/kopi-gateway`:

```
kopi ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed kopi-gateway*, /usr/bin/systemctl --no-ask-password start kopi-gateway*, /usr/bin/systemctl --no-ask-password restart kopi-gateway*
```
:::
...
```bash
kopi gateway install               # Install as launchd agent
kopi gateway start                 # Start the service
kopi gateway stop                  # Stop the service
kopi gateway status                # Check status
tail -f ~/.kopi/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.kopi.gateway.plist`. It includes three environment variables:
...
:::info Multiple installations
Like the Linux systemd service, each `KOPI_HOME` directory gets its own launchd label. The default `~/.kopi` uses `ai.kopi.gateway`; other installations use `ai.kopi.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `kopi-cli` | Full access |
| Telegram | `kopi-telegram` | Full tools including terminal |
| Discord | `kopi-discord` | Full tools including terminal |
| WhatsApp | `kopi-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `kopi-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `kopi-slack` | Full tools including terminal |
| Google Chat | `kopi-google_chat` | Full tools including terminal |
| Signal | `kopi-signal` | Full tools including terminal |
| SMS | `kopi-sms` | Full tools including terminal |
| Email | `kopi-email` | Full tools including terminal |
| Home Assistant | `kopi-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `kopi-mattermost` | Full tools including terminal |
| Matrix | `kopi-matrix` | Full tools including terminal |
| DingTalk | `kopi-dingtalk` | Full tools including terminal |
| Feishu/Lark | `kopi-feishu` | Full tools including terminal |
| WeCom | `kopi-wecom` | Full tools including terminal |
| WeCom Callback | `kopi-wecom-callback` | Full tools including terminal |
| Weixin | `kopi-weixin` | Full tools including terminal |
| BlueBubbles | `kopi-bluebubbles` | Full tools including terminal |
| QQBot | `kopi-qqbot` | Full tools including terminal |
| Yuanbao | `kopi-yuanbao` | Full tools including terminal |
| Microsoft Teams | `kopi-teams` | Full tools including terminal |
| API Server | `kopi-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `kopi-webhook` | Full tools including terminal |
| Raft | `kopi-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.kopi/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
