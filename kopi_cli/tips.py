...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or kopi -c.",
    ...
    # --- CLI Flags ---
    "kopi -c resumes your most recent CLI session. kopi -c \"project name\" resumes by title.",
    "kopi -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "kopi -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "kopi chat -t web,terminal enables only specific toolsets for a focused session.",
    "kopi chat -s github-pr-workflow preloads a skill at launch.",
    "kopi chat -q \"query\" runs a single non-interactive query and exits.",
    "kopi chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "kopi chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "kopi --yolo bypasses all dangerous command approval prompts for the entire session.",
    "kopi chat --source telegram tags the session for filtering in kopi sessions list.",
    "kopi -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "kopi doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "kopi dump outputs a compact setup summary — great for bug reports.",
    "kopi config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "kopi config edit opens config.yaml in your default editor.",
    "kopi config check scans for missing or stale configuration options.",
    "kopi sessions browse opens an interactive session picker with search.",
    "kopi sessions stats shows session counts by platform and database size.",
    "kopi sessions prune --older-than 30 cleans up old sessions.",
    "kopi skills search react --source skills-sh searches the skills.sh public directory.",
    "kopi skills check scans installed hub skills for upstream updates.",
    "kopi skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "kopi skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "kopi mcp add github --command npx adds MCP servers from the command line.",
    "kopi mcp serve runs Hermes itself as an MCP server for other agents.",
    "kopi auth add lets you add multiple API keys for credential pool rotation.",
    "kopi completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "kopi logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "kopi backup creates a zip backup of your entire Hermes home directory.",
    "kopi profile create coder creates an isolated profile that becomes its own command.",
    "kopi profile create work --clone copies your current config and keys to a new profile.",
    "kopi update syncs new bundled skills to ALL profiles automatically.",
    "kopi gateway install sets up Hermes as a system service (systemd/launchd).",
    "kopi memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "kopi webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: kopi tools disables unused tools, kopi skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "kopi models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as kopi -w).",
    ...
    "KOPI_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "Profile names become shell commands — 'kopi profile create coder' creates the 'coder' command.",
    "kopi profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "kopi sessions export backup.jsonl exports all sessions for backup or analysis.",
    "kopi -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "kopi skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .kopi/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: kopi cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "kopi gateway install sets it up as a system service that starts on boot.",
    ...
    "Context files (.kopi.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Hermes loads project context from .kopi.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "kopi mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "kopi mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "kopi mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — kopi tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.kopi/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "kopi chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "kopi plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "kopi acp runs Hermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "KOPI_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "kopi auth add nous or kopi auth add openai-codex sets up OAuth-based providers.",
    ...
    "kopi status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.kopi/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "The skills audit log at ~/.kopi/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Hermes state via KOPI_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "KOPI_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in KOPI_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.kopi/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'kopi profile create ls' would be rejected.",
    "kopi profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "kopi claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "KOPI_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via kopi tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.kopi/.env.",
    "Skills from trusted repos (Kopi Ai Agent Pte Ltd) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.kopi/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads ~/.kopi/.env into the running session — pick up new API keys without restarting.',
    ...
    "KOPI_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.kopi/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.kopi/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'kopi curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "kopi curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'kopi curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'kopi auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'KOPI_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "KOPI_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'kopi dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'kopi dashboard embeds the full Hermes TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.kopi/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.kopi/dashboard-plugins/ — no npm build required.',
    ...
    'KOPI_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'KOPI_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'KOPI_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'KOPI_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.kopi/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.kopi/skills/ so drop-in skills appear without restarting the session.',
    ...
    'kopi -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'kopi chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'kopi chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'kopi chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "kopi chat --source tool tags programmatic chats so they don't clutter kopi sessions list.",
    'kopi dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'kopi sessions rename <ID> "new title" renames any past session; kopi sessions delete <ID> removes one.',
    'kopi import restores a session export or profile archive produced by sessions export or profile export.',
    'kopi fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'kopi pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'kopi setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'kopi status --deep runs the full health sweep across every component; plain kopi status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'KOPI_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'KOPI_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.kopi/plugins/ — trust-gated by design.',
    "KOPI_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'KOPI_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'KOPI_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'KOPI_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'KOPI_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'KOPI_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'KOPI_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'KOPI_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'KOPI_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'KOPI_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'KOPI_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    '--source tool chats are excluded from kopi sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.kopi/dashboard-plugins/.',
]
