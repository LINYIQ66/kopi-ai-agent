---
sidebar_position: 1
title: "通过 Nous Portal 运行 KOPI AI AGENT"
...
- 已安装 KOPI AI AGENT（[快速入门](/getting-started/quickstart)）
...
3. 在 `~/.kopi/config.yaml` 中设置 `model.provider: nous`
...
kopi setup --portal                              # 在远程机器上执行，在本地浏览器中打开打印出的 URL

# 方案 B：设备码登录（适用于 Cloud Shell、Codespaces、EC2 Instance Connect）
kopi auth add nous --type oauth
# 然后重新运行 `kopi setup --portal` 以连接 provider + gateway
```
...
`kopi setup --portal` 会在设置过程中让你选择模型，但订阅的意义在于可以访问完整的模型目录——随时可在会话中使用 `/model` 切换：
...
```bash
kopi cron add "Daily AI news summary" "every day at 9am" \
...
对于多人共用一台机器的团队场景，每个人有自己的 Portal 账号 → 每个 home 目录保存各自的 `~/.kopi/auth.json` → 用户之间不共享 token。这是正确的边界划分。

## 故障排查

### 运行 `kopi setup --portal` 后，`kopi portal info` 显示"not logged in"
...
如果某个模型确实不可用，请[提交 issue](https://github.com/LINYIQ66/kopi-ai-agent/issues)——大多数缺失是我们可以更新的路由配置问题。

### 账单未出现在我的 Portal 账号中

`kopi portal info` 会告诉你是否真的在通过 Portal 路由，还是使用了其他 provider。常见原因：

- `model.provider` 设置为 `openrouter`/`anthropic`/等，而非 `nous`
- OAuth refresh 失败后回退到了其他已配置的 provider
- 存在多个 Hermes profiles，你使用的是错误的那个（检查 `kopi profile list`）

### 想要撤销并重新开始

```bash
kopi auth logout nous       # 清除本地 refresh token
