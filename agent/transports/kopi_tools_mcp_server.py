...
MCP server (per `~/.codex/config.toml [mcp_servers.kopi-tools]`) and
...
Scope (what we expose):
  ...
  - kanban_* (complete/block/comment/    — kanban worker + orchestrator
    ...
    unblock/link)                          write ~/.kopi/kanban.db)
...
Run with: python -m agent.transports.kopi_tools_mcp_server
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    "skills_list",
    "text_to_speech",
    # Kanban worker handoff tools — gated on KOPI_KANBAN_TASK env var
    # (set by the kanban dispatcher when spawning a worker). Without these
    # in the callback, a worker spawned with openai_runtime=codex_app_server
    # could do the work but couldn't report completion back to the kernel,
    # making it hang until timeout. Stateless dispatch — they just read
    # the env var and write to ~/.kopi/kanban.db.
    "kanban_complete",
    "kanban_block",
    "kanban_comment",
    "kanban_heartbeat",
    ...
    # only — the kanban tool gates them on KOPI_KANBAN_TASK being unset.
...
def _build_server() -> Any:
    ...
    try:
        ...
        raise ImportError(
            f"kopi-tools MCP server requires the 'mcp' package: {exc}"
        ) from exc
    ...
    mcp = FastMCP(
        "kopi-tools",
        instructions=(
            "KOPI AI AGENT's tool surface, exposed for use inside a Codex "
    ...
    logger.info(
        "kopi-tools MCP server registered %d/%d tools",
        exposed_count,
        len(EXPOSED_TOOLS),
    )
    return mcp
...
def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for `python -m agent.transports.kopi_tools_mcp_server`."""
    ...
    os.environ.setdefault("KOPI_QUIET", "1")
    os.environ.setdefault("KOPI_REDACT_SECRETS", "true")

    try:
        server = _build_server()
    except ImportError as exc:
        sys.stderr.write(f"kopi-tools MCP server cannot start: {exc}\n")
        return 2
    ...
    try:
        ...
        logger.exception("kopi-tools MCP server crashed")
        sys.stderr.write(f"kopi-tools MCP server error: {exc}\n")
        return 1
    return 0
