"""On-demand supply-chain audit for KOPI AI AGENT installs.
...
2. Python deps declared by user-installed plugins under ``~/.kopi/plugins``
...
def _discover_plugins(kopi_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.kopi/plugins``.
...
def run_audit(
    *,
    ...
    home = kopi_home or Path(get_kopi_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, kopi_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(kopi_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `kopi security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, kopi_home=home
    )
    if total == 0:
