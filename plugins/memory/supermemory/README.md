...
- Supermemory API key from [app.supermemory.ai/integrations?connect=kopi](http://app.supermemory.ai/integrations?connect=kopi)
...
Config file: `$KOPI_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `kopi` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `kopi-{identity}` → `kopi-coder`). |
...
```json
{
  "container_tag": "kopi-{identity}"
}
```

For a profile named `coder`, this resolves to `kopi-coder`. The default profile resolves to `kopi-default`. Without `{identity}`, all profiles share the same container.
