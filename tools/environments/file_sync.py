...
_FORCE_SYNC_ENV = "KOPI_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.kopi") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.kopi to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``KOPI_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, kopi_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.kopi/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="kopi-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.kopi/skills/a.md`` →
        ``~/.kopi/skills/a.md``, a new remote file at
        ``/root/.kopi/skills/b.md`` maps to ``~/.kopi/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
