...
def _write_through_provider_state_to_global_root(
    ...
    ``kopi_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``kopi model`` / ``kopi auth`` which
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``kopi`` invocation that defaults to the active provider
        (e.g. setup wizard, ``kopi auth status``) would land on
        ...
        try:
            with _auth_store_lock():
                ...
                # kopi_cli.auth._save_xai_oauth_tokens (#43589); the pool
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        if self.provider == "openai-codex":
            refresh_timeout_seconds = auth_mod.env_float(
                "KOPI_CODEX_REFRESH_TIMEOUT_SECONDS", 20
            )
            lock_timeout = max(
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("kopi_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("kopi_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # re-authed via `kopi model` / `kopi auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `kopi model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``kopi auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `kopi auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `kopi auth remove <provider> <N>` is stable across all source types.
    try:
        from kopi_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        # API-key vs OAuth is a user-visible choice at `kopi setup` ("Claude
        ...
        # Claude Code CLI, kopi_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.kopi/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `kopi setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"kopi_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_kopi_oauth_credentials

        for source_name, creds in (
            ("kopi_pkce", read_kopi_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `kopi auth add nous --label <name>`).  Fall back to the
        ...
        try:
            from kopi_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.kopi/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `kopi auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `kopi auth remove openai-codex` marks
        ...
        # When the user logs in via ``kopi model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``kopi auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from kopi_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.kopi/.env over os.environ — the user's config file is the
    ...
    def _get_env_prefer_dotenv(key: str) -> str:
        ...
        # load_kopi_dotenv() -> apply_onepassword_secrets()).  The raw
    ...
    # Honour user suppression — `kopi auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.kopi/.env.
    ...
    try:
        from kopi_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from kopi_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.kopi/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `kopi auth` command that confirmed the source is gone).
