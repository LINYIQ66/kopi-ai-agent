...
sidebar_position: 1
title: "CLI Interface"
description: "Master the KOPI AI AGENT terminal interface — commands, keybindings, personalities, and more"
---
...
:::tip
Hermes also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `kopi --tui` — see the [TUI](tui.md) guide.
:::
...
kopi chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
kopi --continue             # Resume the most recent CLI session (-c)
kopi --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
kopi chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
kopi -w                         # Interactive mode in worktree
kopi -w -z "Fix issue #123"     # Single query in worktree
```
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `KOPI_YOLO_MODE` is on (either `kopi --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`kopi -c` or `kopi --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
```bash
kopi -s kopi-ai-agent-dev,github-auth
kopi chat -s github-pr-workflow -s github-auth
```
...
Every installed skill in `~/.kopi/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.kopi/config.yaml`:
...
```
KOPI AI AGENT has been suspended. Run `fg` to bring KOPI AI AGENT back.
```
...
Resume this session with:
  kopi --resume 20260225_143052_a1b2c3
...
```bash
kopi --continue                          # Resume the most recent CLI session
kopi -c                                  # Short form
kopi -c "my project"                     # Resume a named session (latest in lineage)
kopi --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
kopi --resume "refactoring auth"         # Resume by title
kopi -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `kopi sessions rename <id> <title>` from the command line. Use `kopi sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Hermes's SQLite state database under `~/.kopi/state.db`. The database keeps:
