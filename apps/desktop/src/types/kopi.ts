...
export interface EnvVarInfo {
  ...
  // in kopi_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `kopi model` uses — instead of
...
export interface ModelOptionProvider {
   ...
   *  the `kopi model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors kopi_cli/projects_db.Project.to_dict().
...
export interface ToolProvider {
   ...
   *  CLI `kopi tools` active-provider detection). */
  is_active: boolean
}
...
/** Shape of `GET /api/kopi/update/check` — the backend's own update state.
