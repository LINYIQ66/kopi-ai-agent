...
prefers Hermes-managed xAI Grok OAuth (via ``kopi auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.kopi/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``kopi tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `kopi auth` to sign in with "
