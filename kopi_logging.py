...
``~/.kopi/logs/`` (profile-aware via ``get_kopi_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``kopi_cli.web_server``,
    ``kopi_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from kopi_constants import get_config_path, get_kopi_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_kopi_session_injector", False):
    ...
    _session_record_factory._kopi_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``kopi logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``kopi logs --component gateway``.
    "gateway": ("gateway", "kopi_plugins", "plugins.platforms"),
...
def setup_logging(
    ...
    *force* is ``True``.
    ...
    home = kopi_home or get_kopi_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_kopi_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._kopi_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the kopi group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from kopi_cli.config import is_managed
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._kopi_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="kopi-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_kopi_queue", False):
