...
def _reset_backend():
    ...
    with patch.dict(os.environ, {"KOPI_COMPUTER_USE_BACKEND": "noop"}, clear=False):
        yield
    reset_backend_for_tests()
...
class TestClickButtonPassthrough:
    """Surface 5 (Kopi Ai Agent Pte Ltd/kopi-ai-agent#47072) — `middle_click` must
...
class TestImageMimeTypePropagation:
    """Surface 7 (Kopi Ai Agent Pte Ltd/kopi-ai-agent#47072): trycua/cua#1961 made
...
class TestMcpInvocationResolution:
    """Surface 8 (Kopi Ai Agent Pte Ltd/kopi-ai-agent#47072): instead of hardcoding
    ...
    def test_falls_back_when_manifest_missing_command(self):
        ...
        resolved driver path (so KOPI_CUA_DRIVER_CMD still wins)."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
...
class TestStructuredElementsConsumption:
    """Surface 2 (Kopi Ai Agent Pte Ltd/kopi-ai-agent#47072): trycua/cua#1961 made
...
class TestCapabilityDiscovery:
    """Surface 4 (Kopi Ai Agent Pte Ltd/kopi-ai-agent#47072): the wrapper learns
...
class TestElementTokenAttachment:
    """Surface 6 (Kopi Ai Agent Pte Ltd/kopi-ai-agent#47072): trycua/cua#1961 added
...
class TestSessionLifecycle:
    ...
    def test_session_id_format(self):
        from tools.computer_use.cua_backend import CuaDriverBackend
        backend = CuaDriverBackend()
        # kopi-{12 hex chars} — short enough to surface in logs
        # without being a privacy hazard, unique enough for concurrent runs.
        assert backend._session_id.startswith("kopi-")
