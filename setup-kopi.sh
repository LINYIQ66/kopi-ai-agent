...
# 5. Symlinks the 'kopi' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ KOPI AI AGENT Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/kopi-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink kopi into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up kopi command..."

KOPI_BIN="$SCRIPT_DIR/venv/bin/kopi"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$KOPI_BIN" "$COMMAND_LINK_DIR/kopi"
echo -e "${GREEN}✓${NC} Symlinked kopi → $COMMAND_LINK_DISPLAY_DIR/kopi"
...
# ============================================================================
# Seed bundled skills into ~/.kopi/skills/
# ============================================================================

KOPI_SKILLS_DIR="${KOPI_HOME:-$HOME/.kopi}/skills"
mkdir -p "$KOPI_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.kopi/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$KOPI_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  kopi status        # Check configuration"
if is_termux; then
    echo "  kopi gateway       # Run gateway in foreground"
else
    echo "  kopi gateway install # Install gateway service (messaging + cron)"
fi
echo "  kopi cron list     # View scheduled jobs"
echo "  kopi doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m kopi_cli.main setup
fi
