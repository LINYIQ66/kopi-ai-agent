...
def _has_kopi_agent_browser() -> bool:
    ...
    # install.sh installs globally into $KOPI_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-HermesHome", str(get_kopi_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = kopi_subprocess_env(inherit_credentials=False)
