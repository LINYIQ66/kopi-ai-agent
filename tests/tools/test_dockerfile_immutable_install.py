"""Contract tests for the Docker image's immutable /opt/kopi install tree."""
...
def test_dockerfile_makes_opt_kopi_readonly_for_kopi_user() -> None:
    ...
    assert "chown -R root:root /opt/kopi" not in text
    assert "chmod -R a+rX /opt/kopi" not in text
...
def test_dockerfile_does_not_chown_install_trees_to_hermes() -> None:
    ...
    forbidden_patterns = (
        ...
        r"chown\s+-R\s+kopi:kopi\s+/opt/kopi/ui-tui",
        r"chown\s+-R\s+kopi:kopi\s+/opt/kopi/gateway",
        r"chown\s+-R\s+kopi:kopi\s+/opt/kopi/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/kopi must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/kopi/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $KOPI_HOME data volume (which a host install may also use).
    The stamp is created by root in the shim-wiring RUN block; the kopi
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/kopi tree.
    assert f"ENV KOPI_LAZY_INSTALL_TARGET={target}" in text
    assert target.startswith("/opt/data/"), "target must be on the durable volume"
    assert "ENV KOPI_LAZY_INSTALL_TARGET=/opt/kopi" not in text
    ...
    # succeed as the unprivileged kopi runtime user.
    stage2 = (REPO_ROOT / "docker" / "stage2-hook.sh").read_text()
    assert '"$KOPI_HOME/lazy-packages"' in stage2, (
