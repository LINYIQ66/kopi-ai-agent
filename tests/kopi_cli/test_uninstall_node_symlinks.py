"""Tests for kopi_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $KOPI_HOME/node and prepends ~/.local/bin to
...
def _make_kopi_node(kopi_home: Path) -> Path:
    """Create a fake $KOPI_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = kopi_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_kopi_node(fake_home):
    kopi_home = fake_home / ".kopi"
    node_bin = _make_kopi_node(kopi_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(kopi_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(kopi_home) == []


def test_removes_dangling_symlink_into_kopi_node(fake_home):
