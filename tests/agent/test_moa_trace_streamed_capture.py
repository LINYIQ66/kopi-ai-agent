...
path via ``kopi chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp KOPI_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point KOPI_HOME at a temp dir and turn moa.save_traces on."""
    kopi_home = tmp_path / ".kopi"
    kopi_home.mkdir()
    monkeypatch.setenv("KOPI_HOME", str(kopi_home))

    # save_moa_turn reads config via kopi_cli.config.load_config; stub it to
