...
def gateway_home(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("KOPI_PREFILL_MESSAGES_FILE", raising=False)
    monkeypatch.delenv("KOPI_EPHEMERAL_SYSTEM_PROMPT", raising=False)
    monkeypatch.delenv("KOPI_GATEWAY_BUSY_INPUT_MODE", raising=False)
    monkeypatch.delenv("KOPI_RESTART_DRAIN_TIMEOUT", raising=False)
    monkeypatch.delenv("KOPI_BACKGROUND_NOTIFICATIONS", raising=False)
    return tmp_path
