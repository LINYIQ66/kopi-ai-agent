...
(`ai.kopi.gateway-<name>.plist`) or systemd user service
(`kopi-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
kopi profile create coder
kopi profile create personal-bot
kopi profile create research
...
```bash
kopi config set gateway.multiplex_profiles true
kopi gateway restart
```

Equivalently, in the default profile's `~/.kopi/config.yaml`:
...
You do **not** run `kopi gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `kopi gateway start` / `run` is a
...
home). `kopi status` reports the multiplexer and the profiles it serves;
`kopi status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/kopi-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: kopi-gateways {start|stop|restart|status|list}"
}
...
```bash
kopi-gateways start      # start every configured profile
kopi-gateways stop       # stop every configured profile
kopi-gateways restart    # restart all
kopi-gateways status     # status across all
kopi-gateways list       # delegates to `kopi gateway list`
```

:::tip
The `default` profile is targeted with `kopi gateway <action>` (no `-p`),
not `kopi -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `kopi -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.kopi.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/kopi-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.kopi.gateway.plist` /
`kopi-gateway.service`.
...
```bash
# Default profile
tail -f ~/.kopi/logs/gateway.log
tail -f ~/.kopi/logs/gateway.error.log

# Named profile
tail -f ~/.kopi/profiles/<name>/logs/gateway.log
tail -f ~/.kopi/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.kopi/logs/gateway.log ~/.kopi/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
kopi logs -f                  # follow default profile
kopi -p coder logs -f         # follow one profile
kopi logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
kopi profile list             # profiles + model + gateway state
kopi-gateways status          # full status across every profile
launchctl list | grep kopi    # macOS — PIDs and labels
systemctl --user list-units 'kopi-gateway-*'   # Linux — units
```
...
The default profile uses `~/.kopi/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
kopi config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.kopi/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=kopi --why="gateways running" \
  sleep infinity &
...
`kopi-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.kopi/.env ~/.kopi/profiles/*/.env
```

## Updating the code

`kopi update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `kopi gateway start` after a previous `kopi gateway stop`. The
...
```bash
ps -ef | grep "kopi_cli.*-p <profile>"
cat ~/.kopi/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.kopi.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.kopi.gateway-<profile>.plist

# Linux
systemctl --user restart kopi-gateway-<profile>.service
```

### Health check

```bash
kopi doctor                  # default profile
kopi -p <profile> doctor     # one profile
```
