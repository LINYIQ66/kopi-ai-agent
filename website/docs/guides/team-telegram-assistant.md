...
This tutorial walks you through setting up a Telegram bot powered by KOPI AI AGENT that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **KOPI AI AGENT installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.kopi/.env`
...
2. **Send `/newbot`** — BotFather will ask you two things:
   ...
   - **Username** — must end in `bot` (e.g., `myteam_kopi_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by KOPI AI AGENT. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u kopi-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `kopi gateway install` to update the plist.
:::
...
```bash
kopi gateway stop && kopi gateway start
```
...
kopi pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.kopi/.env`:
...
Control how much detail the bot shows when using tools. In `~/.kopi/config.yaml`:
...
Customize how the bot communicates by editing `~/.kopi/SOUL.md`:
...
```bash
# From the CLI
kopi cron list          # View all scheduled jobs
kopi cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u kopi-gateway` (Linux) or `~/.kopi/logs/gateway.log` (macOS) |
| Cron job output | `~/.kopi/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.kopi/cron/jobs.json` |
| Pairing data | `~/.kopi/pairing/` |
| Session history | `~/.kopi/sessions/` |
