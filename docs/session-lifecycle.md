...
The canonical transcript store is SQLite via `SessionDB` (from `kopi_state`). The
...
Counts consecutive restarts via a JSON file (`{KOPI_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `kopi update`, `kopi gateway restart`,
or `/restart`.
