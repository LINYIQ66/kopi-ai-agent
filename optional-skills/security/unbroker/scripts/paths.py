...
All per-subject data lives under PDD_DATA_DIR (default: $KOPI_HOME/unbroker),
...
def kopi_home() -> Path:
    return Path(os.environ.get("KOPI_HOME") or (Path.home() / ".kopi"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else kopi_home() / "unbroker"
