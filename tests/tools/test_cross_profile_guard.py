...
def fake_hermes(tmp_path, monkeypatch):
    """Build a two-profile Hermes layout and point KOPI_HOME at
    the kopi-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setenv("KOPI_HOME", str(sec_home))

    import kopi_constants
    monkeypatch.setattr(kopi_constants, "get_default_kopi_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_kopi_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_hermes):
        ...
        assert "kopi-security" in result["error"]
        # File untouched.
    ...
    def test_non_kopi_path_unaffected(self, fake_hermes, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        # Re-import the module so SKILLS_DIR picks up KOPI_HOME (set in
        ...
        assert "not found in active profile 'kopi-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.kopi/profiles/<name>/."""
        # Don't set KOPI_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_kopi_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_kopi_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_hermes):
        """When active profile is 'kopi-security', the prompt warns
