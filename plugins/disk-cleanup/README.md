...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `KOPI_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `KOPI_HOME` or `/tmp/kopi-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$KOPI_HOME/disk-cleanup/` is itself excluded
- `$KOPI_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
