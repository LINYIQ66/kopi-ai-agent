"""CLI entry point for the kopi-ai-agent ACP adapter.

Loads environment variables from ``~/.kopi/.env``, configures logging
...
try:
    ...
    # shadowing Hermes's own modules — ``kopi acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    kopi_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from KOPI_HOME (default ``~/.kopi``)."""
    from kopi_cli.env_loader import load_kopi_dotenv

    kopi_home = get_kopi_home()
    loaded = load_kopi_dotenv(kopi_home=kopi_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", kopi_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="kopi-acp",
        description="Run KOPI AI AGENT as an ACP stdio server.",
    )
    ...
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.kopi/node/ "
...
def _run_setup() -> None:
    from kopi_cli.main import main as kopi_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "kopi", "model"]
        kopi_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``kopi postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting kopi-ai-agent ACP adapter")
