...
the SAME provider universe as ``kopi model`` (the CLI/TUI picker).
...
def test_every_kopi_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `kopi model` universe."""
    ...
    assert not missing, (
        "providers shown in `kopi model` but not configurable in the desktop "
