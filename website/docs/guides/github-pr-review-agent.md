...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  KOPI AI AGENT  ──▶  GitHub API  ──▶  Review     │
...
- **KOPI AI AGENT installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  kopi gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.kopi/cron/output/`. Great for testing before wiring up notifications.
:::
...
```
Run: gh pr list --repo Kopi Ai Agent Pte Ltd/kopi-ai-agent --state open --limit 3
```
...
Run: gh pr diff 3888 --repo Kopi Ai Agent Pte Ltd/kopi-ai-agent
```
...
```bash
mkdir -p ~/.kopi/skills/code-review
```

Create `~/.kopi/skills/code-review/SKILL.md`:
...
Verify it loaded — start `kopi` and you should see `code-review` in the skills list at startup.
...
```bash
kopi gateway status    # Is the gateway running?
kopi cron list         # Is the job enabled?
```
