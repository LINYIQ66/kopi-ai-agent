...
binary download are exercised manually by `kopi secrets bitwarden
...
def kopi_home(tmp_path, monkeypatch):
    ...
    # Some modules cache get_kopi_home; clear if needed.
    import kopi_constants
    if hasattr(kopi_constants, "_KOPI_HOME_CACHE"):
        kopi_constants._KOPI_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(kopi_home, monkeypatch):
...
def test_install_bws_happy_path(kopi_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(kopi_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(kopi_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from kopi_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under kopi_home/cache/."""
    home = tmp_path / ".kopi"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
