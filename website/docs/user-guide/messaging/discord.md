...
sidebar_position: 3
title: "Discord"
description: "Set up KOPI AI AGENT as a Discord bot"
---

# Discord Setup

KOPI AI AGENT integrates with Discord as a bot, letting you chat with your AI assistant through direct messages or server channels. The bot receives your messages, processes them through the KOPI AI AGENT pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, voice messages, file attachments, and slash commands.
...
3. Enter a name for your application (e.g., "KOPI AI AGENT") and accept the Developer Terms of Service.
4. Click **Create**.
...
The bot token is the credential KOPI AI AGENT uses to log in as your bot. Still on the **Bot** page:
...
KOPI AI AGENT uses your Discord User ID to control who can interact with the bot. To find it:
...
## Step 8: Configure KOPI AI AGENT
...
Discord behavior is controlled through two files: **`~/.kopi/.env`** for credentials and env-level toggles, and **`~/.kopi/config.yaml`** for structured settings. Environment variables always take precedence over config.yaml values when both are set.
...
| `KOPI_DISCORD_TEXT_BATCH_DELAY_SECONDS` | No | `0.6` | Grace window the adapter waits before flushing a queued text chunk. Useful for smoothing streamed output. |
| `KOPI_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS` | No | `2.0` | Delay between split chunks when a single message exceeds Discord's length limit. |
...
The `discord` section in `~/.kopi/config.yaml` mirrors the env vars above. Config.yaml settings are applied as defaults — if the equivalent env var is already set, the env var wins.
...
No extra configuration is needed — any skill installed via `kopi skills install` is automatically registered as a Discord slash command on the next gateway restart.
...
Any file type a user uploads is accepted. Authorization to message the agent is the gate — not the file extension. Every upload is downloaded, cached under `~/.kopi/cache/documents/`, and surfaced to the agent as a `DOCUMENT`-typed message event so it can inspect the file with `terminal` (`ffprobe`, `unzip`, `file`, `strings`, etc.) or `read_file`.
...
KOPI AI AGENT supports Discord voice messages:
...
If the gateway log says Discord is connected and REST API checks work, but every inbound message is silent, look for this warning in `~/.kopi/logs/gateway.log`:
...
**Fix**: Check that `kopi gateway` is running. Verify `DISCORD_BOT_TOKEN` in your `.env` file. If you recently reset the token, update it.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `DISCORD_ALLOWED_USERS`.

**Fix**: Add your User ID to `DISCORD_ALLOWED_USERS` in `~/.kopi/.env` and restart the gateway.
...
**Fix**: Set this in `~/.kopi/config.yaml` and restart the gateway:
...
```bash
# ~/.kopi/.env — works alongside or instead of DISCORD_ALLOWED_USERS
...
```bash
# ~/.kopi/.env — env vars win over config.yaml
