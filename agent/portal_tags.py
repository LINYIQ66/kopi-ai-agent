...
Nous can attribute usage to KOPI AI AGENT and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=kopi-ai-agent",
        "client=kopi-client-v<__version__>",
    ]

The version is sourced live from ``kopi_cli.__version__`` so it auto-aligns
...
``kopi_cli.__version__`` is the canonical source of truth.
"""
...
def _kopi_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``kopi_cli`` cannot be imported (should
...
def kopi_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=kopi-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=kopi-client-v{_kopi_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=kopi-ai-agent", kopi_client_tag()]
