...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated KOPI_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="kopi_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".kopi"))
    prev_home = os.environ.get("KOPI_HOME")
    os.environ["KOPI_HOME"] = os.path.join(test_home, ".kopi")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("kopi_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["KOPI_HOME"] = prev_home
