...
``os.environ["KOPI_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``kopi --yolo`` (process-start env), ``KOPI_YOLO_MODE=1``,
and ``kopi config set approvals.mode off`` actually bypassed.
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_does_not_mutate_env_var(self):
        """Toggling /yolo must not write ``KOPI_YOLO_MODE`` — that path is
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``kopi --yolo`` sets ``KOPI_YOLO_MODE`` before tool imports, so
