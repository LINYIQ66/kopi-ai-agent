...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("KOPI_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning(
                "Ignoring invalid KOPI_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.kopi/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("KOPI_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("KOPI_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = env_float(
            "KOPI_DISCORD_LIVENESS_INTERVAL_SECONDS", 60.0
        )
        self._liveness_failure_threshold = env_int(
            "KOPI_DISCORD_LIVENESS_FAILURE_THRESHOLD", 3
        )
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``KOPI_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("KOPI_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_kopi_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.kopi/skills/ for new or removed skills")
        ...
        # kopi_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from kopi_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from kopi_cli.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from kopi_cli.commands import discord_skill_commands_by_category
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        for attempt in range(2):
            try:
                ...
                try:
                    setattr(thread, "_kopi_auto_thread_initial_name", thread_name)
                except Exception:
                    pass
                return thread
            except Exception as direct_error:
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``kopi update --gateway``
    ...
    async def _handle_message(self, message: DiscordMessage, role_authorized: bool = False) -> None:
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_kopi_auto_thread_initial_name", None)
...
def _component_check_auth(
    ...
    the pairing store (``kopi pairing approve``).
    ...
    # so users approved via ``kopi pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``kopi update`` prompts.
...
# is not in this process (e.g. ``kopi cron`` running standalone) and no live
...
def interactive_setup() -> None:
    ...
    from kopi_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    if lis is not None and not os.getenv("KOPI_DISCORD_LIVENESS_INTERVAL_SECONDS"):
        os.environ["KOPI_DISCORD_LIVENESS_INTERVAL_SECONDS"] = str(lis)
    lft = discord_cfg.get("liveness_failure_threshold")
    if lft is not None and not os.getenv("KOPI_DISCORD_LIVENESS_FAILURE_THRESHOLD"):
        os.environ["KOPI_DISCORD_LIVENESS_FAILURE_THRESHOLD"] = str(lft)
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``kopi_cli.gateway.get_env_value`` at call time (not via
    ...
    """
    import kopi_cli.gateway as gateway_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'kopi-ai-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # kopi_cli/setup.py::_setup_discord function.  Same shape as Teams.
