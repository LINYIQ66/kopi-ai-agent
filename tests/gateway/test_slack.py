...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.kopi."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /kopi, /btw, /stop,
        ...
        for expected in ("/kopi", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@kopi`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@kopi"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@kopi")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /kopi subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /kopi btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_kopi_prefix_still_works(self, adapter):
        """Backward compat: /kopi btw foo must still route to /btw foo.

        Old workspace manifests only declared /kopi as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_kopi_freeform_question(self, adapter):
        """/kopi <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/kopi",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_kopi_slash_stashes_context(self, adapter):
        """Legacy /kopi <subcommand> also stashes context."""
        command = {
            "command": "/kopi",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_kopi_question_does_not_stash_context(self, adapter):
        """Free-form /kopi <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/kopi",
