(function () {
  "use strict";
  // kopi-achievements dashboard plugin
  // Originally authored by @PCinkusz — https://github.com/PCinkusz/kopi-achievements (MIT).
  // Bundled into kopi-ai-agent. Upstream repo remains the staging ground for new
  ...
  if (!SDK || !window.__KOPI_PLUGINS__) return;
  ...
  function api(path, options) {
    ...
    // gated OAuth (kopi_session_at cookie via credentials:'include').
    // Hand-rolling fetch + reading window.__KOPI_SESSION_TOKEN__ directly
    ...
    const url = "/api/plugins/kopi-achievements" + path;
  ...
  async function buildShareImage(achievement) {
    ...
    ctx.fillText("HERMES AGENT  ·  kopi-ai-agent.nousresearch.com", 70, H - 40);
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    function download() {
      ...
      a.href = url;
      a.download = "kopi-achievement-" + (achievement.id || "badge") + ".png";
    ...
    function tweetText() {
      ...
      const tmpl = tx(t, "share.tweet_text", "Just unlocked {tier_part}\"{name}\" in KOPI AI AGENT ☤", {
        tier_part: tierPart,
  ...
  window.__KOPI_PLUGINS__.register("kopi-achievements", AchievementsPage);
})();
