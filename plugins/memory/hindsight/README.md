...
Daemon startup logs: `~/.kopi/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p kopi ui start
```
...
Config file: `~/.kopi/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `kopi` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `kopi-{profile}` isolates memory per active Hermes profile. Empty placeholders collapse cleanly (e.g. `kopi-{user}` with no user becomes `kopi`). |
...
The LLM API key is stored in `~/.kopi/.env` as `HINDSIGHT_LLM_API_KEY`.
