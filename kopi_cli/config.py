"""
Configuration management for KOPI AI AGENT.

Config files are stored in ~/.kopi/ for easy access:
- ~/.kopi/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.kopi/.env         - API keys and secrets

This module provides:
- kopi config          - Show current configuration
- kopi config edit     - Open config in editor
- kopi config set      - Set a specific value
- kopi config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``kopi config set`` (which rewrites
    ...
    ``config.yaml`` in place: kopi never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.kopi/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``kopi logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  kopi config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   on every plugin install / ``kopi update``.
...
# * ``KOPI_HOME`` / ``KOPI_PROFILE`` / ``KOPI_CONFIG`` /
#   ``KOPI_ENV`` — Hermes runtime location flags. Writing these into
...
#
# IMPORTANT: ``KOPI_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (KOPI_LANGFUSE_PUBLIC_KEY,
# KOPI_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # NOT a KOPI_* blanket: integration credentials (KOPI_GEMINI_*,
    # KOPI_LANGFUSE_*, KOPI_SPOTIFY_*, ...) ARE allowed.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "(KOPI_HOME, KOPI_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "KOPI_TOOL_PROGRESS", "KOPI_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `kopi plugins enable
    # observability/langfuse` or `kopi tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "KOPI_LANGFUSE_ENV",
    "KOPI_LANGFUSE_RELEASE",
    "KOPI_LANGFUSE_SAMPLE_RATE",
    "KOPI_LANGFUSE_MAX_CHARS",
    "KOPI_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("KOPI_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_kopi_home() / ".managed"
...
def is_managed() -> bool:
    ...
    Two signals: the KOPI_MANAGED env var (set by the systemd service),
    or a .managed marker file in KOPI_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade kopi-ai-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``kopi_cli/`` — i.e. the git checkout for source
    installs, ``/opt/kopi`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$KOPI_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    2. Legacy home-scoped stamp ``$KOPI_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. KOPI_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.kopi``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$KOPI_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.kopi:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$KOPI_HOME``, a
    ...
    ``docker`` and ``kopi update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/kopi-ai-agent`` image bakes a ``docker``
        stamp into ``/opt/kopi`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $KOPI_HOME.
    try:
    ...
    #    host install whose shared $KOPI_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``kopi update``.
    try:
        method = (
            (get_kopi_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``kopi_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$KOPI_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/kopi`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    ...
    ``uv tool install kopi-ai-agent`` places the install at
    ``.../uv/tools/kopi-ai-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``kopi-ai-agent`` happens to be uv-tool-installed on the machine while
    the *active* Hermes is a regular pip/venv install, causing
    ``kopi update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/kopi-ai-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade kopi-ai-agent"
    if method == "docker":
        return "docker pull nousresearch/kopi-ai-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade kopi-ai-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade kopi-ai-agent"
        return "pip install --upgrade kopi-ai-agent"
    return "kopi update"
...
# banner, the TUI/desktop session info panel, and ``kopi update``. NixOS
# stays fully supported (Tier 2) and must never hit this path.

PLATFORM_SUPPORT_DOCS_URL = "https://kopi-ai-agent.nousresearch.com/docs/getting-started/platform-support"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``kopi update`` / ``kopi update --check`` so the wording — and the
...
# Long-form text for ``kopi update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# kopi_cli/main.py; lives here so the wording stays consistent and we
...
✗ ``kopi update`` doesn't apply inside the Docker container.

KOPI AI AGENT runs as a published image (nousresearch/kopi-ai-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/kopi-ai-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate kopi-ai-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/kopi-ai-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/kopi-ai-agent/tags
  • Your config and session history live under ``$KOPI_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``kopi update`` inside Docker.
...
def format_managed_message(action: str = "modify this Hermes installation") -> str:
    ...
    raw = os.getenv("KOPI_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Hermes installation is managed by NixOS "
            f"(KOPI_MANAGED={env_hint}).\n"
            "Edit services.kopi-ai-agent.settings in your configuration.nix and run:\n"
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from KOPI_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, kopi_bin
    or None if container mode is not active, we're already inside the
    container, or KOPI_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("KOPI_DEV") == "1":
        return None
    ...
    container_mode_file = get_kopi_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "kopi-ai-agent")
    exec_user = info.get("exec_user", "kopi")
    kopi_bin = info.get("kopi_bin", "/data/current-package/bin/kopi")
...
# Re-export from kopi_constants — canonical definition lives there.
from kopi_constants import get_kopi_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_kopi_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_kopi_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_kopi_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the KOPI_UID / KOPI_GID env vars set by Docker deployments.
    ...
    ownership. The entrypoint chowns the top-level KOPI_HOME once, but
    subdirectories created at runtime by ``ensure_kopi_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("KOPI_UID", "").strip()
    gid_str = os.environ.get("KOPI_GID", "").strip()
    try:
...
def _chown_to_kopi_uid(path) -> None:
    """Chown ``path`` to ``KOPI_UID:KOPI_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_kopi_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_kopi_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the kopi group can
    share state with the gateway service.

    The mode can be overridden via the KOPI_HOME_MODE environment variable
    (e.g. KOPI_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse KOPI_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``KOPI_UID``/``KOPI_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("KOPI_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("KOPI_CONTAINER") or os.environ.get("KOPI_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set KOPI_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into KOPI_HOME, upgrading legacy empty templates.
...
def ensure_kopi_home():
    """Ensure ~/.kopi directory structure exists with secure permissions.
    ...
    """
    home = get_kopi_home()
    # Named profiles must be created explicitly (e.g. ``kopi profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old KOPI_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_kopi_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_kopi_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"KOPI_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # identity slot (SOUL.md). Empty by default. The KOPI_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        #             KOPI_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force KOPI_HOME/home when it exists (old strict
        ...
        #  "/home/user/.kopi/cache/documents:/output"]
        ...
        # bundled Hermes image, which drops to the `kopi` user via
    ...
    # Opt in via ``kopi chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.kopi/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: kopi sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .kopi.md, or .cursorrules before Hermes applies
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # to compact its own thread (default); kopi =
    ...
    # See tools/kanban_tools.py and kopi_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``kopi dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``kopi kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``kopi kanban decompose`` and the kanban
        ...
        # ``kopi profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `kopi model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `kopi` (and `kopi chat`) launches by default:
        ...
        # REPL and `--tui` (or KOPI_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `kopi --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `kopi -c` muscle memory.  Default off so existing
        # users aren't surprised.  KOPI_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `kopi --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to KOPI_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/kopi`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `kopi pets`. Disabled until
        ...
        "pet": {
            ...
            # get_kopi_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``KOPI_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``KOPI_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``KOPI_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``KOPI_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``KOPI_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/kopi``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.kopi/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.kopi/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.kopi/cache/delegation/
    ...
    "moa": {
        ...
        # <kopi_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.kopi/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${KOPI_SKILL_DIR} and ${KOPI_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `kopi curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `kopi curator run --consolidate` overrides
        ...
        # normally restored on every `kopi update`, so pruning them only
        ...
        # skipped), snapshot ~/.kopi/skills/ into
        # ~/.kopi/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `kopi curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for kopi-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *KOPI AI AGENT*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (KOPI_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.kopi/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or KOPI_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `kopi doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``kopi_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # $KOPI_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via KOPI_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `kopi -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `kopi` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `kopi kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     kopi-ai-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.kopi/logs/.
    ...
    # update model picker lists without shipping a kopi-ai-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://kopi-ai-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `kopi model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal KOPI_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.kopi/sessions/sessions.json entirely.
        ...
        # the KOPI_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.kopi.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.kopi/.env,
        ...
        # user. Bridged to KOPI_MEDIA_DELIVERY_STRICT.
        "strict": False,
        ...
        # cache (~/.kopi/cache/{documents,images,audio,video,screenshots})
        ...
        # to KOPI_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # KOPI_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to KOPI_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.kopi/state.db.
    ...
    "sessions": {
        ...
        # default of ``kopi sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.kopi/sessions/session_{sid}.json`` on every turn
    ...
    "updates": {
        # Run a full ``kopi backup``-style zip of KOPI_HOME before every
        # ``kopi update``.  Backups land in ``<KOPI_HOME>/backups/`` and
        # can be restored with ``kopi import <path>``.  Off by default:
        # zipping a large KOPI_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``kopi update
        ...
        "backup_keep": 5,
        # What `kopi update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<KOPI_HOME>/lsp/bin/`` on first use.
    ...
    # `kopi tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.kopi/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.kopi/.env (or your shell) and never in config.yaml.
            ...
            # ~/.kopi/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.kopi/.env (or the shell)
    ...
    # `kopi desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the KOPI_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'kopi model' for endpoint-specific config)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @kopi:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. KOPI_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #kopi)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: kopi-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'kopi-ai-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    # KOPI_TOOL_PROGRESS and KOPI_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.kopi.config`` entries, then checks
    ...
    try:
        ...
        # should never break `kopi update`.  Skill-config prompting is a
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``kopi_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'kopi setup' to create a valid config")]
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'kopi doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("KOPI_HOME", "~/.kopi")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "kopi update / kopi -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("KOPI_TOOL_PROGRESS")
            old_mode = get_env_value("KOPI_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from KOPI_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from KOPI_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("KOPI_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from KOPI_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$KOPI_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_kopi_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `kopi plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `kopi curator status` has no
    ...
    #   3. Creates `~/.kopi/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_kopi_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_kopi_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `kopi config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `kopi config set`"
                    )
    ...
    # behavior set it to true explicitly via `kopi config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from kopi_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from kopi_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: kopi config set <key> <value>")
    ...
    # `kopi update`, but only the version bump is persisted.
    ...
    # metadata.kopi.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_kopi_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``kopi config set``
    ...
    backends) — while display paths (``kopi status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _kopi_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.kopi/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.kopi/config.yaml.
    ...
    (which change ``KOPI_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/kopi/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_kopi_dotenv()
...
#   openai-codex (OAuth — kopi auth) — OpenAI Codex
#   nous         (OAuth — kopi auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.kopi/config.yaml.\n
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.kopi/.env.
    ...
    interactive menu render (`kopi tools`, `kopi setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `kopi tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.kopi/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.kopi/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.kopi/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.kopi/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.kopi/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.kopi/.env`` over ``os.environ``.
...
def show_config():
    ...
    # Warn on stale KOPI_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("KOPI_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale KOPI_MAX_ITERATIONS={_env_ghost} "
                f"(run 'kopi doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  kopi config edit     # Edit config file", Colors.DIM))
    print(color("  kopi config set <key> <value>", Colors.DIM))
    print(color("  kopi setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `kopi config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `kopi config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: kopi config set <key> <value>")
            print()
            print("Examples:")
            print("  kopi config set model anthropic/claude-sonnet-4")
            print("  kopi config set terminal.backend docker")
            print("  kopi config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'kopi config migrate' to add them")
        ...
        print("  kopi config           Show current configuration")
        print("  kopi config edit      Open config in editor")
        print("  kopi config set <key> <value>   Set a config value")
        print("  kopi config check     Check for missing/outdated config")
        print("  kopi config migrate   Update config with new options")
        print("  kopi config path      Show config file path")
        print("  kopi config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``kopi config`` UI so users
