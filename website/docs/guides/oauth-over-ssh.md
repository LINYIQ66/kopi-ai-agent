...
kopi auth add spotify --no-browser
...
Remote MCP servers (Linear, Sentry, Atlassian, Asana, Figma, etc.) use the same loopback redirect flow. Hermes auto-picks a free port per server and prints the authorize URL when the OAuth flow kicks off — either at startup (when a new server appears in `mcp_servers:`) or when you run `kopi mcp login <server>`.
...
**Pitfall — the 30s config-reload race.** If you edit `~/.kopi/config.yaml` to add an OAuth MCP server from inside a running Hermes session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough time to complete an interactive OAuth flow, and the reload will give up. Use `kopi mcp login <server>` from a fresh terminal instead — it has no such cap and waits the full 5 min for you to paste back.
...
### Tokens land in the wrong `~/.kopi`

The tokens are written under the Linux user that ran `kopi auth add ...`. If your gateway / systemd service runs as a different user (e.g. `root` or a dedicated `kopi` user), authenticate as **that** user so the tokens land in their `~/.kopi/auth.json`. `sudo -u kopi -i` or equivalent.
