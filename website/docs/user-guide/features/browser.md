...
KOPI AI AGENT includes a full browser automation toolset with multiple backend options:
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. New installs can run `kopi setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** as the browser provider via `kopi model` or `kopi tools`.
:::
...
mode, so you need it installed (`kopi setup tools → Browser Automation`
...
Or configure via `kopi tools` → Browser Automation → Camofox.
...
By default, each Camofox session gets a random identity — cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.kopi/config.yaml`:
...
Hermes derives the stable `userId` from the profile-scoped directory `~/.kopi/browser_auth/camofox/` (or the equivalent under `$KOPI_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding Hermes profile's state directory.
...
brave-browser \
  ...
  --user-data-dir=$HOME/.kopi/chrome-debug \
  --no-first-run \
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `kopi config set toolsets '["kopi-cli", "browser"]'`.
:::
...
```
Navigate to https://github.com/LINYIQ66
```
...
```
Type "kopi agent" into the search field @e3
```
...
Screenshots are stored in `~/.kopi/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.kopi/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
