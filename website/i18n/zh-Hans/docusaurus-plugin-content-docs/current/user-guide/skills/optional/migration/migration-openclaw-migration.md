---
title: "Openclaw Migration — 将用户的 OpenClaw 自定义配置迁移到 KOPI AI AGENT"
sidebar_label: "Openclaw Migration"
description: "将用户的 OpenClaw 自定义配置迁移到 KOPI AI AGENT"
---
...
将用户的 OpenClaw 自定义配置迁移到 KOPI AI AGENT。从 `~/.openclaw` 导入 Hermes 兼容的记忆、`SOUL.md`、命令白名单、用户技能及所选工作区资产，并精确报告无法迁移的内容及原因。

## Skill 元数据

| | |
|---|---|
| 来源 | 可选 — 通过 `kopi skills install official/migration/openclaw-migration` 安装 |
| 路径 | `optional-skills/migration/openclaw-migration` |
| 版本 | `1.0.0` |
| 作者 | KOPI AI AGENT (Kopi Ai Agent Pte Ltd) |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `Migration`, `OpenClaw`, `Hermes`, `Memory`, `Persona`, `Import` |
| 相关 skill | [`kopi-ai-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-kopi-ai-agent) |
...
当用户希望以最少的手动清理将其 OpenClaw 配置迁移到 KOPI AI AGENT 时，使用此 skill。
...
**首次设置：** `kopi setup` 向导会自动检测 `~/.openclaw`，并在配置开始前提供迁移选项。

## 此 skill 的功能

它使用 `scripts/openclaw_to_kopi.py` 来：
...
- 将 OpenClaw skill 复制到 `~/.kopi/skills/openclaw-imports/`
- 可选地将 OpenClaw 工作区指令文件复制到所选 Hermes 工作区
- 将兼容的工作区资产（如 `workspace/tts/`）镜像到 `~/.kopi/tts/`
...
请勿猜测更短的路径，如 `~/.kopi/skills/openclaw-migration/...`。

运行辅助脚本前：

1. 优先使用 `~/.kopi/skills/migration/openclaw-migration/` 下的已安装路径。
...
- 在 `~/.kopi/skills/openclaw-imports/` 下可用的 OpenClaw skill
