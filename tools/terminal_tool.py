...
# display_kopi_home imported lazily at call site (stale-module safety during kopi update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of kopi directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "kopi-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = os.getenv("KOPI_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("KOPI_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (KOPI_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (KOPI_INTERACTIVE=1 or a registered sudo password callback):
    ...
    should_prompt_for_sudo = (
        env_var_enabled("KOPI_INTERACTIVE") or has_sudo_prompt_callback
    )
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``kopi-ai-agent=1`` for the current
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.kopi/.env or environment variables."
        )
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for kopi-tagged containers left behind by
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/kopi
        # restart|stop targeting kopi-gateway) must never run inside the
        ...
        # never restart. This mirrors the `kopi gateway restart` guard in
        # kopi_cli/gateway.py and the cron-path guard in kopi_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_KOPI_GATEWAY") == "1":
            from kopi_cli.cron import _contains_gateway_lifecycle_command
            if _contains_gateway_lifecycle_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Run `kopi gateway restart` from a separate shell outside "
                        "the running gateway."
                    ),
                    "status": "error",
                }, ensure_ascii=False)
        ...
        if background:
            ...
            try:
                ...
                # kopi-ai-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in kopi-ai-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        _gw_platform = _gse("KOPI_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("KOPI_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("KOPI_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("KOPI_SESSION_USER_ID", "")
                            _gw_user_name = _gse("KOPI_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("KOPI_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("KOPI_INTERACTIVE"):
                    output += (
