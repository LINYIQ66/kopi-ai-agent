"""``kopi login`` subcommand parser.

Extracted verbatim from ``kopi_cli/main.py:main()`` (god-file Phase 2).
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``kopi login`` was removed in favor of ``kopi auth`` / ``kopi model``
    (the runtime handler in ``kopi_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``kopi login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``kopi --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``kopi login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``kopi model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `kopi auth` to manage credentials, "
            "`kopi model` to select a provider, or `kopi setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `kopi model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: kopi-cli)"
    )
