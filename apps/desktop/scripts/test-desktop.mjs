...
function ensurePackagedApp() {
  if (process.env.KOPI_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function ensureDmg() {
  ...
  if (process.env.KOPI_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.KOPI_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of kopi_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'kopi-ai-agent', 'kopi_cli', 'main.py')
...
Fast rerun (skip rebuild if the packaged app already exists):
  KOPI_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
