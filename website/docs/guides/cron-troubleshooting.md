...
If you're expecting jobs to fire automatically, you need a running gateway (`kopi gateway` for foreground, or `kopi gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `kopi cron tick`.
...
```bash
date
kopi cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.kopi/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.kopi/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.kopi/.env` |
...
| `local` | Write access to `~/.kopi/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `kopi cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `kopi skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `kopi skills list`.
...
2. `~/.kopi/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `kopi cron list`
...
```bash
ls ~/.kopi/scripts/your-script.py   # Must exist
kopi cron edit <job_id> --script ~/.kopi/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `kopi skills install <skill-name>`.
...
Jobs are stored in `~/.kopi/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.kopi/cron/jobs.json
chmod 600 ~/.kopi/cron/jobs.json   # Your user should own it
```
...
```bash
kopi cron list                    # Show all jobs, states, next_run times
kopi cron run <job_id>            # Schedule for next tick (for testing)
kopi cron edit <job_id>           # Fix configuration issues
kopi logs                         # View recent Hermes logs
kopi skills list                  # Verify installed skills
```
...
1. Run the job with `kopi cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.kopi/logs/agent.log` for scheduler messages and `~/.kopi/logs/errors.log` for warnings
3. Open an issue at [github.com/LINYIQ66/kopi-ai-agent](https://github.com/LINYIQ66/kopi-ai-agent) with:
