...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the KOPI_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "KOPI_KANBAN_DB", "KOPI_KANBAN_WORKSPACES_ROOT", "KOPI_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "KOPI_KANBAN_",
    "KOPI_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(kopi_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(kopi_home) / ".env")
