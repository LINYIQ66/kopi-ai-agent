...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("KOPI_HOME", os.path.expanduser("~/.kopi")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("KOPI_HOME", Path.home() / ".kopi")) / "skills" / "red-teaming" / "godmode"
...
KOPI_HOME = Path(os.getenv("KOPI_HOME", Path.home() / ".kopi"))
CONFIG_PATH = KOPI_HOME / "config.yaml"
PREFILL_PATH = KOPI_HOME / "prefill.json"
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "kopi" in model_lower or "nous" in model_lower:
        return "kopi"
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.kopi/prefill.json."""
