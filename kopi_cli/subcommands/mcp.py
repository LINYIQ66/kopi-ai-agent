...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use 'kopi mcp add' to connect to a new server, or\n"
            "'kopi mcp serve' to expose Hermes conversations over MCP."
        ),
    )
    ...
    # flag is omitted, causing `kopi mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `kopi mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `kopi mcp install n8n`)",
    )
