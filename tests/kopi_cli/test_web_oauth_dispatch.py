"""Regression tests for the OAuth dispatcher in kopi_cli.web_server.
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "kopi_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "kopi_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "kopi_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    ...
    def fake_status():
        observed_homes.append(get_kopi_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "kopi auth add fake-oauth",
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `kopi model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `kopi model` but is "
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``kopi auth remove
    xai-oauth``.
    """
    from kopi_cli import auth as auth_mod
    from kopi_cli import web_server as ws
    ...
    # Prior `kopi auth remove xai-oauth` left the source suppressed.
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "kopi auth add hypothetical-pkce-provider",
        "docs_url": "https://example.com",
        "status_fn": None,
    }
...
def test_status_hardcoded_branch_wins_over_generic_fallback():
    ...
    with patch(
        "kopi_cli.auth.get_nous_auth_status",
...
def test_status_unknown_provider_degrades_to_logged_out():
    ...
    with patch("kopi_cli.auth.get_auth_status", return_value={"logged_in": False}):
