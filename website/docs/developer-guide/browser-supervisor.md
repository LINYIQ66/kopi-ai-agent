...
(`kopi-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `kopi-acp`, `kopi-api-server`, and core toolsets (gated on CDP reachability)
- `kopi_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
