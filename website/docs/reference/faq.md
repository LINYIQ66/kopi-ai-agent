...
description: "Frequently asked questions and solutions to common issues with KOPI AI AGENT"
---
...
- **[Nous Portal](/integrations/nous-portal)** — Kopi Ai Agent Pte Ltd's subscription gateway — 300+ models plus web/image/TTS/browser through one OAuth login (recommended for newcomers)
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `kopi auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `kopi model` or by editing `~/.kopi/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). KOPI AI AGENT does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.kopi/`.

### Can I use it offline / with local models?

Yes. Run `kopi model`, select **Custom endpoint**, and enter your server's URL:
...
KOPI AI AGENT itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same KOPI AI AGENT instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
After adding a new provider via `kopi model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `kopi model` (from terminal) |
| Enter/change API keys | `kopi model` (from terminal) |
...
# Or set directly
kopi config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.kopi/.env` for conflicting entries.
:::
...
# Set a valid model
kopi config set KOPI_MODEL anthropic/claude-opus-4.7

# Or specify per-session
kopi chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `kopi chat --provider <alternative>` to route to a different backend
...
- Or switch to the terminal interface for administrative tasks: `kopi chat`
...
cat ~/.kopi/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `kopi gateway setup`
- Check gateway logs: `cat ~/.kopi/logs/gateway.log | tail -50`
...
Configure in `~/.kopi/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.kopi/kopi-ai-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `kopi gateway start` fails
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'kopi gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `kopi gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
kopi gateway install    # Re-snapshots your current PATH
kopi gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.kopi.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `kopi chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `kopi chat -t "terminal"`
...
cd ~/.kopi/kopi-ai-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.kopi/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `kopi profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

### What happens when I run `kopi update`?

`kopi update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `kopi update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.kopi/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `kopi skills config` settings don't seem to take effect.

**Solution:** Use `kopi skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`kopi gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
2. On the **source machine**, create a full backup:
   ...
   kopi backup
   ```
   This creates a zip of your entire `~/.kopi/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/kopi-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/kopi-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   kopi import ~/kopi-backup-<timestamp>.zip
   ```

4. On the new machine, run `kopi setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
kopi profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
kopi profile import ./work-backup.tar.gz work
```
...
### `kopi backup` vs `kopi profile export`

| Feature | `kopi backup` | `kopi profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.kopi` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='kopi-ai-agent' ~/.kopi/ newmachine:~/.kopi/
```
...
kopi config show | head -20

# Re-run model selection
kopi model

# Or test with a known-good model
kopi chat -q "hello" --model anthropic/claude-opus-4.7
```
...
2. **Ask the community:** [Kopi Ai Agent Pte Ltd Discord](https://discord.gg/nousresearch)
