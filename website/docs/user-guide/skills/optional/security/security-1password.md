...
| | |
|---|---|
| Source | Optional — install with `kopi skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by KOPI AI AGENT |
| License | MIT |
...
Set `OP_SERVICE_ACCOUNT_TOKEN` in `${KOPI_HOME:-~/.kopi}/.env` (the skill will prompt for this on first load).
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/kopi-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/kopi-op.sock"
