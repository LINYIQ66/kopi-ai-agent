...
| | |
|---|---|
| Source | Optional — install with `kopi skills install official/productivity/canvas` |
...
4. Name the token (e.g., "KOPI AI AGENT"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${KOPI_HOME:-~/.kopi}/.env`:
...
```bash
CANVAS="python $KOPI_HOME/skills/productivity/canvas/scripts/canvas_api.py"
