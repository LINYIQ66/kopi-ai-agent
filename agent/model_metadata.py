...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Hermes also honours KOPI_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("KOPI_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_kopi_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run KOPI AI AGENT.  Models with fewer
...
def _get_context_cache_path() -> Path:
    ...
    from kopi_constants import get_kopi_home
    return get_kopi_home() / "context_length_cache.yaml"
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from kopi_cli.models import get_copilot_model_context
