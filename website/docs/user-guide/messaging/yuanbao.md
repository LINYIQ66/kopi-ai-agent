...
sidebar_position: 16
title: "Yuanbao"
description: "Connect KOPI AI AGENT to the Yuanbao enterprise messaging platform via WebSocket gateway"
---
...
After initial setup, verify these variables in `~/.kopi/.env`:
...
4. Review gateway logs: `tail -f ~/.kopi/logs/gateway.log`
...
4. Consider enabling verbose logging: `KOPI_LOG_LEVEL=debug`
...
```bash
KOPI_LOG_LEVEL=debug kopi gateway
```
...
```bash
kopi chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
