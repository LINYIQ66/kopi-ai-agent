...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _kopi_env="${KOPI_HOME:-$HOME/.kopi}/.env"; [ -f "$_kopi_env" ] && grep -q "^GITHUB_TOKEN=" "$_kopi_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_kopi_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from KOPI AI AGENT\",
    \"comments\": [
...
```bash
source "${KOPI_HOME:-$HOME/.kopi}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by KOPI AI AGENT. Code looks clean — good test coverage, no security concerns."
