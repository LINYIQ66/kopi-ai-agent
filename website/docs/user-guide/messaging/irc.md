...
> Run `kopi gateway setup` and pick **IRC** for a guided walk-through.

## Prerequisites

- An IRC server to connect to (e.g. `irc.libera.chat`)
- A channel to join (e.g. `#kopi`) — comma-separate to join several
- A nickname for the bot (default: `kopi-bot`)
- Optional: a registered nick + NickServ password if your network requires identification

## Configure Hermes

You can configure IRC two ways — environment variables (for a quick env-only setup) or the `gateway` block in `~/.kopi/gateway-config.yaml`.
...
| `IRC_NICKNAME` | ✅ | Bot nickname (default: `kopi-bot`) |
...
Check status with `kopi gateway status` — IRC connection state is reported there, including for env-only setups.
