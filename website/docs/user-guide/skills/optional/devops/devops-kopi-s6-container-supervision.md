...
description: "Modify, debug, or extend the s6-overlay supervision tree inside the KOPI AI AGENT Docker image — adding new services, debugging profile gateways, understandin..."
---
...
Modify, debug, or extend the s6-overlay supervision tree inside the KOPI AI AGENT Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `kopi skills install official/devops/kopi-s6-container-supervision` |
| Path | `optional-skills/devops/kopi-s6-container-supervision` |
| Version | `1.0.0` |
| Author | KOPI AI AGENT |
| License | MIT |
| Platforms | linux |
| Tags | `docker`, `s6`, `supervision`, `gateway`, `profiles` |
| Related skills | [`kopi-ai-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-kopi-ai-agent), `kopi-ai-agent-dev` |
...
│   ├── 01-kopi-setup                ← docker/stage2-hook.sh
│   │   ├── UID/GID remap
│   │   ├── chown /opt/data
│   │   ├── chown /opt/data/profiles (every boot)
│   │   ├── seed .env / config.yaml / SOUL.md
│   │   └── skills_sync.py
│   └── 02-reconcile-profiles          ← kopi_cli.container_boot
│       ├── chown /run/service (kopi-writable for runtime register)
│       └── walk $KOPI_HOME/profiles/<name>/gateway_state.json
│           → recreate /run/service/gateway-<name>/
│           → auto-start only those with prior_state == "running"
│
├── s6-rc.d (static services, in /etc/s6-overlay/s6-rc.d/)
│   ├── main-kopi/run                ← exec sleep infinity (no-op slot)
│   └── dashboard/run                  ← if KOPI_DASHBOARD=1, runs `kopi dashboard`
│
├── /run/service (s6-svscan watches; tmpfs)
│   ├── gateway-coder/                 ← runtime-registered per-profile
│   │   ├── type        ("longrun")
│   │   ├── run         ("#!/command/with-contenv sh ... exec s6-setuidgid kopi kopi -p coder gateway run")
│   │   ├── down        (marker — present means "registered but don't auto-start")
│   │   └── log/run     (s6-log → $KOPI_HOME/logs/gateways/coder/current)
│   └── ...
│
└── CMD ("main program")               ← /opt/kopi/docker/main-wrapper.sh
    └── routes user args: bare exec | kopi subcommand | kopi (no args)
...
| Path | Role |
|---|---|
| `Dockerfile` | s6-overlay install + cont-init.d wiring + `ENTRYPOINT ["/init", "/opt/kopi/docker/main-wrapper.sh"]` |
| `docker/stage2-hook.sh` | The "old entrypoint logic" — UID remap, chown, seed, skills sync. Runs as cont-init.d/01-kopi-setup. |
| `docker/cont-init.d/02-reconcile-profiles` | Calls `kopi_cli.container_boot` on every boot to restore profile gateway slots from the persistent volume. |
| `docker/main-wrapper.sh` | The container's CMD. Routes user args, drops to kopi via `s6-setuidgid`, exec's the chosen program. |
| `docker/s6-rc.d/main-kopi/run` | No-op `sleep infinity` — slot exists so the s6-rc user bundle is valid; main kopi runs as the CMD, not as a supervised service. |
| `docker/s6-rc.d/dashboard/run` | Conditional service — `exec sleep infinity` unless `KOPI_DASHBOARD` is truthy. |
...
| `kopi_cli/service_manager.py` | `S6ServiceManager`: `register_profile_gateway`, `unregister_profile_gateway`, `start/stop/restart/is_running`, `list_profile_gateways`. |
| `kopi_cli/container_boot.py` | `reconcile_profile_gateways()` — walks persistent profiles, regenerates s6 slots, emits `container-boot.log`. |
| `kopi_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `kopi gateway start/stop/restart` and routes to s6 when running in a container. |

## Why Architecture B (CMD as main program, not s6-supervised)

The original plan (v1–v3) called for main kopi to run as a supervised s6-rc service. Two real s6-overlay v3 mechanics blocked that:

1. **cont-init.d scripts receive no CMD args** — so the stage2 hook can't parse `docker run <image> chat -q "hi"` to set `KOPI_ARGS` for a service `run` script to consume.
...
So we use the s6-overlay-native CMD pattern: `ENTRYPOINT ["/init", "/opt/kopi/docker/main-wrapper.sh"]`. /init prepends the wrapper to user args automatically — so `docker run <image> --version` becomes `/init main-wrapper.sh --version`, and `--version` doesn't get intercepted by /init's POSIX shell. The wrapper drops to kopi via `s6-setuidgid`, then exec's the chosen program. The program's exit code becomes the container exit code, exactly matching the pre-s6 tini contract.

Trade-off: main kopi is unsupervised under s6. That exactly matches its behavior under tini (the pre-s6 image). Dashboard supervision is the only **new** guarantee — and per-profile gateways under `/run/service/` get full supervision.
...
Edit `S6ServiceManager._render_run_script` in `kopi_cli/service_manager.py`. The function is also called by `kopi_cli/container_boot.py::_register_service` during boot reconciliation, so it's the single source of truth. Update the corresponding assertion in `tests/kopi_cli/test_service_manager.py::test_s6_register_creates_service_dir_and_triggers_scan`.
...
Most likely the profile has no model or auth configured. The service slot is correct — the gateway itself is unconfigured. Run `kopi -p <profile> setup` first. The s6 supervisor will keep restarting it; that's the desired behavior (when you fix the config, the next attempt succeeds and stays up).

### Reconciler skipped a profile

The reconciler keys on the **presence of `SOUL.md`** as the "real profile" marker. `kopi profile create` always seeds it. If a profile dir is missing SOUL.md (stray directory, partial restore, backup-in-progress), the reconciler skips it intentionally. Add a `SOUL.md` (even empty) to opt back in.
