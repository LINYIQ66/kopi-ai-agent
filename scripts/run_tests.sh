#!/usr/bin/env bash
# Canonical test runner for kopi-ai-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.kopi/...)
#
# Usage:
...
if [ -f "$HOME/.kopi/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.kopi"
...
exec env -i \
  ...
  ${KOPI_RUN_SLOW_PET_TESTS:+KOPI_RUN_SLOW_PET_TESTS="$KOPI_RUN_SLOW_PET_TESTS"} \
