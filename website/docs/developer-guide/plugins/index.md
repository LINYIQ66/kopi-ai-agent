...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `KOPI_LOCAL_STT_COMMAND` to a shell template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.kopi/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `kopi skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `Kopi Ai Agent Pte Ltd/kopi-ai-agent`. Users install them into `~/.kopi/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the Kopi Ai Agent Pte Ltd Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/LINYIQ66/kopi-ai-agent/blob/main/CONTRIBUTING.md) for the policy.
:::
...
```bash
mkdir -p ~/.kopi/plugins/calculator
cd ~/.kopi/plugins/calculator
```
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `kopi my-plugin <subcommand>`)
...
If your plugin doesn't show up — or shows up but isn't loading — set `KOPI_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
KOPI_PLUGINS_DEBUG=1 kopi plugins list
```
...
The same logs are always written to `~/.kopi/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
kopi logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `kopi plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.kopi/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.kopi/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.kopi/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.kopi/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `kopi plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
auditable in `~/.kopi/logs/agent.log`. Plugins load after built-in
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$KOPI_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $KOPI_HOME/hook_outputs
```
...
Plugins can add their own `kopi <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for kopi my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: kopi my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for kopi my-plugin."""
...
After registration, users can run `kopi my-plugin status`, `kopi my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `kopi name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `kopi chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `KOPI_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `kopi <subcommand>` (e.g. `kopi kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "kopi kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Hermes from a hook.
...
Hermes has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.kopi/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `kopi gateway status` without SDK instantiation.
...
Model Context Protocol (MCP) servers register their own tools into Hermes without any Python plugin. Declare them in `~/.kopi/config.yaml`:
...
Drop a manifest + handler into `~/.kopi/hooks/<name>/`:

```yaml
# ~/.kopi/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.kopi/hooks/long-task-alert/handler.py
...
kopi skills search my-workflow --source myorg/skills-repo
kopi skills install myorg/skills-repo/my-workflow
```
...
For STT, point `KOPI_LOCAL_STT_COMMAND` at a shell template. Supported placeholders: `{input_path}`, `{output_path}`, `{format}`, `{voice}`, `{model}`, `{speed}` (TTS); `{input_path}`, `{output_dir}`, `{language}`, `{model}` (STT). Any path-interacting CLI is automatically a plugin.
...
```toml
# pyproject.toml
[project.entry-points."kopi_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install kopi-plugin-calculator
# Plugin auto-discovered on next kopi startup
```
