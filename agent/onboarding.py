...
def openclaw_residue_hint_cli() -> str:
    ...
    Points users at ``kopi claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``kopi claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "run `kopi claw cleanup` (renames it to ~/.openclaw.pre-migration — "
