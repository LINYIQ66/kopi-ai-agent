...
| | |
|---|---|
| Source | Optional — install with `kopi skills install official/blockchain/hyperliquid` |
...
The script reads `${KOPI_HOME:-~/.kopi}/.env` for two optional defaults:
...
Helper script: `~/.kopi/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${KOPI_HOME:-~/.kopi}/.env`.
