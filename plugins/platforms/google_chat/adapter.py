...
# ``model_tools`` import time, so ``kopi status``, ``kopi chat``, etc.
...
# ``kopi_plugins.platforms__google_chat.adapter`` once the plugin
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a kopi MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${KOPI_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _kopi_home = _get_kopi_home()
        except (ModuleNotFoundError, ImportError):
            _kopi_home = _Path.home() / ".kopi"
        self._thread_count_store = _ThreadCountStore(
            _kopi_home / "google_chat_thread_counts.json"
        )
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("KOPI_HOME", str(_Path.home() / ".kopi"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${KOPI_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a kopi MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``kopi setup``.

    The setup wizard at ``kopi_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.kopi/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. kopi-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.kopi/.env")
    print_info("Restart the gateway: kopi gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``kopi cron`` running as a
    separate process from ``kopi gateway``).  Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'kopi-ai-agent[google_chat]'",
