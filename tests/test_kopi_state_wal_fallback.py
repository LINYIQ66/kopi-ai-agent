...
Without this fallback, users on NFS-mounted ``KOPI_HOME`` silently lose
...
def _reset_wal_fallback_warned_paths():
    ...
    kopi_state._wal_fallback_warned_paths.clear()
    yield
    kopi_state._wal_fallback_warned_paths.clear()
...
class TestGetLastInitError:
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("kopi_state.sqlite3.connect", side_effect=gated_connect):
