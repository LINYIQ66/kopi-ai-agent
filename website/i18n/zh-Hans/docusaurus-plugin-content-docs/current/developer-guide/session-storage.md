...
多个 kopi 进程（gateway + CLI 会话 + worktree agent）共享同一个 `state.db`。`SessionDB` 类通过以下方式处理写入竞争：
...
该路径由 `kopi_constants.get_kopi_home()` 推导，默认解析为 `~/.kopi/`，或 `KOPI_HOME` 环境变量的值。
