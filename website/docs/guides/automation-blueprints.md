...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`kopi webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `kopi webhook subscribe`) |
...
```bash
kopi webhook subscribe github-pr-review \
  --events "pull_request" \
...
3. For each merged PR, check if it modified:
   ...
   - CLI commands (kopi_cli/commands.py, kopi_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (kopi_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
kopi webhook subscribe deploy-verify \
  --events "deployment" \
...
```python title="~/.kopi/scripts/check-uptime.py"
...
kopi cron create "every 30m" \
  ...
  --script ~/.kopi/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
kopi webhook subscribe auto-port \
  --events "pull_request" \
