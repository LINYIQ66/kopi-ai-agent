"""Regression test: `kopi dashboard --tui` must not hard-crash.

Older Hermes desktop app shells (<= 0.15.x) spawn the backend as::

    kopi dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
def _run_cli(args, timeout=60):
    """Invoke the real kopi_cli.main parser in a subprocess.
    ...
    return subprocess.run(
        [sys.executable, "-m", "kopi_cli.main", *args],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
