"""Tests for Codex auth — tokens stored in Hermes auth store (~/.kopi/auth.json)."""
...
def _setup_kopi_auth(kopi_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_kopi_auth(kopi_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("KOPI_HOME", str(kopi_home))
    ...
    monkeypatch.setattr("kopi_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    kopi_home = tmp_path / "kopi"
    _setup_kopi_auth(kopi_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("KOPI_HOME", str(kopi_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((kopi_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``kopi auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``kopi auth add openai-codex``.
    ...
    (kopi_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `kopi auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``kopi auth add openai-codex`` for accounts B and C
    ...
    (kopi_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``kopi auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``kopi auth add openai-codex`` as a workaround when there was no
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    ...
    _save_codex_tokens({"access_token": "kopi-at", "refresh_token": "kopi-rt"})
    ...
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "kopi-at"


def test_resolve_returns_kopi_auth_store_source(tmp_path, monkeypatch):
    kopi_home = tmp_path / "kopi"
    _setup_kopi_auth(kopi_home)
    monkeypatch.setenv("KOPI_HOME", str(kopi_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run kopi auth".
    """
    from kopi_cli.auth import (
    ...
    assert "kopi auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from kopi_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from kopi_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "kopi_cli.auth.resolve_codex_runtime_credentials",
    ...
    monkeypatch.setattr("kopi_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("kopi_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch kopi_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("kopi_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
