...
sidebar_position: 14
title: "API Server"
description: "Expose kopi-ai-agent as an OpenAI-compatible API for any frontend"
---

# API Server

The API server exposes kopi-ai-agent as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox, and hundreds more — can connect to kopi-ai-agent and use it as a backend.

Your agent handles requests with its full toolset (terminal, file operations, web search, memory, skills) and returns the final response. When streaming, tool progress indicators appear inline so frontends can show what the agent is doing.

:::tip One backend covers models + tools
Hermes itself needs a configured provider and tool backends for the API server to be useful. A [Nous Portal](/user-guide/features/tool-gateway) subscription handles both — 300+ models plus web/image/TTS/browser via the Tool Gateway. Run `kopi setup --portal` once before starting the API server and frontends like Open WebUI or LobeChat get a fully tool-equipped backend.
:::
...
curl http://localhost:8642/v1/chat/completions \
  ...
  -d '{"model": "kopi-ai-agent", "messages": [{"role": "user", "content": "Hello!"}]}'
```
...
Lists the agent as an available model. The advertised model name defaults to the [profile](/user-guide/profiles) name (or `kopi-ai-agent` for the default profile). Required by most frontends for model discovery.
...
Create a new scheduled job. Body accepts the same shape as `kopi cron` — prompt, schedule, skills, provider override, delivery target.
...
When a frontend sends a `system` message (Chat Completions) or `instructions` field (Responses API), kopi-ai-agent **layers it on top** of its core system prompt. Your agent keeps all its tools, memory, and skills — the frontend's system prompt adds extra instructions.
...
:::warning Security
The API server gives full access to kopi-ai-agent's toolset, **including terminal commands**. `API_SERVER_KEY` is **required for every deployment**, including the default loopback bind on `127.0.0.1`. Keep `API_SERVER_CORS_ORIGINS` narrow to control browser access when you explicitly allow browser callers.
:::
...
| `API_SERVER_MODEL_NAME` | _(profile name)_ | Model name on `/v1/models`. Defaults to profile name, or `kopi-ai-agent` for default profile. |
