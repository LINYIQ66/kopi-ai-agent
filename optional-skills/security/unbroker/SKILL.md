...
- **Optional upgrades** (the skill works zero-config without these; `setup --auto` turns on every
  one it detects, reading credentials from the shell env **and from `$KOPI_HOME/.env`** so keys
  ...
  - Email automation, two credential-free-or-not options:
    - **Browser mode (no password): `setup --email-mode browser`.** The agent sends opt-out/CCPA
      ...
      `chrome --remote-debugging-port=9222 --user-data-dir="$HOME/.kopi/chrome-debug"` (a dedicated
...
The engine stores data under `$PDD_DATA_DIR` (default `$KOPI_HOME/unbroker`), written
...
- **Dossiers are plaintext by default** (JSON, `0600` under `KOPI_HOME`). For at-rest encryption run
  ...
  a full-`KOPI_HOME` read; set `PDD_AGE_IDENTITY` to a separate volume for real key separation.
