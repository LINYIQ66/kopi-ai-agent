"""
KOPI AI AGENT Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.kopi/ (configs, sessions, logs)
"""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# KOPI AI AGENT" comment and following line
                if '# KOPI AI AGENT' in line or '# kopi-ai-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('kopi' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing hermes
                if 'kopi' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_node_symlinks(kopi_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``kopi`` command:
    ...
    """
    node_dir = (kopi_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``kopi gateway run`` processes
    ...
    try:
        from kopi_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from kopi_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``KOPI_HOME`` and ``KOPI_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\kopi\git\cmd``, ``%LOCALAPPDATA%\kopi\git\bin``,
#      ``%LOCALAPPDATA%\kopi\git\usr\bin``, ``%LOCALAPPDATA%\kopi\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\kopi\git\`` and Node to
#      ``%LOCALAPPDATA%\kopi\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``kopi dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\kopi\gateway-service\`` and sometimes
...
def _kopi_path_markers(kopi_home: Path) -> list[str]:
    """Path-entry substrings that identify Hermes-owned User-PATH entries."""
    root = str(kopi_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare kopi-ai-agent install dir.
    markers = [root + "\\kopi-ai-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if KOPI_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "kopi".  We don't want to catch
    # unrelated entries like "ckopi-foo" or "ephermeral", so we look for
    # backslash-kopi as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(kopi_home: Path) -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _kopi_path_markers(kopi_home)
            kept: list[str] = []
...
def remove_kopi_env_vars_windows() -> list[str]:
    """Delete KOPI_HOME and KOPI_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("KOPI_HOME", "KOPI_GIT_BASH_PATH"):
                try:
...
def _is_default_kopi_home(kopi_home: Path) -> bool:
    """Return True when ``kopi_home`` points at the default (non-profile) root."""
    try:
        from kopi_constants import get_default_kopi_root
        return kopi_home.resolve() == get_default_kopi_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its KOPI_HOME directory.

    We shell out to ``kopi -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current KOPI_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m kopi_cli.main` so we don't depend on a `kopi`
    #    wrapper that may be half-removed mid-uninstall.
    kopi_invocation = [_sys.executable, "-m", "kopi_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                kopi_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's KOPI_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``kopi uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$KOPI_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from kopi_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(kopi_home)
    ...
    if not summary["gui_installed"]:
        ...
        print(f"  Checked: {kopi_home}, and the standard app locations for this OS.")
        return
    ...
    print()
    if agent_is_installed(kopi_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Hermes agent at {kopi_home / 'kopi-ai-agent'}")
        print(f"  • Your config, sessions, and secrets under {kopi_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Hermes agent is still installed. Run 'kopi' to use the CLI,")
    print("or 'kopi uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.kopi/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.kopi/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie KOPI_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_kopi_home(kopi_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.kopi data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ KOPI AI AGENT Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {kopi_home / 'config.yaml'}")
    print(f"  Secrets: {kopi_home / '.env'}")
    print(f"  Data:    {kopi_home / 'cron/'}, {kopi_home / 'sessions/'}, {kopi_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their KOPI_HOME dirs. Otherwise
...
def _print_uninstall_dry_run(*, project_root: Path, kopi_home: Path, full_uninstall: bool) -> None:
    ...
    if full_uninstall:
        print(f"  • Hermes config/data: {kopi_home}")
        if _is_default_kopi_home(kopi_home):
        ...
        print(f"  • Keep Hermes config/data: {kopi_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``kopi`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$KOPI_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in kopi_home so the marker matching is
        ...
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(kopi_home))))
        ...
        log_info("Removing KOPI_HOME / KOPI_GIT_BASH_PATH User env vars...")
        removed_env = remove_kopi_env_vars_windows()
        if removed_env:
    ...
    log_info("Removing kopi command...")
    ...
    removed_node_links = remove_node_symlinks(kopi_home)
    ...
    #     step-5 rmtree(kopi_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE KOPI_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from kopi_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(kopi_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.kopi/, just remove the kopi-ai-agent subdir
            if kopi_home in project_root.parents or project_root.parent == kopi_home:
    ...
    #     under KOPI_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(kopi_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(kopi_home)
    ...
    # 5. Optionally remove ~/.kopi/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile KOPI_HOME dirs live under
        ...
        try:
            if kopi_home.exists():
                shutil.rmtree(kopi_home)
                log_success(f"Removed {kopi_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {kopi_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {kopi_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://kopi-ai-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://kopi-ai-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using KOPI AI AGENT! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m kopi_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import kopi_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``kopi_constants`` + ``kopi_cli.colors``
    (and lazily ``kopi_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m kopi_cli.uninstall")
    parser.add_argument(
        "--mode",
