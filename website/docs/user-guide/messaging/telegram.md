...
sidebar_position: 1
title: "Telegram"
description: "Set up KOPI AI AGENT as a Telegram bot"
---

# Telegram Setup

KOPI AI AGENT integrates with Telegram as a full-featured conversational bot. Once connected, you can chat with your agent from any device, send voice memos that get auto-transcribed, receive scheduled task results, and use the agent in group chats. The integration is built on [python-telegram-bot](https://python-telegram-bot.org/) and supports text, voice, images, and file attachments.
...
2. Send `/newbot`
3. Choose a **display name** (e.g., "KOPI AI AGENT") — this can be anything
4. Choose a **username** — this must be unique and end in `bot` (e.g., `my_kopi_bot`)
...
If you have local or plugin commands that should stay visible in Telegram's `/` picker, prioritize them in `~/.kopi/config.yaml`:
...
KOPI AI AGENT uses numeric Telegram user IDs to control access. Your user ID is **not** your username — it's a number like `123456789`.
...
You can also set it manually in `~/.kopi/.env`:
...
If you'd rather have the **agent itself** handle audio — for diarization, a custom transcription tool, or just archiving the recording — set `stt.enabled: false` in `~/.kopi/config.yaml`:
...
```
[The user sent a voice message: /home/<user>/.kopi/cache/audio/<hash>.ogg]
```
...
Add the URLs under `platforms.telegram.extra` in `~/.kopi/config.yaml`:
...
grep -E "Using custom Telegram base_url|Using Telegram local_mode" ~/.kopi/logs/gateway.log | tail
```
...
```bash
tail -f ~/.kopi/logs/gateway.log | grep -iE "telegram|cache"
```

You should see a `[Telegram] Cached user voice at /home/<user>/.kopi/cache/audio/...` line and **no** "too large" rejection. Combined with `stt.enabled: false` (above), the path to the original audio file then lands in the agent's inbound message for downstream processing.

## Group Chat Usage

KOPI AI AGENT works in Telegram group chats with a few considerations:
...
# named profiles
kopi -p research gateway start
kopi -p research gateway status
kopi -p research gateway stop
```

For a small fixed fleet, use a shell loop or script that calls `kopi gateway <action>` for the default profile and `kopi -p <profile> gateway <action>` for each named profile. This is more reliable than assuming a single process-level command controls every named profile on every service manager.
...
Add topics under `platforms.telegram.extra.dm_topics` in `~/.kopi/config.yaml`:
...
When this flag is on, Hermes still generates an internal session title (used by `kopi sessions`, the TUI, etc.) but never edits the Telegram topic name. Useful when you organise topics by hand under BotFather Threaded Mode and don't want every first reply to overwrite the title.
...
Add topic bindings under `platforms.telegram.extra.group_topics` in `~/.kopi/config.yaml`:
...
| Problem | Solution |
|---------|----------|
| Bot not responding at all | Verify `TELEGRAM_BOT_TOKEN` is correct. Check `kopi gateway` logs for errors. |
...
| Voice messages not transcribed | Verify STT is available: install `faster-whisper` for local transcription, or set `GROQ_API_KEY` / `VOICE_TOOLS_OPENAI_KEY` in `~/.kopi/.env`. |
...
Configure the response timeout via `agent.clarify_timeout` in `~/.kopi/config.yaml` (default `600` seconds). If you don't respond within the timeout, the agent unblocks with a sentinel message and adapts rather than hanging.
...
```bash
KOPI_TELEGRAM_NOTIFICATIONS=all
```
