...
-----------------------------------------------------------------------------
    ...
    ~/.kopi/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/kopi/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_kopi_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/kopi"
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("KOPI_CHECKPOINT_TIMEOUT", 30)))
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``kopi checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `kopi checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
# look for ``HEAD``/``info/exclude``/``KOPI_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at KOPI_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "KOPI_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `kopi checkpoints` CLI
# ---------------------------------------------------------------------------
