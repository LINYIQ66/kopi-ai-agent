...
Credential storage mirrors every other Hermes channel:
    ...
    * management metadata -> ``~/.kopi/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a kopi dependency
...
def _auth_json_path() -> Path:
    ...
    try:
        ...
        return Path(os.path.expanduser("~/.kopi")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.kopi/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.kopi/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.kopi/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: kopi_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `kopi photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `kopi photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `kopi photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `kopi photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `kopi photon setup`)"
