...
Resolution order for config file:
  1. $KOPI_HOME/honcho.json  (instance-local, enables isolated Hermes instances)
...
def resolve_active_host() -> str:
    ...
    Resolution order:
      1. KOPI_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``kopi.<profile>``
      3. Fallback: ``"kopi"`` (default profile)
    """
    explicit = os.environ.get("KOPI_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $KOPI_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.kopi/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_kopi_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_kopi_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.kopi
    ...
    def from_global_config(
        ...
        Resolution: $KOPI_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        ...
        # A hosts.kopi block or explicit enabled flag means the user
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'kopi honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        # the "go run kopi honcho setup" hint they used to.
        try:
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `kopi honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                kopi_cfg = load_config()
                honcho_cfg = kopi_cfg.get("honcho", {})
