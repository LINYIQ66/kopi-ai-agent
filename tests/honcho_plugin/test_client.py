...
from kopi_cli.profiles import _get_default_kopi_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "kopi"
        assert config.workspace_id == "kopi"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.kopi/honcho.json
        monkeypatch.setenv("KOPI_HOME", str(tmp_path / "isolated"))
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"kopi": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="kopi-ai-agent"
        ):
            result = config.resolve_session_name("/home/user/kopi-ai-agent/subdir")
        assert result == "kopi-ai-agent"
...
class TestResolveConfigPath:
    def test_prefers_kopi_home_when_exists(self, tmp_path):
        kopi_home = tmp_path / "kopi"
        kopi_home.mkdir()
        local_cfg = kopi_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: KOPI_HOME points at ~/.kopi/profiles/<name>, so
        # _get_default_kopi_home() must resolve back to ~/.kopi — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".kopi"
        ...
        assert _get_default_kopi_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_kopi_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".kopi".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".kopi"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    ...
    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"KOPI_HONCHO_HOST": "kopi.coder"}):
            assert resolve_active_host() == "kopi.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KOPI_HONCHO_HOST", None)
            # Temporarily remove kopi_cli.profiles to simulate import failure
            saved = sys.modules.get("kopi_cli.profiles")
            sys.modules["kopi_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "kopi"
            finally:
                if saved is not None:
                    sys.modules["kopi_cli.profiles"] = saved
                else:
                    sys.modules.pop("kopi_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="kopi_coder")
        assert config.host == "kopi_coder"
        assert config.workspace_id == "kopi"  # shared workspace
        assert config.ai_peer == "kopi_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="kopi")
        assert config.host == "kopi"
        assert config.workspace_id == "kopi"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "kopi": {"aiPeer": "kopi", "peerName": "alice"},
                "kopi_coder": {
                    "aiPeer": "kopi_coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="kopi_coder", config_path=config_file,
        )
        assert config.host == "kopi_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "kopi_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="kopi_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "kopi_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "kopi.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="kopi_dreamer",
        ...
        assert config.host == "kopi_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "kopi_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"kopi": {"enabled": True, "aiPeer": "kopi"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"kopi": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_kopi_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="kopi",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("kopi_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("kopi_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_kopi_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="kopi",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("kopi_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
