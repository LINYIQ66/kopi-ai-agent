...
- Pets are installed into your profile's `pets/` directory
  (`<KOPI_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `kopi pets list [query] [--limit N]` |
| List installed pets | `kopi pets list --installed` |
| Install a pet | `kopi pets install <slug> [--select] [--force]` |
...
| Preview/animate | `kopi pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `kopi pets off` |
| Remove an installed pet | `kopi pets remove <slug>` |
| Diagnose setup | `kopi pets doctor` |

`kopi pets show` flags:
...
- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `kopi pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `kopi tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `KOPI_PET_IMAGE_PROVIDER` env var (e.g. `KOPI_PET_IMAGE_PROVIDER=openrouter`).
...
Run `kopi pets doctor` — it reports:
