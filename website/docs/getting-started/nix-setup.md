...
**For non-NixOS users**, this only changes the install step. Everything after (`kopi setup`, `kopi gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage kopi the same way you manage any other NixOS service.
:::
...
That's it. `nixos-rebuild switch` creates the `kopi` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
Setting `addToSystemPackages = true` does two things: puts the `kopi` CLI on your system PATH **and** sets `KOPI_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `kopi` in your shell creates a separate `~/.kopi/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `kopi` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.

- The routing is transparent: `kopi chat`, `kopi sessions list`, `kopi version`, etc. all exec into the container under the hood
- All CLI flags are forwarded as-is
- If the container isn't running, the CLI retries briefly (5s with a spinner for interactive use, 10s silently for scripts) then fails with a clear error — no silent fallback
- For developers working on the kopi codebase, set `KOPI_DEV=1` to bypass container routing and run the local checkout directly
...
Users listed in `hostUsers` are automatically added to the `kopi` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo kopi chat` manually.
:::
...
kopi version
kopi config       # shows the generated config
```
...
```yaml
# secrets/kopi.yaml (encrypted with sops)
kopi-env: |
...
:::tip
Environment variables in `env` values are resolved from `$KOPI_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
Tokens are stored in `$KOPI_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.
...
# Ensure: chown kopi:kopi, chmod 0600
```
...
When kopi runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `kopi setup` | Config is declarative — edit `settings` in your Nix config |
| `kopi config edit` | Config is generated from `settings` |
| `kopi config set <key> <value>` | Config is generated from `settings` |
| `kopi gateway install` | The systemd service is managed by NixOS |
| `kopi gateway uninstall` | The systemd service is managed by NixOS |
...
When container mode is enabled, kopi runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the kopi package itself do **not** trigger recreation.

:::warning Writable layer loss
When the identity hash changes (image upgrade, new volumes, new container options), the container is destroyed and recreated from a fresh pull of `container.image`. Any `apt install`, `pip install`, or `npm install` packages in the writable layer are lost. State in `/data` and `/home/kopi` is preserved (these are bind mounts).

If the agent relies on specific packages, consider baking them into a custom image (`container.image = "my-registry/kopi-base:latest"`) or scripting their installation in the agent's SOUL.md.
:::

### GC Root Protection

The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current kopi package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `kopi plugins install` needed.
...
The package's `site-packages` is added to PYTHONPATH in the kopi wrapper. `importlib.metadata` discovers the entry point at session start.
...
:::note
A build-time collision check prevents plugin packages from shadowing core kopi dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
nix build .#checks.x86_64-linux.managed-guard       # KOPI_MANAGED blocks mutation
...
| `bundled-skills` | Skills directory exists, contains SKILL.md files, `KOPI_BUNDLED_SKILLS` is set in wrapper |
...
| `addToSystemPackages` | `bool` | `false` | Add `kopi` CLI to system PATH and set `KOPI_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$KOPI_HOME/.env` at activation time |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `kopi gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the kopi user's per-user profile so terminal commands, skills, and cron jobs all see them |
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$KOPI_HOME/plugins/`. Each must contain `plugin.yaml` |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.kopi` symlink to the service stateDir and are auto-added to the `kopi` group |
...
```
/var/lib/kopi/                     # stateDir (owned by kopi:kopi, 0750)
├── .kopi/                         # KOPI_HOME
