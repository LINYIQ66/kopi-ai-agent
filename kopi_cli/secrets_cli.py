"""CLI handlers for ``kopi secrets bitwarden ...``.
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``kopi_cli.main`` as part of building the top-level
    ``kopi secrets`` parser.
    """
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        "  Status:  [cyan]kopi secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]kopi secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]kopi secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]kopi secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`kopi secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]kopi secrets bitwarden setup[/cyan] and pick EU or "
