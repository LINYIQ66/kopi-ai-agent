...
sidebar_position: 1
title: "Nous Portal"
description: "One subscription, 300+ frontier models, the Tool Gateway, and Nous Chat — the recommended way to run KOPI AI AGENT"
---
...
| **Hermes** | Hermes-4-70B, Hermes-4-405B (chat, see [note below](#a-note-on-kopi-4)) |
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes KOPI AI AGENT's tool calls through Nous-managed infrastructure. Five backends, one login:
...
Your Portal account also covers [chat.nousresearch.com](https://chat.nousresearch.com) — Kopi Ai Agent Pte Ltd's web chat interface with the same model catalog. Useful when you're away from your terminal, or for non-agent conversation work.

### No credentials in your dotfiles

Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.kopi/auth.json` is the only credential on disk, and Hermes mints short-lived JWTs from it per request — see [Token handling](#token-handling) below.
...
Kopi Ai Agent Pte Ltd's own **Hermes 4** family (Hermes-4-70B, Hermes-4-405B) is available through the Portal at heavily discounted rates. These are **frontier hybrid-reasoning chat models** — strong at math, science, instruction following, schema adherence, roleplay, and long-form writing.

They are **not recommended for use inside KOPI AI AGENT**, however. Hermes 4 is tuned for chat and reasoning, not the rapid-fire tool-calling loop the agent relies on. Use them for [Nous Chat](https://chat.nousresearch.com), for research workflows, or via the [subscription proxy](/user-guide/features/subscription-proxy) from other tooling — but for agent work, pick a frontier agentic model from the catalog instead:
...
2. Stores the refresh token at `~/.kopi/auth.json`
3. Lets you pick a Nous model from the curated list (or skip to keep your current one)
4. Sets Nous as your inference provider in `~/.kopi/config.yaml` (when you pick a model)
...
Your existing providers stay configured. You can switch between them with `/model` mid-session or `kopi model` between sessions — the Portal becomes one of your available providers, not your only one.
...
```bash
kopi portal            # log in to Nous Portal + set it up (one-shot onboarding)
kopi portal info       # login status, subscription info, model + gateway routing
kopi portal status     # alias for `portal info`
kopi portal tools      # detailed Tool Gateway catalog with per-tool routing
kopi portal open       # open the subscription management page in your browser
```

`kopi portal` (with no subcommand) is the human-readable alias for `kopi auth add nous --type oauth` — it logs you in, lets you pick a Nous model, sets Nous as your inference provider, and offers the Tool Gateway opt-in (identical to `kopi setup --portal`, and the same Nous flow as the first-time quick setup).

`kopi portal info` gives you the high-level overview:
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through Nous, that's supported. Use `kopi tools` to pick backends per tool:
...
After `kopi setup --portal`, `~/.kopi/config.yaml` will look like:
...
The OAuth refresh token is stored separately at `~/.kopi/auth.json` (not in `config.yaml` — credentials and configuration are kept separate by design).
...
or use `kopi model` and re-select Nous Portal.

### Got a "re-authentication required" message mid-session

Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `kopi auth add nous` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
If a model is genuinely missing, [open an issue](https://github.com/LINYIQ66/kopi-ai-agent/issues) — we surface the Portal's catalog to Hermes and gaps usually mean a routing config we can update.

### Bills not appearing on my Portal account

Check `kopi portal info` first — if it shows you're using a different provider (`Model: currently openrouter` instead of `using Nous as inference provider`), your local config has drifted. Run `kopi model`, pick Nous Portal, and the next request will route through your subscription.
