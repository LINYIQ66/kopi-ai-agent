...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/kopi'
...
// /api/profiles/active `current`). "default" is the root ~/.kopi. This is the
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.kopiDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new KOPI_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.kopiDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.kopiDesktop?.getConnection
...
// Switch to the default (root ~/.kopi) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.kopiDesktop?.touchBackend?.(target).catch(() => undefined)
}
