...
Rules:
  ...
  - empty dirs    → always delete (under KOPI_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly KOPI_HOME and /tmp/kopi-*
Never touches: ~/.kopi/logs/ or any system directory.
"""
...
try:
    ...
    def get_kopi_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("KOPI_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".kopi").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$KOPI_HOME/logs/``."""
    return get_kopi_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$KOPI_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under KOPI_HOME or ``/tmp/kopi-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    kopi_home = get_kopi_home()
    try:
        path.resolve().relative_to(kopi_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/kopi-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("kopi-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "kopi-ai-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $KOPI_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so KOPI_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        kopi_home = get_kopi_home()
        for parent in ("cron", "cronjobs"):
            base = kopi_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside KOPI_HOME)")
        return False
...
def guess_category(path: Path) -> Optional[str]:
    ...
    kopi_home = get_kopi_home()
    try:
        rel = path.resolve().relative_to(kopi_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "kopi-ai-agent",
        }:
            return None
        ...
        # Path isn't under KOPI_HOME (e.g. /tmp/kopi-*) — fall through.
        pass
