...
description: "Overview of KOPI AI AGENT's tools — what's available, how toolsets work, and terminal backends"
---
...
| **X Search** | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool — gated on xAI credentials (SuperGrok OAuth or `XAI_API_KEY`); off by default, opt in via `kopi tools` → 🐦 X (Twitter) Search. |
...
| **Automation** | `cronjob` | Scheduled tasks with create/list/update/pause/resume/run/remove actions. Outbound delivery is handled by cron's own delivery, the `kopi send` CLI, and the gateway notifier — not by an agent-callable tool. |
...
Paid [Nous Portal](https://portal.nousresearch.com) subscribers can use web search, image generation, TTS, and browser automation through the **[Tool Gateway](tool-gateway.md)** — no separate API keys needed. Run `kopi model` to enable it, or configure individual tools with `kopi tools`.
:::
...
kopi chat --toolsets "web,terminal"
...
See [Toolsets Reference](/reference/toolsets-reference) for the full set, including platform presets such as `kopi-cli`, `kopi-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
# Configure
kopi config set terminal.backend singularity
kopi config set terminal.singularity_image ~/python.sif
```
...
modal setup
kopi config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.kopi/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.kopi/.env`.
:::
