...
- **One-shot from a script that's already running** (CI step, post-commit hook, deploy script, externally-scheduled monitor): use [`kopi send`](/guides/pipe-script-output) to pipe stdout or a file straight to Telegram / Discord / Slack / etc. without setting up a cron entry.
:::
...
```python title="~/.kopi/scripts/watch-site.py"
import hashlib, json, os, urllib.request

URL = "https://example.com/pricing"
STATE_FILE = os.path.expanduser("~/.kopi/scripts/.watch-site-state.json")
...
```bash
/cron add "every 1h" "If the script output says CHANGE DETECTED, summarize what changed on the page and why it might matter. If it says NO_CHANGE, respond with just [SILENT]." --script ~/.kopi/scripts/watch-site.py --name "Pricing monitor" --deliver telegram
```
...
```bash
/cron add "every 6h" "Check the GitHub repository Kopi Ai Agent Pte Ltd/kopi-ai-agent for:
- New issues opened in the last 6 hours
- New PRs opened or merged in the last 6 hours
- Any new releases

Use the terminal to run gh commands:
  gh issue list --repo Kopi Ai Agent Pte Ltd/kopi-ai-agent --state open --json number,title,author,createdAt --limit 10
  gh pr list --repo Kopi Ai Agent Pte Ltd/kopi-ai-agent --state all --json number,title,author,createdAt,mergedAt --limit 10
...
```python title="~/.kopi/scripts/collect-prices.py"
import json, os, urllib.request
from datetime import datetime

DATA_DIR = os.path.expanduser("~/.kopi/data/prices")
...
If there's a significant move, explain what happened." \
  --script ~/.kopi/scripts/collect-prices.py \
  --name "Price tracker" \
  --deliver telegram
```
