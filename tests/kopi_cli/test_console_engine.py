...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/kopi-console-test.zip",
    "import /tmp/kopi-console-test.zip",
...
def test_console_status_hides_cli_next_step_footer(
    ...
    import kopi_cli.status as status_mod
    ...
    def fake_show_status(_args):
        ...
        print("\x1b[2m  Run 'kopi doctor' for detailed diagnostics\x1b[0m")
        print("\x1b[2m  Run 'kopi setup' to configure\x1b[0m")
        print()
    ...
    assert "kopi doctor" not in result.output
    assert "kopi setup" not in result.output
...
def test_console_status_hides_osc_linked_cli_next_step_footer(
    ...
    def fake_show_status(_args):
        ...
        print()
        print(osc_link("\u2500" * 60))
        print(osc_link("  Run 'kopi doctor' for detailed diagnostics"))
        print(osc_link("  Run 'kopi setup' to configure"))
        print()
...
def test_console_help_uses_cli_subcommand_summaries():
    ...
    assert "Run `kopi skills list`" not in help_text
    assert "Run `kopi tools list`" not in help_text
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_kopi_home):
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_kopi_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_kopi_home):
...
def test_repl_refuses_non_interactive_confirmation(_isolate_kopi_home):
...
def test_main_console_subcommand_smoke(_isolate_kopi_home):
    import subprocess

    result = subprocess.run(
        [sys.executable, "-m", "kopi_cli.main", "console"],
