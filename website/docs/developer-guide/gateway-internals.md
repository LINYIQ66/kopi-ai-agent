...
1. `resolve_command()` from `kopi_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.kopi/.env` | API keys, bot tokens, platform credentials |
| `~/.kopi/config.yaml` | Model settings, tool configuration, display options |
...
- **Explicit target delivery** — the send engine specifying `telegram:-1001234567890`, exposed via the [`kopi send` CLI](/guides/pipe-script-output) for shell scripts and via cron `deliver:` targets
...
Hooks are discovered from `gateway/builtin_hooks/` (an extension point — currently empty in the shipped distribution; `_register_builtin_hooks()` is a no-op stub) and `~/.kopi/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `kopi gateway start` / `kopi gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.kopi/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `kopi gateway stop` stops only the current profile's gateway. `kopi gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
