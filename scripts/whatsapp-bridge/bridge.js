 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.kopi/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.kopi', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (KOPI_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.KOPI_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.kopi', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.KOPI_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.kopi', 'document_cache');
const AUDIO_CACHE_DIR = process.env.KOPI_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.kopi', 'audio_cache');
...
// `kopi update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *KOPI AI AGENT*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['KOPI AI AGENT', 'Chrome', '120.0'],
    ...
    switch (type) {
      case 'image':
        if (ext === 'gif') {
          ...
          let tmpGifMp4 = null;
          try {
            tmpGifMp4 = path.join(tmpdir(), `kopi_gif_${randomBytes(6).toString('hex')}.mp4`);
            execFileSync(
              'ffmpeg',
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `kopi_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execFileSync(
              'ffmpeg',
