...
entries via ``kopi mcp catalog`` or the interactive ``kopi mcp picker``,
and install them with ``kopi mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into kopi-ai-agent. Presence in the
...
- Manifests pin transport details (commands, args, refs). MCPs are never
  auto-updated; users explicitly re-run ``kopi mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.kopi/.env`` (the
...
from kopi_constants import get_kopi_home, get_optional_mcps_dir
...
from kopi_cli.cli_output import prompt as _prompt_input
...
def _install_root() -> Path:
    ...
    root = get_kopi_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.kopi/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.kopi/.env via save_env_value()."""
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``kopi mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `kopi mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `kopi mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `kopi mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `kopi mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `kopi auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`kopi auth {entry.auth.provider}` if you have not "
