...
from kopi_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
_UPSTREAM_REPO_URL = "https://github.com/LINYIQ66/kopi-ai-agent.git"
...
def _fetch_pypi_latest(package: str = "kopi-ai-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    ...
    Two paths: if ``KOPI_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = kopi_home / ".update_check"
    embedded_rev = os.environ.get("KOPI_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # KOPI_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `kopi update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/kopi/update/check`
    ...
    try:
        from kopi_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $KOPI_HOME/kopi-ai-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = kopi_home / "kopi-ai-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    ...
    because ``$KOPI_HOME/kopi-ai-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``kopi_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from kopi_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/LINYIQ66/kopi-ai-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    canonical Kopi Ai Agent Pte Ltd/kopi-ai-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"KOPI AI AGENT v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else KOPI_CADUCEUS
    except Exception:
        _bskin = None
        _hero = KOPI_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Kopi Ai Agent Pte Ltd[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Kopi Ai Agent Pte Ltd[/]")

    if os.getenv("KOPI_YOLO_MODE"):
    ...
    try:
        from kopi_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from kopi_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else KOPI_AGENT_LOGO
        console.print(_logo)
        console.print()
