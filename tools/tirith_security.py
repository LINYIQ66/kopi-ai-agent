...
it is automatically downloaded from GitHub releases to $KOPI_HOME/bin/tirith.
...
def _failure_marker_path() -> str:
    ...
    return os.path.join(_get_kopi_home(), ".tirith-install-failed")
...
def _kopi_bin_dir() -> str:
    """Return $KOPI_HOME/bin, creating it if needed."""
    d = os.path.join(_get_kopi_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $KOPI_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_kopi_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $KOPI_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $KOPI_HOME/bin/tirith
    ...
    kopi_bin = os.path.join(_kopi_bin_dir(), "tirith")
    if os.path.isfile(kopi_bin) and os.access(kopi_bin, os.X_OK):
        _resolved_path = kopi_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return kopi_bin
