...
- 将 `kopi` 链接到 `$PREFIX/bin`，使其保留在 Termux PATH 中
...
### 5. 将 `kopi` 添加到 Termux PATH

```bash
ln -sf "$PWD/venv/bin/kopi" "$PREFIX/bin/kopi"
```

`$PREFIX/bin` 在 Termux 中已默认在 PATH 中，因此这样做可以让 `kopi` 命令在新 shell 中持续可用，无需每次重新激活虚拟环境。
...
### `kopi doctor` 提示缺少 ripgrep 或 Node
