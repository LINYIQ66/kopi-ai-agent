...
The ``kopi update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m kopi_cli.main gateway run`` — image name
    ``pythonw``, not ``kopi.exe`` — so the ``taskkill /IM kopi.exe`` guard
    ...
    # The kopi.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM kopi.exe' in text
