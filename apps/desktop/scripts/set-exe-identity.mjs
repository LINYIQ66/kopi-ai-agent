...
// so EVERY packed build — first install, `kopi desktop`, the installer's
...
// path (which rebuilds via `kopi desktop --build-only`, never install.ps1)
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ...
      CompanyName: 'Kopi Ai Agent Pte Ltd',
      LegalCopyright: 'Copyright (c) 2026 Kopi Ai Agent Pte Ltd'
    }
  })
