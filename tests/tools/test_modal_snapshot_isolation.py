...
def _restore_tool_modules():
    original_kopi_home = os.environ.get("KOPI_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_kopi_home is None:
            os.environ.pop("KOPI_HOME", None)
        else:
            os.environ["KOPI_HOME"] = original_kopi_home
        _reset_modules(("tools", "kopi_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    kopi_cli = types.ModuleType("kopi_cli")
    kopi_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["kopi_cli"] = kopi_cli
    kopi_home = tmp_path / "kopi-home"
    os.environ["KOPI_HOME"] = str(kopi_home)
    sys.modules["kopi_cli.config"] = types.SimpleNamespace(
        get_kopi_home=lambda: kopi_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="kopi-ai-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": kopi_home / "modal_snapshots.json",
