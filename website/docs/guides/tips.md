...
description: "Practical advice to get the most out of KOPI AI AGENT — prompt tips, CLI shortcuts, context files, memory, cost optimization, and security"
---
...
Forgot something from your last session? Run `kopi -c` to resume exactly where you left off, with full conversation history restored. You can also resume by title: `kopi -r "my research project"`.
...
Want Hermes to have a stable default voice? Edit `~/.kopi/SOUL.md` (or `$KOPI_HOME/SOUL.md` if you use a custom Hermes home). Hermes now seeds a starter SOUL automatically and uses that global file as the instance-wide personality source.
...
Name your sessions with `/title auth-refactor` or `/title research-llm-quantization`. Named sessions are easy to find with `kopi sessions list` and resume with `kopi -r "auth-refactor"`. Unnamed sessions pile up and become impossible to distinguish.

### DM Pairing for Team Access

Instead of manually collecting user IDs for allowlists, enable DM pairing. When a teammate DMs the bot, they get a one-time pairing code. You approve it with `kopi pairing approve telegram XKGH5N7P` — simple and secure.
...
:::tip
By default, messaging sessions never auto-reset — context lives until you `/reset` or compression kicks in. If you want sessions to reset automatically (after idle time or daily at a fixed hour), opt in via the `session_reset` section in `~/.kopi/config.yaml`.
:::
...
TERMINAL_DOCKER_IMAGE=kopi-sandbox:latest
```
