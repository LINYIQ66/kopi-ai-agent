...
//! KOPI_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for KOPI_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub kopi_home: Option<String>,
}
...
#[tauri::command]
pub async fn launch_kopi_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_kopi_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Hermes desktop at {}. The desktop build step \
             may have been skipped or failed. Run `kopi desktop` from a \
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_kopi_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
...
pub(crate) fn resolve_kopi_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_kopi_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
...
pub(crate) fn kopi_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".kopi-bootstrap-complete").exists()
        && resolve_kopi_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_kopi_desktop_exe(install_root).ok_or_else(|| {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_kopi_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::kopi_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // the repo INTO at $HermesHome\kopi-ai-agent. Mirrors kopi_constants.
    let kopi_home = args
        .kopi_home
        .clone()
        .unwrap_or_else(|| crate::paths::kopi_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&kopi_home).join("kopi-ai-agent");

    // Copy ourselves to KOPI_HOME/hermes-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_kopi_home() {
        tracing::warn!(?err, "failed to copy installer into KOPI_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    powershell::run_script(script_path, args, sink, kopi_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    #[test]
    fn resolve_kopi_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_kopi_desktop_app(&root)
    ...
    #[test]
    fn resolve_kopi_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_kopi_desktop_app(&root).is_none(),
