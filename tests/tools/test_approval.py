...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("kopi_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("kopi_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestWindowsShellDestructiveCommands:
    def test_cmd_del_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd /c del /f /q C:\tmp\kopi-victim\file.txt"
        )
    ...
    def test_cmd_rmdir_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd.exe /k rmdir /s /q C:\tmp\kopi-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows cmd destructive delete"
    ...
    def test_powershell_remove_item_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"powershell -NoProfile -Command Remove-Item -Recurse -Force C:\tmp\kopi-victim"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_pwsh_rm_alias_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"pwsh -c rm -Recurse -Force C:\tmp\kopi-victim"
        )
        assert dangerous is True
        assert key is not None
        assert "delete" in desc.lower()
    ...
    def test_powershell_bare_remove_item_requires_approval(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            r"powershell Remove-Item -Recurse -Force C:\tmp\kopi-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows PowerShell destructive delete"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"KOPI_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_kopi_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.kopi/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_kopi_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $KOPI_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_kopi_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$KOPI_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestHermesConfigWriteProtection:
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.kopi/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.kopi/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.kopi/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.kopi/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.kopi/config.yaml")
        assert dangerous is True
        assert "kopi config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.kopi/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_kopi_home_config(self):
    ...
    def test_sed_in_place_absolute_kopi_home_env(self):
        env_path = get_kopi_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_kopi_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $KOPI_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.kopi/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_kopi_home_config(self):
        config_path = get_kopi_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.kopi/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_kopi_home_env(self):
        env_path = get_kopi_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.kopi/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.kopi/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.kopi/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.kopi/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.kopi/config.yaml")
        assert dangerous is False
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_kopi_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $KOPI_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.kopi/config.yaml/.env must require approval — the
    ...
    def test_cp_to_kopi_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.kopi/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    ...
    def test_windows_kopi_home_config_folds(self, monkeypatch):
        ...
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.kopi\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.kopi/kopi-ai-agent && source venv/bin/activate && python -m kopi_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m kopi_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m kopi_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "kopi_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart kopi-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_kopi_gateway_stop_detected(self):
        cmd = "kopi gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_kopi_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `kopi` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "kopi -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_kopi_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "kopi --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_kopi_gateway_multiple_flags_detected(self):
        cmd = "kopi -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_kopi_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "kopi -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_kopi_gateway_start_not_flagged(self):
        cmd = "kopi gateway start"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_kopi_detected(self):
        """pkill targeting kopi/gateway processes must be caught."""
...
class TestPgrepKillExpansion:
    ...
    def test_kill_dollar_pgrep_detected(self):
        cmd = 'kill -9 $(pgrep -f "kopi.*gateway")'
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestLaunchctlGatewayLifecycle:
    ...
    def test_launchctl_stop_kopi_detected(self):
        cmd = "launchctl stop ai.kopi.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "kopi" in desc.lower()

    def test_launchctl_kickstart_kopi_detected(self):
        cmd = "launchctl kickstart -k system/ai.kopi.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_kopi_detected(self):
        cmd = "launchctl bootout system/ai.kopi.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_kopi_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.kopi.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("KOPI_GATEWAY_SESSION", "KOPI_CRON_SESSION",
                      "KOPI_YOLO_MODE",
                      "KOPI_SESSION_KEY", "KOPI_INTERACTIVE")
        }
        os.environ.pop("KOPI_YOLO_MODE", None)
        os.environ.pop("KOPI_INTERACTIVE", None)
        # KOPI_CRON_SESSION takes priority over KOPI_GATEWAY_SESSION in
        ...
        os.environ.pop("KOPI_CRON_SESSION", None)
        os.environ["KOPI_GATEWAY_SESSION"] = "1"
        os.environ["KOPI_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("kopi_cli.config.load_config", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"KOPI_INTERACTIVE": "1"}, clear=False):
