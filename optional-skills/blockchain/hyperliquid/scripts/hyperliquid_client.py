#!/usr/bin/env python3
"""
Hyperliquid CLI Tool for KOPI AI AGENT
-------------------------------------
...
def _kopi_home() -> Path:
    return Path(os.environ.get("KOPI_HOME", "~/.kopi")).expanduser()
...
def _resolve_user(user: Optional[str]) -> str:
    ...
    sys.exit(
        ...
        f"{DEFAULT_USER_ENV} in your environment or {_kopi_home() / '.env'}."
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Hyperliquid CLI Tool for KOPI AI AGENT")
