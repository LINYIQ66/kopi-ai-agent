...
class TestWriteDenyExactPaths:
    ...
    def test_kopi_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.kopi/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "kopi_root"
        ...
        # Sanity check: KOPI_HOME does point to the profile dir, not the root.
        from kopi_constants import get_kopi_home, get_default_kopi_root
        assert get_kopi_home() == profile_home
        assert get_default_kopi_root() == root
