#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for KOPI AI AGENT.
...
# Ensure sibling modules (_kopi_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _kopi_home import display_kopi_home, get_kopi_home

KOPI_HOME = get_kopi_home()
TOKEN_PATH = KOPI_HOME / "google_token.json"
CLIENT_SECRET_PATH = KOPI_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = KOPI_HOME / "google_oauth_pending.json"
...
def install_deps():
    ...
    print("  pip install 'kopi-ai-agent[google]'")
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_kopi_home()}/google_token.json")
