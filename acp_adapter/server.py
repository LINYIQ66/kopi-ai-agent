"""ACP agent server — exposes KOPI AI AGENT via the Agent Client Protocol."""
...
from tools.approval import (
    reset_kopi_interactive_context,
    set_kopi_interactive_context,
)

logger = logging.getLogger(__name__)

try:
    from kopi_cli import __version__ as KOPI_VERSION
except Exception:
    KOPI_VERSION = "0.0.0"
...
class HermesACPAgent(acp.Agent):
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from kopi_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from kopi_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_kopi_session_id: str,
        previous_kopi_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.kopi.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_kopi_session_id,
                previous_kopi_session_id=previous_kopi_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_kopi_session_id: Optional[str] = None,
        previous_kopi_session_id: Optional[str] = None,
    ) -> None:
        ...
        split during a turn), pass ``previous_kopi_session_id`` so the
        attached ``_meta.kopi.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # kopi_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_kopi_session_id or session_id,
            previous_kopi_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["kopi-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="kopi-ai-agent", version=KOPI_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        # tools.approval (set_kopi_interactive_context) rather than
        # os.environ["KOPI_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue KOPI_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind KOPI_SESSION_KEY for this session so per-session caches
            ...
            interactive_token = set_kopi_interactive_context(True)
            ...
            previous_session_id = os.environ.get("KOPI_SESSION_ID")
            ...
            try:
                ...
                if interactive_token is not None:
                    reset_kopi_interactive_context(interactive_token)
                # Restore KOPI_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("KOPI_SESSION_ID", None)
                else:
                    os.environ["KOPI_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            pre_turn_kopi_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (KOPI_SESSION_KEY in
        ...
        # _meta.kopi.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_kopi_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_kopi_id
            and pre_turn_kopi_id
            and post_turn_kopi_id != pre_turn_kopi_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_kopi_session_id=post_turn_kopi_id,
                    previous_kopi_session_id=pre_turn_kopi_id,
                )
            except Exception:
                logger.debug(
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"KOPI AI AGENT v{KOPI_VERSION}"
