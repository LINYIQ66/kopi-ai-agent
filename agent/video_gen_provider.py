...
as ``kind: backend``) or ``~/.kopi/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``kopi tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$KOPI_HOME/cache/videos/``, creating parents as needed."""
    from kopi_constants import get_kopi_home

    path = get_kopi_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$KOPI_HOME/cache/videos/``.
