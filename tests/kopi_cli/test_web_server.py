"""Tests for kopi_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_kopi_home):
    """Drop the example-dashboard fixture into the per-test KOPI_HOME
    ...
    ``KOPI_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, kopi-achievements, model providers
    ...
    # plugin in exactly as a real operator would with `kopi plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down KOPI_HOME.
    app = web_server.app
...
class TestSessionTokenInjection:
    """The desktop shell mints KOPI_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("KOPI_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("KOPI_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_kopi_home):
        """Create a TestClient and isolate the state DB under the test KOPI_HOME."""
        try:
        ...
        import kopi_state
        from kopi_constants import get_kopi_home
        from kopi_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(kopi_state, "DEFAULT_DB_PATH", get_kopi_home() / "state.db")
    ...
    def test_status_active_session_count_uses_read_only_db(self, monkeypatch, tmp_path):
        ...
        fake_db_path.touch()
        monkeypatch.setattr(kopi_state, "DEFAULT_DB_PATH", fake_db_path)
        ...
        monkeypatch.setattr("kopi_state.SessionDB", _FakeDB)
    ...
    def test_status_active_session_count_fresh_install_returns_zero(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(kopi_state, "DEFAULT_DB_PATH", tmp_path / "absent.db")

        def _boom(*a, **k):
            raise AssertionError("SessionDB must not be constructed when db file is absent")

        monkeypatch.setattr("kopi_state.SessionDB", _boom)
    ...
    def test_dashboard_update_capability_detects_generic_container(self, monkeypatch):
        ...
        monkeypatch.setattr(kopi_constants, "is_container", lambda: True)
    ...
    def test_dashboard_update_capability_allows_git_in_container(self, monkeypatch):
        """A git checkout inside a container (e.g. bind-mounted in kopi-webui)
    ...
    def test_get_memory_provider_config_returns_safe_defaults(self):
        ...
        assert fields["bank_id"]["value"] == "kopi"
    ...
    def test_post_memory_provider_setup_persists_values_without_activation(self):
        from kopi_cli.config import load_config, load_env
    ...
    def test_get_media_404_for_missing_file(self):
        ...
        missing = get_kopi_home() / "images" / "nope.png"
    ...
    def test_desktop_audio_routes_registered(self):
        ...
        without failing CI.
        """
        from kopi_cli.web_server import app
    ...
    def test_update_kopi_returns_docker_guidance_without_spawning(self, monkeypatch):
        import kopi_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn kopi update")

        # Bypass the managed-externally gate so we reach the docker install check.
        monkeypatch.setattr(web_server, "_dashboard_local_update_managed_externally", lambda: False)
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_kopi_action", fail_spawn)
        web_server._ACTION_PROCS.pop("kopi-update", None)
        web_server._ACTION_RESULTS.pop("kopi-update", None)

        resp = self.client.post("/api/kopi/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "kopi-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/kopi-ai-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/kopi-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/kopi-ai-agent:latest" in line for line in status_data["lines"])

    def test_update_kopi_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        import kopi_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("managed runtime update guard should not spawn kopi update")
    ...
    def test_update_kopi_spawns_on_non_docker_install(self, monkeypatch):
        import kopi_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "git")
        monkeypatch.setattr(web_server, "_spawn_kopi_action", fake_spawn)
        web_server._ACTION_PROCS.pop("kopi-update", None)
        web_server._ACTION_RESULTS.pop("kopi-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "kopi-update"}
        assert calls == [(["update"], "kopi-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("kopi-update", None)
        web_server._ACTION_RESULTS.pop("kopi-update", None)
        web_server._ACTION_PROCS["kopi-update"] = proc

        resp = self.client.get("/api/actions/kopi-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "kopi-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["kopi-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_action_status_tails_large_log_without_read_text(self, tmp_path, monkeypatch):
        ...
        log_path = tmp_path / web_server._ACTION_LOG_FILES["kopi-update"]
        log_path.write_text(
            "stale-start\n"
        ...
        resp = self.client.get("/api/actions/kopi-update/status?lines=3")
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from kopi_cli.web_server import _channel_managed_env_keys
    ...
    def test_get_env_vars_surfaces_catalog_providers(self):
        ...
        tencent-tokenhub, copilot were configurable via `kopi model` but
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from kopi_cli.web_server import (
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `kopi import` doesn't auto-abort at the overwrite prompt."""
        import kopi_cli.web_server as ws
        ...
        monkeypatch.setattr(ws, "_spawn_kopi_action", fake_spawn)
    ...
    def test_ops_backup_uses_hosted_kopi_home(self, tmp_path, monkeypatch):
        from pathlib import Path

        import kopi_cli.web_server as ws

        hosted_home = tmp_path / "opt-data"
        monkeypatch.setenv("KOPI_HOME", str(hosted_home))
        captured = {}
    ...
    def test_ops_backup_download_streams_dashboard_backup(self, tmp_path):
        ...
        archive = backup_dir / "kopi-backup-test.zip"
    ...
    def test_ops_import_upload_rejects_invalid_zip(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_kopi_action", fail_spawn)
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from kopi_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from kopi_cli.config import save_env_value
        from kopi_cli.web_server import _SESSION_TOKEN
    ...
    def test_teams_messaging_metadata_links_setup_guide(self):
        ...
        from kopi_cli.web_server import _build_catalog_entry
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                "status": "ready",
                "bot_username": "kopi_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_kopi_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                "status": "ready",
                "bot_username": "kopi_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_kopi_action", fail_spawn_action)
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``kopi gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                "status": "ready",
                "bot_username": "kopi_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/kopi_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_spa_assets_are_read_as_utf8(self, monkeypatch, tmp_path):
        ...
        css_resp = spa_client.get("/assets/app.css", headers={"x-forwarded-prefix": "/kopi"})
    ...
    def test_headless_serve_disables_spa_even_with_a_dist(self, monkeypatch, tmp_path):
        """`kopi serve` (KOPI_SERVE_HEADLESS) must NOT serve the SPA even
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.setenv("KOPI_SERVE_HEADLESS", "1")
        app_ = FastAPI()
        ws.mount_spa(app_)
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from kopi_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from kopi_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``kopi model`` custom flow. Regression for the desktop loop where a
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "kopi-4"}
        cfg["auxiliary"] = {
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from kopi_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_kopi_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_list_includes_default(self):
        from kopi_constants import get_kopi_home
        get_kopi_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from kopi_constants import get_kopi_home
        import kopi_cli.profiles as profiles_mod
        ...
        named = kopi_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_kopi_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_kopi_for_default_profile(self):
        from kopi_constants import get_kopi_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "kopi setup"
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles_mod.shutil, "which", lambda name: "/opt/kopi/bin/kopi")
        ...
        if is_windows:
            assert lines == ["@echo off", "kopi -p writer %*"]
        else:
            assert lines == ["#!/bin/sh", 'exec /opt/kopi/bin/kopi -p writer "$@"']
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_kopi_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_kopi_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_kopi_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_kopi_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_kopi_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_kopi_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_kopi_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_kopi_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_kopi_home() / "profiles" / "builder"
        token = set_kopi_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_kopi_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import kopi_cli.skills_config as skills_config
        import kopi_cli.web_server as web_server
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from kopi_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from kopi_cli.web_server import _denormalize_config_from_web
        from kopi_cli.config import save_config
...
class TestGatewayBusyReadout:
    ...
    def test_restart_drain_timeout_surfaced_and_numeric(self, monkeypatch):
        ...
        monkeypatch.setenv("KOPI_RESTART_DRAIN_TIMEOUT", "90")
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from kopi_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.kopi/dashboard-themes/."""
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_kopi_home):
        ...
        monkeypatch.setattr(
            kopi_state, "DEFAULT_DB_PATH", get_kopi_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``kopi_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in kopi_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_kopi_home):
        ...
        # Pin the SessionDB to the isolated KOPI_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_kopi_home, _install_example_plugin):
        ...
        installs it into the per-test ``KOPI_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into KOPI_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, kopi-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (kopi-achievements is loaded by default).
        resp = self.client.get("/api/plugins/kopi-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``kopi --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["KOPI_TUI_DASHBOARD"] == "1"
        assert env["KOPI_TUI_INLINE"] == "1"
        assert env["KOPI_TUI_DISABLE_MOUSE"] == "1"
    ...
    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_kopi_home
    ):
        import kopi_cli.main as main_mod

        config_path = Path(os.environ["KOPI_HOME"]) / "config.yaml"
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/kopi-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/kopi-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"kopi-ws-ok" in buf:
                    break
            assert b"kopi-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from kopi_cli.pty_bridge import PtyUnavailableError
        ...
        import kopi_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        KOPI_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from kopi_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("KOPI_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_kopi_home, _install_example_plugin):
        ...
        the per-test ``KOPI_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_kopi_home):
        ...
        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("KOPI_DESKTOP", raising=False)
