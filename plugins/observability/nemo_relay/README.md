...
not replace KOPI AI AGENT's planner, tools, memory, model provider routing, or
...
With this plugin enabled, KOPI AI AGENT can:
...
The `KOPI_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `KOPI_HOME` that the
agent run will use:

```bash
env KOPI_HOME=/tmp/kopi-nemo-relay-test \
  kopi plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`KOPI_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`KOPI_HOME` is the Hermes profile/config home used by both
`kopi plugins enable ...` and the later `kopi chat ...` run. If unset,
Hermes uses the user's default home, usually `~/.kopi`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export KOPI_HOME=/tmp/kopi-nemo-relay-test
kopi plugins enable observability/nemo_relay
kopi chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `kopi` command you run is built from the
...
The plugin can configure exporters directly from `KOPI_NEMO_RELAY_*`
...
```bash
export KOPI_NEMO_RELAY_ATOF_ENABLED=1
export KOPI_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export KOPI_NEMO_RELAY_ATIF_ENABLED=1
export KOPI_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `KOPI_NEMO_RELAY_ATOF_FILENAME`
- `KOPI_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `KOPI_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `KOPI_NEMO_RELAY_ATIF_AGENT_NAME`
- `KOPI_NEMO_RELAY_ATIF_AGENT_VERSION`
- `KOPI_NEMO_RELAY_ATIF_MODEL_NAME`
- `KOPI_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)
...
```bash
export KOPI_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "KOPI AI AGENT"
agent_version = "local"
```

When `KOPI_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`KOPI_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`KOPI_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Hermes does not
...
cat > "$KOPI_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
```bash
export KOPI_NEMO_RELAY_ATOF_ENABLED=1
export KOPI_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/kopi-nemo-relay-docs/subagent/atof
export KOPI_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export KOPI_NEMO_RELAY_ATOF_MODE=overwrite
export KOPI_NEMO_RELAY_ATIF_ENABLED=1
export KOPI_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/kopi-nemo-relay-docs/subagent/atif
export KOPI_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export KOPI_NEMO_RELAY_ATIF_AGENT_NAME='KOPI AI AGENT E2E'
export KOPI_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export KOPI_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "KOPI AI AGENT E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
```bash
mkdir -p /tmp/kopi-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/kopi-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/kopi-nemo-relay-docs/workdir/beta.txt
cd /tmp/kopi-nemo-relay-docs/workdir

export KOPI_NEMO_RELAY_ATOF_ENABLED=1
export KOPI_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/kopi-nemo-relay-docs/parallel/atof
export KOPI_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export KOPI_NEMO_RELAY_ATOF_MODE=overwrite
export KOPI_NEMO_RELAY_ATIF_ENABLED=1
export KOPI_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/kopi-nemo-relay-docs/parallel/atif
export KOPI_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export KOPI_NEMO_RELAY_ATIF_AGENT_NAME='KOPI AI AGENT E2E'
export KOPI_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
mkdir -p "$KOPI_HOME" /tmp/kopi-middleware-test/nemo-relay
...
cat > /tmp/kopi-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/kopi-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/kopi-middleware-test/atif"
...
export KOPI_NEMO_RELAY_PLUGINS_TOML=/tmp/kopi-middleware-test/nemo-relay/plugins.toml
