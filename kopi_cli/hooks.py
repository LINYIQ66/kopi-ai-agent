"""kopi hooks — inspect and manage shell-script hooks.

Usage::

    kopi hooks list
    kopi hooks test <event> [--for-tool X] [--payload-file F]
    kopi hooks revoke <command>
    kopi hooks doctor

Consent records live under ``~/.kopi/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.kopi/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``kopi hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: kopi hooks {list|test|revoke|doctor}")
        print("Run 'kopi hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.kopi/config.yaml.")
        print("See `kopi hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `kopi hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `kopi hooks test` and `kopi hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `kopi hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `kopi hooks doctor` would execute
