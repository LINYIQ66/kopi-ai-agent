...
profile is active, get_kopi_home() should:
  1. STILL return ~/.kopi (raising would brick 30+ module-level callers)
...
def fresh_constants(monkeypatch, tmp_path):
    """Import kopi_constants fresh and reset the one-shot warn flag."""
    import importlib
...
class TestGetHermesHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        """Classic mode: no active_profile file → silent, returns ~/.kopi."""
    ...
    def test_named_profile_unset_home_warns_once(
        ...
        """active_profile=coder + KOPI_HOME unset → warn loudly, still return fallback."""
        kopi_dir = tmp_path / ".kopi"
        kopi_dir.mkdir()
        (kopi_dir / "active_profile").write_text("coder\n")
        ...
        assert err.count("KOPI_HOME fallback") == 1
        assert "'coder'" in err
        assert "#18594" in err

        # 3. One-shot: second and third calls don't re-warn
        fresh_constants.get_kopi_home()
        fresh_constants.get_kopi_home()
        err2 = capsys.readouterr().err
        assert "KOPI_HOME fallback" not in err2

    def test_kopi_home_set_suppresses_warning(
        self, fresh_constants, tmp_path, capsys, monkeypatch
    ):
        """Even if active_profile is 'coder', setting KOPI_HOME suppresses warning."""
        profile_dir = tmp_path / ".kopi" / "profiles" / "coder"
        profile_dir.mkdir(parents=True)
        (tmp_path / ".kopi" / "active_profile").write_text("coder\n")
    ...
    def test_unreadable_active_profile_no_crash(
        ...
        (kopi_dir / "active_profile").write_bytes(b"\xff\xfe\x00\x00")
