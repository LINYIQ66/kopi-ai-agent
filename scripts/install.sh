...
# Python-driven tool session (e.g. KOPI terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:Kopi Ai Agent Pte Ltd/kopi-ai-agent.git"
REPO_URL_HTTPS="https://github.com/LINYIQ66/kopi-ai-agent.git"
KOPI_HOME="${KOPI_HOME:-$HOME/.kopi}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${KOPI_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$KOPI_INSTALL_DIR"
...
#   code at /usr/local/lib/kopi-ai-agent, command at /usr/local/bin/kopi,
#   data still at /root/.kopi (KOPI_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$KOPI_HOME/.no-bundled-skills so future"
            echo "                   'kopi update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> KOPI.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.kopi/kopi-ai-agent"
            echo "                   default (root, Linux): /usr/local/lib/kopi-ai-agent"
            echo "  --hermes-home PATH  Data directory (default: ~/.kopi, or \$KOPI_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, KOPI installs the code under"
            echo "  /usr/local/lib/kopi-ai-agent and links the command into"
            echo "  /usr/local/bin/kopi (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$KOPI_HOME"
            echo "  (default /root/.kopi).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$KOPI_HOME/kopi-ai-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs kopi setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ KOPI AI AGENT Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Kopi Ai Agent Pte Ltd.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `kopi update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (KOPI-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download KOPI AI AGENT","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install kopi command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `kopi` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $KOPI_HOME/kopi-ai-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $KOPI_HOME/kopi-ai-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/kopi-ai-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $KOPI_HOME/kopi-ai-agent — then preserve it)
#
# Always no-op when the user set --dir or $KOPI_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in KOPI_HOME.
    if is_termux; then
        INSTALL_DIR="$KOPI_HOME/kopi-ai-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$KOPI_HOME/kopi-ai-agent/.git" ]; then
            INSTALL_DIR="$KOPI_HOME/kopi-ai-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/kopi-ai-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/kopi-ai-agent"
        ...
        # /usr/local/bin/kopi wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/kopi"
        log_info "  Data:    $KOPI_HOME (unchanged)"
...
# Point a KOPI-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $KOPI_HOME/node/bin — which is NOT on
...
# KOPI's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no KOPI-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$KOPI_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$KOPI_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$KOPI_HOME/node/etc/npmrc"
}

get_kopi_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/kopi" ]; then
        echo "$link_dir/kopi"
    else
        echo "kopi"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://kopi-ai-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Hermes owns its own uv at $KOPI_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (kopi_cli/managed_uv.py) looks in the same
    # place, so install.sh and `kopi update` stay in sync.
    local _managed_uv="$KOPI_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $KOPI_HOME/bin ..."
    mkdir -p "$KOPI_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/kopi-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/kopi-uv-installer.$$.sh")"
    ...
    # directly into $KOPI_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$KOPI_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_node() {
    ...
    # Repair pre-existing KOPI-managed installs where `npm install -g` lands
    ...
    if [ -x "$KOPI_HOME/node/bin/node" ] && node_satisfies_build "$("$KOPI_HOME/node/bin/node" --version)"; then
        export PATH="$KOPI_HOME/node/bin:$PATH"
        log_success "Node.js $("$KOPI_HOME/node/bin/node" --version) found (KOPI-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing KOPI-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.kopi/node/..."
    ...
    # Place into ~/.kopi/node/ and symlink binaries into the same bin dir
    # the kopi command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$KOPI_HOME/node"
    mkdir -p "$KOPI_HOME"
    mv "$extracted_dir" "$KOPI_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$KOPI_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$KOPI_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$KOPI_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$KOPI_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.kopi/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. KOPI install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "KOPI AI AGENT itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `kopi update` path
                # (#4735).
                ...
                local stash_name
                stash_name="kopi-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"kopi-ai-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    ...
    if [ "$USE_VENV" = true ]; then
        KOPI_BIN="$INSTALL_DIR/venv/bin/hermes"
    else
        KOPI_BIN="$(which kopi 2>/dev/null || echo "")"
        if [ -z "$KOPI_BIN" ]; then
            log_warn "kopi not found on PATH after install"
            return 0
        fi
    fi
    ...
    # Create a user-facing shim for the kopi command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $KOPI_BIN. Without
...
EOF
    ...
    log_success "Installed kopi launcher → $command_link_display_dir/hermes"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "kopi command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        log_info "kopi not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# KOPI AI AGENT — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# KOPI AI AGENT — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# KOPI AI AGENT — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so kopi works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.kopi directory structure (config at top level, code in subdir)
    mkdir -p "$KOPI_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.kopi/.env (top level, easy to find)
    if [ ! -f "$KOPI_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$KOPI_HOME/.env"
            log_success "Created ~/.kopi/.env from template"
        else
            touch "$KOPI_HOME/.env"
            log_success "Created ~/.kopi/.env"
        fi
    else
        log_info "~/.kopi/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$KOPI_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.kopi/config.yaml (top level, easy to find)
    if [ ! -f "$KOPI_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$KOPI_HOME/config.yaml"
            log_success "Created ~/.kopi/config.yaml from template"
        fi
    else
        log_info "~/.kopi/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in kopi_cli/default_soul.py — the
    ...
    if [ ! -f "$KOPI_HOME/SOUL.md" ]; then
        cat > "$KOPI_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.kopi/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.kopi/"

    # Seed bundled skills into ~/.kopi/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `kopi update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'kopi update'." \
            > "$KOPI_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $KOPI_HOME/.no-bundled-skills"
        log_info "  Future 'kopi update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.kopi/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.kopi/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$KOPI_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$KOPI_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.kopi/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# KOPI AI AGENT browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# KOPI AI AGENT browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (kopi --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `kopi update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'kopi setup' after install."
        return 0
    fi
    ...
    # Run kopi setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m kopi_cli.main setup < /dev/tty
    else
        python -m kopi_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    WHATSAPP_SESSION="$KOPI_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'kopi whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                KOPI_CMD="$(get_kopi_command_path)"
                $KOPI_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'kopi whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'kopi gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $KOPI_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $KOPI_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: kopi gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: kopi gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $KOPI_CMD gateway > "$KOPI_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.kopi/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: kopi gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: kopi gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $KOPI_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $KOPI_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $KOPI_HOME/cron/, sessions/, logs/"
    ...
    echo -e "   ${GREEN}kopi setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}kopi config${NC}       View/edit configuration"
    echo -e "   ${GREEN}kopi config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}kopi gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}kopi update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'kopi' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'kopi' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'kopi' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$KOPI_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$KOPI_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$KOPI_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$KOPI_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$KOPI_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    KOPI_CMD="$(command -v kopi 2>/dev/null || echo "")"
    if [ -n "$KOPI_CMD" ]; then
        log_info "Running kopi setup..."
        "$KOPI_CMD" setup
    else
        log_warn "kopi command not found on PATH"
        log_info "Try: python -m kopi_cli.main setup"
    fi
}
...
# _purge_electron_build_cache() used by `kopi desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the KOPI-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `kopi desktop`),
    ...
    if [ "$OS" = "linux" ]; then
        ...
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/KOPI.app" \
            "$desktop_dir/release/mac/KOPI.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # checkout stays clean for the next `kopi update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the KOPI-managed Node
            # provisioned during prerequisites/node-deps (at $KOPI_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $KOPI_HOME.
    # $KOPI_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'kopi update' on this host install.
