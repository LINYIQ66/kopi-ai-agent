...
def _clean_state():
    ...
    saved = {}
    for k in ("KOPI_INTERACTIVE", "KOPI_GATEWAY_SESSION", "KOPI_EXEC_ASK", "KOPI_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["KOPI_INTERACTIVE"] = "1"
        session_key = os.getenv("KOPI_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No KOPI_INTERACTIVE or KOPI_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
