...
默认情况下，cron 任务继承创建它的 gateway/CLI 所属的 Hermes profile。传入 `--profile <name>`（CLI）或 `profile=`（cronjob 工具）可将任务重定向到不同的 profile——调度器会解析该 profile 的 `KOPI_HOME`，在运行期间临时切换到该 profile，加载其 `.env` 和 `config.yaml`，并在其中执行任务：

```bash
# 将任务固定到 `night-ops` profile，无论在哪里调度
kopi cron create "every 1d at 03:00" \
...
:::note 串行化
设置了 `profile` 的任务也串行运行，原因与 `workdir` 固定任务相同：切换 `KOPI_HOME` 是进程全局变更，两个 profile 固定任务并行运行会产生竞争。未固定的任务仍在正常并行池中运行。
:::
...
```bash
kopi cron list
kopi cron pause <job_id>
kopi cron resume <job_id>
kopi cron run <job_id>
kopi cron remove <job_id>
kopi cron status
kopi cron tick
```
...
```bash
kopi gateway install     # 安装为用户服务
sudo kopi gateway install --system   # Linux：服务器开机启动的系统服务
kopi gateway             # 或在前台运行
...
`~/.kopi/cron/.tick.lock` 处的文件锁防止重叠的调度器 tick 重复运行同一批任务。
...
| `"local"` | 仅保存到本地文件（`~/.kopi/cron/output/`） | CLI 上的默认值 |
...
如果 agent 的最终响应以 `[SILENT]` 开头，投递将被完全抑制。输出仍会保存到本地以供审计（位于 `~/.kopi/cron/output/`），但不会向投递目标发送任何消息。
...
或设置 `KOPI_CRON_SCRIPT_TIMEOUT` 环境变量。解析顺序为：环境变量 → config.yaml → 默认 120 秒。
...
`.sh`/`.bash` 文件在 `/bin/bash` 下运行；其他文件在当前 Python 解释器（`sys.executable`）下运行。脚本必须位于 `~/.kopi/scripts/`（与预运行脚本门控相同的沙箱规则）。
...
Hermes 会通过 `write_file` 将检查脚本写入 `~/.kopi/scripts/`，然后调用：
...
- 任务 2 触发时，Hermes 从 `~/.kopi/cron/output/{job1_id}/*.md` 读取任务 1 的最新输出
...
Cron 在全新的 agent 会话中运行每个任务，不附加任何聊天平台。默认情况下，cron agent 获得**你在 `kopi tools` 中为 `cron` 平台配置的工具集**——不是 CLI 默认值，也不是所有工具。
...
当任务上设置了 `enabled_toolsets` 时，它优先生效；否则 `kopi tools` 的 cron 平台配置生效；否则 Hermes 回退到内置默认值。这对成本控制很重要：在每个小型"获取新闻"任务中携带 `moa`、`browser`、`delegation` 会在每次 LLM 调用时膨胀工具 schema prompt。
...
:::tip
Hermes 自身的 `~/.kopi/state.db` 是内部 schema，会在版本间变更。不要从预运行门控中查询它——指向你自己的数据库或 feed。
:::

致谢：此方案集由 @iankar8 在 [#2654](https://github.com/LINYIQ66/kopi-ai-agent/pull/2654) 中的探索所启发，该 PR 提议将 sql/file/command 触发器作为并行机制添加。`script` + `wakeAgent` 门控已以零成本覆盖了所有三种情况，因此该工作以文档形式落地。
...
任务存储在 `~/.kopi/cron/jobs.json`。任务运行的输出保存到 `~/.kopi/cron/output/{job_id}/{timestamp}.md`。
