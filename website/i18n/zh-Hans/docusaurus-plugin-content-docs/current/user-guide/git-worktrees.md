...
KOPI AI AGENT 常用于大型、长期维护的仓库。当你需要：
...
- CLI：运行 `kopi` 或 `kopi chat` 时所在的目录
...
- 新目录：`../repo-feature`
- 新分支：`feature/kopi-experiment`，已在该目录中检出
...
- 在各自的分支上工作（`feature/kopi-a` 与 `feature/kopi-b`）。
...
- `~/.kopi/checkpoints/` 下的 Hermes checkpoint 数据在移除 worktree 时不会自动清理，但通常体积很小。
...
- **以实验内容命名分支**
  - 例如：`feature/kopi-checkpoints-docs`、`feature/kopi-refactor-tests`。
- **频繁提交**
...
如需并行运行多个 agent，在多个终端中分别运行 `kopi -w`——每次调用都会自动获得独立的 worktree 和分支。
