...
如果你编写的是 worker 代码本身——即运行在通道*内部*的 agent——kanban 生命周期与参考细节会自动注入到 worker 的系统提示中（[`agent/prompt_builder.py`](https://github.com/LINYIQ66/kopi-ai-agent/blob/main/agent/prompt_builder.py) 中的 `KANBAN_GUIDANCE` 块）。
...
对于 Hermes profile 通道，调度器的 `_default_spawn` 会在任务固定的工作区内运行 `kopi -p <assignee> chat -q <prompt>`（或当 `kopi` shim 不在 `$PATH` 时使用等效的模块形式），并设置以下环境变量：

| 变量 | 携带内容 |
|---|---|
| `KOPI_KANBAN_TASK` | worker 正在操作的任务 id |
| `KOPI_KANBAN_DB` | 每个看板 SQLite 文件的绝对路径 |
| `KOPI_KANBAN_BOARD` | 看板 slug |
| `KOPI_KANBAN_WORKSPACES_ROOT` | 看板工作区树的根目录 |
| `KOPI_KANBAN_WORKSPACE` | *本*任务工作区的绝对路径 |
| `KOPI_KANBAN_RUN_ID` | 当前运行的 id（用于生命周期门控） |
| `KOPI_KANBAN_CLAIM_LOCK` | claim 锁字符串（`<host>:<pid>:<uuid>`） |
| `KOPI_PROFILE` | worker 自身的 profile 名称（用于 `kanban_comment` 作者归因） |
| `KOPI_TENANT` | 租户命名空间（如果任务有的话） |
...
- **使用 block 而非 complete**，`reason` 以 `review-required: ` 为前缀，使仪表板 / `kopi kanban show` 将该行显示为等待审查。
...
仪表板以摘要、元数据块和退出状态徽章渲染运行历史。CLI 用户可运行 `kopi kanban tail <task_id>` 实时跟踪，或运行 `kopi kanban runs <task_id>` 查看历史尝试列表。

## 现有通道形态

### Hermes profile 通道（默认）

当前所有 kanban worker 采用的形态：assignee 是 profile 名称，调度器生成 `kopi -p <profile>`，worker 会自动获得注入的 `KANBAN_GUIDANCE` 系统提示块，并使用 `kanban_*` 工具终止运行。除定义 profile 外无需任何额外配置。

为你的 fleet 创建 profile 时，选择与你希望 orchestrator 路由到的*角色*相匹配的名称。orchestrator（如果存在）通过 `kopi profile list` 发现你的 profile 名称——系统不假设固定的名单（orchestrator 侧的契约也是注入的 `KANBAN_GUIDANCE` 的一部分）。
...
- **滞留任务检测** — assignee 在 `kanban.stranded_threshold_seconds`（默认 30 分钟）内始终未产生 claim 的 ready 任务，会在 `kopi kanban diagnostics` 中显示为 `stranded_in_ready` 警告。严重程度在 2 倍阈值时升级为 error，在 6 倍时升级为 critical。可通过单一信号捕获拼写错误的 assignee、已删除的 profile 以及宕机的外部 worker 池——与标识无关，无需维护每个看板的白名单。

## 相关资源

- [Kanban 概览](./kanban) — 面向用户的介绍。
- [Kanban 教程](./kanban-tutorial) — 开启仪表板的完整演练。
- [`KANBAN_GUIDANCE`](https://github.com/LINYIQ66/kopi-ai-agent/blob/main/agent/prompt_builder.py) — 注入到每个 kanban worker 系统提示中的 worker + orchestrator 生命周期。