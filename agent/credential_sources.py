...
Hermes seeds its credential pool from many places:
    ...
    env:<VAR>     — os.environ / ~/.kopi/.env
    claude_code   — ~/.claude/.credentials.json
    kopi_pkce   — ~/.kopi/.anthropic_oauth.json
...
unify here is **removal**:
    ...
    ``kopi auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), kopi_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.kopi/.env  → clear it
    ...
    """
    from kopi_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"{env_var} until you run `kopi auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `kopi auth add anthropic` to re-enable if needed.",
    ])


def _remove_kopi_pkce(provider: str, removed) -> RemovalResult:
    """~/.kopi/.anthropic_oauth.json is ours — delete it outright."""
    from kopi_constants import get_kopi_home

    result = RemovalResult()
    oauth_file = get_kopi_home() / ".anthropic_oauth.json"
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `kopi auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `kopi auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``kopi auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `kopi model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    via ``kopi auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `kopi auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `kopi auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `kopi auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="kopi_pkce",
        remove_fn=_remove_kopi_pkce,
        description="~/.kopi/.anthropic_oauth.json",
    ))
    register(RemovalStep(
