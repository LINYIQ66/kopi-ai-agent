"""CLI handlers for ``kopi migrate ...``.

Currently exposes only ``kopi migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``kopi migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: kopi migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `kopi migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `kopi doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
