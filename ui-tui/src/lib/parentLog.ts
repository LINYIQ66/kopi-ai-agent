...
const logDir = join(process.env.KOPI_HOME?.trim() || join(homedir(), '.kopi'), 'logs')
...
// a real ~/.kopi (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('kopi-tui: parent lifecycle log unavailable\n')
    }
  }
}
