"""Windows ConPTY bridge for the `kopi dashboard` chat tab.

Drop-in counterpart to ``kopi_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``kopi_cli.web_server`` consumes: ``spawn``, ``read``,
