"""Progressive tool disclosure ("tool search") for KOPI AI AGENT.
...
* Core tools defined in ``toolsets._KOPI_CORE_TOOLS`` are *never* deferred.
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_KOPI_CORE_TOOLS)
    except Exception:
        return frozenset()
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_KOPI_CORE_TOOLS``. Core tools are never deferred
