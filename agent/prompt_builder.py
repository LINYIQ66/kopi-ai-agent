...
from kopi_constants import get_kopi_home, get_skills_dir, is_wsl
...
def _find_kopi_md(cwd: Path) -> Optional[Path]:
    ...
    # could pick up a .kopi.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _KOPI_MD_NAMES:
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.kopi/kanban.db`. Your task id is in "
    "`$KOPI_KANBAN_TASK`; your workspace is `$KOPI_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $KOPI_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $KOPI_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${KOPI_KANBAN_BRANCH:-wt/$KOPI_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$KOPI_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`kopi profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `kopi kanban <verb>` for board operations. Use "
...
#
# The kopi-ai-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to kopi-ai-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `kopi computer-use doctor` and share the output. That command "
...
def build_environment_hints() -> str:
    ...
    extra = (os.getenv("KOPI_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_kopi_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("KOPI_PLATFORM") or os.environ.get("KOPI_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.kopi/skills/`` directory.  External dirs
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: kopi setup, kopi setup tools, kopi setup terminal, kopi status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from KOPI_HOME and return its content, or None.
    ...
    """
    try:
        from kopi_cli.config import ensure_kopi_home
        ensure_kopi_home()
    except Exception as e:
        logger.debug("Could not ensure KOPI_HOME before loading SOUL.md: %s", e)

    soul_path = get_kopi_home() / "SOUL.md"
...
def _load_kopi_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    ...
    if not kopi_md_path:
        return ""
    try:
        content = kopi_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = kopi_md_path.name
        try:
            rel = str(kopi_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".kopi.md", context_length=context_length,
            read_path=str(kopi_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", kopi_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    SOUL.md from KOPI_HOME is independent and always included when present.
    ...
    project_context = (
        _load_kopi_md(cwd_path, context_length)
    ...
    # SOUL.md from KOPI_HOME only — skip when already loaded as identity
    if not skip_soul:
