...
def test_kopi_client_tag_includes_current_version():
    """The client tag must reflect kopi_cli.__version__ verbatim."""
    from kopi_cli import __version__
    from agent.portal_tags import kopi_client_tag

    assert kopi_client_tag() == f"client=kopi-client-v{__version__}"


def test_kopi_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import kopi_client_tag

    tag = kopi_client_tag()
    assert tag.startswith("client=kopi-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import kopi_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=kopi-ai-agent" in tags
    assert kopi_client_tag() in tags
    assert len(tags) == 2
