...
`~/.kopi/config.yaml`, `~/.kopi/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/kopi`:

```text
/etc/kopi/
├── config.yaml     # managed config layer (wins over ~/.kopi/config.yaml)
└── .env            # managed env layer (wins over ~/.kopi/.env + shell)
```
...
The location can be relocated with the `KOPI_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `KOPI_HOME` — set by the same administrator who owns the managed
...
export KOPI_MANAGED_DIR=/opt/org/kopi-policy
```

:::warning
A user who can set `KOPI_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `kopi doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/kopi/config.yaml` (managed) | `/etc/kopi/.env` (managed) |
| 2 | `~/.kopi/config.yaml` (user) | `~/.kopi/.env` (user) |
...
```bash
kopi config        # shows a header naming the managed source + the pinned keys
kopi doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Hermes refuses and names the source:

```bash
$ kopi config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/kopi/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `kopi config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/kopi

# Pin some config values for every user on this machine
sudo tee /etc/kopi/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/kopi/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/kopi
sudo chmod 0644 /etc/kopi/config.yaml /etc/kopi/.env
```
...
`kopi doctor` to confirm the policy is being applied).
