"""Tests for kopi_cli/completion.py — shell completion script generation."""
...
from kopi_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real kopi structure."""
    p = argparse.ArgumentParser(prog="kopi")
...
class TestGenerateZsh:
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_kopi_profiles'" in out
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c kopi -f" in out
...
class TestSubcommandDrift:
    def test_SUBCOMMANDS_covers_required_commands(self):
        ...
        """
        import inspect
        from kopi_cli.main import _coalesce_session_name_args
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_kopi_profiles()" in out
        assert 'profiles_dir="$HOME/.kopi/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'kopi profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _kopi_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_kopi_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _kopi_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_kopi_profiles()" in out
        assert "$HOME/.kopi/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__kopi_profiles" in out
        count = out.count("(__kopi_profiles)")
