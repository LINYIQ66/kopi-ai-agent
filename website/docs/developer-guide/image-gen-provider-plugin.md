...
description: "How to build an image-generation backend plugin for KOPI AI AGENT"
---
...
2. **User** — `~/.kopi/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
3. **Pip** — packages declaring a `kopi_agent.plugins` entry point

Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `kopi tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `kopi tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.kopi/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `kopi plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `kopi tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `kopi tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `kopi tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            #   - base64 data: save under $KOPI_HOME/cache/images/ via save_b64_image()
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `kopi plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `kopi tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `kopi tools` model picker |
...
Some backends return image URLs (fal, Replicate); others return base64 payloads (OpenAI gpt-image-2). For the base64 case, use `save_b64_image()` — it writes to `$KOPI_HOME/cache/images/<prefix>_<timestamp>_<uuid>.<ext>` and returns the absolute `Path`. Pass that path (as `str`) as `image=` in `success_response()`. Gateway delivery (Telegram photo bubble, Discord attachment) recognizes both URLs and absolute paths.

## User overrides

Drop a user plugin at `~/.kopi/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `kopi plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.

## Testing

```bash
export KOPI_HOME=/tmp/kopi-imggen-test
mkdir -p $KOPI_HOME/plugins/image_gen/my-backend
# …copy __init__.py + plugin.yaml into that dir…

export MY_BACKEND_API_KEY=your-test-key
kopi plugins enable my-backend

# Pick it as the active provider
echo "image_gen:" >> $KOPI_HOME/config.yaml
echo "  provider: my-backend" >> $KOPI_HOME/config.yaml

# Exercise it
kopi -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `kopi tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
