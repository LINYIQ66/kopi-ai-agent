...
shared ``kopi_bootstrap.harden_import_path`` guard.
...
def test_entry_calls_shared_harden_guard_before_heavy_imports():
    """entry.py must call kopi_bootstrap.harden_import_path() before it
    ...
    assert harden_call_line is not None, (
        "entry.py must call kopi_bootstrap.harden_import_path()"
    )
...
def test_entry_does_not_reimplement_guard_inline():
    ...
    assert '{"", "."}' not in source and "{'', '.'}" not in source, (
        "entry.py should delegate to kopi_bootstrap.harden_import_path, "
...
def test_guard_handles_absolute_cwd_path():
    ...
    original = sys.path[:]
    try:
        sys.path[:] = ["/home/user/tg-ws-proxy", "/opt/kopi", "/usr/lib"]
        kopi_bootstrap.harden_import_path(src_root="/opt/kopi")
        assert sys.path[0] == "/opt/kopi"
        assert sys.path.index("/opt/kopi") < sys.path.index(
