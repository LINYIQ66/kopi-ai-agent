...
VERSION_FILE = REPO_ROOT / "kopi_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level KOPI_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (kopi journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "kopi.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.kopi/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows kopi-shim resolution + prefer --update on recovery; #52378)
    ...
    "274096618+kopi-ai-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "hermes@nousresearch.com": "Kopi Ai Agent Pte Ltd",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on kopi mcp add)
    ...
    "teknium@kopi-ai-agent": "teknium1",
    ...
    "github@nadyakopi.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (KOPI_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (KOPI_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: kopi-eval community link)
    ...
    "vigo@kopi": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level KOPI_HOME state files)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"kopi-ai-agent[acp]=={semver}"
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/LINYIQ66/kopi-ai-agent",
    ...
    lines.append(f"# KOPI AI AGENT v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for KOPI AI AGENT. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="KOPI AI AGENT Release Tool")
    ...
    print(f"{'='*60}")
    print("  KOPI AI AGENT Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"KOPI AI AGENT v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"KOPI AI AGENT v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'KOPI AI AGENT v{new_version} ({calver_date})' "
