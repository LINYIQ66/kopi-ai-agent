...
3. Sets `model.provider: nous` in `~/.kopi/config.yaml`
...
kopi setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
kopi auth add nous --type oauth
# Then re-run `kopi setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "KOPI AI AGENT release notes" and summarize the top 3 hits.
```
...
`kopi setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
These rows appear in `kopi tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Hermes runs the Portal login inline (without changing your inference provider or your other tools).
...
```bash
kopi cron create "every day at 9am" \
...
For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.kopi/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `kopi portal info` shows "not logged in" after `kopi setup --portal`
...
```bash
kopi config set model.provider nous
```
...
Re-verify with `kopi portal info`.
...
If a model is genuinely unavailable, [open an issue](https://github.com/LINYIQ66/kopi-ai-agent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`kopi portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Hermes profiles where you're using the wrong one (check `kopi profile list`)

### Want to revoke and start clean

```bash
kopi auth logout nous       # wipes the local refresh token
