...
function pushNow(): void {
  window.kopiDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.kopiDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.kopiDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.kopiDesktop?.petOverlay?.close()
}
