# nix/nixosModules.nix — NixOS module for kopi-ai-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: kopi runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $KOPI_HOME/.env
# and read by kopi at startup — no container recreation needed for env changes.
#
# Tool resolution: the kopi wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.kopi-ai-agent;
    effectivePackage =
    ...
    kopi-ai-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;
    ...
    # KOPI_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "hermes-documents" { } (
      ...
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'KOPI_DOC_EOF'\n${value}\nKOPI_DOC_EOF"
        ) cfg.documents
      )
    );
    ...
    containerEntrypoint = pkgs.writeShellScript "hermes-container-entrypoint" ''
      ...
      KOPI_GID="''${KOPI_GID:?KOPI_GID must be set}"

      # ── Group: ensure a group with GID=$KOPI_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$KOPI_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        ...
        if command -v groupadd >/dev/null 2>&1; then
          ...
          addgroup -g "$KOPI_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$KOPI_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$KOPI_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        ...
        if command -v useradd >/dev/null 2>&1; then
          ...
          adduser -u "$KOPI_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$KOPI_UID:$KOPI_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure KOPI_HOME is owned by the target user.
      ...
      if [ -n "''${KOPI_HOME:-}" ] && [ -d "$KOPI_HOME" ]; then
        find "$KOPI_HOME" \! -user "$KOPI_UID" -exec chown "$KOPI_UID:$KOPI_GID" {} +
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$KOPI_UID" --regid="$KOPI_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $KOPI_HOME/.env.
    ...
    # Default: /var/lib/kopi/workspace → /data/workspace.
  ...
  in {
    options.services.kopi-ai-agent = with lib; {
      ...
      package = mkOption {
        type = types.package;
        default = kopi-ai-agent;
        description = "The kopi-ai-agent package to use.";
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $KOPI_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $KOPI_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `kopi gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the kopi user's per-user profile
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          kopi_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as kopi (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in kopi-ai-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the kopi CLI to environment.systemPackages and export
          KOPI_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.kopi symlink to the service
            stateDir. These users are automatically added to the kopi group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.kopi-ai-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the kopi CLI to system PATH and export KOPI_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.kopi/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.KOPI_HOME = "${cfg.stateDir}/.kopi";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.kopi-ai-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the kopi user's per-user profile so the
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.kopi        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.kopi/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.kopi/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.kopi/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.kopi/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.kopi/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      # ── Activation: link config + auth + documents ────────────────────
      {
        system.activationScripts."kopi-ai-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          ...
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.kopi ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.kopi ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.kopi -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.kopi/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.kopi/$_subdir"
            chmod 2770 "${cfg.stateDir}/.kopi/$_subdir"
            find "${cfg.stateDir}/.kopi/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.kopi/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.kopi/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.kopi/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.kopi/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.kopi/.managed
          chmod 0644 ${cfg.stateDir}/.kopi/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.kopi/.container-mode <<'KOPI_CONTAINER_MODE_EOF'
    ...
    KOPI_CONTAINER_MODE_EOF
          ...
          '' else ''
            rm -f ${cfg.stateDir}/.kopi/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.kopi";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.kopi" ]; then
                  rm -f "${symlinkPath}"
                  echo "kopi-ai-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.kopi -> stateDir/.kopi for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.kopi";
                target = "${cfg.stateDir}/.kopi";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "kopi-ai-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.kopi/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.kopi/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.kopi/auth.json
              fi
            ''}
          ''}
    ...
    KOPI_NIX_ENV_EOF
        ...
        find ${cfg.stateDir}/.kopi/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.kopi/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.kopi/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.kopi-ai-agent = {
          ...
          serviceConfig = {
            ...
            # $KOPI_HOME/.env by the activation script. load_kopi_dotenv()
            ...
            # so interactive users in the kopi group can read/write them.
            UMask = "0007";
        ...
        systemd.services.kopi-ai-agent = {
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              KOPI_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              KOPI_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env KOPI_UID="$KOPI_UID" \
                --env KOPI_GID="$KOPI_GID" \
                --env KOPI_HOME=${containerDataDir}/.kopi \
                --env KOPI_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/kopi gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
