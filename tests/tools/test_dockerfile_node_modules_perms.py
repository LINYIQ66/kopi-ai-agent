...
Older images made /opt/kopi/ui-tui and /opt/kopi/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV KOPI_TUI_DIR=/opt/kopi/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R kopi:kopi /opt/kopi/ui-tui" not in text
    assert "chown -R kopi:kopi /opt/kopi/node_modules" not in text
