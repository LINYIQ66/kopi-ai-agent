...
"""
    ...
    assert "Tyler Williams > Active Projects: KOPI AI AGENT" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `kopi cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # kopi_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.kopi/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "kopi_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    ...
    # All-lowercase matches → lowercase ``kopi``; this preserves the
    # real filesystem path ``~/.kopi`` (Hermes home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "kopi should always respond concisely"
...
def test_rebrand_text_replaces_legacy_bot_names():
    ...
    assert mod.rebrand_text("clawdbot prefers tabs") == "kopi prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Hermes was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "kopi uses Python"
...
def test_rebrand_text_preserves_filesystem_path_casing():
    ...
    rewrite to lowercase ``.kopi`` (the real Hermes home), not the broken
    ``.Hermes``.
    ...
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.kopi/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .kopi directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.kopi'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "kopi config path: ~/.kopi/"
