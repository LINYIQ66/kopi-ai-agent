...
Usage:
    python -m kopi_cli.main web          # Start on http://127.0.0.1:9119
    python -m kopi_cli.main web --port 8080
"""
...
from kopi_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
try:
    ...
    # running `kopi dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["KOPI_WEB_DIST"]) if "KOPI_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``kopi gateway run`` — but the
    desktop app spawns a ``kopi dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same KOPI_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import kopi_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from kopi_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire kopi_cli.gateway import into a background thread so the event
    # loop is not blocked and KOPI_DASHBOARD_READY fires without delay.
...
# KOPI_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
#
# This list is defined in ``kopi_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from kopi_cli.dashboard_auth.public_paths import (
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``kopi-0day``
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from kopi_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from kopi_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from kopi_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.refresh_cua_driver": {
        "type": "bool",
        "description": (
            "Refresh an already-installed cua-driver during kopi update. "
...
class ModelAssignment(BaseModel):
    ...
    # ``kopi model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from kopi_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "KOPI_DASHBOARD_FILES_ROOT"
...
# KOPI_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to KOPI_HOME.
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no KOPI_HOME resolution, so it blocks these trees wherever
...
def _default_kopi_root_is_opt_data() -> bool:
    raw = os.environ.get("KOPI_HOME", "").strip()
    if not raw:
        return False
    try:
        from kopi_constants import get_default_kopi_root

        root = get_default_kopi_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``kopi update``.
    ...
    if _default_kopi_root_is_opt_data():
        return True
    try:
        from kopi_constants import is_container
...
async def fs_write_text(payload: FsWriteText):
    ...
    tmp = target.with_name(f".{target.name}.kopi-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``kopi_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from kopi_cli import web_git as _web_git  # noqa: E402
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from kopi_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_kopi_home() at call time
    ...
    try:
        ...
        # Windows install the first import of kopi_cli.gateway blocks the
        ...
        try:
            from kopi_cli.dashboard_auth import list_providers as _list_providers
        ...
        try:
            from kopi_cli.auth import get_nous_session_validity
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the kopi home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_kopi_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_kopi_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from kopi_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from kopi_cli.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_kopi_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_kopi_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_kopi_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from kopi_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.kopi/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_kopi_home() / "logs"
...
def _spawn_kopi_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``kopi <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``kopi_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "kopi_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "KOPI_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["kopi", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``kopi gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``kopi gateway restart`` children race each other on the
    ...
    return _spawn_kopi_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``kopi gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``KOPI_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
async def update_hermes():
    ...
    if _dashboard_local_update_managed_externally():
        ...
        _record_completed_action("kopi-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "kopi-update",
    ...
    try:
        proc = _spawn_kopi_action(["update"], "kopi-update")
    except Exception as exc:
        _log.exception("Failed to spawn kopi update")
...
async def check_kopi_update(force: bool = False):
    ...
    ``POST /api/kopi/update`` actually runs ``kopi update``.
    ...
    try:
        ...
        if force:
            try:
                (get_kopi_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.kopi/config.yaml`` under ``tts.``.
    """
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        kopi_home / f"{name}.json",
        kopi_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_kopi_home()))
            return
...
# in kopi_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(
    ...
    """
    try:
        from kopi_cli.inventory import build_models_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `kopi model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from kopi_cli.auth import get_provider_auth_state
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from kopi_cli.moa_config import normalize_moa_config
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.kopi/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from kopi_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (kopi_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from kopi_cli.nous_subscription import apply_nous_managed_defaults
                from kopi_cli.tools_config import _get_platform_tools
        ...
        # endpoint, mirroring the ``kopi model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from kopi_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``kopi model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from kopi_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from kopi_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `kopi model` provider — would be
        ...
        # Vertex — a `kopi model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `kopi model` picker uses (not desktop-only prefix guesses).
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in kopi gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in kopi gateway setup",
...
def _whatsapp_session_path() -> Path:
    ...
    return get_kopi_dir("platforms/whatsapp/session", "whatsapp/session")
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's KOPI_HOME.
...
# the canonical ``kopi auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_kopi_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_kopi_oauth_credentials,
            _get_kopi_oauth_file,
        )
    except ImportError:
        read_kopi_oauth_credentials = None  # type: ignore
        _get_kopi_oauth_file = None  # type: ignore

    kopi_creds = None
    if read_kopi_oauth_credentials:
        try:
            kopi_creds = read_kopi_oauth_credentials()
        except Exception:
            kopi_creds = None
    if kopi_creds and kopi_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "kopi_pkce",
    ...
    try:
        ...
        pass
    try:
        from kopi_cli.config import get_env_value
    except ImportError:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "kopi auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "kopi auth add minimax-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "kopi auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from kopi_cli import auth as hauth
        if provider_id == "nous":
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``kopi model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `kopi model` universe (zero-edit for new plugins).
    try:
        from kopi_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"kopi auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("kopi_pkce", "claude_code", ...)
    ...
    sync with the `kopi model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_kopi_oauth_file
                oauth_file = _get_kopi_oauth_file()
            ...
            try:
                from kopi_cli.auth import clear_provider_auth
        ...
        try:
            from kopi_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.kopi/.anthropic_oauth.json AND credential pool
...
# Guarded so kopi web still starts if anthropic_adapter is unavailable;
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("KOPI_PORTAL_BASE_URL")
    ...
    if provider_id == "xai-oauth":
        from kopi_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from kopi_cli.auth import persist_nous_credentials
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # prior ``kopi auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``kopi model`` re-login path in _login_xai_oauth).
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from kopi_cli.auth import _save_codex_tokens
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `kopi sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `kopi sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_kopi_home()
    db = _open_session_db_for_profile(body.profile)
    try:
        filters = dict(
...
async def get_logs(
    ...
    from kopi_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_kopi_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from kopi_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, KOPI_HOME)."""
    from kopi_cli import profiles as profiles_mod
...
#
# Wraps the same config data layer the CLI uses (kopi_cli.mcp_config), so
# servers managed here show up under `kopi mcp list` and vice versa.  Secrets
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from kopi_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from kopi_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from kopi_cli.mcp_config import (
        _get_mcp_servers,
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the KOPI_HOME
...
async def auth_mcp_server(name: str, profile: Optional[str] = None):
    ...
    Mirrors ``kopi mcp login``: wipe cached OAuth state so the probe forces
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds KOPI_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (kopi_cli.webhook); the webhook
...
# spawn the real `kopi gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_kopi_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_kopi_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from kopi_cli.auth import read_credential_pool
...
async def run_doctor():
    try:
        proc = _spawn_kopi_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_kopi_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_kopi_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"kopi-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `kopi import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_kopi_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from kopi_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from kopi_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <kopi_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_kopi_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_kopi_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``kopi`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_kopi_action`` tracks one process/log per name, so a shared
...
# Human-readable labels for each hub source id (matches `kopi skills search`
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from kopi_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`kopi -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the KOPI_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_kopi_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "kopi setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local KOPI_HOME override so the write lands in the target
    ...
    """
    from kopi_constants import set_kopi_home_override, reset_kopi_home_override

    token = set_kopi_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local KOPI_HOME override (same mechanism as
    ...
    """
    from kopi_constants import set_kopi_home_override, reset_kopi_home_override
    from kopi_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the KOPI_HOME override. Returns the
    number of skills newly disabled.
    """
    from kopi_constants import set_kopi_home_override, reset_kopi_home_override
    from kopi_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's KOPI_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``kopi profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from KOPI_HOME).
    """
    from kopi_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``kopi profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the KOPI_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``kopi profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from kopi_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_kopi_home()`` at call
       time — the context-local override from ``set_kopi_home_override``
    ...
    retargeted to the *current* ``get_kopi_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a KOPI_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_kopi_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_kopi_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from kopi_cli.skills_config import get_disabled_skills
...
async def get_toolsets(profile: Optional[str] = None):
    from kopi_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``kopi tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``kopi tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `kopi tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``kopi tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.kopi/.env`` via ``save_env_value`` —
    the same store ``kopi tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``kopi tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``kopi -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same KOPI_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``kopi computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``kopi --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from kopi_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from kopi_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from kopi_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from kopi_cli.dashboard_auth.audit import AuditEvent, audit_log
        from kopi_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``kopi --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``KOPI_TUI_RESUME`` env var —
    matching what ``kopi_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``KOPI_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``KOPI_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``KOPI_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``KOPI_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_kopi_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``kopi -p <name>`` performs. The in-process
    ``KOPI_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from kopi_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("KOPI_TUI_DISABLE_MOUSE", "1")
    env.setdefault("KOPI_TUI_INLINE", "1")
    ...
    env["KOPI_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["KOPI_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["KOPI_TUI_RESUME"] = resume

    if sidecar_url:
        env["KOPI_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile KOPI_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["KOPI_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``KOPI_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("KOPI_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from kopi_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="kopi-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="kopi-console",
                )
...
def _dashboard_console_context() -> str:
    ...
    return "hosted" if _default_kopi_root_is_opt_data() else "local"
...
# KOPI_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`kopi_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from kopi_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/kopi/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /kopi`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__KOPI_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `kopi serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("KOPI_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (kopi serve): web UI disabled — use "
            "`kopi dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/kopi``)
        ...
        ``__KOPI_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__KOPI_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__KOPI_BASE_PATH__="{prefix}";'
                f"window.__KOPI_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__KOPI_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__KOPI_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__KOPI_BASE_PATH__="{prefix}";'
                f"window.__KOPI_AUTH_REQUIRED__={gated_js};"
                f"</script>"
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.kopi/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.kopi/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_kopi_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.kopi/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as kopi_cli.plugins):
    1. User plugins:    ~/.kopi/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.kopi/plugins/  (only if KOPI_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from kopi_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_kopi_home() / "plugins", "user"),
    ...
    # ``kopi_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("KOPI_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".kopi" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    try:
        from kopi_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_kopi_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from kopi_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from kopi_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from kopi_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from kopi_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.kopi/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"kopi_dashboard_plugin_{plugin['name']}"
...
from kopi_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``KOPI_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("KOPI_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``KOPI_SERVE_HEADLESS``), so
    the banner announces the bind rather than a browser URL.
    """
    import uvicorn

    try:
        from kopi_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the kopi-0day MCP-persistence campaign abused unauthenticated public
    ...
    # KOPI_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            ready_token = "KOPI_BACKEND_READY" if headless else "KOPI_DASHBOARD_READY"
