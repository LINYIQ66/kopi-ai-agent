...
| Related skills | [`systematic-debugging`](/docs/user-guide/skills/bundled/software-development/software-development-systematic-debugging), [`node-inspect-debugger`](/docs/user-guide/skills/bundled/software-development/software-development-node-inspect-debugger), `debugging-kopi-tui-commands` |
...
- A long-running process (kopi gateway, tui_gateway) misbehaves and you can't restart it
...
The kopi test runner and pytest both support this:
...
```bash
source /home/bb/kopi-ai-agent/.venv/bin/activate
pip install debugpy
```
...
```json
{
  ...
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/kopi-ai-agent" }
  ]
}
```
...
Easiest: add `breakpoint()` near the suspect line, then run `kopi` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `kopi --tui`)
...
debugpy.wait_for_client()
```
Start `kopi --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
