...
sidebar_position: 11
title: "飞书 / Lark"
description: "将 KOPI AI AGENT 配置为飞书或 Lark 机器人"
---

# 飞书 / Lark 配置

KOPI AI AGENT 可作为全功能机器人与飞书和 Lark 集成。连接后，你可以在私信或群聊中与 Agent 对话，在 home chat 中接收 cron job 结果，并通过标准 gateway 流程发送文本、图片、音频和文件附件。
...
4. 为应用开启 **机器人** 能力。
5. 运行 `kopi gateway setup`，选择 **飞书 / Lark**，并在提示时输入凭据。
...
Gateway 驱动的更新提示使用原生飞书 `Yes` / `No` 卡片，而非回退到纯文本回复。当 `kopi update --gateway` 需要确认时，适配器将所选答案记录到 Hermes 的 `.update_response` 文件中，并将卡片内联替换为已解决状态。
...
规则存储在 `~/.kopi/feishu_comment_rules.json`（pairing 授权存储在 `~/.kopi/feishu_comment_pairing.json`），支持基于 mtime 缓存的热重载——编辑后无需重启 gateway，下一个评论事件即生效。
...
| 设置 | 环境变量 | 默认值 |
|---------|---------|---------|
| 静默期 | `KOPI_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| 每批最大消息数 | `KOPI_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| 每批最大字符数 | `KOPI_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| 设置 | 环境变量 | 默认值 |
|---------|---------|---------|
| 静默期 | `KOPI_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
入站消息使用消息 ID 去重，TTL 为 24 小时。去重状态持久化到 `~/.kopi/feishu_seen_message_ids.json`，重启后仍有效。

| 设置 | 环境变量 | 默认值 |
|---------|---------|---------|
| 缓存大小 | `KOPI_FEISHU_DEDUP_CACHE_SIZE` | 2048 条 |
...
| `KOPI_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | 最大去重消息 ID 追踪数量 |
| `KOPI_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | 文本突发防抖静默期 |
| `KOPI_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | 每批文本合并的最大消息数 |
| `KOPI_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | 每批文本合并的最大字符数 |
| `KOPI_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | 媒体突发防抖静默期 |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | 设置两个环境变量，或通过 `kopi gateway setup` 配置 |
...
飞书 / Lark 使用 `kopi-feishu` 平台预设，包含与 Telegram 及其他基于 gateway 的消息平台相同的核心工具。