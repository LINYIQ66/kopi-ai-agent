...
> Run `kopi gateway setup` and pick **SimpleX** for a guided walk-through.
...
| `KOPI_SIMPLEX_TEXT_BATCH_DELAY` | Optional | Quiet-period seconds (default: `0.8`) used to concatenate rapid-fire inbound text messages into one event |
...
2. Use **DM pairing** — send any message to the bot and it will reply with a pairing code. Enter that code via `kopi pairing approve simplex <CODE>`.
...
`simplex:group:12` as a cron `deliver=` target or in a `kopi send` call.
...
Or target a specific contact via the cron job's `deliver:` field, or from a shell script with the [`kopi send` CLI](/guides/pipe-script-output):

```bash
kopi send simplex:<contact-id> "Done!"
```
