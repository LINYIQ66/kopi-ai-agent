...
are cached in-process and on disk under ``<kopi_home>/cache/op_cache.json``
so back-to-back short-lived ``kopi`` invocations don't re-shell ``op`` for
...
# In-process cache.  The key folds in str(home_path) so a KOPI_HOME switch
...
def apply_onepassword_secrets(
    *,
    ...
    Called by ``load_kopi_dotenv()`` after the .env files have loaded.
