...
:::tip
At creation, an unpinned job (one you don't give an explicit `provider`/`model`) follows the global default selected by `kopi model` — and Hermes **snapshots** that provider and model on the job. If the global default later changes, the job **fails closed**: it skips the run, makes no inference call, and sends an alert telling you to pin the provider/model explicitly (`cronjob action=update job_id=… provider=… model=…`) to proceed. This prevents an unattended job from silently inheriting a switch to a paid provider/model and spending money you didn't intend (#44585). To make a job deliberately track your global default, pin it to the new values after changing them. `kopi setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
kopi cron create "every 2h" "Check server status"
kopi cron create "every 1h" "Summarize new feed items" --skill blogwatcher
kopi cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
kopi cron create "every 1d at 09:00" \
...
```bash
kopi cron edit <job_id> --schedule "every 4h"
kopi cron edit <job_id> --prompt "Use the revised task"
kopi cron edit <job_id> --skill blogwatcher --skill maps
kopi cron edit <job_id> --add-skill maps
kopi cron edit <job_id> --remove-skill blogwatcher
kopi cron edit <job_id> --clear-skills
```
...
```bash
kopi cron list
kopi cron pause <job_id_or_name>
kopi cron resume <job_id_or_name>
kopi cron run <job_id_or_name>
kopi cron remove <job_id_or_name>
kopi cron edit <job_id_or_name> [...flags]
kopi cron status
kopi cron tick
```
...
```bash
kopi gateway install     # Install as a user service
sudo kopi gateway install --system   # Linux: boot-time system service for servers
kopi gateway             # Or run in foreground
...
1. loads jobs from `~/.kopi/cron/jobs.json`
...
A file lock at `~/.kopi/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.kopi/cron/output/`) | Default on CLI |
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.kopi/cron/output/`), but no message is sent to the delivery target.
...
Or set the `KOPI_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.kopi/scripts/` (same sandboxing rule as the pre-run script gate).
...
Hermes will write the check script to `~/.kopi/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.kopi/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.kopi/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.kopi/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.kopi/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Hermes reads Job 1's most recent output from `~/.kopi/cron/output/{job1_id}/*.md`
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `kopi tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `kopi tools` cron-platform config wins; otherwise Hermes falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.
...
```bash
#!/bin/bash
# ~/.kopi/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.kopi/scripts/.feed-changed.last"
...
:::tip
Hermes's own `~/.kopi/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/LINYIQ66/kopi-ai-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.kopi/cron/jobs.json`. Output from job runs is saved to `~/.kopi/cron/output/{job_id}/{timestamp}.md`.
