...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("KOPI_YOLO_MODE", ""))
...
# os.environ["KOPI_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export KOPI_INTERACTIVE.
_kopi_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "kopi_interactive",
    default=None,
)


def set_kopi_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["KOPI_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``KOPI_INTERACTIVE`` env var for legacy callers.
    """
    return _kopi_interactive_ctx.set("1" if interactive else "")


def reset_kopi_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_kopi_interactive_context`."""
    _kopi_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``KOPI_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _kopi_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("KOPI_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("KOPI_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("KOPI_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("KOPI_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set KOPI_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind KOPI_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds KOPI_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("KOPI_CRON_SESSION"):
        return False
    if env_var_enabled("KOPI_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $KOPI_HOME, or by the resolved absolute
# active profile home path such as /home/kopi/.kopi/config.yaml. The
# resolved-absolute form is folded into the ~/.kopi/ patterns at detection
...
# go stale when KOPI_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_KOPI_ENV_PATH = (
    r'(?:~\/\.kopi/|'
    r'(?:\$home|\$\{home\})/\.kopi/|'
    r'(?:\$kopi_home|\$\{kopi_home\})/)'
    r'\.env\b'
)
# ~/.kopi/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _KOPI_ENV_PATH; matches the KOPI_HOME override form as
# well as ~/.kopi/.
_KOPI_CONFIG_PATH = (
    r'(?:~\/\.kopi/|'
    r'(?:\$home|\$\{home\})/\.kopi/|'
...
DANGEROUS_PATTERNS = [
    ...
    # `kopi` and `gateway` (e.g. `kopi -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bhermes\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart kopi gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "kopi update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `kopi gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart kopi-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart kopi-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(hermes|gateway|cli\.py)\b', "kill kopi/gateway process (self-termination)"),
    ...
    # the `kopi gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.kopi.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(hermes|ai\.kopi)\b', "stop/restart kopi launchd service (kills running agents)"),
    ...
    # ~/.kopi/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Hermes-managed security file (~/.kopi/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_KOPI_CONFIG_PATH}|{_KOPI_ENV_PATH})', "in-place edit of Hermes config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_KOPI_CONFIG_PATH}|{_KOPI_ENV_PATH})', "in-place edit of Hermes config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_KOPI_CONFIG_PATH}|{_KOPI_ENV_PATH})', "in-place edit of Hermes config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.kopi/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / KOPI_HOME even when those are set after this module is
    ...
    command = _rewrite_resolved_kopi_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / KOPI_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.kopi``); the matched
...
def _rewrite_resolved_kopi_home(command: str) -> str:
    """Rewrite the resolved absolute Hermes home prefix to ``~/.kopi/``.

    Resolves the active ``KOPI_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.kopi/`` so the static ``_KOPI_CONFIG_PATH`` / ``_KOPI_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/kopi/.kopi/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$KOPI_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from kopi_constants import get_kopi_home
        home = get_kopi_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.kopi")
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["KOPI_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "KOPI_SPINNER_PAUSE" in os.environ:
            del os.environ["KOPI_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active() -> bool:
    ...
    into one place:
      - process-scoped ``--yolo`` / ``KOPI_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            KOPI_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if env_var_enabled("KOPI_CRON_SESSION"):
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set KOPI_INTERACTIVE or "
                "KOPI_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set KOPI_INTERACTIVE or "
            "KOPI_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("KOPI_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``KOPI_INTERACTIVE``) fails CLOSED — a plugin-
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("KOPI_EXEC_ASK")
        ...
        if env_var_enabled("KOPI_CRON_SESSION"):
            if _get_cron_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from kopi_cli.config import load_config as _load_cfg
