# OpenAI-Compatible API Server for KOPI AI AGENT
...
SSE streaming. By exposing this endpoint, kopi-ai-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  kopi-ai-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.kopi/.env`
2. Run `kopi gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with kopi-ai-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns kopi-ai-agent as a model) |
...
Authorization: Bearer kopi-api-key-here
...
But kopi-ai-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "kopi-ai-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured kopi-ai-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of kopi-ai-agent's core prompt
...
Once implemented, kopi-ai-agent works as a drop-in backend for:
