"""Tests for kopi_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert name.startswith("kopi_")
        assert name.endswith("_bot")
        assert len(name) == len("kopi_") + 16 + len("_bot")
        assert len(name) <= 32
    ...
    def test_slug_uses_telegram_safe_base32_chars(self):
        name = generate_bot_username()
        slug = name.removeprefix("kopi_").removesuffix("_bot")
        assert len(slug) == 16
...
class TestPollForToken:
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "kopi_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "kopi_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("kopi_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("kopi_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("kopi_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("kopi_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("kopi_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "kopi_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
    ...
    def test_eventual_success(self):
        ...
        with patch("kopi_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("kopi_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from kopi_cli.setup import _setup_telegram_auto

        assert callable(_setup_telegram_auto)

    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        from kopi_cli import setup

        seen = {}
        profile_home = tmp_path / ".kopi" / "profiles" / "oracle"
        profile_home.mkdir(parents=True)

        monkeypatch.setattr(setup, "get_kopi_home", lambda: profile_home)
        ...
        monkeypatch.setattr(
            "kopi_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from kopi_cli.setup import _profile_name_from_kopi_home
