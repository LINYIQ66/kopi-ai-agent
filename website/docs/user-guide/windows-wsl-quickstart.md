...
description: "Run KOPI AI AGENT on Windows via WSL2 — setup, filesystem access between Windows and Linux, networking, and common pitfalls"
...
KOPI AI AGENT now supports **both** native Windows and WSL2.  This page covers the WSL2 path; for the native PowerShell install see the dedicated **[Windows (Native) Guide](./windows-native.md)**.
...
The kopi gateway (and anything else you want to keep running) is easier to manage with systemd. On modern WSL, enable it once inside your distro:
...
- Your Hermes install (`~/.kopi/`) — Linux side. The installer already does this.
...
Clone inside WSL. Always, unless you have a specific reason not to. A typical Hermes workflow (`kopi chat`, tool calls that `rg`/`ripgrep` the repo, file watchers, background gateway) will be dramatically faster and more reliable against `~/code/myrepo` than `/mnt/c/Users/you/myrepo`.

One exception: **MCP bridges that launch Windows binaries.** If you're using `chrome-devtools-mcp` through `cmd.exe` (see [MCP guide: WSL → Windows Chrome](/guides/use-mcp-with-hermes#wsl2-bridge-kopi-in-wsl-to-windows-chrome)), Windows may complain with a `UNC` warning if Hermes's current working directory is `~`. In that case, start Hermes from somewhere under `/mnt/c/` so the Windows process has a drive-letter cwd.
...
- Using the **OpenAI-compatible API server** (exposed by `kopi gateway` when `API_SERVER_ENABLED=true`) from a Windows-side tool. See the [API Server feature page](/user-guide/features/api-server).
...
home directory, and launches Hermes. If `kopi` is not on PATH yet, open WSL
...
If you enabled systemd per the setup section above, `kopi gateway` and the API server work the way they do on any Linux machine. Use the gateway setup wizard:
...
**Massive slowness on `git status` / `kopi chat` in a repo.**
...
**`kopi` not found after running the installer.**
...
- **[MCP guide → WSL → Windows Chrome](/guides/use-mcp-with-hermes#wsl2-bridge-kopi-in-wsl-to-windows-chrome)** — controlling your signed-in Windows Chrome from Hermes in WSL.
