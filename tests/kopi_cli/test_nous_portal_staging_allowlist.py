...
``KOPI_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` in its
...
class TestPortalEnvOverrideHelper:
    def test_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("KOPI_PORTAL_BASE_URL", raising=False)
        monkeypatch.delenv("NOUS_PORTAL_BASE_URL", raising=False)
        assert _nous_portal_env_override() is None

    def test_kopi_portal_base_url_wins(self, monkeypatch):
        monkeypatch.setenv(
            "KOPI_PORTAL_BASE_URL", "https://portal.staging-nousresearch.com/"
        )
        monkeypatch.delenv("NOUS_PORTAL_BASE_URL", raising=False)
        assert (
...
class TestResolveAccessTokenEnvOverrideWins:
    ...
    def _run_and_capture(self, monkeypatch, auth):
        ...
        caplog_records = []
        logger = logging.getLogger("kopi_cli.auth")
