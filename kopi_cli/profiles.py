...
Each profile is a fully independent KOPI_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.kopi/profiles/<name>/`` by default.

The "default" profile is ``~/.kopi`` itself — backward compatible,
zero migration needed.

Usage::

    kopi profile create coder          # fresh profile + bundled skills
    kopi profile create coder --clone  # also copy config, .env, SOUL.md, skills
    kopi profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    kopi -p coder chat                 # or via flag
    kopi profile use coder             # set as sticky default
    kopi profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See kopi_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.kopi``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   kopi-ai-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `kopi profile create --no-skills`.  When present in
...
# `kopi skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         ...
         (``~/.kopi``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_kopi_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.kopi) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "kopi-ai-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when KOPI_HOME equals the
...
# artifacts* at the root of KOPI_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``kopi_constants``.
...
_RESERVED_NAMES = frozenset({
    "kopi", "default", "test", "tmp", "root", "sudo",
})

# Hermes subcommands that cannot be used as profile names/aliases
_KOPI_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the kopi root, NOT to the current KOPI_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where KOPI_HOME points outside
    ``~/.kopi``, profiles live under ``KOPI_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_kopi_home() / "profiles"


def _get_default_kopi_home() -> Path:
    """Return the default (pre-profile) KOPI_HOME path.

    In standard deployments this is ``~/.kopi``.
    In Docker/custom deployments where KOPI_HOME is outside ``~/.kopi``
    (e.g. ``/opt/data``), returns KOPI_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_kopi_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its KOPI_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_kopi_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, kopi subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _KOPI_SUBCOMMANDS:
        return f"'{canon}' conflicts with a kopi subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            kopi_exe = shutil.which("kopi") or "kopi"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(kopi_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    try:
        ...
        # not be migrated. The next `kopi doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``kopi -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``kopi profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``kopi -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``kopi profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``KOPI_HOME``.
    """
    try:
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_kopi_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, kopi_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own KOPI_HOME.
    ...
    The returned ``kopi_home`` is the path to pass to
    ``set_kopi_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_kopi_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``kopi update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.kopi)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.kopi/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `kopi -p <name> auth add`) had no file until first
    ...
    # Write the opt-out marker so seed_profile_skills() and `kopi update`'s
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `kopi profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `kopi -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches KOPI_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``kopi profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "KOPI_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_kopi_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    ...
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``KOPI_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `kopi -p <canon> profile
    ...
    kopi_markers = ("kopi_cli.main", "kopi-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # a resolved executable named `kopi`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_kopi = (
                any(marker in joined for marker in kopi_markers)
                or exe_name == "kopi"
                or exe_name.startswith("kopi")
            )
            if not is_hermes:
                continue
            ...
            # ...or by KOPI_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("KOPI_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.kopi).\n"
            "To remove everything, use: kopi uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_kopi_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``KOPI_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from kopi_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set KOPI_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("KOPI_HOME")
    try:
        os.environ["KOPI_HOME"] = str(profile_dir)
        from kopi_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["KOPI_HOME"] = old_home
        elif "KOPI_HOME" in os.environ:
            del os.environ["KOPI_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.kopi/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: kopi profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from KOPI_HOME.

    Returns ``"default"`` if KOPI_HOME is not set or points to ``~/.kopi``.
    Returns the profile name if KOPI_HOME points into ``~/.kopi/profiles/<name>``.
    Returns ``"custom"`` if KOPI_HOME is set to an unrecognized path.
    """
    from kopi_constants import get_kopi_home
    kopi_home = get_kopi_home()
    resolved = kopi_home.resolve()

    default_resolved = _get_default_kopi_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      KOPI_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Hermes file the
      user may have lying alongside KOPI_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.kopi itself — its parent is ~/ and its
        # directory name is ".kopi", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: kopi profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.kopi itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.kopi). "
            "Specify a different name: kopi profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="kopi_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"kopi_{old_name}"
    legacy_old_host = f"kopi.{old_name}"
    new_host = f"kopi_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_kopi_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("kopi_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a KOPI_HOME path string.

    Called early in the CLI entry point, before any kopi modules
    are imported, to set the KOPI_HOME environment variable.
    """
