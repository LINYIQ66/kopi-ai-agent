...
```bash
kopi gateway setup        # 交互式配置所有消息平台
```
...
```bash
kopi gateway              # 在前台运行
kopi gateway setup        # 交互式配置消息平台
kopi gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo kopi gateway install --system   # 仅 Linux：安装开机启动的系统服务
kopi gateway start        # 启动默认服务
kopi gateway stop         # 停止默认服务
kopi gateway status       # 检查默认服务状态
kopi gateway status --system         # 仅 Linux：显式检查系统服务
```
...
| `/update` | 将 KOPI AI AGENT 更新至最新版本 |
| `/help` | 显示可用命令 |
...
**默认情况下会话永不自动重置** —— 上下文会一直保留，直到你手动 `/reset` 或触发上下文压缩。如果你希望会话自动重置，可在 `~/.kopi/config.yaml` 的 `session_reset` 部分选择启用：
...
在 `~/.kopi/gateway.json` 中配置各平台的覆盖设置：
...
# 其他配对命令：
kopi pairing list          # 查看待审核和已批准的用户
kopi pairing revoke telegram 123456789  # 撤销访问权限
```
...
在 `~/.kopi/config.yaml` 中控制显示多少工具活动信息：
...
当运行后台会话的 agent 使用 `terminal(background=true)` 启动长时间运行的进程（服务器、构建等）时，网关可以向你的聊天推送状态更新。通过 `~/.kopi/config.yaml` 中的 `display.background_process_notifications` 控制：
...
kopi gateway start                 # 启动服务
kopi gateway stop                  # 停止服务
kopi gateway status                # 检查状态
journalctl --user -u kopi-gateway -f  # 查看日志
...
:::info 多个安装
如果你在同一台机器上运行多个 Hermes 安装（使用不同的 `KOPI_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.kopi` 使用 `kopi-gateway`；其他安装使用 `kopi-gateway-<hash>`。`kopi gateway` 命令会自动针对当前 `KOPI_HOME` 对应的正确服务。
:::

### macOS（launchd）

```bash
kopi gateway install               # 安装为 launchd agent
kopi gateway start                 # 启动服务
kopi gateway stop                  # 停止服务
kopi gateway status                # 检查状态
tail -f ~/.kopi/logs/gateway.log   # 查看日志
```

生成的 plist 文件位于 `~/Library/LaunchAgents/ai.kopi.gateway.plist`。它包含三个环境变量：
...
- **KOPI_HOME** — 将网关限定到你的 Hermes 安装。

:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `kopi gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::

:::info 多个安装
与 Linux systemd 服务类似，每个 `KOPI_HOME` 目录都有自己的 launchd 标签。默认的 `~/.kopi` 使用 `ai.kopi.gateway`；其他安装使用 `ai.kopi.gateway-<suffix>`。
:::
...
|----------|---------|--------------|
| CLI | `kopi-cli` | 完全访问 |
| Telegram | `kopi-telegram` | 完整工具，包括终端 |
| Discord | `kopi-discord` | 完整工具，包括终端 |
| WhatsApp | `kopi-whatsapp` | 完整工具，包括终端 |
| Slack | `kopi-slack` | 完整工具，包括终端 |
| Google Chat | `kopi-google_chat` | 完整工具，包括终端 |
| Signal | `kopi-signal` | 完整工具，包括终端 |
| SMS | `kopi-sms` | 完整工具，包括终端 |
| Email | `kopi-email` | 完整工具，包括终端 |
| Home Assistant | `kopi-homeassistant` | 完整工具 + HA 设备控制（ha_list_entities、ha_get_state、ha_call_service、ha_list_services） |
| Mattermost | `kopi-mattermost` | 完整工具，包括终端 |
| Matrix | `kopi-matrix` | 完整工具，包括终端 |
| DingTalk | `kopi-dingtalk` | 完整工具，包括终端 |
| Feishu/Lark | `kopi-feishu` | 完整工具，包括终端 |
| WeCom | `kopi-wecom` | 完整工具，包括终端 |
| WeCom Callback | `kopi-wecom-callback` | 完整工具，包括终端 |
| Weixin | `kopi-weixin` | 完整工具，包括终端 |
| BlueBubbles | `kopi-bluebubbles` | 完整工具，包括终端 |
| QQBot | `kopi-qqbot` | 完整工具，包括终端 |
| Yuanbao | `kopi-yuanbao` | 完整工具，包括终端 |
| Microsoft Teams | `kopi-teams` | 完整工具，包括终端 |
| API Server | `kopi-api-server` | 完整工具（去除 `clarify`、`send_message`、`text_to_speech`——程序化访问没有交互用户） |
| Webhooks | `kopi-webhook` | 完整工具，包括终端 |
...
1. **网关日志**（`~/.kopi/logs/gateway.log` 或 systemd / launchd 单元日志）。搜索平台名称以及 `circuit breaker`、`paused` 或 `disabled`。触发事件包含失败次数和最后一个错误。
