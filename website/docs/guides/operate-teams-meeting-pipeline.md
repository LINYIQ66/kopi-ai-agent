...
```bash
kopi teams-pipeline token-health
kopi teams-pipeline token-health --force-refresh
```
...
```bash
kopi teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
kopi teams-pipeline maintain-subscriptions
kopi teams-pipeline maintain-subscriptions --dry-run
```
...
Hermes ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.kopi/scripts/`. First create the script:

```bash
mkdir -p ~/.kopi/scripts
cat > ~/.kopi/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec kopi teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.kopi/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
kopi cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/kopi-teams-pipeline-maintain.service`:
...
Type=oneshot
User=hermes
EnvironmentFile=/etc/kopi/env
ExecStart=/usr/local/bin/kopi teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/kopi-teams-pipeline-maintain.timer`:
...
sudo systemctl enable --now kopi-teams-pipeline-maintain.timer
systemctl list-timers kopi-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/kopi teams-pipeline maintain-subscriptions >> /var/log/kopi/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.kopi/.env` at the top of a wrapper script that crontab calls.
...
```bash
kopi teams-pipeline subscriptions   # should show expirationDateTime advanced
kopi teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
kopi teams-pipeline list
kopi teams-pipeline list --status failed
kopi teams-pipeline show <job-id>
```

### Replay a stored job

```bash
kopi teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
kopi teams-pipeline fetch --meeting-id <meeting-id>
kopi teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `kopi teams-pipeline maintain-subscriptions --dry-run`
- inspect `kopi teams-pipeline list --status failed`
...
- run `kopi teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `kopi teams-pipeline run`
...
- [ ] `kopi teams-pipeline validate` returns an OK snapshot
- [ ] `kopi teams-pipeline token-health --force-refresh` succeeds
