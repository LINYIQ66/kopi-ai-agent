...
``kopi gateway restart`` (or ``launchctl kickstart ai.kopi.gateway``
or ``systemctl restart kopi-gateway``).  When the cron fires, the
...
``kopi cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``kopi gateway``, ``launchctl ... kopi-gateway``,
``systemctl ... kopi-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_KOPI_GATEWAY=1``, and
``kopi gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `kopi gateway restart|stop` — the canonical foot-gun.
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<KOPI_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.kopi/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_kopi_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `kopi gateway restart` from a shell outside "
