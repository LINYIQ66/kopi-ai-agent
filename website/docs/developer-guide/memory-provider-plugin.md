...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          kopi_home (str): Active KOPI_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `kopi memory setup` | **Yes** |
| `save_config(values, kopi_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `kopi memory setup`:
...
Every field in `get_config_schema()` is prompted during `kopi memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$KOPI_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, kopi_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(kopi_home) / "my-provider.json"
...
All storage paths **must** use the `kopi_home` kwarg from `initialize()`, not hardcoded `~/.kopi`:

```python
# CORRECT — profile-scoped
from kopi_constants import get_kopi_home
data_dir = get_kopi_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.kopi/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `kopi my-provider status`, `kopi my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `kopi <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `kopi --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: kopi my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the kopi my-provider argparse tree.
