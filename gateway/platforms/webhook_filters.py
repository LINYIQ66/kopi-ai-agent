...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.kopi to the active profile home."""
    ...
    if raw == "~/.kopi":
        return kopi_home
    if raw.startswith("~/.kopi/"):
        return kopi_home / raw.removeprefix("~/.kopi/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return kopi_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under KOPI_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from kopi_constants import get_kopi_home

    scripts_root = (get_kopi_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.kopi" or raw_text.startswith("~/.kopi/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__kopi_ignore__") is True
        ):
            return False, None
