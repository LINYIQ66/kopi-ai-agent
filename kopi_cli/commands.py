...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.kopi/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update KOPI AI AGENT to the latest version", "Info"),
    CommandDef("version", "Show KOPI AI AGENT version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`kopi_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/kopi`` subcommand mapping, and
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``kopi skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``kopi skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``kopi skills list`` and
...
# unrelated command landed. These claim their slots right after /kopi,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /kopi <command>).
...
# native slot, the alias spelling stays reachable via /kopi reset).
...
# ``/kopi <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/kopi" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /kopi credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /kopi billing.
#   - moa: high-cost slash mode, available through /kopi moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /kopi debug on Slack.
_SLACK_VIA_KOPI_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/kopi <verb>`` subcommand.
    ...
    can still reach them via ``/kopi <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/kopi`` is always reserved as the first entry so the
    legacy ``/kopi <subcommand>`` form keeps working for anything that
    ...
    # Reserve /kopi as the catch-all top-level command.
    entries.append(("kopi", "Talk to Hermes or run a subcommand", "[subcommand] [args]"))
    seen.add("kopi")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_KOPI_ONLY:
            # Intentionally Slack-via-/kopi only (see _SLACK_VIA_KOPI_ONLY).
            return
    ...
    # everything except /kopi, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://kopi-ai-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /kopi handler.

    Maps both canonical names and aliases so /kopi bg do stuff works
    the same as /kopi background do stuff.

    Plugin-registered slash commands are included so ``/kopi <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from kopi_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
