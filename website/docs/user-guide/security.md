...
KOPI AI AGENT is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
The approval system supports three modes, configured via `approvals.mode` in `~/.kopi/config.yaml`:
...
1. **CLI flag**: Start a session with `kopi --yolo` or `kopi chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `KOPI_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `KOPI_YOLO_MODE` environment variable which is checked before every command execution.
...
Configure the timeout in `~/.kopi/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.kopi/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.kopi/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` kopi/gateway | Self-termination prevention |
...
The `KOPI_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.kopi/config.yaml`:
...
:::tip
Use `kopi config edit` to review or remove patterns from your permanent allowlist.
:::
...
Set allowed user IDs as comma-separated values in `~/.kopi/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.kopi/.env to allow open access,
...
3. The bot owner runs `kopi pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.kopi/config.yaml`:
...
kopi pairing approve telegram ABC12DEF

# Revoke a user's access
kopi pairing revoke telegram 123456789

# Clear all pending codes
kopi pairing clear-pending
```

:::tip Docker users: run pairing commands as the `kopi` user
The official Docker image runs the gateway as the unprivileged `kopi` user
...
```bash
docker exec -u kopi kopi-ai-agent kopi pairing approve telegram ABC12DEF
```
...
[i10270]: https://github.com/LINYIQ66/kopi-ai-agent/issues/10270
:::

**Storage:** Pairing data is stored in `~/.kopi/pairing/` with per-platform JSON files:
...
Container resources are configurable in `~/.kopi/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.kopi/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `KOPI_HOME`. Skills declare these in frontmatter:
...
When loaded, Hermes checks if these files exist in the active profile's `KOPI_HOME` and registers them for mounting:
...
4. **Store secrets securely** — keep API keys in `~/.kopi/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.kopi/logs/` for unauthorized access attempts
10. **Keep updated** — run `kopi update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.kopi/.env`:
...
TERMINAL_SSH_KEY=~/.ssh/kopi_agent_key
```
...
Hermes ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `kopi_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `kopi doctor` for the full remediation.
- **`kopi doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Hermes installs these **lazily** on first use rather than eagerly under `kopi-ai-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `kopi tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `kopi tools`.
