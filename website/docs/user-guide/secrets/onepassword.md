...
Resolve provider API keys from [1Password](https://1password.com/) at process startup instead of storing them in plaintext inside `~/.kopi/.env`. You keep your keys as 1Password items and reference them by `op://vault/item/field`; rotating a credential becomes a single change in 1Password.
...
2. You map environment-variable names to `op://` references in `~/.kopi/config.yaml`.
3. Every time `kopi` (or the gateway, or a cron job) starts, after `~/.kopi/.env` has loaded, Hermes runs `op read` for each reference and sets the resolved values into `os.environ`.
...
- **Service accounts** (recommended for servers/CI): create a service account in 1Password, grant it read access to the relevant vault, and export its token as `OP_SERVICE_ACCOUNT_TOKEN` in `~/.kopi/.env`. The token is the credential — treat it like any other bearer token.
...
1. **In `~/.kopi/.env` (recommended).** `kopi secrets onepassword setup --token <token>` writes the token to `~/.kopi/.env`, exactly like Bitwarden's `BWS_ACCESS_TOKEN`. Because `load_kopi_dotenv()` always loads `.env`, the token is available everywhere with zero extra setup. This is the simplest reliable option.

2. **In `~/.kopi/.op.env` (gitignored).** If you'd rather keep the service-account token out of `.env` — for example so `.env` can be checked into a private dotfiles repo while the token stays out of version control — place it in `~/.kopi/.op.env`:

   ```bash
   echo 'OP_SERVICE_ACCOUNT_TOKEN=ops_...' > ~/.kopi/.op.env
   chmod 600 ~/.kopi/.op.env
   ```
...
3. **Via systemd `EnvironmentFile` (Linux gateway).** If you run the gateway under systemd, you can inject the token directly into the service environment:
   ...
   ```ini
   [Service]
   EnvironmentFile=-/home/youruser/.kopi/.op.env
   ```
...
```bash
kopi secrets onepassword setup
```
...
```bash
kopi secrets onepassword set OPENAI_API_KEY    "op://Private/OpenAI/api key"
kopi secrets onepassword set ANTHROPIC_API_KEY "op://Private/Anthropic/credential"
```

### 4. Preview and confirm

```bash
kopi secrets onepassword sync     # dry-run: resolve now, show what would apply
kopi secrets onepassword status   # config + binary + references + auth
```

From now on, every `kopi` invocation resolves the references at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `kopi secrets onepassword setup` | Verify `op`, set account / token env var, enable |
| `kopi secrets onepassword status` | Show config, binary, auth, and configured references |
| `kopi secrets onepassword set ENV_VAR "op://…"` | Map an env var to a reference (stored stripped + validated) |
| `kopi secrets onepassword remove ENV_VAR` | Drop a mapping |
| `kopi secrets onepassword sync` | Dry-run: resolve references now and show what would apply |
| `kopi secrets onepassword sync --apply` | Resolve and export into the current shell's environment |
| `kopi secrets onepassword disable` | Flip `enabled: false`; leaves mappings in place |

`op` and `1password` are accepted as aliases for `onepassword`.

## Configuration

Defaults in `~/.kopi/config.yaml`:
...
Successful, complete pulls are cached in-process and on disk under `<kopi_home>/cache/op_cache.json` (written atomically, mode `0600`), so back-to-back short-lived `kopi` invocations don't re-shell `op` for every reference. The cache:
...
- A 1Password service-account token can read every secret the account has access to. Store it in `~/.kopi/.env` (not `config.yaml`), and revoke + regenerate from 1Password if it leaks.
...
- **Single-machine personal setups** where `~/.kopi/.env` is fine.
