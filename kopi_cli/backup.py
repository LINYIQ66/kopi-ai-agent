"""
Backup and import commands for kopi CLI.

`kopi backup` creates a zip archive of the entire ~/.kopi/ directory
(excluding the kopi-ai-agent repo and transient files).

`kopi import` restores from a backup zip, overlaying onto the current
KOPI_HOME root.
"""
...
from kopi_constants import get_default_kopi_root, get_kopi_home, display_kopi_home
...
# ``kopi-ai-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/kopi-ai-agent/``
...
# KOPI_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "kopi-ai-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under KOPI_HOME) —
...
# File names that ``kopi import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE KOPI_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside KOPI_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to kopi root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``kopi-ai-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/kopi-ai-agent/`` — must be preserved.
        if part == "kopi-ai-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside KOPI_HOME.
...
def run_backup(args) -> None:
    ...
    kopi_root = get_default_kopi_root()

    if not kopi_root.is_dir():
        print(f"Error: Hermes home directory not found at {kopi_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"kopi-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"kopi-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_kopi_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(kopi_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(kopi_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``kopi-ai-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "kopi-ai-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(kopi_root)
    ...
    # outside KOPI_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_kopi_home()}."
        )
    ...
    print(f"\nRestore with: kopi import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Look for telltale files that a kopi home would have
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.kopi/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a kopi dir name
        if prefix in {".kopi", "kopi"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_kopi_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (kopi_root / "config.yaml").exists()
        has_env = (kopi_root / ".env").exists()
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under KOPI_HOME.
            ...
            target = kopi_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(kopi_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_kopi_home()}."
            )
        ...
        profiles_dir = kopi_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # kopi_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: kopi profile list  (after installing hermes)")

        # Guidance
        print()
        if not (kopi_root / "kopi-ai-agent").is_dir():
            print("Note: The kopi-ai-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: kopi update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  kopi -p {pname} gateway install")

        print("Done. Your Hermes configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and kopi backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to KOPI_HOME).
...
# — `kopi update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $KOPI_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside KOPI_HOME and the entry is silently
...
def _quick_snapshot_root(kopi_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(kopi_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside KOPI_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``kopi update``.
    ...
    Args:
        ...
        kopi_home: Override for the Hermes home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(kopi_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for kopi backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_kopi_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, kopi_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``kopi_root`` to ``out_path``.
...
def _pre_update_backup_dir(kopi_home: Optional[Path] = None) -> Path:
    home = kopi_home or get_kopi_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of KOPI_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<KOPI_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``kopi update``) should continue even if the backup fails.
    """
    kopi_root = kopi_home or get_default_kopi_root()
    if not kopi_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(kopi_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, kopi_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `kopi claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of KOPI_HOME under ``backups/`` before a
    ``kopi claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``kopi import <archive>``.  Writes to
    ``<KOPI_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `kopi import` and the
