...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'kopi_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'kopi_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'kopi_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/kopi/linux-unpacked/kopi', 'linux', {}), '/opt/kopi/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/kopi', 'linux', {}), null)
})
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.kopi/kopi-ai-agent/venv/bin/python',
    pythonPath: null,
  ...
  assert.match(script, /'-m' 'kopi_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/kopi\/linux-unpacked'/)
  assert.match(script, /export KOPI_HOME='\/home\/x\/\.kopi'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.kopi/kopi-ai-agent',
    agentRoot: '/home/x/.kopi/kopi-ai-agent',
  ...
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.kopi\/kopi-ai-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'kopi_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  ...
  assert.match(script, /'-m' 'kopi_cli\.uninstall' '--mode' 'lite'/)
})
