...
def _translate_one_server(
    ...
    if not isinstance(kopi_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(kopi_cfg.get("command"))
    has_url = bool(kopi_cfg.get("url"))
    ...
    for key in kopi_cfg:
        ...
        elif key not in _KNOWN_KOPI_KEYS:
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (KOPI_HOME, PYTHONPATH) could
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="kopi-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a KOPI_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed kopi-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.kopi`` symlink that happens to live under ``/private/var/folders``
...
def _build_kopi_tools_mcp_entry() -> dict:
    ...
    so a kopi installed under /opt/, /usr/local/, or a venv all work.
    KOPI_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # KOPI_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_kopi_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever KOPI_HOME its launcher sets
    ...
    # — that would override the launcher's KOPI_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("KOPI_HOME", tmp_path) would
    ...
    kopi_home = os.environ.get("KOPI_HOME") or ""
    if kopi_home and _looks_like_test_tempdir(kopi_home):
        kopi_home = ""
    if kopi_home:
        env["KOPI_HOME"] = kopi_home
    # PYTHONPATH passes through so a worktree-launched kopi finds the
    ...
    env["KOPI_QUIET"] = "1"
    env["KOPI_REDACT_SECRETS"] = env.get("KOPI_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_kopi_tools: bool = True,
) -> MigrationReport:
    """Translate Hermes mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        kopi_config: full ~/.kopi/config.yaml dict
        ...
        expose_kopi_tools: when True (default), register Hermes' own
    ...
    kopi_servers = (kopi_config or {}).get("mcp_servers") or {}
    if not isinstance(kopi_servers, dict):
    ...
    for name, cfg in kopi_servers.items():
    ...
    # The server itself is agent/transports/kopi_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_kopi_tools:
        translated["kopi-tools"] = _build_kopi_tools_mcp_entry()
        if "kopi-tools" not in report.migrated:
            report.migrated.append("kopi-tools")
