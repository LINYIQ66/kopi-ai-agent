...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with kopi-ai-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.kopi/plugins/<name>/``
3. **Project plugins** – ``./.kopi/plugins/<name>/`` (opt-in via
   ``KOPI_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``kopi_agent.plugins``
   entry-point group.
...
from kopi_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``KOPI_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("KOPI_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``KOPI_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.kopi/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("KOPI_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    logger.debug(
        "KOPI_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # separate `kopi -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `kopi kanban dispatch`),
...
ENTRY_POINTS_GROUP = "kopi_agent.plugins"

_NS_PARENT = "kopi_plugins"
...
class PluginManifest:
    ...
    #              in ~/.kopi/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``kopi plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        ...
        running under ``~/.kopi/profiles/<name>``, or ``"custom"`` when
        ``KOPI_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``kopi honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``kopi <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`kopi_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from kopi_cli.dashboard_auth import (
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``kopi model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: kopi_cli.main imports plugins indirectly
        from kopi_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.kopi/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        if env_var_enabled("KOPI_SAFE_MODE"):
            logger.info("KOPI_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to kopi_cli/. Two layouts are
        ...
        # 2. User plugins (~/.kopi/plugins/)
        user_dir = get_kopi_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.kopi/plugins/)
        if _env_enabled("KOPI_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".kopi" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set KOPI_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with kopi and must
            ...
            # all ~20 of them added several seconds to every `kopi`
            # invocation — including plain `kopi chat`, which never touches a
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `kopi plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``kopi plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`kopi plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `kopi plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``kopi_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``kopi_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="kopi-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``kopi tools`` TUI so plugin-provided toolsets appear
