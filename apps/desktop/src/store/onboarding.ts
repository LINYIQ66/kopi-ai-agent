...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/kopi'
...
const CONFIGURED_CACHE_KEY = 'kopi-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'kopi-onboarding-skipped-v1'
const POLL_MS = 2000
...
// mirroring kopi_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
async function fetchProviderDefaultModel(
  ...
  // `kopi model` does (for Nous it honors the user's free/paid tier, so a
...
async function openSignInUrl(url: string) {
  if (window.kopiDesktop?.openExternal) {
    try {
      await window.kopiDesktop.openExternal(url)
