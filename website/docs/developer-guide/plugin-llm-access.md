...
kopi-ai-agent itself). For the async surface (`acomplete()` /
  ...
  entries:
    my-plugin:
      llm:
        ...
        # `kopi model` and config.yaml model.provider).
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.kopi/auth.json` / env, including the credential pool when
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/LINYIQ66/kopi-ai-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/LINYIQ66/kopi-ai-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/LINYIQ66/kopi-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/LINYIQ66/kopi-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
