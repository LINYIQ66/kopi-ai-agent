...
def _fresh_import():
    """Return a freshly-imported kopi_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("kopi_bootstrap", None)
    import kopi_bootstrap  # noqa: WPS433
    return kopi_bootstrap
...
class TestEntryPointsImportBootstrap:
    ...
    ENTRY_POINTS = [
        ...
        "run_agent.py",          # kopi-ai-agent (console_script)
        "acp_adapter/entry.py",  # kopi-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import kopi_bootstrap' and that
        ...
        """
        # Resolve relative to the kopi-ai-agent repo root.  Tests live
        # at tests/test_kopi_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # kopi start even when kopi_bootstrap hasn't been
        ...
        assert first_import_name == "kopi_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'kopi_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import kopi_bootstrap' line to be the first import."
        )
...
class TestHardenImportPath:
    ...
    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("KOPI_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["KOPI_PYTHON_SRC_ROOT"] = env
            elif "KOPI_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["KOPI_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/kopi")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("KOPI_PYTHON_SRC_ROOT", None)
            else:
                os.environ["KOPI_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/kopi", "/usr/lib/python"])
    ...
    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/kopi", "/opt/kopi", ""])
        assert result.count("/opt/kopi") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["KOPI_PYTHON_SRC_ROOT"] = "/env/kopi"
            hb.harden_import_path()
            assert sys.path[0] == "/env/kopi"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("KOPI_PYTHON_SRC_ROOT", None)
